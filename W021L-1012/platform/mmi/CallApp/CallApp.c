...
//                                        callPIType      pi,
//                                        AECHAR          *name);
//static void CallApp_Handle_SS_StateChange(CCallApp*pMe, AEETNotifyInfo  *p_Ss_Info);

//static void CallApp_Handle_Phone_StateChange(CCallApp *pMe,AEETNotifyInfo  *p_Phone_Info);

static void CallApp_Handle_Call_StateChange(CCallApp *pMe,AEETNotifyInfo  *p_Call_Info);

static void CallApp_ProcessCallStateDATA(CCallApp *pMe,
                                        AEETNotifyInfo  *pCallInfo,
    ...
    switch (eCode)
    {
        ...
        case EVT_APP_STOP:
            ...
            if(pMe->m_b_auto_redial)
            {
                ...
                {
                    WSTRLCPY(pMe->m_DialString,pMe->m_call_info.call_number,AEET_MAX_DIGITS_LENGTH);
        ...
        case EVT_BATT_POWERDOWN:
            ...
            {	
               ...
               //ICM_EndCall(pMe->m_pITelephone, pMe->m_CallsTable->cd);
            }
            return TRUE;
...
static int CallApp_CallNumber(ICallApp *p, AECHAR *number)
{
    ...
    MSG_FATAL("CallApp_CallNumber",0,0,0);
                              ...
                              uint16 wParam, uint32 dwParam)
{
    ...
    switch (Notify_msg->cls)
    {
        ...
        {
            AEETNotifyInfo *EventInfo = (AEETNotifyInfo *)(Notify_msg->pData);
            ...
            switch (Notify_msg->dwMask)
            {
                ...
                case AEET_NMASK_OTHER_CALL:
                    //CALL_ERR("AEET_NMASK_VOICE_CALL %x",EventInfo->event,0,0);
                ...
                case AEET_NMASK_DATA_CALL:
                    CALL_ERR("AEET_NMASK_DATA_CALL %x",EventInfo->event,0,0);
                ...
                //    CALL_ERR("AEET_NMASK_PHONE %x",EventInfo->event,0,0);
                ...
                //case AEET_NMASK_SS:
                //    CALL_ERR("AEET_NMASK_SS %x",EventInfo->event,0,0);
                                             ...
                                             AEETNotifyInfo  *p_Phone_Info)
{
    ...
    switch (p_Phone_Info->event)
    {
        ...
        {
            CALL_MSG_HIGH("AEET_EVENT_PH_OPRT_MODE %d",p_Ph_Info->oprt_mode,0,0);
        ...
        case AEET_EVENT_PH_SYSTEM_PREF:        /* System preference changed */
            CALL_MSG_HIGH("AEET_EVENT_PH_SYSTEM_PREF",0,0,0);
            break;

        case AEET_EVENT_PH_ANSWER_VOICE:       /* Answer voice as data was changed. */
            CALL_MSG_HIGH("AEET_EVENT_PH_ANSWER_VOICE %d",p_Ph_Info->answer_voice,0,0);
        ...
        case AEET_EVENT_PH_NAM_SEL:        /* NAM selection was changed */
            CALL_MSG_HIGH("AEET_EVENT_PH_NAM_SEL %d",p_Ph_Info->nam_sel,0,0);
        ...
        case AEET_EVENT_PH_CURR_NAM:       /* Current NAM was changed. */
            CALL_MSG_HIGH("AEET_EVENT_PH_CURR_NAM %d",p_Ph_Info->curr_nam,0,0);
        ...
        case AEET_EVENT_PH_IN_USE_STATE:       /* In use state was changed */
            CALL_MSG_HIGH("AEET_EVENT_PH_IN_USE_STATE %d",p_Ph_Info->is_in_use,0,0);
            break;

        case AEET_EVENT_PH_CDMA_LOCK_MODE:     /* CDMA lock mode was changed. */
            CALL_MSG_HIGH("AEET_EVENT_PH_CDMA_LOCK_MODE %d",p_Ph_Info->cdma_lock_mode,0,0);
        ...
        case AEET_EVENT_PH_MAINTREQ:       /* CDMA maintenance required command. */
            // TBD
            CALL_MSG_HIGH("AEET_EVENT_PH_MAINTREQ",0,0,0);
            break;

        case AEET_EVENT_PH_STANDBY_SLEEP:      /* Entering powerdown sleep mode */
            CALL_MSG_HIGH("AEET_EVENT_PH_STANDBY_SLEEP",0,0,0);
            break;
        case AEET_EVENT_PH_STANDBY_WAKE:       /* Exiting powerdown sleep mode */
            CALL_MSG_HIGH("AEET_EVENT_PH_STANDBY_WAKE %d",pMe->m_lastCallState,0,0);
        ...
        case AEET_EVENT_PH_ACMMAX_SET:         /* Accumulated Call Meter was set */
            CALL_MSG_HIGH("AEET_EVENT_PH_ACMMAX_SET",0,0,0);
            break;
                                             ...
                                             AEETNotifyInfo  *p_Call_Info)
{
    ...
    switch (p_cm_call_info->call_type)
    {
        case AEET_CALL_TYPE_OTAPA:
            //CALL_ERR("AEET_CALL_TYPE_OTAPA",0,0,0);
        ...
        case AEET_CALL_TYPE_PD:
            ...
            CALL_ERR("AEET_CALL_TYPE_SMS??",0,0,0);
            return;
        ...
        case AEET_CALL_TYPE_NONE:
            ...
            //CALL_ERR("AEET_CALL_TYPE_VOICE",0,0,0);
        ...
        default:
        {
            ...
            //if((p_Call_Info->event == AEET_EVENT_CALL_SIGNAL) && (AEET_IS_VOICECALL_CONNECTED(pMe->m_pITelephone)))
            if((p_Call_Info->event == AEET_EVENT_CALL_SIGNAL))
            {
                //  call_table->signal.signal_type=AEET_SIGNAL_CDMA_IS54B;
                // call_table->signal.signal.cdma_tone=AEET_CDMA_TONE_BUSY;
    ...
    if (newState != STATE_NULL)
    {
        ...
        else
        {
            	...
            	&& pMe->m_nStartCallType != START_INCOMING_ANSWERED)
            {
                ...
                //in some case ,if we press end key in incoming dialog,dialer app been stop,in AEET_EVENT_CALL_END,we may set new state to STATE_MISSEDCALL,in this case ,we return
                else
                {
                    return;
                }
...
static void CallApp_ProcessCallStateOTAPA(CCallApp*pMe,AEETNotifyInfo   *pCallInfo,
	                                        CallAppState             *newState)
{
    ...
    switch (pCallInfo->event)
    {
        ...
        case AEET_EVENT_CALL_END:
            //CALL_ERR("AEET_EVENT_CALL_END",0,0,0);
	                                       ...
	                                       CallAppState             *newState)
{
    ...
    switch (pCallInfo->event)
    {
        ...
        case AEET_EVENT_CALL_ORIG:    /* phone originated a call */
            //CALL_ERR("AEET_EVENT_CALL_CONNECT",0,0,0);
            if(pMe->m_CallsTable)
            {
	                                            ...
	                                            CallAppState             *newState)/* phone originated a call */
{
    ...
    *newState = STATE_CALLING;
    ...
    //CallApp_Change_Call_Table_Call_ID(pMe,call_table->call_info.other_party_no,call_table->cd);
}
	                                           ...
	                                           CallAppState             *newState)/* Originated/incoming call was ended */
{
    ...
    switch (pMe->m_callEndStatus)
    {
        ...
        //test only
        //case 1:/*cdg2.6*/
        //    IALERT_StartAlerting(pMe->m_pAlert, AEEALERT_CALLTYPE_VOICE, call_table->call_info.other_party_no, AEET_CDMA_TONE_ABBR_INTERCEPT);
        //    break;

        //case 2:/*cdg2.6*/
        //    IALERT_StartAlerting(pMe->m_pAlert, AEEALERT_CALLTYPE_VOICE, call_table->call_info.other_party_no, AEET_CDMA_TONE_ABBR_REORDER);
        //    break;
                                                            ...
                                                            CallAppState             *newState)
{
    ...
    {
        CALL_ERR("Received AEET_EVENT_CALL_SIGNAL with no signal info.",
                0, 0, 0);
        return;
    }
...
*/
static void CallApp_ProcessCallStateVoiceDisplay(CCallApp *pMe,
	AEETNotifyInfo  *pCallInfo, CallAppState *newState)
{
    ...
    //(pCallInfo->event == AEET_EVENT_INTERNAL_EXT_DISP))
    {
        ...
        /*OEMCM.c 5409     AEET_EVENT_CALL_DISPLAY
        notify_info->event_data.call.disp_type = AEET_DISP_NORMAL;*/
        ...
        AEETNotifyInfo  *pCallInfo, CallAppState *newState)
{
    ...
    {
        case AEET_EVENT_CALL_ORIG:       /* phone originated a call */
            CALL_ERR("AEET_EVENT_CALL_ORIG=+=",0,0,0);
        ...
        case AEET_EVENT_CALL_ANSWER:     /* Incoming call was answered */
            CALL_ERR("%d AEET_EVENT_CALL_ANSWER=+=",call_table->number.pi,0,0);
        ...
        case AEET_EVENT_CALL_INCOM:      /* phone received an incoming call */
            CALL_ERR("%d AEET_EVENT_CALL_INCOM=+=",call_table->number.pi,0,0);
        ...
        case AEET_EVENT_CALL_CALLER_ID:  /* Caller ID info was received from BS */
            CALL_ERR("%d AEET_EVENT_CALL_CALLER_ID=+=",call_table->number.pi,0,0);
...
#endif
            CALL_ERR("%d %d AEET_EVENT_CALL_REDIR_NUM=+=",call_table->number.pi,call_table->call_info.redirect_party_number.pi,0);
        ...
        case AEET_EVENT_CALL_SIGNAL:       /* signal info was received from BS */
            CALL_ERR("%d %d AEET_EVENT_CALL_SIGNAL=+=",pMe->m_bReceiveCallerID,call_table->number.pi,0);
        ...
        case AEET_EVENT_CALL_CONNECT: /* Originated/incoming call was connected */
            CALL_ERR("%d AEET_EVENT_CALL_CONNECT=+=",call_table->number.pi,0,0);
            pMe->m_bReceiveCallerID     = FALSE;
...
#ifndef CUST_EDITION  //unfinish 
        case AEET_EVENT_CALL_LINE_CTRL: /* Originated was accepted */
            CALL_ERR("%d AEET_EVENT_CALL_LINE_CTRL=+=",call_table->number.pi,0,0);
        ...
        case AEET_EVENT_CALL_OPS:      /* phone sent Flash/Flash with Info to BS */
            CALL_ERR("AEET_EVENT_CALL_OPS=+=",0,0,0);
        ...
        case AEET_EVENT_CALL_SRV_OPT: /* Service option changed */
            CALL_ERR("%d AEET_EVENT_CALL_PRIVACY=+=",call_table->call_info.is_privacy,0,0);
        ...
        case AEET_EVENT_CALL_ABRV_ALERT:   /* CDMA/AMPS abbreviated alert */
            CALL_ERR("AEET_EVENT_CALL_ABRV_ALERT=+=",0,0,0);
        ...
        case AEET_EVENT_CALL_ABRV_REORDER: /* AMPS abbreviated reorder */
            CALL_ERR("AEET_EVENT_CALL_ABRV_REORDER=+=",0,0,0);
            //OEM_Snd_Sound_Id_Start(SND_DEVICE_CURRENT, SND_METHOD_VOICE,
        ...
        case AEET_EVENT_CALL_ABRV_INTERCEPT: /* AMPS abbreviated intercept */
            CALL_ERR("AEET_EVENT_CALL_ABRV_INTERCEPT=+=",0,0,0);
            //OEM_Snd_Sound_Id_Start(SND_DEVICE_CURRENT, SND_METHOD_VOICE,
        ...
        case AEET_EVENT_CALL_DISPLAY:      /* display info was received from BS */
            ...
            //        case AEET_EVENT_INTERNAL_EXT_DISP:  /* extended display was received from BS */
            //#endif /* FEATURE_IS95B_EXT_DISP */
            /*From OEMCM.c 5456,we cann't receive AEET_EVENT_INTERNAL_EXT_DISP
                when OEMCM receive AEET_EVENT_INTERNAL_EXT_DISP,it send AEET_EVENT_CALL_DISPLAY
            */
            CALL_ERR("AEET_EVENT_CALL_DISPLAY=+=",0,0,0);
            //CallApp_ProcessCallStateVoiceDisplay(pMe, pCallInfo, newState);
            //break;

        case AEET_EVENT_CALL_CALLED_PARTY:   /* called party info was received from BS */
            CALL_ERR("%d AEET_EVENT_CALL_CALLED_PARTY=+=", call_table->number.pi, 0, 0);
            CallApp_ProcessCallStateVoiceDisplay(pMe, pCallInfo, newState);
            break;

        case AEET_EVENT_CALL_CONNECTED_NUM:  /* connected number info was received from BS */
            CALL_ERR("%d && %d %d AEET_EVENT_CALL_CONNECTED_NUM=+=",
        ...
        case AEET_EVENT_CALL_ERROR:
            CALL_ERR("%x AEET_EVENT_CALL_ERROR=+=", pCallInfo->event_data.call.call_error, 0, 0);
            *newState = STATE_EXIT;
            break;
    ...
    switch(pSignal->signal_type)
    {
        case AEET_SIGNAL_CDMA_TONE:
            switch(pSignal->signal.cdma_tone)
            {
                ...
                case AEET_CDMA_TONE_ABBR_REORDER:
...
//AEECMCallState CallApp_Get_Current_CallState(AEETCallEventData *call_table)
//{
                                            ...
                                            boolean is_energency, boolean is_restict)
{
    ...
    AECHAR    m_call_number[AEET_MAX_DIGITS_LENGTH] = {0};
    ...
    else
    {
        ...
        WSTRLCPY(m_call_number,number,AEET_MAX_DIGITS_LENGTH);
    ...
    if(b_force_add)
    {
        ...
        if(len == 0)
        {
            WSTRLCPY(m_call_number,L"No number",AEET_MAX_DIGITS_LENGTH);
        }
        else
        {
            WSTRLCPY(m_call_number,number,AEET_MAX_DIGITS_LENGTH);
    ...
    switch (type)
    {
        case PI_RESTRICTED: // restricted
            (void) ISHELL_LoadResString(pMe->m_pShell,
                                        ...
                                        AEET_MAX_DIGITS_LENGTH/*MAX_SIZE_NAME_TEXT*/);
    ...
    WSTRLCPY(pMe->m_call_info.call_number, m_call_number, AEET_MAX_DIGITS_LENGTH);
    ...
    if(pMe->m_CallsTable_Count == 0)//the fisrt call
    {
        ...
        if(temp)
        {
            ...
            WSTRLCPY(temp->call_number, m_call_number, AEET_MAX_DIGITS_LENGTH);
                                            ...
                                            uint8 call_id,uint16 call_type/*uint16 call_status*/,callPIType type)
{
    ...
    if(pMe->m_b_is_PI_ALLOWED == PI_ALLOWED)
    {
        WSTRLCPY(pMe->m_call_info.call_number, number, AEET_MAX_DIGITS_LENGTH);
        if(m_call_name[0] != 0)
        {
            WSTRLCPY(pMe->m_call_info.call_name, m_call_name, MAX_SIZE_NAME_TEXT);
            pMe->m_call_info.in_phonebook = TRUE;
    ...
    if(pMe->m_CallsTable_Count == 1)
    {
        if(pMe->m_CallsTable)
        {
            ...
            if(pMe->m_b_is_PI_ALLOWED == PI_ALLOWED)
            {
                WSTRLCPY(pMe->m_CallsTable->call_number, number, AEET_MAX_DIGITS_LENGTH);
                if(m_call_name[0] != 0)
                {
            ...
            {
                WSTRLCPY(pMe->m_CallsTable->call_number, m_call_number, AEET_MAX_DIGITS_LENGTH);
            }
...
{
    int len = 0;
    char  temp1[AEET_MAX_ALPHA_TAG_LENGTH] = {0};
    char  temp2[AEET_MAX_ALPHA_TAG_LENGTH] = {0};
    int i = 0;
    int j = 0;
    if(buf == NULL)
    {
    ...
    STRTOWSTR(temp2, buf, AEET_MAX_ALPHA_TAG_LENGTH);                        
...
{
    tBTA_AG_RES_DATA data;
    //AEETCallEventData  *CallInfo = (AEETCallEventData  *)pCallInfo;
    int xx;
    tBTUI_AG_CB *bt_ui_ag_cb = bcmapp_ag_get_ag_cb();
    ...
    switch(event)
    {
        ...
        case AEET_EVENT_CALL_INCOM:      /* phone received an incoming call */
            ERR("AEET_EVENT_CALL_INCOM",0,0,0);
            b_incoming = TRUE;
            pMe->m_bt_audio = SEND_BT_CALL;
        ...
        case AEET_EVENT_CALL_CALLER_ID:  /* Caller ID info was received from BS */
            ...
            ERR("AEET_EVENT_CALL_CALLER_ID %d",pMe->m_b_incall,0,0);
            b_incoming = TRUE;
            data.str[0]='\"';
        ...
        case AEET_EVENT_CALL_CONNECT: /* Originated/incoming call was connected */
            ERR("AEET_EVENT_CALL_CONNECT %d",b_incoming,0,0);
            pMe->m_b_add_btag_menu = TRUE;
