                                           ...
                                           uint32      dwParam)
{
    ...
    switch (eCode)
    {
        ...
        case EVT_DIALOG_START:
        {
            ...
            //else if ((AEECMPrivacyPref)privacy_pref == AEET_PRIVACY_PREF_STANDARD)
            //{
        ...
        case EVT_KEY:
            ...
            switch ((AVKType)wParam)
            {
                ...
                case AVK_ENDCALL:				
                    ...
                    //CALL_ERR("AVK_ENDCALL", 0,0,0);
                    // End call
                    //ASSERT(AEET_CALL_STATE_CONV == pMe->m_lastCallState);
        ...
        case EVT_KEY:
            ...
            switch ((AVKType)wParam)
            {
                ...
                case AVK_CLR:
                    ...
                    //if (AEET_CALL_STATE_CONV != pMe->m_lastCallState &&pMe->m_bincoming_rsk == IDS_REJECT)
...
static void CallApp_IncomingCall_Dlg_Init(CCallApp *pMe)
{
    ...
    //if (AEET_CALL_STATE_CONV == pMe->m_lastCallState)//waiting call
    if(pMe->m_b_incall )
    {
        // Inband call?
...
MAKE_CALL_VALUE CallApp_MakeCall(CCallApp *pMe)
{
    ...
    // Can only make emergency calls while emgcall is TRUE
    //if (CallApp_IsEmergencyMode(pMe->m_pITelephone)/*pMe->idle_info.IsEmgCall*/ &&
    //                                        !CallApp_IsEmergency_Number(pMe->m_DialString))
    //{
    ...
    if(pMe->m_b_incall == FALSE)
    {
        ...
        //    if(CheckAEEReturnStatus(ICM_OriginateVoiceCall(pMe->m_pITelephone, wbuf, &nCallID)) == FALSE)
        //    {
                                   ...
                                   boolean      louder)
{
...

    {
        ...
        //ICM_GetCallInfo(pMe->m_pITelephone, 0, ci, sizeof(AEECMCallInfo));
...
static void CallApp_ConvReleasePKey(CCallApp *pMe)
{
    ...
    {
        byte strLen;
       // AECHAR wsbuf[AEET_MAX_DIGITS_LENGTH] = {0};
        ...
        //ICM_BurstDTMF(pMe->m_pITelephone, pMe->m_CallsTable->call_id, dur->nOn, dur->nOff, wsbuf);
...
static void CallApp_BurstDTMF(CCallApp *pMe,AECHAR *dtmf,int len)  //len 的长度要注意
{
    int         i   = 0;
   // uint8 CallIDs[DIALERAPP_MAX_NUM_CALLS];
    const DTMFToneDuration *dur = CallApp_GetDTMFDuration(pMe);
...
static void CallApp_Calc_Cursor_Rect(CCallApp* pMe, AEERect *pRect)
{
    int xPos = 0, yPos = 0, dy = pMe->m_rc.dy, Line_Pixel = 2;
