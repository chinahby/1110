...
// Format variables, strings, characters and patterns for Slovenian (Slovenia)
...
static const AECHAR Currency_NegativePattern []           = { '-','#',' ','S','I','T',0 }; 
static const AECHAR Currency_PositivePattern[]            = { '#',' ','S','I','T',0 }; 
static const AECHAR Currency_CurrencySymbol[]             = { 'S','I','T',0 }; 
...
/* -----------------------------------------------------------------------
    LocaleSL_Ctor 
   --- */
static void LocaleSL_Ctor(LocaleSL *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleSL_New  
   --- */
int LocaleSL_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleSL *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleSL, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleSL_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule); 
...
int LocaleSL_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALESL) {
      return LocaleSL_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
