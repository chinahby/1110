...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/data/1x/1xrlp/main/lite/src/dsrlpdel.c#1 $ $DateTime: 2007/11/06 01:21:33 $ $Author: nsivakum $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(fr_detect_ptr->remove_idx != fr_detect_ptr->add_idx)
  {
    ...
    for(i = 0; i < fr_arr_ptr->index; i++)
    {
      ...
      {
        rscb_ptr->ver_ptr->fcn_ptr_table->rx_new_data
        (
          rscb_ptr,
      ...
      {
        ERR("NULL ptr in del frame array",0,0,0);
      }
    }
    fr_arr_ptr->index = 0;
  ...
  if (fr_detect_ptr->del_fr_arr[fr_detect_ptr->add_idx].index > 0)
  {
    ...
    if (fr_detect_ptr->del_fr_arr[fr_detect_ptr->add_idx].index != 0)
    {
      ERR("NON EMPTY ARRAY...MEM LEAK???",0,0,0);
    }
  }
} /* dsrlpdel_remove() */
#endif /* MSM5000_IRAM_FWD */
