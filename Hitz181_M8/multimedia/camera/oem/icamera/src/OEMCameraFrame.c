...
static int FrameBitmap_GetYUVBitmap(IBitmap *po, IBitmap **ppIBitmap)
{
    ...
    if (pYCbCr) 
    {
        ...
        // if camera frame is rotated, save the rotation in YCbCr object
        if (pme->m_rawFrame.rotation)
        {
          //hack
          YCbCr* pYCbCrInternal = (YCbCr*) pYCbCr;
          if (pme->m_rawFrame.rotation == 90)
          {
            pYCbCrInternal->rotate = YCBCR_ROTATION_90CW;
          }
          else if (pme->m_rawFrame.rotation == 180)
          {
            pYCbCrInternal->rotate = YCBCR_ROTATION_180;
          }
          else if (pme->m_rawFrame.rotation == 270)
          {
