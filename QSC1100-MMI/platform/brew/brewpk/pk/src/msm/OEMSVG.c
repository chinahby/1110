...
/*=====================================================================
  ...
  $Header: //depot/asic/msmshared/apps/brew_3.1.5/pk/src/msm/OEMSVG.c#36 $
  $DateTime: 2007/03/29 18:14:55 $
  $Author: lindai $
...
int CMediaSVGHandleInputEventFocus(CMediaSVG* pMediaSVG, int16 focusEvent)
{
    ...
    case MM_SVG_FOCUS_EVENT_NORTH:
        alEvent = SVGAL_FOCUS_EVENT_MOVE;
        dir = SVGAL_FOCUS_EVENT_NORTH;
        break;

    case MM_SVG_FOCUS_EVENT_NORTH_EAST:
        alEvent = SVGAL_FOCUS_EVENT_MOVE;
        dir = SVGAL_FOCUS_EVENT_NORTH_EAST;
        break;

    case MM_SVG_FOCUS_EVENT_EAST:
        alEvent = SVGAL_FOCUS_EVENT_MOVE;
        dir = SVGAL_FOCUS_EVENT_EAST;
        break;

    case MM_SVG_FOCUS_EVENT_SOUTH_EAST:
        alEvent = SVGAL_FOCUS_EVENT_MOVE;
        dir = SVGAL_FOCUS_EVENT_SOUTH_EAST;
        break;

    case MM_SVG_FOCUS_EVENT_SOUTH:
        alEvent = SVGAL_FOCUS_EVENT_MOVE;
        dir = SVGAL_FOCUS_EVENT_SOUTH;
        break;

    case MM_SVG_FOCUS_EVENT_SOUTH_WEST:
        alEvent = SVGAL_FOCUS_EVENT_MOVE;
        dir = SVGAL_FOCUS_EVENT_SOUTH_WEST;
        break;

    case MM_SVG_FOCUS_EVENT_WEST:
        alEvent = SVGAL_FOCUS_EVENT_MOVE;
        dir = SVGAL_FOCUS_EVENT_WEST;
        break;

    case MM_SVG_FOCUS_EVENT_NORTH_WEST:
        alEvent = SVGAL_FOCUS_EVENT_MOVE;
        dir = SVGAL_FOCUS_EVENT_NORTH_WEST;
        break;
...
int CMediaSVGHandleUserInput(CMediaSVG* pMediaSVG, int32 param1, int32 param2)
{
    ...
    case MM_SVG_EVENT_ROTATE:
        ...
        SVGALRotate(pMediaSVG->alCtx, (int) rotData->cx, (int) rotData->cx, (int) rotData->nAngle);
        //CMediaSVGCreateNotifyMsg(pMediaSVG, MM_CMD_SETMEDIAPARM, MM_PARM_SVG_INPUT, MM_STATUS_DONE ,
    ...
    if((SUCCESS == ret) && (pMediaSVG->m_pMediaSVGState == MEDIASVG_STATE_PLAYING))
    {
      ...
      CMediaSVGNextFrame(pMediaSVG, FALSE, 0);
    }
...
int CMediaSVGHandleBkColor(CMediaSVG* pMediaSVG, int32 param1, int32 param2)
{
    ...
    SVGALSetBackgroundColor(pMediaSVG->alCtx, red, green, blue);
...
int CMediaSVGHandleSetMediaData(CMediaSVG* pMediaSVG, int32 param1, int32 param2)
{
    ...
    switch(pMediaSVG->m_pMediaSVGState)
    {
        case MEDIASVG_STATE_IDLE:
        {
            ...
            //we've set the content successfully, move to ready state
...
int OEMSVGInit(CMediaSVG* pMediaSVG, void** ppalCtx, uint32 width, uint32 height, uint32 dpi)
{
    ...
    if( SVGALInit(ppalCtx, width, height, dpi) != SVGAL_SUCCESS)
    {
    ...
    pMediaSVG->m_TotalTimeCallback  = 0;
...
int OEMCMediaSVGPlay(IMedia* pm)
{
    ...
    else if(pMediaSVG->m_pMediaSVGState == MEDIASVG_STATE_READY)
    {
        ...
        SVGALPlay(pMediaSVG->alCtx, pMediaSVG->m_isClientFrameGenEnabled);
...
int OEMCMediaSVGSeek(IMedia* pm, AEEMediaSeek eSeek, int32 lSeekValue)
{
    ...
    CMediaSVGCreateNotifyMsg(pMediaSVG, MM_CMD_PLAY, 0, MM_STATUS_SEEK, NULL, 0, FALSE, FALSE);
...
int OEMCMediaSVGPause(IMedia* pm)
{
    ...
    if(pMediaSVG->m_pMediaSVGState == MEDIASVG_STATE_PLAYING)
    {
        ...
        CMediaSVGCreateNotifyMsg(pMediaSVG, MM_CMD_PLAY, 0, MM_STATUS_PAUSE, (void*) pMediaSVG->m_currDocTime,
                                 sizeof(pMediaSVG->m_currDocTime), FALSE, FALSE);
...
int OEMCMediaSVGResume(IMedia* pm)
{
    ...
    if(pMediaSVG->m_pMediaSVGState == MEDIASVG_STATE_PLAY_PAUSE)
    {
        ...
        //Inform the Lower Layers that Resume has been done
        SVGALResume(pMediaSVG->alCtx, pMediaSVG->m_isClientFrameGenEnabled);
...
int OEMCMediaSVGGetTotalTime(IMedia* pm)
{
    ...
    }else if(pMediaSVG->m_pMediaSVGState == MEDIASVG_STATE_READY)
    {
        ...
        if((-1 == totalTime) || (0x0FFFFFFF < totalTime))
        {
            ...
            //TODO: USe Macro
            pMediaSVG->m_TotalTimeCallback = (uint32)0x0FFFFFFF;
        }else
        {
            pMediaSVG->m_TotalTimeCallback = (uint32)totalTime;
        }
        CMediaSVGCreateNotifyMsg(pMediaSVG, MM_CMD_GETTOTALTIME, 0, MM_STATUS_DONE, 
                (void*) pMediaSVG->m_TotalTimeCallback, sizeof(pMediaSVG->m_TotalTimeCallback), TRUE, TRUE);
...
int OEMCMediaSVGQueryInterface(IMedia* pm, AEECLSID clsId, void** outInterface)
{
    ...
    else if(clsId == AEECLSID_SVGDOM || clsId == AEEIID_SVGDOM)
    {
        ...
        {
            //requesting the DOM interface can happen when some content has already been bound,
            //(and potentially some frames were generated) or when no content has been bound, in
            //which case we bind it here
            if( (ret = OEMCMediaSVGPlay((IMedia*) pMediaSVG) ) != SUCCESS)
            {
                *outInterface = NULL;
                return ret;
            }
