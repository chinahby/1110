...
SERVICES: IModule implementation file for A1Lite
...
DESCRIPTION
  ...
  3. Provide a CreateInstance() function (which is identical to the A1LiteClsCreateInstance()
...
#include "A1LiteModGen.h"  // Contains Sample AEEModule declarations
#include "A1LiteAppGen.h"
                     ...
                     PFNMODCREATEINST pfnMC,PFNFREEMODDATA pfnMF)
{
   ...
   *ppMod = NULL;
   ...
   pMe->m_piEnv = A1LiteEnv_New(pEnv, pIShell, (IModule *)pMe);
...
Description:
   ...
   This function invokes the A1LiteClsCreateInstance() function that the module
                                 ...
                                 AEECLSID ClsId,void **ppObj)
{
   ...
   // For a dynamic module, they must supply the A1LiteClsCreateInstance() 
   //   function. Hence, invoke it. For a static app, they will have 
   //   registered the create Instance function. Invoke it.
   if (pme->pfnModCrInst) {
      nErr = pme->pfnModCrInst(ClsId, pIShell, pIModule, pme->m_piEnv, ppObj);
#if !defined(AEE_STATIC)
   } else {
      nErr = A1LiteClsCreateInstance(ClsId, pIShell, pIModule, pme->m_piEnv, ppObj);
#endif
   }
