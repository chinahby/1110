...
FILE: SSOperations_AEEAppGen.c // singleton-change
...
#include "SSOperations_AEEAppGen.h"  // singleton-change // AEEApplet definitions 
#include "SSOperations_AEEModGen.h"  // singleton-change // AEEMod definitions and utility function
...
static uint32 AEEApplet_Release(IApplet * po)
{
   ...
   // singleton-change - Remove reference to this instance from Module.
   if (pme->clsID == AEECLSID_SS_AGENT) 
   {
        ((AEEMod*)(pme->m_pIModule))->pCSSAgentApp = NULL;
   }
...
PROTOTYPE:
   boolean SSOperations_AEEApplet_New(int16 nSize,
                         AEECLSID clsID,
                         IShell * pIShell, 
                         IModule * pIModule, 
                         IApplet **ppobj, 
...
boolean SSOperations_AEEApplet_New(int16 nIn,
                      AEECLSID clsID,
                      IShell * pIShell, 
                      IModule * pIModule, 
                      IApplet **ppobj, 
