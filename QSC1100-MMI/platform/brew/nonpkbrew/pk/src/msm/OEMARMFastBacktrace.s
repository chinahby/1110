#include "customer.h"
        ...
        ;
        ; int HT_SYSTEM_CHECK_TEXT_SECTION_POINTER(void *ptr);
        ;

        IMPORT HT_SYSTEM_CHECK_TEXT_SECTION_POINTER
	...
	;
        ...
        ;-----------------------------------------------------------------------
        ; signed long HT_ARM926EJS_UNWIND_STACK(void **, unsigned long);
        ;-----------------------------------------------------------------------
        ; inputs:
	;
	...
	;       that HT_ARM926EJS_UNWIND_STACK unwinds, the return address in 
	...
	; caller of HT_ARM926EJS_UNWIND_STACK, [r0, #4] will contain the 
	; return address of the caller, and so on.
        ;-----------------------------------------------------------------------

HT_ARM926EJS_UNWIND_STACK PROC
...
THUMB_PUSH
	...
	; If it is not, then quit.
 	stmfd   sp!, {r0-r3}          	; Save r0-r3 on the stack
        ...
        blx     HT_SYSTEM_CHECK_TEXT_SECTION_POINTER 	; call function 
...
ERROR_ARMMODE
#ifdef FEATURE_L4
	ldr     r6, %f98		; load addr of __thread_stub
	add	r6, r6, #0xC		; advance r6 to branch instruction
	cmp	r3, r6			; see if we are in __thread_stub
	ldmeqfd sp!, {r4-r11,pc}	; return without error since we have 
					; reached __thread_stub
#endif
	b	ERROR
ERROR_MAXCOUNT
	b	ERROR
...
#ifdef FEATURE_L4
98      DCD     __thread_stub
#else
98      DCD     rex_task_preamble
#endif

        EXPORT HT_ARM926EJS_UNWIND_STACK
