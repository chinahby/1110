...
vc_jpeg_decode display_jpg_done = VC_JPEG_DECODE_INIT;   /*留着备用*/
#else
                                    ...
                                    uint32 dwParam)
{
   ...
   {
      MG_FARF(ADDR, ("pCurNode NULL, or in build or menu empty!"));
      return FALSE;
   }
                                                       ...
                                                       uint32 dwParam)
{
   ...
   MG_FARF(ADDR, ("VideoAddDlg eCode:%d, wParam:%d, dwParam:%d",
            eCode, wParam, dwParam));
