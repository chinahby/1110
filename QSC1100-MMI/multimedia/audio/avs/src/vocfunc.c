...
$Header: //source/qcom/qct/multimedia/audio/6k/ver1/avs/main/latest/src/vocfunc.c#56 $ $DateTime: 2009/05/31 22:50:11 $ $Author: c_sampat $
...
voc_status_type voc_adec_adpcm_start (
  ...
  else if(check_interlock(VOC_UNLOCK_GRAPH)) {
    ret_val = VOC_CMD_FAILED_S;
  }
#endif /* FEATURE_GRAPH_ADPCM */
...
)
{
  ...
  if ((state->config == VOC_CAP_IS733)
   ...
   } else if (state->config == VOC_CAP_IS127) { /* IS127 - EVRC Service Option */
    switch (rate_factor) {
      case 8:                          /* rate 4/8 */
      case 0:                          /* 8/8 (1) */
        INTLOCK();
...
void voc_update_slowtalk_config( void )
{
  ...
  if (voc_check_in_call(state->acquire_mask) &&
      (check_interlock(VOC_UNLOCK_VOCODER)
#ifdef FEATURE_AVS_INCALL_MIXER_ADEC_PB
      || check_interlock(VOC_UNLOCK_MIXER_ADEC)
#endif /* FEATURE_AVS_INCALL_MIXER_ADEC_PB */
     ))
