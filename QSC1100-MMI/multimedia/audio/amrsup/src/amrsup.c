...
$Header: //source/qcom/qct/multimedia/audio/6k/common/amrsup/main/latest/src/amrsup.c#2 $ $DateTime: 2008/08/13 00:23:59 $ $Author: subhashj $
...
/* <EJECT> */
#if defined(FEATURE_WCDMA) || defined(FEATURE_AMR_IF1) \
    || defined(FEATURE_AUDFMT_AMR)
/*===========================================================================
...
#endif /* FEATURE_WCDMA || FEATURE_AMR_IF1 || FEATURE_AUDFMT_AMR */
...
)
{
  ...
  {
	MSG_ERROR("Invalid AMR_Mode : %d",amr_mode,0,0);
	return 0;
  }
  ...
  switch (frame_type)
  {
    ...
    default:
      MSG_ERROR("Unsupported frame type %d", frame_type, 0, 0);
      /* fall thru */
  ...
  switch (if1_frame_info->frame_type_index)
  {
    ...
    case AMRSUP_FRAME_TYPE_INDEX_AMR_SID:  /* = AMRSUP_FRAME_TYPE_INDEX_2385 */
      ...
      *amr_mode = (amrsup_mode_type)(if1_frame_info->frame_type_index);
      ...
      /* fall thru for AMR-NB SID frame */


    case AMRSUP_FRAME_TYPE_INDEX_AWB_SID:
      /* ===== Decoding SID frame ===== */
      ...
      *amr_mode = (amrsup_mode_type)(((if1_frame[4] >> 3) & 0x01)
      ...
      if (if1_frame_info->frame_type_index == AMRSUP_FRAME_TYPE_INDEX_AWB_SID)
      {
        *amr_mode = (amrsup_mode_type)(if1_frame[4] & 0x0f);
        *amr_mode = (amrsup_mode_type)((uint8)(*amr_mode) + AMRSUP_MODE_0660);
    ...
    case AMRSUP_FRAME_TYPE_INDEX_AWB_SPL:
      *frame_type = AMRSUP_SPEECH_LOST;
      *amr_mode = last_mode;
    ...
    default:
      MSG_ERROR("Unsupported frame type index %d", \
                if1_frame_info->frame_type_index, 0, 0);
      /* fall thru */
...
{
  uint32 dst_mask = 0x00000001 << ((*dst_bit_index) & 0x7);
  uint8  *dst = &dst_frame[((unsigned int) *dst_bit_index) >> 3];
  uint32 src_bit, src_mask;
...
{
  uint32 src_mask = 0x00000001 << ((*src_bit_index) & 0x7);
  uint8  *src = &src_frame[((unsigned int) *src_bit_index) >> 3];
  uint32 src_val  = (uint32) *src++;
  uint32 dst_bit, dst_mask;
  ...
  */
  ...
  switch (frame_type) {
    ...
    case AMRSUP_SID_UPDATE:
      ...
      */
      if2_frame[5] = (uint8)amr_mode & AMRSUP_FRAME_TYPE_INDEX_SPEECH_MASK;
...
)
{
  ...
  */
  ...
  if(mode <= (int)AMRSUP_MODE_1220) {
    ...
    */
    *amr_mode 
    = (amrsup_mode_type)(if2_frame[5] & AMRSUP_FRAME_TYPE_INDEX_SPEECH_MASK);
  ...
  switch (frame_type) {
    case AMRSUP_SID_FIRST:
      ...
      */
      ...
      /* Set the mode (Bit 36 - 38 = amr_mode with bits swapped)
      */
      amr_data[4] |= (((uint8)amr_mode << 3) & 0x08)
        | (((uint8)amr_mode << 1) & 0x04) | (((uint8)amr_mode >> 1) & 0x02);
      break;
...
)
{
  ...
  */
  ...
  switch (frame_type) {
    ...
    default:
      MSG_ERROR("The specified Frame Type is unsupported.", 0, 0, 0);
      break;
  }
