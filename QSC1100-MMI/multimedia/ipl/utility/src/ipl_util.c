...
$Header: //source/qcom/qct/multimedia/ipl/utility/main/latest/src/ipl_util.c#6 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/05/09   babakf  Added lp support to calc_median
...
)
{
  ...
  transparentR = ((transparentValue & 0xF800) >> 8);
  transparentG = ((transparentValue & 0x07E0) >> 3);
  transparentB = ((transparentValue & 0x001F) << 3);
  ...
  if (to_img_ptr == o_img_ptr)
  {
    for(row = 0; row < outHeight * outWidth; row++) 
    {
      if ((abs(transparentR - ((*inImgPtr & 0xF800) >> 8)) < thresh) && 
          (abs(transparentG - ((*inImgPtr & 0x07E0) >> 3)) < thresh) &&
...
)
{
  ...
  // cleanup histograms 
  memset(&img_hist->mv[0][0],0,IPL_MAX_HIST_CHAN*img_hist->size*sizeof(uint32));
...
API_EXTERN uint32 ipl_calc_median(ipl_image_type *in, ipl_rect_type *crop)
{
  ...
  else if (in->cFormat == IPL_YCrCb420_LINE_PK) 
  {
    ...
    p = in->clrPtr;
    p += (crop->x + crop->y*in->dx/2);
    pitch = (in->dx - crop->dx);
    for (y = 0; y < crop->dy/2; y++)
    {
      for (x = 0; x < crop->dx/2; x++)
      {
        Cr[(*p)]++;
        Cb[(*p)]++;
        p++;
