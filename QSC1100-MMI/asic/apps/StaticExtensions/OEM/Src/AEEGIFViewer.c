...
#error code not present
   ...
   // If no scaling required, blit the bitmap to screen
   if (pme->m_rc.dx == pme->m_pIDIB->cx && 
       pme->m_rc.dy == pme->m_pIDIB->cy) {
...
static void COEMGIFViewer_ScaleImage(COEMGIFViewer * pMe, int n1, int n2, int x, int y, AEERasterOp rop)
{
   ...
   if ((IBitmapScale*)0 != pMe->pScale && 
      ...
      IBitmap *pbmDst = IDisplay_GetDestination(pMe->m_pDisplay);
      ...
      if ((IBitmap*)0 != pbmDst) {
         ...
         IBitmap_GetInfo(pBitmap, &bi, sizeof(bi));

         rcSrc.x = pMe->m_xOffset;
         rcSrc.y = pMe->m_yOffset;
         rcSrc.dx = pMe->m_info.cxFrame;
         ...
         IBitmapScale_Scale(pMe->pScale, 
               pBitmap, &rcSrc, pbmDst, &rcDst, &rcClip, nTrans);
