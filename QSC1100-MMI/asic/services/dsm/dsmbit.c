...
$Header: //depot/asic/msmshared/services/dsm/dsmbit.c#9 $ $DateTime: 2007/01/31 13:23:20 $ $Author: mbonnet $
...
)
{
  ...
  uint8  num_bytes;            /* Number of bytes to extract */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (offset == 0)
  {
    ...
    if (item_ptr == NULL || length_packet != byte_offset)
    {
  ...
  else //offset != 0
  {
    ...
    if (item_ptr == NULL || length_packet != (byte_offset + 1))
    {
      return num_pushed_bits;
    }
  }
