...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/securemsm/crypto/main/latest/src/seccrypti.h#10 $ 
  $DateTime: 2009/04/15 10:58:57 $ $Author: etarolli $
...
typedef struct seccryptdsp_common_data_s   seccryptdsp_common_data_type;
...
typedef struct secpkxdsp_instance_s        secpkxdsp_instance_type;
typedef struct secencdsp_instance_s        secencdsp_instance_type;
typedef struct sechshdsp_instance_s        sechshdsp_instance_type;
...
/* -------------------------------------------------------------------- */
/* Wrapper to clean/invalidate selected data from the ARM data WB cache          */
/* -------------------------------------------------------------------- */

#ifdef FEATURE_SEC_WB_CACHE

#include "armmmu.h"

#define SECCRYPT_INVALIDATE_ARM_CACHE( __start_addr, __length )  \
          mmu_dcache_inval_region(__start_addr, __length)

#define SECCRYPT_CLEAN_WB_CACHE( __start_addr, __length )  \
          mmu_dcache_clean_region(__start_addr, __length)
#else

#include "cache_mmu.h"

#define SECCRYPT_INVALIDATE_ARM_CACHE( __start_addr, __length )  \
          mmu_invalidate_data_cache_lines(__start_addr, __length)
...
/* -------------------------------------------------------------------- */
#define SECCRYPT_INSTANCE_POOL_SIZE  20

/* -------------------------------------------------------------------- */
/* Pool size of available crypto commands                               */
/* -------------------------------------------------------------------- */
#define SECCRYPT_CMD_POOL_SIZE       20
...
/* -------------------------------------------------------------------- */
/* See platform definition in seccrypt.h */
...
struct seccrypt_instance_s
{
   ...
   seccrypt_platform_enum_type          op_platform;
   seccrypt_platform_enum_type          previous_platform;
   seccrypt_platform_enum_type          preferred_platform;
...
#if defined(FEATURE_SEC_IPC_SVC) || defined(CS_ISIGNAL)
   void                                 *ipc_req_msg;
#endif /* FEATURE_SEC_IPC_SVC || CS_ISIGNAL */
   ...
   struct
   {
      ...
      seccrypt_async_exec_cb_param_data_type  async_exec_cb;
...
#ifdef FEATURE_SEC_DEBUG
   struct
   {
      /* Shortcut to the state struct */
      struct seccrypts_state_info_s*     state_info_ptr;

      /* Shortcuts to the platform-specific structs */
      struct seccryptarm_common_data_s*  arm_info_ptr;
      struct seccryptdsp_common_data_s*  dsp_info_ptr;
      struct seccrypthw_common_data_s*  hw_info_ptr;

      /* Shortcuts to the algorithm-specific structs */
      struct
      {
         struct __pkx_common_info_s*      common_info_ptr;
         struct __pkx_mod_exp_storage_s*  mod_exp_storage_ptr;
         struct __pkx_dh_storage_s*       dh_storage_ptr;
         struct __pkx_rsa_storage_s*      rsa_storage_ptr;
      } pkx;

      struct
      {
         struct __enc_common_info_s*      common_info_ptr; 
         struct __enc_des_cipherinfo_s*   des_cipherinfo_ptr;
         struct __enc_des3_cipherinfo_s*  des3_cipherinfo_ptr;
         struct __enc_aes_cipherinfo_s*   aes_cipherinfo_ptr;
         struct __enc_arc4_cipherinfo_s*  arc4_cipherinfo_ptr;
      } enc;

      struct
      {
         struct __sechsh_info_s*          common_info_ptr;
      } hsh;
   ...
   union
   {
      ...
      seccryptdsp_common_data_type  dsp;
...
(
seccrypt_instance_type*      instance_ptr,
seccrypt_platform_enum_type  new_platform
);
...
(
seccrypt_instance_type*    instance_ptr,
seccrypt_param_enum_type   param,
seccrypt_param_type*       param_data_ptr
);
