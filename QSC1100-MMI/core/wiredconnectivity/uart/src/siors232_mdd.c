...
/*===========================================================================
  ...
  $Header: //depot/asic/msmshared/drivers/uart/02.00.XX/siors232_mdd.c#6 $ $DateTime: 2009/03/24 16:47:54 $ $Author: cmihalik $
...
---------------------------------------------------------------------------*/
LOCAL sio_port_id_type siors_mdd_stream_port[SIO_MAX_STREAM];
...
LOCAL void siors_mdd_callback_entry ( void *, siors_mdd_callback_type);
...
LOCAL void siors_mdd_schedule_empty_tx_fifo_chk
(
  sio_mdd_port_type   *mdd_port
)
{
  uint32                  i_save;        /* Save last interrupt state      */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
LOCAL void siors_mdd_call_stream_close_func_isr
(
  int32 time_ms, 
...
LOCAL void siors_mdd_schdl_stream_chng
(
  sio_mdd_port_type   *mdd_port,
...
{
  uint32 i_save;                         /* Temporary for saving current
                                             interrupt state for purposes 
                                             of locking interrupts         */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  INTLOCK_SAV( i_save);
...
SIDE EFFECTS
  None
===========================================================================*/
LOCAL void siors_mdd_time_stream_change_isr(int32 time_ms, timer_cb_data_type data)
{
  sio_mdd_port_type   *mdd_port;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  mdd_port = (sio_mdd_port_type *)data;
...
LOCAL void siors_mdd_check_no_cable( sio_mdd_port_type *mdd_port )
{
  uint32                  i_save;        /* Save last interrupt state      */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(mdd_port->is_clock_enabled)
...
SIDE EFFECTS
  None
===========================================================================*/
LOCAL void siors_mdd_check_no_cable_cb_isr(int4 dummy)
{
...
LOCAL void siors_mdd_get_more_data_for_tx
(
  sio_mdd_port_type   *mdd_port
)
{
...
===========================================================================*/
LOCAL void siors_mdd_tx_pkt_done
(
  sio_mdd_port_type   *mdd_port
)
{
...
LOCAL void siors_mdd_rx_hrd_fctl_tail_chr_isr
(
...
LOCAL void siors_mdd_rx_hrd_fctl_generic_isr
(
  sio_mdd_port_type   *mdd_port
)
{
...
LOCAL void siors_mdd_rx_hrd_fctl_packet_isr
(
  sio_mdd_port_type   *mdd_port
)
{
...
void siors_mdd_initialize_state
( 
  sio_mdd_port_type   *mdd_port
)
{
...
LOCAL sio_status_type siors_mdd_alloc_stream_rx
(
...
===========================================================================*/
void siors_mdd_dealloc_stream_rx
(
  sio_stream_id_type stream_id,              /* Stream ID                  */
  sio_mdd_port_type   *mdd_port              /* Port                       */
)
...
sio_status_type siors_mdd_alloc_stream_tx
(
  sio_stream_id_type   stream_id,            /* Stream ID                  */
  sio_mdd_port_type   *mdd_port,             /* Port                       */
...
void siors_mdd_unceremonious_close
(
  sio_stream_id_type stream_id,           /* Stream ID                     */
  sio_mdd_port_type   *mdd_port              /* Port                       */
)
...
void siors_mdd_dealloc_stream_tx
(
  sio_stream_id_type   stream_id,             /* Stream ID                   */
  sio_mdd_port_type   *mdd_port,              /* Port                        */
  void               (*close_func_ptr)(void)  /* Function to call when 
...
===========================================================================*/
#ifdef FEATURE_FIRST_UART
void siors_mdd_do_bitrate_change1 (void)
{
  sio_mdd_port_type   *mdd_port;
  mdd_port = &(siors_mdd_ports[UART_DEVICE_1]);
...
#ifdef FEATURE_SECOND_UART
void siors_mdd_do_bitrate_change2 (void)
{
  sio_mdd_port_type   *mdd_port;
  mdd_port = &(siors_mdd_ports[UART_DEVICE_2]);
...
#ifdef FEATURE_THIRD_UART
void siors_mdd_do_bitrate_change3 (void)
{
  sio_mdd_port_type   *mdd_port;
  mdd_port = &(siors_mdd_ports[UART_DEVICE_3]);
...
SIDE EFFECTS
  None
===========================================================================*/
void siors_mdd_sched_baudrate_change
(
  sio_stream_id_type stream_id,              /* Stream ID                  */
  sio_mdd_port_type   *mdd_port,
...
SIDE EFFECTS
  None
===========================================================================*/
LOCAL void siors_mdd_baudrate_change_now
(
  sio_mdd_port_type   *mdd_port,             /* Port                       */
  sio_bitrate_type     new_bitrate           /* new bitrate                */
)
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    case SIO_IOCTL_CD_ASSERT:
      ...
      //siors_set_dcd_output(rs_port, TRUE);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    case SIO_IOCTL_CD_DEASSERT:
      ...
      //siors_set_dcd_output(rs_port, FALSE);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    case SIO_IOCTL_ENABLE_DTR_EVENT:
      ...
      //siors_enable_dtr_event(rs_port, param);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    case SIO_IOCTL_DISABLE_DTR_EVENT:
      ...
      //siors_disable_dtr_event(rs_port, param);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    case SIO_IOCTL_DTE_READY_ASSERTED:
      ...
      // siors_check_dte_ready_asserted(rs_port, param);
...
===========================================================================*/
LOCAL void siors_mdd_rx_activity
(
  sio_mdd_port_type             *mdd_port 
)
{
...
===========================================================================*/
LOCAL void siors_mdd_callback_entry
(
    void *pHead,
