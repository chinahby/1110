...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_pm_super.h#3 $ $DateTime: 2008/12/12 09:42:18 $ $Author: richardp $
 ...
 * the member tables[2][SUPERBLOCK_PAGE_ENTRIES] from structure nand_info.
 * Any updates to structure superblock_data or structure nand_info should
 * reflect in SUPER_BL_DATA_ENT_SIZE. Any failure to update the
 * SUPERBLOCK_DATA_SIZE with the change to superblock_data or nand_info would
 * be caught at fs_pm_init () which would validate the calculations.
 */
#define SUPERBLOCK_DATA_SIZE   240

/* 8 here is derived from the structure member of nand_info
 * uint32 tables[2][SUPERBLOCK_PAGE_ENTRIES]  which is
 * (4 * 2 * SUPERBLOCK_PAGE_ENTRIES bytes)
 */
#define SUPERBLOCK_PAGE_ENTRIES  ((EFS_PAGE_SIZE - SUPERBLOCK_DATA_SIZE)/(8))
