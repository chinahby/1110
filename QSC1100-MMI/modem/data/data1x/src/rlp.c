...
INITIALIZATION AND SEQUENCING REQUIREMENTS
 ...
 Copyright (c) 1993-2008 by QUALCOMM Incorporated.  All Rights Reserved. 
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/data/1x/1xrlp/main/lite/src/rlp.c#4 $ $DateTime: 2008/08/28 01:54:40 $ $Author: spurthic $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/27/08   cs      Lint critical error fixed
07/29/08   sn      Fixed Klocwork errors.
02/07/08   sn      Changed PS_RLP to DSM_PS_RLP flag.
11/08/02   atp     Added freeing of meta_info to prevent memory leak.
01/14/02   rsl     Added ps.h to compile with FEATURE_IS95B_MDR undefined.
...
#include "ds.h"  /* Contains the enum for the UI last call status func.    */
#include "ps.h"
#include "rlp.h"
#include "dsm.h"
#include "crc.h"
...
LOCAL dword rlp_rx_stat[ RLP_ERASURE+1][ 4];
LOCAL dword rlp_tx_stat[ RLP_ERASURE+1][ 4];
LOCAL dword rlp_rx_stat_tot[ RLP_ERASURE+1][ 4];
LOCAL dword rlp_tx_stat_tot[ RLP_ERASURE+1][ 4];
...
)
{
  ...
  word isave;                         /* save status for interrupt locking */
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    ERR( "NAK Frame FIFO full, attempt: %d", attempt_num, 0, 0);
    return;
  }
  /*-------------------------------------------------------------------------
...
#error code not present
  ...
  {
    ERR( "Not all Re-xmits queued", 0, 0, 0);
    rlp_gen_stats.re_xmits_missed++;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    ERR( "RLP Reset: Max V(N/R), %d/%d", rlp_state_info.needed_seq_num,
               current_seq_num, 0);
    rlp_gen_stats.resets++;
    rlp_reset();
    return( TRUE);
...
#error code not present
      ...
      {
        MSG_HIGH("Hole in RLP sequence", 0, 0, 0);
      }
      /*---------------------------------------------------------------------
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  else if( GT_SEQ_CHECK( current_seq_num, rlp_state_info.needed_seq_num))
  {
    ...
    while( item_ptr != NULL)
    {
           ...
           current_seq_num)
      {
        ...
        -------------------------------------------------------------------*/
        ERR( "Reassy Dup: %u, V(N/R): %u/%u", current_seq_num,
                   rlp_state_info.needed_seq_num,
                   rlp_state_info.expected_seq_num);
        (void)dsm_free_buffer( new_item_ptr);
  ...
  {
    ERR("Old duplicate: %u, V(N/R): %u/%u", current_seq_num,
              rlp_state_info.needed_seq_num, rlp_state_info.expected_seq_num);
    (void)dsm_free_buffer( new_item_ptr);
  }
  /*------------------------------------------------------------------------
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  q_put( rlp_post_rx_q_ptr, &data_frame_ptr->link);
    ...
    case RLP_EIGHTH_RATE:
      ...
      if( primary_chan)
      {
            ...
            frame_ptr->pri_idle.nr_crc)
        {
          ...
          if( rlp_state_info.state == RLP_ESTABLISHED_STATE)
          {
            ERR( "RLP Idle NR CRC error", 0, 0, 0);
...
#error code not present
                 ...
                 pri_idle.seq_num     != rlp_saved_eighth.seq_num)
        {
          /*-----------------------------------------------------------------
            ...
            not incremented.
          -----------------------------------------------------------------*/
            ERR( "Idle no match to previous", 0, 0, 0);
            rlp_pri_frate_errs.idle_fr_err++;
            return_value         = RLP_RX_FRAME_ERR;
        }
        else   /* idle frame is good - so far */
        {
          ...
          else if( last2_idles_same)
          {
                              ...
                              rlp_state_info.expected_seq_num))
            {
              ...
              -----------------------------------------------------------*/
              ERR( "3 Matching 1/8, now: %d, was: %d",
                        frame_ptr->pri_idle.seq_num,
                        rlp_state_info.expected_seq_num, 0);
              frame_typ            = RLP_IDLE_FR;
              return_value         = RLP_RX_NO_DATA_AVAILABLE;
            }
...
#error code not present
    ...
    case RLP_FULL_RATE:
                 ...
                 rlp_prifull_table[rlp_mux].format_a)
      {
        /*-----------------------------------------------------------
          ...
          section of IS-707.
        -----------------------------------------------------------*/
        ERR( "Primary Full Rate type err", 0, 0, 0);
    ...
    case RLP_QUARTER_RATE:
         ...
         not a valid rateset & fr_rate combination */      
      {
        ...
        {
           MSG_HIGH( " Quarter rate frame for rateset 1 received ",0,0,0);
        }
        else
        {
          /* ---------------------------------------------------------
...
#error code not present
        ...
        {
           ERR( "Frame Err: %u, Rate: %u, Chan: %u", gen_frame_status, 
                   fr_rate, rlp_mux);
        }
  ...
  if( return_value == RLP_RX_FRAME_ERR)
  {
    ...
    if( rlp_state_info.state == RLP_ESTABLISHED_STATE) 
    {
      ...
      {
          ERR( "RLP Reset, Max Erasures: %d", rlp_state_info.contig_erase,
                0, 0);
          rlp_gen_stats.resets++;
          rlp_reset();
       }
  ...
  {
    ERR( "RLP Channel changed, Resync", 0, 0, 0);
    rlp_gen_stats.resets++;
    rlp_reset();
    (void)dsm_free_buffer( rlp_item_ptr);
    return( RLP_RX_NO_DATA_AVAILABLE);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  else if( rlp_state_info.nak_frames_cnt != 0)
  {
    ...
    if( rlp_nak_array[rlp_state_info.nak_out_index].in_process == FALSE)
    {
      ...
      ----------------------------------------------------------------------*/
      MSG_MED("Send NAK: %u - %u, attempt: %d",
  ...
  else if( rlp_state_info.re_xmit_frames_cnt != 0)
  {
    ...
    if( rlp_tx_segfr_active)
    {
      ...
      *frame_ptr                       = (byte *)&tx_frame.frame;
      ...
      (void)memcpy( (void *)tx_frame.frame.gen_data.data, 
