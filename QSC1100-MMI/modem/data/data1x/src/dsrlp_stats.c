...
$Header: //source/qcom/qct/modem/data/1x/1xrlp/main/lite/src/dsrlp_stats.c#1 $ $DateTime: 2007/11/06 01:21:33 $ $Author: nsivakum $
...
---------------------------------------------------------------------------*/
log_rlp_frames_type dsrlp_log_rx_rlp_frm_buf;
log_rlp_frames_type dsrlp_log_tx_rlp_frm_buf;
...
//vlechan
//boolean      dsrlp_log_rx_sch0_erasure = FALSE;
...
)
{
  ...
  (void) dsrlp_get_stats_ex( req_ptr->sr_id, 0, &rlp_stats_ptr );
  ...
  if( rlp_stats_ptr != NULL ) 
  {
    /*-----------------------------------------------------------------------
      Erase it all         
      -----------------------------------------------------------------------*/
    ts_get((unsigned long *) ( &rlp_stats_ptr->reset_timestamp ));
...
===========================================================================*/
log_rlp_pkt_type * dsrlp_log_rx_init_frame(void)
{
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (log_status(dsrlp_log_rx_rlp_frm_buf.hdr.code))
  {
    ...
    -----------------------------------------------------------------------*/
    if (dsrlp_log_rx_rlp_frm_buf.hdr.len >= LOG_RLP_FRM_BUF_THRESHOLD)
    {
      dsrlp_log_rx_send_frames();
    }
    ...
    if (frm_ptr == &dsrlp_log_rx_rlp_frm_buf.var_len_buf.pkt[0])
    {
      ...
      ---------------------------------------------------------------------*/
      ts_get((unsigned long *) dsrlp_log_rx_rlp_frm_buf.hdr.ts);
    }

    /*-----------------------------------------------------------------------
      Update packet length 
    -----------------------------------------------------------------------*/
    dsrlp_log_rx_rlp_frm_buf.hdr.len += FPOS(log_rlp_pkt_type, payload);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  -------------------------------------------------------------------------*/
  if (dsrlp_log_rx_rlp_frm_buf.hdr.len > FPOS(log_rlp_frames_type, var_len_buf))
  {
