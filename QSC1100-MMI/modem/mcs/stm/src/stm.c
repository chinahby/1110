...
Externalized Functions:
 ...
 - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  Copyright (c) 2002-2009 QUALCOMM Incorporated. All Rights Reserved
...
$Header: //source/qcom/qct/modem/mcs/stm/rel/1.0/src/stm.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/19/09   aps/sst Fixed memory leak
...
#define STM_ENTER_ATOMIC_SECT()  \
        {                      \
          INTLOCK();           \
          TASKLOCK();          \
        }

#define STM_LEAVE_ATOMIC_SECT()  \
        {                      \
          TASKFREE();          \
          INTFREE();           \
        }
...
(
  stm_group_type const    *group,              /* pointer to group */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( sm == NULL )
  {
    /* if the sm is NULL, make sure we free the payload */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* copy the message */
  memcpy((void*)dst_msg_header, (void*)src_msg_header,
...
(
  stm_group_type const *group,  /* group that the state machine is in */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  rec_sig = stm_get_wait_sigs(group) & filter_mask & rex_get_sigs(rex_self());
  ...
  for ( i = 0; (i < group->num_sigs) && (rec_sig != 0); i++ )
  {
    ...
    {
      /* This is the error case, where the signal mapper was unable
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while ( !group->stopped )
  {
    ...
    if ( proc_sig( group, (boolean)(!has_global_group) ) )
    {
      processed = TRUE;
      continue;
    }
    ...
    /* Process a signal for the global group (if there is one) that was not
       ...
       not. */
    if ( has_global_group &&
         proc_sig( group->global_group, TRUE ) )
    {
      processed = TRUE;
      continue;
    }
