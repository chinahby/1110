...
Initialization and Sequencing Requirements:
 ...
 - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  Copyright (c) 2002 - 2006 QUALCOMM Incorporated. All Rights Reserved
...
$Header: //source/qcom/qct/modem/mcs/stm/rel/1.0/src/stm_bm.c#1 $
...
/* Since interrupt can be unlocked on a task switch to make a section
   truely atomic we have to _both_ int lock and task lock. */
#define ATOMIC_SECT_BEGIN(save)    INTLOCK_SAV(save);TASKLOCK_SAV()
#define ATOMIC_SECT_END(save)      TASKFREE_SAV();INTFREE_SAV(save)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( list == NULL )
  {
    MSG_ERROR( "invalid parameter: list is NULL", 0, 0, 0 );
    return NULL;
  }

  if ( max_lists == 0 )
  {
    MSG_ERROR( "invalid parameter: max_lists is 0", 0, 0, 0 );
    return NULL;
  }
  
  #ifdef STM_BM_DEBUG
  MSG_HIGH( "stm_bm_create(%ld, 0x%lx)", max_lists, (uint32)list, 0 );
  #endif
...
)
{
  ...
  uint32            i;            /* loop counter */
  uint32            isave;        /* int save registers */

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if ( handle == NULL )
  {
    MSG_ERROR( "invalid parameter: handle is NULL", 0, 0, 0 );
    return FALSE;
  }

  if ( buf_mem == NULL )
  {
    MSG_ERROR( "invalid parameter: buf_mem is NULL", 0, 0, 0 );
    return FALSE;
  }

  if ( buf_size == 0 )
  {
    MSG_ERROR( "invalid parameter: buf_size = 0", 0, 0, 0 );
    return FALSE;
  }

  if ( n_buf == 0 )
  {
    MSG_ERROR( "invalid parameter: n_buf is 0", 0, 0, 0 );
    return FALSE;
  }
  
  #ifdef STM_BM_DEBUG
  MSG_HIGH( "stm_bm_add_buffer_list(0x%lx, %ld)", (uint32)handle, buf_size, 0 );
  MSG_HIGH( "stm_bm_add_buffer_list(,, %ld, 0x%lx)", n_buf, (uint32)buf_mem, 0 );
  #endif

  ATOMIC_SECT_BEGIN(isave);
  ...
  if ( new_list == NULL )
  {
    ATOMIC_SECT_END(isave);
    return FALSE;     /* buffer list is full */
  }
  ...
  ATOMIC_SECT_END(isave);
  return TRUE;    /* buffer list was added */
...
)
{
  ...
  uint32            isave;         /* interrupt register save storage */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( buf_size == 0 )
  {
    MSG_ERROR( "invalid parameter: buf_size is 0", 0, 0, 0 );
    return FALSE;
  }

  #ifdef STM_BM_DEBUG
  MSG_HIGH( "stm_bm_remove_buffer_list(0x%lx, %ld)", (uint32)handle, buf_size, 0);
  #endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( size == 0 )
  {
    MSG_ERROR( "invalid parameter: size is 0", 0, 0, 0 );
    return NULL;
  }
  ...
  #ifdef STM_BM_DEBUG
  MSG_HIGH( "stm_bm_get_buffer(0x%lx, %ld) returned 0x%lx",
...
)
{
  ...
  uint32            isave;    /* interrupt save registers */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( buf == NULL )
  {
    MSG_ERROR( "invalid parameter: buffer ptr is NULL", 0, 0, 0 );
    return FALSE;
  }

  #ifdef STM_BM_DEBUG
  MSG_HIGH( "stm_bm_return_buffer(0x%lx, 0x%lx)", (uint32)handle, (uint32)buf, 0);
  #endif
