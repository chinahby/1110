...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_mnand_log.c#1 $ $DateTime: 2008/12/12 11:11:10 $ $Author: richardp $
                   ...
                   fs_mnand_super_t super, int fresh_start)
{
  ...
  fs_paranoia_register (&mnand_log_page, sizeof(mnand_log_page),
                        "mnand_log_page");
...
fs_mnand_start_txn_in_new_buffer (fs_mnand_log_t log)
{
  ...
  memcpy (log_buff, (log_buff + log->start_txn_idx), curr_txn_size);
                     ...
                     uint32 arg1, uint32 arg2, uint32 arg3, uint32 arg4)
{
  ...
  for (i=0; i<arg_cnt; ++i)
  {
    memcpy (&log->log_page->buff[log->buff_idx], &args[i], 4);
    log->buff_idx += 4;
  }
}
...
fs_mnand_log_replay_logs (fs_mnand_log_t log, void *pm_data)
{
  ...
  do
  {
    ...
    for (idx = 0; idx <= FS_MNAND_LOG_MAX_TRANS_BYTES; )
    {
      ...
      {
        memcpy (&args[i], (uint32*)&log->log_page->buff[idx], 4);
        idx += 4;
      }
