...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/inc/caii.h#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $
...
typedef struct
{
  ...
  caii_nnscr_num_bits_record_type recs[16];
...
typedef struct
{
  ...
  caii_nnscr_var_rate_record_type recs[16];
...
typedef struct
{
  ...
  caii_nnscr_ltu_record_type row[16];
...
typedef struct
{
  ...
  caii_nnscr_partition_mux_type row[32];
...
typedef struct
{
  ...
  byte num_bits_tables_count;   /* Number of instances of the flexible rate
                                   table included */
  caii_nnscr_num_bits_table_type bit_table[8];
  ...
  byte var_rate_tables_count;   /* Number of instances of the variable rate
                                   mask table included */
  caii_nnscr_var_rate_table_type var_table[8];
  ...
  caii_nnscr_ltu_table_type table[4];
  ...
  caii_nnscr_partition_table_type part_table[4];
...
#if !defined(FEATURE_MSG_PACK_UNIT_TEST) && defined(FEATURE_IS2000_REL_A)
...
typedef struct
{
  ...
  byte rev_fch_rc_map;    /* R-FCH RC information */
...
#endif /* FEATURE_IS2000_REL_A_CC || FEATURE_IS2000_REL_B */
...
#define CAII_UNIV_PAGE_PARTIAL_ADDR_MATCH 0xffffffff
#define CAII_UNIV_PAGE_FIRST_SEG          0xff
#define CAII_UNIV_PAGE_SECOND_SEG         0
#define CAII_EXT_MS_SDU_LEN_MAX           15
...
typedef struct
{
  byte config_msg_seq;       /* Configuration message sequence number */
  byte acc_msg_seq;          /* Access parameters message sequence number */
  boolean read_next_slot;    /* Pages are carried into next slot */
  boolean read_next_slot_bcast;
    /* Enhanced broadcast pages are carried into next slot */
} caii_univ_page_common_type;
...
/* Page record format 0 */
typedef struct
{
  byte msg_seq;           /* Message sequence number */
  dword imsi_s_33_16;     /* 33rd - 16th bits of IMSI_S */
  boolean ext_ms_sdu_length_incl;
    /* Additional MS record indicator */
  byte ext_ms_sdu_length; /* Length of additional MS record (only present if
                             ext_ms_sdu_length_incl is set to 1) */
  word service_option;    /* Service option */
  byte add_ms_record[CAII_EXT_MS_SDU_LEN_MAX];
    /* Additional MS record (only present if ext_ms_sdu_length is > 0) */
} caii_univ_page_rec_fmt_0_type;

/* Page record format 1 */
typedef struct
{
  byte msg_seq;                   /* Message sequence number */
  byte imsi_11_12;                /* The 11th and 12th digits of the IMSI */
  dword imsi_s_33_16;             /* 33rd - 16th bits of IMSI_S */
  boolean ext_ms_sdu_length_incl; /* Additional MS record indicator */
  byte ext_ms_sdu_length;         /* Length of additional MS record (only
                                     present if ext_ms_sdu_length_incl is set to
                                     1) */
  word service_option;            /* Service option */
  byte add_ms_record[CAII_EXT_MS_SDU_LEN_MAX];
    /* Additional MS record (only present if ext_ms_sdu_length is > 0) */
} caii_univ_page_rec_fmt_1_type;

/* Page record format 2 */
typedef struct
{
  byte msg_seq;                   /* Message sequence number */
  word mcc;                       /* Mobile country code */
  ...
  byte add_ms_record[CAII_EXT_MS_SDU_LEN_MAX];
    /* Additional MS record (only present if ext_ms_sdu_length is > 0) */
} caii_univ_page_rec_fmt_2_type;

/* Page record format 3 */
typedef struct
{
  byte msg_seq;                   /* Message sequence number */
  word mcc;                       /* Mobile country code */
  ...
  byte add_ms_record[CAII_EXT_MS_SDU_LEN_MAX];
    /* Additional MS record (only present if ext_ms_sdu_length is > 0) */
} caii_univ_page_rec_fmt_3_type;

/* Page record format 4 */
typedef struct
{
  byte msg_seq;                   /* Message sequence number */
  byte imsi_addr_num;             /* Number of IMSI address digits */
  ...
  byte add_ms_record[CAII_EXT_MS_SDU_LEN_MAX];
    /* Additional MS record (only present if ext_ms_sdu_length is > 0) */
} caii_univ_page_rec_fmt_4_type;

/* Page record format 5 */
typedef struct
{
  byte msg_seq;                   /* Message sequence number */
  byte imsi_addr_num;             /* Number of IMSI address digits */
  ...
  byte add_ms_record[CAII_EXT_MS_SDU_LEN_MAX];
    /* Additional MS record (only present if ext_ms_sdu_length is > 0) */
} caii_univ_page_rec_fmt_5_type;

/* Page record format 6 */
typedef struct
{
  byte reserved_len;
  byte reserved[31];
} caii_univ_page_rec_fmt_6_type;

/* Page record format 7 */
typedef struct
{
  byte reserved_len;
  byte reserved[31];
} caii_univ_page_rec_fmt_7_type;

/* Page record format 8 */
typedef struct
{
  byte msg_seq;                   /* Message sequence number */
  word tmsi_code_addr_31_16;      /* 31st - 16th bits of TMSI code */
  boolean ext_ms_sdu_length_incl; /* Additional MS record indicator */
  byte ext_ms_sdu_length;         /* Length of additional MS record (only
                                     present if ext_ms_sdu_length_incl is set to
                                     1) */
  ...
  byte add_ms_record[CAII_EXT_MS_SDU_LEN_MAX];
    /* Additional MS record (only present if ext_ms_sdu_length is > 0) */
} caii_univ_page_rec_fmt_8_type;

/* Page record format 9 */
typedef struct
{
  byte msg_seq;                   /* Message sequence number */
  byte tmsi_code_addr_23_16;      /* 23rd - 16th bits of TMSI code */
  boolean ext_ms_sdu_length_incl; /* Additional MS record indicator */
  byte ext_ms_sdu_length;         /* Length of additional MS record (only
                                     present if ext_ms_sdu_length_incl is set to
                                     1) */
  ...
  byte add_ms_record[CAII_EXT_MS_SDU_LEN_MAX];
    /* Additional MS record (only present if ext_ms_sdu_length is > 0) */
} caii_univ_page_rec_fmt_9_type;

/* Page record format 10 */
typedef struct
{
  byte msg_seq;                   /* Message sequence number */
  boolean ext_ms_sdu_length_incl; /* Additional MS record indicator */
  ...
  byte add_ms_record[CAII_EXT_MS_SDU_LEN_MAX];
    /* Additional MS record (only present if ext_ms_sdu_length is > 0) */
} caii_univ_page_rec_fmt_10_type;

/* Page record format 11 */
typedef struct
{
  byte msg_seq;                   /* Message sequence number */
  byte tmsi_zone_len;             /* Length of TMSI zone */
  qword tmsi_zone;                /* TMSI zone */
  word tmsi_code_addr_31_16;      /* 31st - 16th bits of TMSI code */
  boolean ext_ms_sdu_length_incl; /* Additional MS record indicator */
  byte ext_ms_sdu_length;         /* Length of additional MS record (only
  ...
  byte add_ms_record[CAII_EXT_MS_SDU_LEN_MAX];
    /* Additional MS record (only present if ext_ms_sdu_length is > 0) */
} caii_univ_page_rec_fmt_11_type;

/* Page record format 12 */
typedef struct
{
  byte burst_type;
  byte addr_len;
  byte bc_addr[CAII_BC_ADDR_MAX];
} caii_univ_page_rec_fmt_12_type;

/* Page record fromat 13.0 */
typedef struct
{
  word reserved;
} caii_univ_page_rec_fmt_13_0_type;

/* Page record fromat 13.1 */
typedef struct
{
  byte reserved;
} caii_univ_page_rec_fmt_13_1_type;

/* Page record fromat 13.3 */
typedef struct
{
  byte reserved_len;
  byte reserved[17];
} caii_univ_page_rec_fmt_13_3_type;

/* Page record fromat 14.0 */
typedef struct
{
  word reserved;
} caii_univ_page_rec_fmt_14_0_type;

/* Page record fromat 14.1 */
typedef struct
{
  qword reserved;
} caii_univ_page_rec_fmt_14_1_type;

/* Page record fromat 14.2 */
typedef struct
{
  qword reserved;
} caii_univ_page_rec_fmt_14_2_type;

/* Page record fromat 14.3 */
typedef struct
{
  byte reserved_len;
  byte reserved[31];
} caii_univ_page_rec_fmt_14_3_type;

/* Page record format 15.0 */
typedef struct
{
  byte addr_len;                    /* Length of broadcast address */
  byte bc_addr_remainder[CAII_BC_ADDR_MAX-2];
    /* Remaining octets of broadcast address */
  byte ext_bcast_sdu_length_ind; /* Additional broadcast record indicator */
  byte ext_bcast_sdu_length;
    /* Length of additional broadcast record (only present if
       ext_bcast_sdu_length_ind is set to 2 or 3) */
  byte bcn;                  /* Index of BCCH */
  word time_offset;                 /* Time offset */
  byte repeat_time_offset;
    /* Repeat time offset (only present if ext_bcast_sdu_length_ind is set to 1
       or 3) */
  byte add_bcast_record[CAII_EXT_BC_SDU_LEN_MAX];
    /* Additional broadcast record (only present if ext_bcast_sdu_length_ind is
       set to 1 or 3) */
} caii_univ_page_rec_fmt_15_0_type;

/* Page record fromat 15.3 */
typedef struct
{
  byte reserved_len;
  byte reserved[31];
} caii_univ_page_rec_fmt_15_3_type;
...
typedef struct
{
  ...
  union
  {
    caii_univ_page_rec_fmt_0_type    fmt0;
    caii_univ_page_rec_fmt_1_type    fmt1;
    caii_univ_page_rec_fmt_2_type    fmt2;
    caii_univ_page_rec_fmt_3_type    fmt3;
    caii_univ_page_rec_fmt_4_type    fmt4;
    caii_univ_page_rec_fmt_5_type    fmt5;
    caii_univ_page_rec_fmt_6_type    fmt6;
    caii_univ_page_rec_fmt_7_type    fmt7;
    caii_univ_page_rec_fmt_8_type    fmt8;
    caii_univ_page_rec_fmt_9_type    fmt9;
    caii_univ_page_rec_fmt_10_type   fmt10;
    caii_univ_page_rec_fmt_11_type   fmt11;
    caii_univ_page_rec_fmt_12_type   fmt12;
    caii_univ_page_rec_fmt_13_0_type fmt13_0;
    caii_univ_page_rec_fmt_13_1_type fmt13_1;
    caii_univ_page_rec_fmt_13_3_type fmt13_3;
    caii_univ_page_rec_fmt_14_0_type fmt14_0;
    caii_univ_page_rec_fmt_14_1_type fmt14_1;
    caii_univ_page_rec_fmt_14_2_type fmt14_2;
    caii_univ_page_rec_fmt_14_3_type fmt14_3;
    caii_univ_page_rec_fmt_15_0_type fmt15_0;
    caii_univ_page_rec_fmt_15_3_type fmt15_3;
  } type;
} caii_univ_page_rec_type;

/* Page block of Unsegmented Universal Page Message */
typedef struct
{
  word offset;                        /* Offset to start next translation */
  word length;                        /* Length of page block in bits */
  byte data[CAI_FCCCH_BODY_SIZE];     /* Page block */
#ifdef FEATURE_IS2000_REL_A_CC_BCSMS
  word first_page_rec_offset;         /* Offset of first page record */
#endif /* FEATURE_IS2000_REL_A_CC_BCSMS */
} caii_univ_page_block_type;

/* Unsegemented Universal Page Message */
typedef struct
{
#ifdef FEATURE_MSG_PACK_UNIT_TEST
#error code not present
#endif /* FEATURE_MSG_PACK_UNIT_TEST */
  byte msg_type;                                         /* Message type */
  caii_univ_page_common_type common;                     /* Common fields */
  caii_univ_page_interleaved_addr_type interleaved_addr; /* Partial address */
#ifdef FEATURE_MSG_PACK_UNIT_TEST
#error code not present
#else
  caii_univ_page_block_type page_block; /* Page block info */
#endif /* FEATURE_MSG_PACK_UNIT_TEST */
} caii_univ_page_type;

/* Universal Page Segment message */
typedef struct
{
#ifdef FEATURE_MSG_PACK_UNIT_TEST
#error code not present
#endif /* FEATURE_MSG_PACK_UNIT_TEST */
  byte msg_type;           /* Message type */
  byte segment_seq;        /* Segment sequence number */
  word page_seg_block_len; /* Length of page segment block in bits*/
  byte page_seg_block[CAI_FCCCH_BODY_SIZE]; /* Page segment block */
} caii_univ_page_seg_type;

/* Messsage field from which to start next translation */
typedef enum
{
  CAII_UPM_CONFIG_MSG_SEQ,
  CAII_UPM_BCAST_INCLUDED,
  CAII_UPM_NUM_BCAST,
  CAII_UPM_IMSI_INCLUDED,
  CAII_UPM_NUM_IMSI,
  CAII_UPM_TMSI_INCLUDED,
  CAII_UPM_NUM_TMSI,
  CAII_UPM_RES_TYPE_INCLUDED,
  CAII_UPM_NUM_RES_TYPE,
  CAII_UPM_BURST_TYPE,
  CAII_UPM_ADDR_BLOCK,
  CAII_UPM_DONE
} caii_univ_page_field_type;

/* ---------------------------------------------------------------------
** Per Release A LAC standard requirement, the Base Station shall not
** transmit PDUs derived from the same Universal Page Block in different
** FCCCH slots. So the largest reassembly buffer need for segmented
** Universal Page Message is (38400 * 80 * 1000 / 8) = 384 bytes.
** --------------------------------------------------------------------- */
#define CAII_SEG_UPM_MAX_MSG_BYTES 384

/* Page block of Segmented Universal Page Message */
typedef struct
{
  word offset;                           /* Offset to start next translation */
  word length;                           /* Length of page block in bits */
  byte data[CAII_SEG_UPM_MAX_MSG_BYTES]; /* Page block */
#ifdef FEATURE_IS2000_REL_A_CC_BCSMS
  word first_page_rec_offset;            /* Offset of first page record */
#endif /* FEATURE_IS2000_REL_A_CC_BCSMS */
} caii_seg_univ_page_block_type;

/* Segmented Universal Page buffer */
typedef struct
{
  boolean valid;
    /* Indicates if buffer contains valid segmented Universal Page message */
  caii_univ_page_field_type msg_field;
    /* Messsage field from which to start next translation */
  qword frame;
    /* Frame that First UPM segment received */
  byte segment_seq;
    /* Segment sequence number */
  caii_univ_page_common_type common;
    /* Common fields */
  caii_univ_page_interleaved_addr_type interleaved_addr;
    /* Interleaved address fields */
  caii_seg_univ_page_block_type page_block;
} caii_seg_univ_page_type;
...
#if defined(FEATURE_PASSPORT_MODE) || defined(FEATURE_MSG_PACK_UNIT_TEST)
...
#ifdef FEATURE_IS2000_REL_D
#error code not present
#endif                       /* identifiers to be restored */
...
/* Use caii_extended_alert_w_info_type from Extended Alert With Information
 ...
 typedef struct
 {
    ...
    byte      d_sig_encrypt_mode;
    byte      num_recs;
    caii_security_record_type recs[7];
    byte      key_size;
...
#endif /* FEATURE_IS2000_REL_A */
#ifdef FEATURE_IS2000_REL_A_CC
  caii_a41_sys_parm_type          a41spm;
  caii_mc_rr_parm_type            mcrrpm;
  caii_a41_rand_type              a41rm;
  caii_en_ac_parm_type            enacpm;
  caii_univ_nl_type               unlm;
  caii_univ_page_type             upm;
  caii_univ_page_seg_type         upsm;
#endif /* FEATURE_IS2000_REL_A_CC */
#ifdef FEATURE_IS2000_REL_C
