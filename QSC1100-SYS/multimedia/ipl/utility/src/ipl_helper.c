...
$Header: //source/qcom/qct/multimedia/ipl/utility/main/latest/src/ipl_helper.c#6 $
...
// decide if we want to log memory usage or not
  ...
  #define ipl_mm_malloc(a,b,c,d)  mm_malloc(a,b,c,d); 
  #define ipl_mm_free(a,b,c)      mm_free(a,b,c);  
#else
  #define IPL_GROUP 1 //lint !e750
  #define ipl_mm_malloc(a,b,c,d)  malloc(a);
  #define ipl_mm_free(a,b,c)      free(a);
#endif
...
// feature should be defined to save a few ms by avoiding malloc/free 30 
...
static const char extension[32][15] = 
{
  ...
  "argb888",     /* RGB0 data (32 bit word, padded on low order)    */
...
)
{
  ...
  else
  {
    ...
    // therefore, lets malloc
    MSG_LOW("ipl_malloc_img: mallocing memory\n", 0,0,0);

	  retval = (uint8 *) ipl_mm_malloc(bytes, IPL_GROUP, __FILE__, __LINE__);
...
DESCRIPTION
  This function frees memory alloced by ipl_malloc

DEPENDENCIES
  None

ARGUMENTS IN
  in  points to the buffer to free
...
===========================================================================*/
API_EXTERN void ipl_free (void * in)
{
  MSG_LOW("ipl_free: freeing bytes\n", 0,0,0);
  ...
  {
    if (in == NULL)
      MSG_LOW("ipl_free_img: not going to free NULL imgPtr\n", 0,0,0);
    else
    {
      MSG_LOW("ipl_free_img: freeing imgPtr\n", 0,0,0);
      ipl_mm_free(in, __FILE__, __LINE__);
    }
  }
  in = NULL;
}
...
)
{
  ...
  MSG_LOW("ipl_malloc_img: requesting %d bytes\n", isize,0,0);
  MSG_LOW("ipl_malloc_img: requesting %d bytes\n", csize,0,0);
  ...
  else
  {
    // Even though we have buffer, it was not big enough to use  
    // therefore, lets malloc
	  in->imgPtr = (uint8 *)ipl_mm_malloc(isize, IPL_GROUP, __FILE__, __LINE__);

	  if (in->imgPtr == NULL)
    {
      MSG_LOW("ipl_malloc_img: malloc failed!\n", 0,0,0);
      return IPL_NO_MEMORY;
    }

	  //malloc(isize);
	  //in->imgPtr = malloc(isize);
    ...
    if (csize != 0)
    {
	    in->clrPtr = (uint8 *) ipl_mm_malloc(csize,IPL_GROUP, __FILE__, __LINE__);
	    if (!in->clrPtr)
      {
        free(in->imgPtr);
        return IPL_NO_MEMORY;
...
DESCRIPTION
  This function is used to free memory for a given image of type ipl_image_type
...
{
  MSG_LOW("ipl_free_img: freeing bytes\n", 0,0,0);
  ...
  else
  {
    ...
    {
      if (in->clrPtr == NULL)
        MSG_LOW("ipl_free_img: not going to free NULL clrPtr\n", 0,0,0);
      else
      {
        MSG_LOW("ipl_free_img: releasing clrPtr\n", 0,0,0);
	      ipl_mm_free(in->clrPtr, __FILE__, __LINE__);
      }
    }
  }
  in->imgPtr = NULL;
...
ipl_debug_write_tile(ipl_image_type *in, const ipl_rect_type *crop, const char * str)
{
  ...
  {
    MSG_HIGH("Could not malloc memory for temp image\n", 0,0,0);
    return IPL_NO_MEMORY;
  }
