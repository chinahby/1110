...
Notice that changes are listed in reverse chronological order.
  ...
  $Header: //source/qcom/qct/multimedia/mmservices/mpeg4fileformat/parser/main/latest/src/videofmt.c#1 $
...
)
{
    ...
    default:
        ...
        MSG_ERROR ("video_fmt_open: Invalid file format code: %d",
                   format, 0, 0);
...
)
{
    ...
    {
    case VIDEO_FMT_MP4:
        video_fmt_mp4w_create (params, callback_ptr, client_data);
        break;

    case VIDEO_FMT_INVALID:
    default:
        /* Indicate the format is invalid or not supported. */
        MSG_ERROR ("video_fmt_create: Invalid file format code: %d",
                   format, 0, 0);
        callback_ptr (VIDEO_FMT_FAILURE, client_data, NULL, NULL);
    }
}
