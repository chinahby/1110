...
$Header: //depot/asic/msmshared/services/pbm/pbm_conversion.c#13 $
...
#ifdef FEATURE_MMGSDI /* No SIM cards mean no SIM encoding is needed. */
...
static int pbmconvert_unicode_compare(const void *first_arg, const void *second_arg);


#ifdef FEATURE_MMGSDI /* No SIM cards mean no SIM encoding is needed. */
#if !defined(FEATURE_UI_DUALPROC_APPS) /* The Apps side in dual processor build
                                          build does not care about SIM cards */
...
static void pbmconvert_parse_input(const void *in_str, uint16 in_len,
                                   SIM_CODING_MODES *out_mode, uint16 *out_len,
...
static boolean pbmconvert_convert_ucs2_to_ucs2_80( uint16 in_ucs2, uint8 **out_octets,
                                                   uint16 *out_len, uint16 out_base);

static boolean pbmconvert_convert_ucs2_to_ucs2_81 (uint16 in_ucs2, uint8 **out_octets,
                                                   uint16 *out_len, uint16 out_base);

static boolean pbmconvert_convert_ucs2_to_ucs2_82 (uint16 in_ucs2, uint8 **out_octets,
                                                   uint16 *out_len, uint16 out_base);

static boolean pbmconvert_convert_ucs2_to_gsm7    (uint16 in_ucs2, uint8 **out_octets,
                                                   uint16 *out_len, uint16 out_base);


static uint16 pbmconvert_pbm8_source(const void **in_ptr);
static uint16 pbmconvert_ucs2_source(const void **in_ptr);

static boolean pbmconvert_convert_one_ucs2_to_pbm8(uint16 ucs2_char, uint8 *pbm8_ptr);


static boolean pbmconvert_convert_one_ucs2_to_gsm7(uint16 ucs2_char, uint8 *gsm7_ptr,
                                                   uint16 *gsm7_len);

static boolean pbmconvert_sim_ucs2_to_ucs2(uint8 **in_string, uint16 *in_len, 
                                           uint16 **out_octets, uint16 ucs2_base);
static boolean pbmconvert_sim_gsm7_to_ucs2(uint8 **in_string, uint16 *in_len,
                                           uint16 **out_octets, uint16 ucs2_base);
static boolean pbmconvert_sim_ucs2_halfpage_to_ucs2(uint8 **in_string, uint16 *in_len,
                                                    uint16 **out_octets,
                                                    uint16 ucs2_base);
...
{
  SIM_CODING_MODES out_mode = SIM_CODING_GSM7;
...
static void pbmconvert_parse_input(const void *in_str, uint16 in_len,
                                   ...
                                   PFSOURCE_FN src_fn, boolean gsm7_only)
{
  int     index;                // Loop variable over input string.
  uint16  ucs2_char;            // Unicode character we are converting.
  uint16  output_size;          // Octets possible to use.

  uint16  curr_gsm7_length;     // Current Unicode needs how many GSM7 octets?
  ...
  uint16  best_converts;        // To determine which format to use, save best.

  /* Make sure no NULL pointers are given. */
  PBM_CHECK_PTR_RET(in_str,      VOID);
  PBM_CHECK_PTR_RET(out_mode,    VOID);
  PBM_CHECK_PTR_RET(out_len,     VOID);
  PBM_CHECK_PTR_RET(out_base,    VOID);
  PBM_CHECK_PTR_RET(src_fn,      VOID);
  PBM_CHECK_PTR_RET(out_converts,VOID);
       ...
       index++)
  {
    ...
    if (pbmconvert_convert_one_ucs2_to_gsm7(ucs2_char, gsm7_chars,
                                          &curr_gsm7_length))
    {
    ...
    if (!gsm7_only)
    {
      ...
      {
        /* Could it be a GSM7 character?  If not, try 0x81 format. */
        if (curr_gsm7_length == 0)
        {
          ...
          else
          {
            ...
            else
            {
              ...
              {
                /* Nope, we can't use this any longer. */
                use_pattern1 = FALSE;
              }
            }
          }
        ...
        if (use_pattern1)
        {
          ...
          /* If it was not encoded as GSM7, it took one octet. */
          if (curr_gsm7_length == 0) {
            ucs81_len++;
          } else {
            /* GSM7 may take one or two. */
      ...
      {
        /* Could it be a GSM7 character?  If not, try 0x82 format. */
        if (curr_gsm7_length == 0)
        {
     ...
     */
    if (all_gsm7) { if (gsm7_len >= output_size) break; }
    else if (use_pattern1) { if (ucs81_len >= output_size) break; }
    else if (use_pattern2) { if (ucs82_len >= output_size) break; }
    else if (ucs80_len >= output_size) break;
  }
  ...
  *out_converts = best_converts;
  ...
  if (!gsm7_only)
  {
    ...
    if (ucs82_converts > best_converts)
    {
      ...
      // All characters meet GSM7 or are the remaining ones within 128 of each
      // other?
                                           ...
                                           uint16 **out_octets, uint16 ucs2_base)
{
  ...
  /* Check input parameters. */
  PBM_CHECK_PTR_RET(in_string, TRUE);
  PBM_CHECK_PTR_RET(*in_string, TRUE);
  PBM_CHECK_PTR_RET(out_octets, TRUE);
  PBM_CHECK_PTR_RET(*out_octets, TRUE);
  PBM_CHECK_PTR_RET(in_len, TRUE);
                                           ...
                                           uint16 **out_octets, uint16 ucs2_base)
{
  ...
  if ((*in_len >= 1) && ((curr_value = **in_string) != 0xFF))
  {
    *in_string += 1;        // We looked at one character.
    ...
    {
      PBM_MSG_HIGH("Unexpected character with high bit set 0x%x", curr_value, 0, 0);
          ...
          if (second_octet == 0x1b)
          {
            ...
            *in_len-=2;
            ...
            if (*in_len++ >= 1) 
            {
              ...
              convert_gsm7_to_unicode((*in_len++ & 0x7f),
                                      (uint16 *) *out_octets);
              *out_octets+=1;
              *in_len-=1;
            }
                                ...
                                uint16 *out_octets, uint16 out_len)
{
  ...
  /* Check input parameters. */
  PBM_CHECK_PTR_RET(in_str, retval);
  PBM_CHECK_PTR_RET(out_octets, retval);
...
static int pbmconvert_unicode_compare(const void *first_arg, const void *second_arg)
{
  ...
  PBM_CHECK_PTR_RET(first, 0);
  PBM_CHECK_PTR_RET(second, 0);
...
void pbmconvert_init_conversion(void)
{
  ...
  memcpy(unicode_to_pbm8, pbm8_to_unicode, sizeof(unicode_to_pbm8));
...
#if !defined(FEATURE_UI_DUALPROC_APPS) /* The Apps side of a dual proc
                                          build does not care about SIM */
  // copy gsm7_to_unicode into unicode_to_gsm7;
  memcpy(unicode_to_gsm7, gsm7_to_unicode, sizeof(unicode_to_gsm7));
...
static uint16 pbmconvert_pbm8_source(const void **in_ptr)
{
  ...
  /* Check input parameters. */
  PBM_CHECK_PTR_RET(in_ptr, ret_val);
  PBM_CHECK_PTR_RET(*in_ptr, ret_val);
...
/*===========================================================================
FUNCTION pbmconvert_ucs2_source

DESCRIPTION
  uint16 pbmconvert_ucs2_source(const void **in_ptr)

  in_ptr: [in/out] Source pointer that is moved after we get the octet.

  Used to abstract the fact we may be presented with either a PBM8 or
  UCS2 string.  This function just returns the given value, incrementing
  the pointer by two.
                                ...
                                uint8 *pbm8_str, uint16 pbm8_str_len)
{
  ...
  /* Parameter checking. */
  PBM_CHECK_PTR_RET(ucs2_str, FALSE);
  PBM_CHECK_PTR_RET(pbm8_str, FALSE);
                ...
                boolean s1_is_wstr, boolean s2_is_wstr)
{
   ...
   */
  if (s1_is_wstr) {
    src_fn1 = pbmconvert_ucs2_source;
  } else {
    src_fn1 = pbmconvert_pbm8_source;
  }
  if (s2_is_wstr) {
    src_fn2 = pbmconvert_ucs2_source;
  } else {
