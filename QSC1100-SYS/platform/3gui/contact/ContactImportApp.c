...
/*=============================================================================
  ...
  $Header: //depot/asic/msmshared/apps/Contact/ContactImportApp.c#9 $
...
09/10/03   adm     Added features for DUAL SLOT 
...
static const AEEAppInfo saiAppList[] = {
   {
...
/*=============================================================================
FUNCTION: ContactImportMod_GetModInfo
...
=============================================================================*/
PFNMODENTRY ContactImportMod_GetModInfo(IShell      *ps,
                                        AEECLSID   **ppClasses,
                                        AEEAppInfo **pApps,
                                        uint16      *pnApps,
                                        uint16      *pwMinPriv)
{
   ...
   *ppClasses = NULL;
   *pApps = (AEEAppInfo *) saiAppList;
   *pnApps = ARR_SIZE(saiAppList);
   return(PFNMODENTRY)ImportAppMod_Load;
}
      ...
      case EVT_APP_START:
         {
            ...
            IMENUCTL_GetRect(pMe->m_softKey,
                             &rc);
            IMENUCTL_SetActive(pMe->m_softKey,
                               TRUE);
            ...
            rc.dy = rc.y;
            rc.y = as->rc.y;
            ISTATIC_SetRect(pMe->m_staticBox,
                            &rc);
            ...
            // Setup the initial screen
               ...
               if (bFileExists) {
                  (void) SNPRINTF(pMe->m_szMsgBuf, sizeof(pMe->m_szMsgBuf),
         ...
         switch (wParam) {
            ...
            case ID_SK_STARTEXPORT:
               ...
               CImportApp_AddSoftKeyItem(pMe->m_softKey,
                                         "Cancel",
                                         ID_SK_CANCEL);
               STRLCPY(pMe->m_szMsgBuf, "Exporting:", sizeof(pMe->m_szMsgBuf));
            ...
            case ID_SK_EXIT:
               (void) ISHELL_CloseApplet(pMe->a.m_pIShell,
                                         FALSE);
               return TRUE;
...
static boolean CImportApp_InitImportTable(CImportApp *pMe)
{
   ...
   pMe->m_importTbl = MALLOC(sizeof(ItemImportTableEntry)*(IMPORTITEM_COUNT+1));
...
DESCRIPTION: Ensures that the specified number contains valid digits. Letters
             are replaced by appropriate numbers and other characters are
             ignored.
...
static void CImportApp_CheckNumber(AECHAR *num)
{
   while (*num) {
      if ( ( (*num < (AECHAR)'0') || (*num > (AECHAR)'9') ) &&
         ...
         default:
            {
               // The following code ignores non-valid characters which are
               // non-letters
               AECHAR *tmp_num;

               for (tmp_num = num; *tmp_num != '\0'; tmp_num++) {
                  *tmp_num = *(tmp_num + 1);
               }
               num--;
            }
...
static void CImportApp_ExportRecord(CImportApp *pMe)
{
   ...
   for (;;) {
      ...
      // Don't export FDN and SDN entries...
      if ((AEE_ADDR_CAT_SIM_FDN == IADDRREC_GetCategory(pMe->m_rec)) || 
          (AEE_ADDR_CAT_SIM_FDN2 == IADDRREC_GetCategory(pMe->m_rec)) ||
          (AEE_ADDR_CAT_SIM_SDN == IADDRREC_GetCategory(pMe->m_rec)) ||
          (AEE_ADDR_CAT_SIM_SDN2 == IADDRREC_GetCategory(pMe->m_rec)))
      {
         IADDRREC_Release(pMe->m_rec);
         pMe->m_rec = NULL;
      } else {
   ...
   for (j=0;j<nTotalFields;j++) {
      ...
      if (fld.fID == AEE_ADDRFIELD_NAME) {
         if (bNameFound == FALSE) {
            bNameFound = TRUE;
            SNPRINTF(pMe->m_szBuf, sizeof(pMe->m_szBuf),
            ...
            if ((AEE_ADDR_CAT_SIM_ADN != wCategory) || (AEE_ADDR_CAT_SIM_ADN2 != wCategory))
#endif /* FEATURE_MMGSDI*/
            {
               switch (wCategory) {
               case AEE_ADDR_CAT_BUSINESS:
                  STRLCPY(pMe->m_szBuf, IMPORTITEM_BIZCONT, sizeof(pMe->m_szBuf));
                  break;
               case AEE_ADDR_CAT_PERSONAL:
                  STRLCPY(pMe->m_szBuf, IMPORTITEM_PERCONT, sizeof(pMe->m_szBuf));
                  break;
               default:
                  STRLCPY(pMe->m_szBuf, IMPORTITEM_DEFCONT, sizeof(pMe->m_szBuf));
                  break;
               }

               STRLCAT(pMe->m_szBuf, NL, sizeof(pMe->m_szBuf));
         ...
         if (fieldStr != NULL) {
            STRLCPY(pMe->m_szBuf, fieldStr, sizeof(pMe->m_szBuf));
            STRLCAT(pMe->m_szBuf, ": ", sizeof(pMe->m_szBuf));
            STRLCAT(pMe->m_szBuf, pMe->m_szBuf96, sizeof(pMe->m_szBuf));
            STRLCAT(pMe->m_szBuf, NL, sizeof(pMe->m_szBuf));
   ...
   STRLCPY(pMe->m_szBuf, IMPORTITEM_NEXTREC, sizeof(pMe->m_szBuf));
   STRLCAT(pMe->m_szBuf, NL, sizeof(pMe->m_szBuf));
