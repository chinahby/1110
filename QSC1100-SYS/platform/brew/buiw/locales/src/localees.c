...
// Format variables, strings, characters and patterns for Spanish (Spain)
...
/* -----------------------------------------------------------------------
    LocaleES_Ctor 
   --- */
static void LocaleES_Ctor(LocaleES *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleES_New  
   --- */
int LocaleES_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleES *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleES, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleES_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule); 
...
int LocaleES_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALEES) {
      return LocaleES_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
