...
// Format variables, strings, characters and patterns for UK ENGLISH
...
/* -----------------------------------------------------------------------
    CreateInstance
   --- */
int xModule_LocaleENGBCreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   // Note, when implementing your own ILocale object, obtain
   // a valid BREW class id from the class id generator online
   // and use it here.
   ...
   if (dwItem != -1) {
      ...
      if (pMe->gtFormats[dwItem].cFlags & FORMATTABLE_FLAG_INT) {
         ...
         if (pawNew) {
            ...
            (void)WSTRLCPY(pawNew, (AECHAR *)p1, (uint32)WSTRLEN((AECHAR *)pawNew));
...
static int Locale_wstrncmp(ILocale *po, AECHAR *s1, AECHAR *s2, unsigned int length)
{
   // UK English - as implemted by this ILocale - can use BREW's library function
   (void)po;
...
/* -----------------------------------------------------------------------
    ...
    For UK English, lines are broken at the first hard break or last
