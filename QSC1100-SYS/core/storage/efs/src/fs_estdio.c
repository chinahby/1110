 ...
 *
 *  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_estdio.c#1 $ $DateTime: 2009/01/30 16:38:40 $ $Author: richardp $
 *
...
#define STDIO_INIT_LOCK(s)     rex_init_crit_sect(&s->f_lock)
/** Request a lock */
#define STDIO_LOCK(s)  rex_enter_crit_sect(&s->f_lock)
/** Release a lock */
#define STDIO_UNLOCK(s)  rex_leave_crit_sect(&s->f_lock)
 ...
 */
rex_crit_sect_type efs_fdescriptor_cs;
...
efs_estdio_init (void)
{
  ...
  if (efs_statvfs ("/", &efs_info))
    FS_ERR_FATAL ("efs_stdio init failed", 0, 0, 0);

  efs_block_size = efs_info.f_bsize;

  /* Check if block size is power of two. */
  if ((efs_block_size & (efs_block_size - 1)) != 0)
    FS_ERR_FATAL ("efs_stdio block size invalid", 0, 0, 0);

  rex_init_crit_sect (&efs_fdescriptor_cs);
 ...
 * use efs_fget_rbk_file_fd() to get the file descriptor of the rollback file.
...
alloc_fpointer (void)
{
  ...
  rex_enter_crit_sect (&efs_fdescriptor_cs);
  ...
  rex_leave_crit_sect (&efs_fdescriptor_cs);
...
efs_efopen (const char *path, const char *mode)
{
  ...
  fp->file_pos = efs_lseek (fp->file_desc, 0, SEEK_CUR);
 ...
 * occurs. It also takes care calling efs_lseek() to change the file position.
 ...
 */
static fs_size_t
read_from_device (char *data_out, fs_size_t bytes, int fd, fs_off_t file_pos)
{
  int nread;
  uint32 total = 0;

  /* Seek to the current file position */
  if (efs_lseek (fd, file_pos, SEEK_SET) == -1)
    return (0);
...
efs_efread (void *ptr, fs_size_t size, fs_size_t nitems, EFS_EFILE *stream)
{
  ...
  if (FULLY_BUFFERED (stream))
  {
    ...
    if (bytes_from_buffer > 0)
    {
      ...
      memcpy (ptr, stream->buffer + stream->curr_index, bytes_from_buffer);
    ...
    if (bytes_to_read > 0)
    {
      ...
      if ((bytes_to_buffer) >= (2 * bytes_to_read))
      {
        ...
        memcpy (ptr, stream->buffer, bytes_to_read);
...
write_to_device (char *ptr, fs_size_t bytes, EFS_EFILE* stream)
{
  ...
  if (efs_lseek (stream->file_desc, stream->file_pos, SEEK_SET) == -1)
    return 0;
...
efs_efwrite (void *ptr, fs_size_t size, fs_size_t nitems, EFS_EFILE *stream)
{
  ...
  if (FULLY_BUFFERED (stream))
  {
    ...
    if (bytes_to_buffer >= bytes_to_write || stream->byte_count != 0)
    {
      ...
      memcpy (stream->buffer + stream->curr_index, ptr, bytes_to_buffer);
    ...
    else
    {
      ...
      memcpy (stream->buffer, ptr, bytes_to_write);
...
efs_efseek (EFS_EFILE *stream, long offset, int whence)
{
  ...
  if (whence == SEEK_END)
  {
    result = efs_lseek (stream->file_desc, offset, whence);
  ...
  else
  {
    if (whence == SEEK_SET)
      ...
      result = stream->file_pos + stream->curr_index + offset;
  ...
  if (!OK_TO_WRITE (stream) && stream->file_pos <= result
      && (uint32) result < (uint32) (stream->file_pos + stream->byte_count))
  {
    stream->curr_index = result - stream->file_pos;
  }
  else
  {
...
efs_eftell (EFS_EFILE *stream)
{
  ...
  else
  {
    ...
    if (stream->stream_errno != 0 || stream->state == STREAM_INACTIVE ||
      ...
      result = stream->file_pos + stream->curr_index;
