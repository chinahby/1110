...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_efs2.c#7 $ $DateTime: 2008/12/12 11:11:10 $ $Author: richardp $
...
#error code not present
 ...
 fsb_buffer_initialize (&static_fs_buf, fs_pm);
  ...
  tmp = mp->cluster_size / (sizeof (struct efs2_inode));
  tmp2 = 0;
  while (tmp > 1) {
    tmp >>= 1;
    tmp2++;
...
inode_io (struct fs_mount_efs *mp, fs_inode_t inum,
    struct efs2_inode *inode, int op)
{
  fs_cluster_id clust;
  int offset;
  struct efs2_inode *data;
...
m_create_inode (struct fs_mount *mpp, fs_mode_t mode, uint32 gid, uint32 uid)
{
  ...
  if (result == 0)
  {
    ...
    if (vp != NULL)
    {
      ...
      fs_efs2_inode_construct (&vp->p.inode, mode, gid, uid);
