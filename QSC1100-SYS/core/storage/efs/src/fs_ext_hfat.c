...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_ext_hfat.c#4 $ $DateTime: 2009/01/30 12:47:45 $ $Author: richardp $
...
e_hfat_readdir (void *iterator, struct fs_dirent *dirent)
{
  ...
  switch (iter->state)
  {
    case ITER_STATE_DOT:       /* First fake root entry: "." */
      ...
      fs_strlcpy (dirent->d_name, ".", sizeof (dirent->d_name));
      break;
    ...
    case ITER_STATE_NORMAL:
...

      {
        ...
        fs_strlcpy (dirent->d_name, utf8_name, sizeof (dirent->d_name));
...
static uint32
fs_hfat_time_to_epoch (uint16 hfat_time, uint16 hfat_date)
{
  uint32 epoch_time;
  ...
  epoch_time = fs_time_jul_to_secs (&hfat_jul);

  /* Convert to EPOCH time by adding the delta CDMA_TO_FS_TIME_DELTA */
  epoch_time += CDMA_TO_FS_TIME_DELTA;
...
e_hfat_getstat (const char *name, struct fs_stat *buf)
{
  ...
  else
  {
    ...
    buf->st_ctime = fs_hfat_time_to_epoch (fstat.modifiedtime,
                                           fstat.modifieddate);
