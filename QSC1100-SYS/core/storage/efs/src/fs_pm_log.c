...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_pm_log.c#4 $ $DateTime: 2008/12/12 09:42:18 $ $Author: richardp $
                  ...
                  fs_log_page_write_cb_t log_page_write_cb, void *priv_data)
{
  ...
  log->last_written_log = FS_INVALID_PAGE_ID;
...
#error code not present
  ...
  {
    static FS_ALIGN uint8 verify_buffer[FS_LOG_PAGE_SIZE];
    int result;
    result = fs_device_read_page (log->dev,
        log->queue[log->q_flush],
        verify_buffer,
        FS_FOP_LOG);
