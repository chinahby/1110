...
#define STD_ARRAY_SIZE(a) (sizeof((a))/sizeof((a)[0]))
...
extern unsigned long std_swapl(unsigned long ul);
...
Description:
   ...
   takes the size of the destination buffer as an argument and guarentees 
...
Return Value:
   Length of the string in characters that preceed the terminating NUL 
   character.
...
Description:
   The std_strcmp() compares two null-terminated character strings.
...
Description:
   The std_strncmp() compares at most n bytes of two null-terminated character strings.
...
Description:
   The std_stricmp() compares two null-terminated character strings, case-folding any 
     ASCII characters.
...
Description:
   The std_strnicmp() compares at most n bytes of 2 null-terminated character strings, 
...
Description:
   ...
   destination buffer as an argument and guarentees to zero-terminate the
   result and not to overflow the nDestSize size.
   
   This function copies up to size-1 characters from the pszSrc string 
...
Comment:
  ...
  Howver, calling strlcpy() with UINT_MAX will result in the entire source
...
Description:
   ...
   guarentees not to create an improperly terminated string and not to
...
Parameters:
   ...
   cpszNeedle: null-terminated string to search for
...
Prototype:
   ...
   void *std_memrchrbegin(const void* s, int c, int nLen);
...
Description:
   The std_memchrsend() finds the first occurrence of a list of 
...
Description:
   The std_strchrsend()finds the first occurrence of a list of 
...
Parameters:
   s: string to search
   cpszSrch: a null-terminated list of characters to search for
...
Description:
   ...
   portion, using forward slashes, adding necessary slashes and deleting extra 
    slashes.  This function guarantees null-termination of pszDest
...
Comments:
   Passing an "fs:/" path to this function may produce undesireable 
...
Description:
   ...
   output to a string.  These functions guarantee null-termination of
...
===pre>
   ...
   ======|=======================================================
     c   | Output a single character.
         |
     s   | Output a null-terminated single-byte character string.
         |
...
===/pre>   
   ...
   specifies the minimum number of digits to output.  Values are paddded
...
===/pre>   
   ...
   specifier, but "j" has been introduced in ISO C99.
