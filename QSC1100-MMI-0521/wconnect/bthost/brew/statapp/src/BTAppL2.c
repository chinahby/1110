...
$Header: //source/qcom/qct/wconnect/bthost/brew/statapp/rel/00.00.26/src/BTAppL2.c#1 $
...
boolean BTApp_BuildL2ResultsMenu( CBTApp* pMe )
{
  ...
  if ( TOP_MENU != BT_APP_MENU_L2_RESULTS )
  {
    ...
    ISHELL_LoadResString( pMe->a.m_pIShell, BTAPP_RES_FILE, IDS_L2, 
  ...
  ISHELL_LoadResString( pMe->a.m_pIShell, BTAPP_RES_FILE, IDS_LABEL_KBITS_PER_SEC,
                        wBuf1, sizeof( wBuf1 ) );
  WSPRINTF( &pMe->pText1[ uLen], 
            LONG_TEXT_BUF_LEN - uLen,
            wBuf1,
...
boolean BTApp_BuildL2TestMenu( CBTApp* pMe )
{
  ...
  // set the title
  IMENUCTL_SetTitle( pMe->m_pIMenu, BTAPP_RES_FILE, IDS_L2, NULL );
