...
AECHAR *FormatPhoneNumberForDisplay(AECHAR const *number)
{
   ...
   if (*srcEnd != (AECHAR) '\0') {
      ...
      MEMCPY(&buf[destLen], srcEnd, (uint32) extraLen * sizeof(AECHAR));
   }
...
void ClockCtl_Init(ClockCtl *ctl,
                   ...
                   boolean bLargeFnt)
{
    ctl->m_focus = CLOCKCTL_FOCUS_NONE;
    ctl->m_dwDispTime = 0;
    ctl->m_largeFnt = bLargeFnt;
    ctl->m_props = 0;
    ctl->m_minInc = 1;
    ...
    ITIMECTL_AddRef(ctl->m_pt);
    IDISPLAY_AddRef(ctl->m_pd);
    ISHELL_AddRef(ctl->m_ps);

    (void) ISHELL_CreateInstance(ctl->m_ps,
                                 AEECLSID_CONFIG,
                                 (void **)&(ctl->m_pcfg));
...
{
    if (ctl->m_pt) {
       ITIMECTL_Release(ctl->m_pt);
       ctl->m_pt = NULL;
    }

    if (ctl->m_pd) {
       IDISPLAY_Release(ctl->m_pd);
       ctl->m_pd = NULL;
    }

    if (ctl->m_ps) {
       ISHELL_Release(ctl->m_ps);
       ctl->m_ps = NULL;
    }

    if (ctl->m_pcfg) {
       ICONFIG_Release(ctl->m_pcfg);
       ctl->m_pcfg = NULL;
    }
}
...
=============================================================================*/
void ClockCtl_SetTime(ClockCtl *ctl, uint32 newTime)
{
    ctl->m_dwDispTime = newTime;
}
...
{
    AEEFont           fnt;
    AECHAR            timeString[CLOCKCTL_MAX_TIMESTR_LEN];
    AECHAR            oneChar[2] = {0,0};
    int               i, pnFits;
    int16             strPixWidth, charPixHt;
    uint32            dwFlags;
    AEERect           rc;

    ITIMECTL_GetRect(ctl->m_pt, &rc);
    ...
    // Clear the area
    IDISPLAY_FillRect(ctl->m_pd, &rc, RGB_WHITE);

    GetTimeString(ctl->m_pcfg, ctl->m_pt, ctl->m_ps,
                  (ctl->m_dwDispTime % CLOCKCTL_MS_IN_DAY),
                  timeString);

    if (ctl->m_focus == CLOCKCTL_FOCUS_NONE) {
       // Simple draw in case no character needs to be highlighted
       (void) IDISPLAY_DrawText(ctl->m_pd,
                                fnt,
                                timeString,
                                -1,
                                0, // param ignored due to IDF_ALIGN_MIDDLE flag
                                0, // param ignored due to IDF_ALIGN_CENTER flag
                                &rc,
                                IDF_ALIGN_CENTER | IDF_ALIGN_MIDDLE
                                );
    }
    else {
       // tough part, some characters need highlighting, draw char by char
       // compute the length and width of the string and figure out where
       // to align the string in the display
       strPixWidth = (int16) IDISPLAY_MeasureTextEx(ctl->m_pd,
                                                    fnt,
                                                    timeString,
                                                    -1,
                                                    (int) rc.dx,
       ...
       charPixHt = (int16) IDISPLAY_GetFontMetrics(ctl->m_pd,
                                                   fnt,
                                                   NULL,
                                                   NULL
                                                   );
       rc.x = rc.x + (rc.dx - strPixWidth) / 2;
       rc.y = rc.y + (rc.dy - charPixHt) / 2;
       rc.dy = charPixHt;

       // print character by character after deciding whether to highlight
       for (i = 0; i < WSTRLEN(timeString); i++) {
          dwFlags = IDF_ALIGN_CENTER;
          switch (ctl->m_focus) {
          case CLOCKCTL_FOCUS_HOUR:
             if ((i == 0) || (i == 1)) {
                dwFlags |= IDF_TEXT_INVERTED;
             }
             break;
          case CLOCKCTL_FOCUS_MIN:
             if ((i == 3) || (i == 4)) {
                dwFlags |= IDF_TEXT_INVERTED;
             }
             break;
          case CLOCKCTL_FOCUS_AMPM:
             if ((i == 6) || (i == 7)) {
                dwFlags |= IDF_TEXT_INVERTED;
             }
             break;
          ...
          oneChar[0] = timeString[i];
          rc.dx = (int16) IDISPLAY_MeasureText(ctl->m_pd,
                                               fnt,
                                               oneChar);
          (void) IDISPLAY_DrawText(ctl->m_pd,
                                   fnt,
...
boolean ClockCtl_HandleKeyEvent(ClockCtl *ctl, AVKType key)
{
    switch (key) {
    case AVK_UP:
       // UP key bumps up the vaious fields of time (Hour, Min or AM/PM)
       switch (ctl->m_focus) {
       case CLOCKCTL_FOCUS_HOUR:
          ctl->m_dwDispTime += CLOCKCTL_MS_IN_HOUR;
          if (ctl->m_dwDispTime >= CLOCKCTL_MS_IN_DAY) {
             ctl->m_dwDispTime -= CLOCKCTL_MS_IN_DAY;
          }
          break;
       case CLOCKCTL_FOCUS_MIN:
          ctl->m_dwDispTime += CLOCKCTL_MS_IN_MIN * ctl->m_minInc;
          if (ctl->m_dwDispTime >= CLOCKCTL_MS_IN_DAY) {
             ctl->m_dwDispTime -= CLOCKCTL_MS_IN_DAY;
          }
          break;
       case CLOCKCTL_FOCUS_AMPM:
          if (ctl->m_dwDispTime >= 12 * CLOCKCTL_MS_IN_HOUR) {
             ctl->m_dwDispTime -= 12 * CLOCKCTL_MS_IN_HOUR;
          }
          else {
             ctl->m_dwDispTime += 12 * CLOCKCTL_MS_IN_HOUR;
          }
    ...
    case AVK_DOWN:
       switch (ctl->m_focus) {
       case CLOCKCTL_FOCUS_HOUR:
          if (ctl->m_dwDispTime < CLOCKCTL_MS_IN_HOUR) {
             ctl->m_dwDispTime += CLOCKCTL_MS_IN_DAY - CLOCKCTL_MS_IN_HOUR;
          }
          else {
             ctl->m_dwDispTime -= CLOCKCTL_MS_IN_HOUR;
          }
          break;
       case CLOCKCTL_FOCUS_MIN:
          if (ctl->m_dwDispTime < CLOCKCTL_MS_IN_MIN * ctl->m_minInc) {
             ctl->m_dwDispTime += CLOCKCTL_MS_IN_DAY -
                                             CLOCKCTL_MS_IN_MIN * ctl->m_minInc;
          }
          else {
             ctl->m_dwDispTime -= CLOCKCTL_MS_IN_MIN * ctl->m_minInc;
          }
    ...
    case AVK_RIGHT:
       ...
       switch (ctl->m_focus)  {
       case CLOCKCTL_FOCUS_HOUR:
          ctl->m_focus = CLOCKCTL_FOCUS_MIN;
          break;
       case CLOCKCTL_FOCUS_MIN:
             ctl->m_focus = CLOCKCTL_FOCUS_AMPM;
          break;
       case CLOCKCTL_FOCUS_AMPM:
          ctl->m_focus = CLOCKCTL_FOCUS_HOUR;
          break;
       default:
          return FALSE;
       }
