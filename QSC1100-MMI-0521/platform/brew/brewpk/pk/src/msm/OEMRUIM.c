...
static int OEMRUIM_GetCHVStatus(IRUIM *pMe, AEECHVStatus *pCHVStatus)
{
   ...
   if ((gCallBack.rpt_type != UIM_STATUS_R)
      ...
      if (gCallBack.rpt.status.num_bytes >= 22) {
         ...
         data = (uim_sel_mf_df_rsp_type*)gCallBack.rpt.status.data;

         // Check CHV1 'enabled' status (Note that the bit comes back from the UIM inverted) 
         pCHVStatus->chv1_enabled = (data->file_char & 0x80) ? 0 : 1;

         // Remaining CHV1 count
         pCHVStatus->chv1CountRemain = data->chv1_status & 0x0f;
         //* Remaining Unblock CHV1 count 
         pCHVStatus->chv1UnblockRemain = data->uchv1_status & 0x0f;
         // Remaining CHV2 count 
         pCHVStatus->chv2CountRemain = data->chv2_status & 0x0f;
         //* Remaining Unblock CHV2 count 
         pCHVStatus->chv2UnblockRemain = data->uchv2_status & 0x0f;
#else
...
#define RUIM_ID_SIZE ((word)(sizeof(((nv_item_type *) 0)-> esn.esn )))
   ...
   if (OEM_SVCGetConfig(CFGI_CARDID_LEN, (void*)pnLen, sizeof(int)) != SUCCESS)
   {
      return EFAILED;
   }
   ...
   if ((OEM_SVCGetConfig(CFGI_CARDID, (void *)pId, *pnLen) == SUCCESS))
   {
      return AEE_SUCCESS;
   }
   else
...
===========================================================================*/
extern void oemui_rex_wait(rex_sigs_type waitSig);
