...
$Header: //depot/asic/msmshared/apps/brew_3.1.5/pk/src/OEMJPEG.c#26 $
...
uint32  COEMJPEGViewer_Release(IImage *po)
{
  ...
  if(pme && pme->m_nRefs > 0)
  {
    ...
    else
    {
      ...
      pme->m_pOEMJPEGViewerSys->pOEMJPEGViewer = NULL;
    }
...
static void COEMJPEGViewer_ReadableCB(COEMJPEGViewer *pme)
{
  ...
  case JPEG_READ_STATE:
    ...
    if((pme->m_nRead) > 0)
    {
      ...
      if(((isJPEGEXIFHeaderValid = jpeg_exifread(&pme->m_ExifInfo, pme->m_pJPEG, (uint32)pme->m_nRead))) ||
         (jpeg_jfifread(&pme->m_ExifInfo, pme->m_pJPEG, (uint32)pme->m_nRead)))
      {
                          ...
                          void *server_data)
{
  ...
  if(pmeSys && (!pmeSys->pOEMJPEGViewer))
  {
    switch(command)
    {
