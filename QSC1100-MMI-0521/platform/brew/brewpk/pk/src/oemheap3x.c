...
$Header: //depot/asic/msmshared/apps/brew_3.1.5/pk/src/oemheap3x.c#8 $
...
#define MAX_SRC_FILE_NAME 32
typedef struct _HNode
{
   uint32        dwSize;
   struct _HNode *pPrev;
   char           szFile[MAX_SRC_FILE_NAME];
   uint32        dwLine;
...
static void  HeapFreeInternal(MemHeap *heap, void * ptr)
{
   ...
   if (pb->bFree == DELAYED_FREE && df_enabled)
   {
       ...
       DBGPRINTF("HeapFreeInternal free block %p: DOUBLE FREE (SAVED LOGS)!", ptr, 0, 0);
   ...
   if(pb->bFree == TRUE)
   {
      DBGPRINTF("HeapFreeInternal free block %p: DOUBLE FREE!", ptr, 0, 0);
#ifdef FEATURE_ASSERT_ON_DOUBLE_FREE
      ASSERT(FALSE);
#endif//FEATURE_ASSERT_ON_DOUBLE_FREE
      return;
...
// #ifdef HEAP_DEBUGGING
   ...
   {
      DBGPRINTF("HeapFreeInternal free block %p: FREEING GARBAGE!", ptr, 0, 0);
      //ASSERT(FALSE);
      return;
   }
// #endif
...
__inline void str_cpyn(char *pDest, int cbCpy, char *pSrc)
{
	while(cbCpy-- > 0)	{
		if('\0' == *pSrc)
			break;
...
void *Heap_DbgMark(void *p, char *pszPath, uint32 dwLine)
{
   if(p && pszPath && pszPath[0])
   {
#if MIN_BREW_VERSIONEx(3,1,5)
      HNode *pb = PTR_TO_NODE(((char *)p - (sizeof(uint32)*2)));
#else
      HNode *pb = PTR_TO_NODE(((char *)p - sizeof(uint32)));
#endif

      char *pszFile = strrchr(pszPath, '/');
      if(NULL == pszFile)
         pszFile = strrchr(pszPath, '\\');
      if(NULL == pszFile)
         pszFile = pszPath;
      else
         pszFile++;

      str_cpyn(pb->szFile, MAX_SRC_FILE_NAME-1, pszFile);
      pb->dwLine = dwLine;
   }
...
void print_alloc_pos(void *ptr)
{
    ...
    if (ptr)
    {
         ...
         DBGPRINTF("Filename: %s, Line: %d", pb->szFile, pb->dwLine);
    }
}
...
void *HeapReallocInternal(MemHeap *heap, void * pIn, uint32 size)
{
   ...
   if (df_enabled && pb->bFree == DELAYED_FREE)
   {
       ...
       DBGPRINTF("HeapReallocInternal free block %p: DOUBLE FREE (SAVED LOGS)!", pIn, 0, 0);
#ifdef FEATURE_ASSERT_ON_DOUBLE_FREE
       ASSERT(FALSE);
#endif//FEATURE_ASSERT_ON_DOUBLE_FREE
       return (NULL);
   ...
   if(pb->bFree == TRUE)
   {
      DBGPRINTF("HeapReallocInternal free block %p: DOUBLE FREE!", pIn, 0, 0);
#ifdef FEATURE_ASSERT_ON_DOUBLE_FREE
      ASSERT(FALSE);
#endif//FEATURE_ASSERT_ON_DOUBLE_FREE
      return (NULL);
...
// #ifdef HEAP_DEBUGGING
   ...
   {
      DBGPRINTF("HeapReallocInternal free block %p: FREEING GARBAGE!", pIn, 0, 0);
      //ASSERT(FALSE);
      return (NULL);
   }
// #endif
...
ORNew:
   ...
   while(1)
   {
      ...
      if (pb)
      {
         ...
         //STRCPY(pb->szFile, g_dbg_malloc_filename);
         //pb->dwLine = g_dbg_malloc_line;
         pb->szFile[0] = '\0';
         pb->dwLine = 0;
         pb->bFree = FALSE;
...
// in the event HEAPTRACKER is turned on
   ...
   /* DEBUG Code
   DBGPRINTF("GetMemStats: Tot: %d, Max: %d, Free: %d", heap->dwTotalBytes, max,
...
static HNode * PushUnlockedNode(MemHeap *heap, HNode * pb)
{
   ...
   {
      DBGPRINTF("PushUnlockedNode invalid node ptr %p", pb, 0, 0);
#ifdef HEAP_DEBUGGING
      ASSERT(FALSE);
#endif
      return(NULL);
...
unsigned long AEEkHeap_Unlock(AEEkHeap *pHeap, void *p)
{
   ...
   {
      DBGPRINTF("AEEkHeap_Unlock free block %p", p, 0, 0);
#ifdef HEAP_DEBUGGING
      ASSERT(FALSE);
#endif
   }
...
uint32 AEEkHeap_GetMemStatsExt(uint32 * pdwTotal, uint32 * pdwMax)
{
    if  (NULL != gHeap3_x)
    {
        return AEEkHeap_GetMemStats((AEEkHeap *)gHeap3_x, pdwTotal, pdwMax);
    }
