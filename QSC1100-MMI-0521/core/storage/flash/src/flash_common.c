...
/*===========================================================================
  ...
  $Header: //depot/asic/msmshared/drivers/flash/MSM_FLASH.01.04/flash_common.c#13 $
...
SIDE EFFECTS
  None
===========================================================================*/
void flash_initialize_device_structure()
{
  int i=0;
...
SIDE EFFECTS
  None
===========================================================================*/
fsi_device_t flash_allocate_device_structure()
{
  int i = 0;
  ...
  {
    FLASH_ERR_FATAL(" Exceeded Max clients supported",0,0,0);
  }
...
SIDE EFFECTS
  None
===========================================================================*/
void flash_release_device_structure(fsi_device_t device_desc)
{
  int i = 0;
  ...
  {
    FLASH_ERR_FATAL(" Superflous close attempted",0,0,0);
  }
...
fs_device_t flash_open(char *partition_name, int whichflash)
{
  ...
  /* Do pointer NULL checking here so that subsequence calls that have
   * the param fsi_device passed do not need to check any more
   */
  if (fsi_device == NULL)
  {
    FLASH_ERR_FATAL(
      " No available client for the device", 0, 0, 0);
  }
  ...
  if (result == FS_DEVICE_OK)
  {
    ...
    if ((fs_device->open_partition != 0) && (partition_name != NULL))
    {
    ...
    else if(partition_name != NULL)
    {
      ...
      if ((strncmp (partition_name, MIBIB_DEVICE_ALL_PARTI_NAME,
                   FLASH_PART_NAME_LENGTH) == 0) && (flash_type==FLASH_NOR))
      {
    ...
    {
      /* We were called in non-partitioned mode with a NULL partition name
       * pointer. Return indicating failure. */
