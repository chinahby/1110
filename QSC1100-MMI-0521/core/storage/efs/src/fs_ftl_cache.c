...
/*=============================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_ftl_cache.c#3 $
...
ftl_cache_init (int16 driveno)
{
  ...
  rex_init_crit_sect (&pt->fs_ftl_cache_crit_sec);
...
ftl_cache_open (int16 driveno)
{
  ...
  pt = &cpartitions[driveno];
  rex_enter_crit_sect (&pt->fs_ftl_cache_crit_sec);
  if (pt->ftl_raw_device != FTL_NO_DEVICE) {
    /* The device is already in use.*/
    rex_leave_crit_sect (&pt->fs_ftl_cache_crit_sec);
    return FTL_CACHED_NO_DEVICE;
  }
...
int ftl_cache_force_sync_all (void)
{
  ...
  for (i = 0; i < MAX_FTL_PARTITIONS; i++) {
    if (cpartitions[i].ftl_raw_device != FTL_NO_DEVICE) {
      ...
      rex_enter_crit_sect (&cpartitions[i].fs_ftl_cache_crit_sec);
      while (cache_sync_oldest_block (pt))
        ;
      rex_leave_crit_sect (&cpartitions[i].fs_ftl_cache_crit_sec);
      pt->sync_timer_active = 0;
