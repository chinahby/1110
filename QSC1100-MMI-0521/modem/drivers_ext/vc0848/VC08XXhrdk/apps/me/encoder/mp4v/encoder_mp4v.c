...
* 		
=======================================================================*/
#include "encoder_mp4v.h"
#include "aeestdlib.h"
#if(ME_KM_ENCODE_S263_EN|ME_KM_ENCODE_MP4V_EN)

#if ME_KM_OBJ_STACK
static me_encoder_t 	_g_encoder_mp4v;
#endif
...
ME_U32 encoder_mp4v_init(void *recinfor, void* inputinfor, void **encodeinfor)
{
	...
	encinfo.Enc_Quality_Level=5; //10;//				//  1-31 the higher, the better image quality.
	encinfo.Enc_Half_Pixsel_Estimation_Enable=1;	//
	encinfo.Enc_Short_HeaderSurpport=0;			//0:not 1: yes def:0
	encinfo.Enc_Mpeg4_ResyncMarker_Disable=1;	//0:not 1: yes
    encinfo.Enc_Mpeg4_QuantType=0;			//0:H.263 1:mpeg-2
	encinfo.Enc_Mpeg4_4mv_Disable=0;			//
	encinfo.Enc_Mpeg4_Rounding_Enable=1;		//
    encinfo.Enc_Mpeg4_intra_dc_vlc=0;	
...
{
	#if ME_KM_FUNNAME
	ME_MESSAGE(("->encoder_mp4v_release()"));
	#endif
	ME_ASSERT_PARAM_RETURN((encodeinfor!=NULL));
#ifndef _WIN32	
	VIM_HAPI_CaptureMp4_Release();
#endif
	return ME_SUCCESS;
}
...
{
	#if ME_KM_FUNNAME
	ME_MESSAGE(("->encoder_mp4v_set_inputparam()"));
	#endif
...
{
	#if ME_KM_FUNNAME
	ME_MESSAGE(("->encoder_mp4v_stop()"));
	#endif
	ME_ASSERT_PARAM_RETURN((encodeinfor!=NULL));

#ifndef _WIN32	
	VIM_HAPI_StopCaptureMp4();	
	//VIM_HAPI_CaptureMp4_Release();
#endif
...
{
	#if ME_KM_FUNNAME
	ME_MESSAGE(("->encoder_mp4v_resume()"));
	#endif
	ME_ASSERT_PARAM_RETURN((encodeinfor!=NULL));
	return ME_SUCCESS;
}
...
{
	#if ME_KM_FUNNAME
	ME_MESSAGE(("->encoder_s263_get_max_src_size()"));
	#endif
	*maxsrcsize = 0;
	ME_ASSERT_PARAM_RETURN((encodeinfor!=NULL));
	*maxsrcsize = 176*144*2;	
	return ME_SUCCESS;
}
...
Parameters:
       void *decodeinfor:	encoder information structor point
       ME_U32 maxsrcsize: audio max src stream data length(bytes)
...
{
	#if ME_KM_FUNNAME
	ME_MESSAGE(("->encoder_mp4v_get_decode_dst_buffersize()"));
	#endif
	
	*maxdstsize = 0;
	ME_ASSERT_PARAM_RETURN((encodeinfor!=NULL));

	//*maxdstsize = mp4v_encinfor.I_Frame_Estimat_Size*30;	
	*maxdstsize = mp4v_encinfor.OneBufferSize;	
	return ME_SUCCESS;
}
...
ME_U32 encoder_mp4v_encodeframes(void *encodeinfor, void *src, void *dst)
{
	...
	ME_U32 dst_address, enc_flag = 0;
#ifndef _WIN32	
	VIM_HAPI_MP4_ENC_TIME ticks_enc;
#endif
	#if ME_KM_FUNNAME
	ME_MESSAGE(("->encoder_mp4v_encodeframes()"));
	#endif
	ME_ASSERT_PARAM_RETURN((encodeinfor!=NULL));
	...
	//check decode flag
	if (mp4v_encinfor.enc_start_flag)
		enc_flag|=VIM_MPEG4_MP4_START;
	if (mp4v_encinfor.vop_start_flag)
		enc_flag|=VIM_MPEG4_VOP_START;

 	//reset encode configuration
	mp4v_encinfor.enc_start_flag 	= FALSE;		//when need start decode mp4 set this flag
	mp4v_encinfor.vop_start_flag 	= FALSE;		//when one frame end, need set this flag
	ticks_enc.bMod_Time_Base = mp4v_encinfor.enc_ticks_base;
	ticks_enc.nTime_Inc = mp4v_encinfor.enc_ticks_inc;
	//get dst address
	dst_address	= dst_buffer->buffer_address + dst_buffer->cursor_write;
	//start decode done
	if(enc_flag&VIM_MPEG4_MP4_START)
	{
		VIM_HAPI_StartCaptureMp4((ME_U32)dst_address);
		//me_printf("\nME_MESSAGE:	VIM_HAPI_StartCaptureMp4\n");
	}
	else
	{
		VIM_HAPI_CaptureMp4_StartNextVop((ME_U32)dst_address,
									      	mp4v_encinfor.key_vop_flag,
									      	mp4v_encinfor.last_frame_size,
								     		(PVIM_HAPI_MP4_ENC_TIME)&ticks_enc);
		//me_printf("\nME_MESSAGE:	VIM_HAPI_CaptureMp4_StartNextVop\n");
	}
#endif
	return ME_SUCCESS;
}
...
ME_U32 encoder_mp4v_register(void)
{
	...
	#if ME_KM_FUNNAME
	ME_MESSAGE(("->encoder_mp4v_register()"));
	#endif
	
#if ME_KM_OBJ_STACK
	pthis	= &_g_encoder_mp4v;
#else
	...
	return me_encoder_register(pthis);	/*×¢²ámp4v encoder*/
}
#endif  //#if(ME_KM_ENCODE_S263_EN|ME_KM_ENCODE_MP4V_EN)
