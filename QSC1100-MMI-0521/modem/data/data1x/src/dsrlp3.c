...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/data/1x/1xrlp/main/lite/src/dsrlp3.c#2 $ $DateTime: 2008/04/17 02:06:32 $ $Author: nsivakum $
...
08/31/04   atp        Fixed bug in rx_frame_det() when SEQ_HI present while
                      parsing MuxPDU5-FmtD combo (CR48770).
...
(
   dsrlpi_sess_ctl_blk_type     *rscb_ptr,      /* ptr to sess ctl block   */
   dsrlp_rev_frame_type         *rev_fr_ptr     /* ptr to frame to be built*/
);
...
dsrlpi_ver_const_type dsrlp3_constants =
{
  ...
  DSRLPSEQ_32BIT_SEQ_MODULUS,                /* Seq Modulus for this ver   */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  *bits_left = *bits_left - RLP3_NAK_HOLE_CNT_LEN;
  ...
  MSG_HIGH("Sending Unseg NAKS. RLP:%d",rscb_ptr->sr_id,0,0);
  ...
  while ((*bits_left) > bits_per_nak)
  {
    ...
    -----------------------------------------------------------------------*/
    nak_id = dsrlpnak_get_nak(rscb_ptr,nak_id,FALSE);
    ...
    ------------------------------------------------------------------------*/
    dsrlpnak_nak_was_sent(rscb_ptr,nak_id);
  } /* while */
  ASSERT(num_naks > 0);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  *bits_left = *bits_left - RLP3_NAK_SEG_CNT_LEN;
  ...
  MSG_HIGH("Sending SEG NAKS:RLP:%d",rscb_ptr->sr_id,0,0);
  ...
  while ((*bits_left) > bits_per_nak)
  {
    /*-----------------------------------------------------------------------
      Get a nak_id and put data into frame
    -----------------------------------------------------------------------*/
    nak_id = dsrlpnak_get_nak(rscb_ptr,nak_id,TRUE);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  dsrlpi_ctl_used = fwd_frame_ptr->fr_item_ptr->used;
  ...
  {
    if (rscb_ptr->state.state == DSRLPI_SYNC_STATE)
    {
      return(DSRLPI_NULL);
    }
    else
    {
      MSG_LOW("return RLP_ERASURE", 0, 0 ,0);
  ...
  if (fwd_frame_ptr->fr_rate == DSRLP_RATE_1)
  {
    ...
    else if (mux_pdu_type == DSRLP_MUX_PDU_5)
    {
      ...
      if ((len_indicator & RLP3_MUXPDU5_LEN_IND_MASK) == 0x00)
      {
          ...
          -----------------------------------------------------------------*/
          MSG_LOW("Received frame with Len Indicator 00 RLP:%d ",rscb_ptr->sr_id,0,0);
      ...
      else if ((len_indicator & RLP3_MUXPDU5_LEN_IND_MASK) == 0x04)
      {
          ...
          -----------------------------------------------------------------*/
          MSG_LOW("Received frame with Len Indicator 01 RLP: %d ",rscb_ptr->sr_id,0,0);
         ...
         MSG_LOW("Length received is %d RLP:%d",pdu5_len,rscb_ptr->sr_id,0);
      ...
      else if ((len_indicator & RLP3_MUXPDU5_LEN_IND_MASK) == 0x08)
      {
         ...
         Byte   |         6 bits             | 2 bits            |   
            ...
            and then obtain the 6 bits of length and 2 bits of type field.
          -----------------------------------------------------------------*/
          MSG_LOW("Received frame with Len Indicator 10 RLP: %d ",rscb_ptr->sr_id,0,0); 
          ...
          -----------------------------------------------------------------*/
          pdu5_temp = pdu5_len =  ntohs(pdu5_len);
          ...
          MSG_LOW("For Len_ind 10, Len rxed is %d,Mux:%d RLP:%d",
                   pdu5_len,
      ...
      {
          /*-----------------------------------------------------------------
            We received the length indicator as 11, which is reserved.
          -----------------------------------------------------------------*/
          MSG_HIGH ("Invalid Length Indicator RLP: %d",rscb_ptr->sr_id,0,0);
      ...
      )
    {
                                                ...
                                                == RLP3_FMT_D_DATA_SQI_MASK )
      {
        ...
        {
          ERR("Bad PDU - SW ERR %d", mux_pdu_type, 0, 0);
          dsm_free_packet( &(fwd_frame_ptr->fr_item_ptr));
		  MSG_LOW("return RLP_ERASURE", 0, 0 ,0);
          return(DSRLPI_RLP_ERASURE);
        }
      ...
      else /* SQI not set */
      {
                                                ...
                                                == RLP3_FMT_D_DATA_SSP_MASK )
        {
          ...
          {
              MSG_HIGH ("Unknown PDU type: Please verify RLP: %d",
                        rscb_ptr->sr_id,0,0);
              dsm_free_packet( &(fwd_frame_ptr->fr_item_ptr));
			  MSG_LOW("return RLP_ERASURE", 0, 0 ,0);
              return(DSRLPI_RLP_ERASURE);
      ...
      /*---------------------------------------------------------------------
       ...
       --------------------------------------------------------------------*/
      dsm_pullup(&(fwd_frame_ptr->fr_item_ptr), NULL, offset);
      ...
      )
    {
      /*---------------------------------------------------------------------
       ...
       --------------------------------------------------------------------*/
        MSG_HIGH( "Bad type bits for RLP3 - %x RLP:%d", 
                  type_bits,rscb_ptr->sr_id , 0);
        return(DSRLPI_RLP_ERASURE);
    }
  ...
  if ( (ctl_byte & RLP3_UNSEG_DATA_MASK) == RLP3_UNSEG_DATA_MASK_RSLT )
  {
    ...
    if( (fwd_frame_ptr->fr_item_ptr == NULL) || 
		(dsm_length_packet(fwd_frame_ptr->fr_item_ptr) < length) )
    {
      MSG_HIGH("BS fr too short len = %d used = %d RLP:%d",
               length,dsrlpi_ctl_used,rscb_ptr->sr_id);
      return( DSRLPI_RLP_ERASURE);
    }
  ...
  else if ( (ctl_byte & RLP3_SEG_DATA_MASK) == RLP3_SEG_DATA_FLD_CTL )
  {
    ...
    if( fwd_frame_ptr->fr_item_ptr == NULL)
    {
      MSG_HIGH("Malformed frame, ctl_byte=0x%xi RLP:%d", 
               ctl_byte, rscb_ptr->sr_id, 0);
      return( DSRLPI_RLP_ERASURE);
    }
      ...
      )
    {
                        ...
                        offset+(RLP3_CTL_FCS_BYTE_LEN*8))) != CRC_16_L_OK)
      {
        /*-------------------------------------------------------------------
         ...
         ------------------------------------------------------------------*/
        ERR("Bad FCS for INIT frame", 0, 0, 0);
		MSG_LOW("return RLP_ERASURE", 0, 0 ,0);
        return (DSRLPI_RLP_ERASURE);
      }
        ...
        case RLP3_NAK_NAK_TYPE_0:
          ...
          for (i=0; i<(nak_count + 1); i++)
          {
            ...
            /*---------------------------------------------------------------
             ...
             --------------------------------------------------------------*/
            if( dsrlprtx_put_rexmit_info( rscb_ptr, 
                                          nak_first_seq, 
                                          nak_last_seq) == FALSE)
            {
              resync_rlp = TRUE;
			...
			MSG_MED("BS NAK0: 0x%x -> 0x%x, V(S)=0x%x",
                   nak_first_seq, 
                   nak_last_seq,
        ...
        case RLP3_NAK_NAK_TYPE_1:
          ...
          for (i=0; i<(nak_count + 1); i++)
          {
            ...
            /*---------------------------------------------------------------
              ...
              for re-xmit list.
             --------------------------------------------------------------*/
            if( dsrlprtx_put_rexmit_info(rscb_ptr, nak_seq, nak_seq)== FALSE)
            {
              resync_rlp = TRUE;
            }
            else
            ...
            for(bit_pos = 1; bit_pos <= 8; bit_pos++)
            {
              if (nak_map_byte & mask)
              {
                ...
                /*-----------------------------------------------------------
                  Create entry for re-xmit list.
                 ----------------------------------------------------------*/
                if( dsrlprtx_put_rexmit_info(rscb_ptr, tmp_seq, tmp_seq)
                        == FALSE)
                {
                  resync_rlp = TRUE;
                }
			...
			MSG_MED("BS NAK1 0x%x, Map:0x%x :V(S)=0x%x",
                    nak_seq,
                    nak_map_byte,
                    rscb_ptr->state.next_seq_num);
          } /* for nak_count */
...
#error code not present
        ...
        case RLP3_NAK_NAK_TYPE_2:
          ...
          for (i=0; i<(nak_count + 1); i++)
          {
			...
			MSG_MED("BS NAK2: 0x%x -> 0x%x, V(S)=0x%x",
                   nak_first_seq,
                   nak_last_seq,
                   rscb_ptr->state.next_seq_num);
        ...
        case RLP3_NAK_NAK_TYPE_3:
          ...
          for (i=0; i<(nak_count + 1); i++)
          {
			...
			MSG_MED("BS NAK3: 0x%x Len:0x%x :V(S)=0x%x",
				  nak_first_seq,
				  length,
				  rscb_ptr->state.next_seq_num);
                        ...
                        offset+(RLP3_CTL_FCS_BYTE_LEN*8))) != CRC_16_L_OK)
      {
        ...
        {
          dsrlprtx_remove_last_element( rscb_ptr);
        }

        ERR("Bad FCS for NAK frame, type=%d, ", nak_type, 0, 0);
        dsm_free_packet( &(fwd_frame_ptr->fr_item_ptr));
		MSG_LOW("return RLP_ERASURE", 0, 0 ,0);
        return (DSRLPI_RLP_ERASURE);
      }
      ...
      if (resync_rlp == TRUE)
      {
        MSG_HIGH("Bad NAK from BS - resync RLP:%d",rscb_ptr->sr_id,0,0);
        dsm_free_packet( &(fwd_frame_ptr->fr_item_ptr));
      ...
      /*---------------------------------------------------------------------
       ...
       --------------------------------------------------------------------*/
      DSRLPSEQ_SEQ_GEN_FROM_SEQ_HI(rscb_ptr->state.peer_vn, seq_hi, 
                                   fr_seq_num);
      ...
      ---------------------------------------------------------------------*/
      if (DSRLPSEQ_GT_SEQ_CHECK(rscb_ptr->state.peer_vn,
                                rscb_ptr->state.next_seq_num,
                                rscb_ptr->ver_ptr->seq_modulus))
      {
        MSG_HIGH("RESET:  BS V(n) %x > MS V(S) %x RLP: %d",
                 rscb_ptr->state.peer_vn, 
                 rscb_ptr->state.next_seq_num, rscb_ptr->sr_id);
        dsm_free_packet( &(fwd_frame_ptr->fr_item_ptr));
        dsrlpi_reset(rscb_ptr);
        /*-------------------------------------------------------------------
           Report RLP reset event
      ...
      /*---------------------------------------------------------------------
       ...
       --------------------------------------------------------------------*/
      MSG_HIGH("BS V(n) via NAK = 0x%x %d",rscb_ptr->state.peer_vn,rscb_ptr->sr_id,0);
    ...
    else if ( (ctl_byte & RLP3_IDLEFILL_CTL_MASK) == RLP3_FILL_CTL_FLD )
    {
      ...
      if (DSRLPSEQ_GE_SEQ_CHECK(rscb_ptr->state.next_seq_num,
                                tmp_seq,
                                rscb_ptr->ver_ptr->seq_modulus))
      {
        /*-------------------------------------------------------------------
         ...
         ------------------------------------------------------------------*/
        rscb_ptr->state.peer_vn = tmp_seq;
      }

      rscb_ptr->stats.rx_fill_fr_cnt++;
    ...
    else
    {
      ...
      if (ctl_byte != 0x3F)
      {
         MSG_HIGH("Unknown frame RLP3 fmt 0x%x RLP: %d", ctl_byte, rscb_ptr->sr_id, 0);
      }
      MSG_LOW("return RLP_ERASURE", 0, 0 ,0);
      return (DSRLPI_RLP_ERASURE);
    }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (curr_info_ptr->seq_num == rscb_ptr->state.expected_seq_num)
  {
                            ...
                            modulus))
  {
    ...
    -----------------------------------------------------------------------*/
    if (dsrlpdel_q_empty(rscb_ptr->state.del_fr_id) == FALSE)
    {
      return( DSRLPI_DONE);
    }
    ...
    {
      MSG_HIGH("RLP: %d", rscb_ptr->sr_id,0,0);
      MSG_HIGH("big gap: V(R): 0x%x, V(N): 0x%x, idle seq: 0x%x",
               rscb_ptr->state.expected_seq_num,
    ...
    -----------------------------------------------------------------------*/
    if ((rscb_ptr->state.nak_rounds_rev != 0) && (dsrlpnak_insert(rscb_ptr,
                                          rscb_ptr->state.expected_seq_num,
    ...
    {
      rscb_ptr->state.expected_seq_num = curr_info_ptr->seq_num;
      if (rscb_ptr->state.nak_rounds_rev == 0)
      {
        rscb_ptr->state.needed_seq_num = curr_info_ptr->seq_num;
      }
    }
  }
  else /* rcvd num is less than expected */
  {
    ...
    -----------------------------------------------------------------------*/
    MSG_HIGH("Rx idle w/old seq 0x%x V_S=0x%x RLP:%d",
              curr_info_ptr->seq_num,
              rscb_ptr->state.expected_seq_num,rscb_ptr->sr_id);
...
{
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  MSG_MED("Rxed Erasure. Used  = %d RLP:%d",dsrlpi_ctl_used,rscb_ptr->sr_id,0);
  MSG_MED("Rxed Erasure. Bytes = 0x%x 0x%x 0x%x",
...
{
  rscb_ptr->state.mux_erasure_cnt++;
  return (DSRLPI_DONE);
} /* rlp3_rx_mux_erasure() */

/*===========================================================================
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /* Below check is to fix Klockwork error. */
  if( rev_fr_ptr->mux_pdu_type >= DSRLP_MUX_PDU_3 )
  {
    ERR("Invalid MUX PDU type", 0, 0, 0);
    return FALSE;
  }
  ...
  /*------------------------------------------------------------------------
    Also get a local ptr to the state, as it is accessed often.  Should
...
#error code not present
  ...
  -------------------------------------------------------------------------*/
  octet_delta = bit_pos - ((bit_pos/BITS_IN_OCTET) * BITS_IN_OCTET);
  ...
  ASSERT((bit_pos - ((bit_pos/BITS_IN_OCTET) * BITS_IN_OCTET)) == 0);

  /*-------------------------------------------------------------------------
    Calculate FCS over these bytes.  Function automatically places CRC into
    frame.
  -------------------------------------------------------------------------*/
  byte_len = (bit_pos/BITS_IN_OCTET) + RLP3_CTL_FCS_BYTE_LEN;
  ...
  item_ptr->used          = dsrlp_used_len_table[rev_fr_ptr->fr_rate]
                                                [rev_fr_ptr->srvc_type]
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    ERR("Called tx_naks when boolean false",0,0,0);
    return FALSE;
  }

  nak_id = dsrlpnak_get_nak(rscb_ptr,0,TRUE);
...
//  }
                                   ...
                                   &frame_len) == FALSE)
  {
    if (dsrlpi_get_min_frame_and_len(rev_fr_ptr,
                                     ...
                                     &frame_len) == FALSE)
    {
      ERR("Rev fr too small for NAK frame Tx",0,0,0);
      return(FALSE);
    }
  }
  ...
  {
    dsrlp3i_bld_naktype0(rscb_ptr,data_ptr,&bits_left);
  }
  else
  {
    dsrlp3i_bld_naktype2(rscb_ptr,data_ptr,&bits_left);
  }
...
//  }
  ...
  if (dsrlpi_get_min_frame_and_len(rev_fr_ptr,
                                   ...
                                   &frame_len) == FALSE)
  {
    MSG_HIGH(" Only %d bytes to tx idle-not enough RLP:%d", 
             frame_len, rscb_ptr->sr_id, 0);
    return(FALSE);
  }
      ...
      (rscb_ptr->state.rnd_trip_fr_cntr == 0))
  {
    ...
    ----------------------------------------------------------------------*/
    if (rscb_ptr->state.idle_timer_active == TRUE)
    {
      ASSERT(rscb_ptr->state.idle_timer > 0);

      if (rscb_ptr->state.idle_timer != 1)
      {
        rscb_ptr->state.idle_timer--; 
        return FALSE;
      }
    }
    else
  ...
  if ((rscb_ptr->state.rnd_trip_fr_cntr > 0)       ||
       ...
       (rscb_ptr->state.idle_timer == 0)))
  {
    ...
    if (rscb_ptr->state.rnd_trip_fr_cntr > 0)
    {
      rscb_ptr->state.rnd_trip_fr_cntr--;
    }

    if (rscb_ptr->state.idle_timer_active == TRUE)
    {
      rscb_ptr->state.idle_timer = DSRLPI_IDLE_TIMER_DEF;
      rscb_ptr->state.idle_fr_xmit_cntr++;

      if (rscb_ptr->state.idle_fr_xmit_cntr == DSRLPI_IDLE_FR_XMIT_CNTR_DEF)
      {
  ...
  -----------------------------------------------------------------------*/
  item_ptr->data_ptr[0] = 
                    DSRLPSEQ_GET_SEQ(rscb_ptr->state.next_seq_num);
  ...
  item_ptr->data_ptr[1]  = 
                  DSRLPSEQ_GET_SEQ_HI(rscb_ptr->state.next_seq_num);
...
//  }
  ...
  if (dsrlpi_get_min_frame_and_len(rev_fr_ptr,
                                   ...
                                   &frame_len) == FALSE)
  {
    MSG_HIGH(" Only %d bytes to tx fill-not enough RLP:%d", 
             frame_len, rscb_ptr->sr_id, 0);
    return(FALSE);
  }
  ...
  -----------------------------------------------------------------------*/
  item_ptr->data_ptr[0] = 
                    DSRLPSEQ_GET_SEQ(rscb_ptr->state.needed_seq_num);
  ...
  item_ptr->data_ptr[1]  = 
                  DSRLPSEQ_GET_SEQ_HI(rscb_ptr->state.needed_seq_num);
