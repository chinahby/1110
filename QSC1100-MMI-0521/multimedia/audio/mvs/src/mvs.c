...
$Header: //source/qcom/qct/multimedia/audio/6k/common/mvs/main/latest/src/mvs.c#2 $ $DateTime: 2008/08/13 00:23:59 $ $Author: subhashj $
...
#include "clk.h"        /* Clock rec/dereg prototypes */
...
)
{
  ...
  if (context != MVS_PKT_CONTEXT_NONE)
  {
    ...
    if (mvs_to_voc_client[client] != mvs_to_voc_client[mvs_enabled_client])
    {
      ...
      {
#if defined(MSMAUD_VOC_4GV_VOIP) || defined(MSMAUD_VOC_IS127_VOIP)
        /* add support for voip specific firmware */
        if(mvs_enabled_client == MVS_CLIENT_VOIP &&
           (mvs_client_states[mvs_enabled_client].desired_config_mode == MVS_MODE_4GV_NB ||
           mvs_client_states[mvs_enabled_client].desired_config_mode == MVS_MODE_IS127)){
           if(mvs_client_states[mvs_enabled_client].desired_config_mode == MVS_MODE_4GV_NB) {
             (void)voc_config_standby (mvs_to_voc_client[client],VOC_CAP_4GV_NB_VOIP);
           }
#ifdef MSMAUD_VOC_IS127_VOIP
           else if(mvs_client_states[mvs_enabled_client].desired_config_mode == MVS_MODE_IS127) {
             (void)voc_config_standby (mvs_to_voc_client[client],VOC_CAP_IS127_VOIP);
           }
#endif /* MSMAUD_VOC_IS127_VOIP */
        } else
#endif /* (MSMAUD_VOC_4GV_VOIP) || (MSMAUD_VOC_IS127_VOIP) */
        (void)voc_config_standby (mvs_to_voc_client[client],
                                  mvs_to_voc_mode[
      ...
      else
      {
        ...
        if(mvs_enabled_client == MVS_CLIENT_VOIP &&
           ...
           mvs_client_states[mvs_enabled_client].desired_config_mode == MVS_MODE_IS127)){
           if(mvs_client_states[mvs_enabled_client].desired_config_mode == MVS_MODE_4GV_NB) {
             (void)voc_config(mvs_to_voc_client[client],VOC_CAP_4GV_NB_VOIP);
           }
#ifdef MSMAUD_VOC_IS127_VOIP
           else if(mvs_client_states[mvs_enabled_client].desired_config_mode == MVS_MODE_IS127) {
             (void)voc_config(mvs_to_voc_client[client],VOC_CAP_IS127_VOIP);
           }
#endif /* MSMAUD_VOC_IS127_VOIP */
        } else
#endif /* (MSMAUD_VOC_4GV_VOIP) || (MSMAUD_VOC_IS127_VOIP) */
        (void)voc_config(mvs_to_voc_client[client],
                         mvs_to_voc_mode[mvs_client_states[mvs_enabled_client]
                                         .desired_config_mode]);
      }
    }
#ifdef FEATURE_MVS_GEN_SILENCE
  ...
  if (mode != MVS_MODE_NONE)
  {
            ...
            != mvs_to_voc_client[mvs_pcm_enabled_client]))
    {
      ...
      if(mvs_pcm_enabled_client == MVS_CLIENT_VOIP && 
        (mode == MVS_MODE_4GV_NB || mode == MVS_MODE_IS127) ) {
          if(mode == MVS_MODE_4GV_NB)
            (void)voc_config (mvs_to_voc_client[mvs_pcm_enabled_client], VOC_CAP_4GV_NB_VOIP);
#ifdef MSMAUD_VOC_IS127_VOIP
          else
            (void)voc_config (mvs_to_voc_client[mvs_pcm_enabled_client], VOC_CAP_IS127_VOIP);
#endif /* MSMAUD_VOC_IS127_VOIP */
      } else
#endif /* (MSMAUD_VOC_4GV_VOIP) || (MSMAUD_VOC_IS127_VOIP) */
      (void)voc_config (mvs_to_voc_client[mvs_pcm_enabled_client],
