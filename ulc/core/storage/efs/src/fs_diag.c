 ...
 *====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
 ...
 /*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_diag.c#13 $
...
fs_diag_md5sum_handler (PACKED void *req_ptr, uint16 pkt_len)
{
  ...
  if (response)
  {
    ...
    if (efs_md5sum (request_path, &md5sum) != 0)
      ...
      memcpy ((void *) response->hash_result, md5sum.hash, 16);
      response->diag_errno = 0;
    }
  }
...
fs_diag_mkdir (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  if (rsp_pkt == NULL)
  {
    MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_mkdir.", 0, 0, 0);
...
fs_diag_rmdir (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  if (rsp_pkt == NULL)
  {
    MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_rmdir.", 0, 0, 0);
    return ((PACKED void *) rsp_pkt);
  }
...
fs_diag_disp (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  if (rsp_pkt == NULL)
  {
    MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_disp.", 0, 0, 0);
    return ((PACKED void *) rsp_pkt);
  }
  ...
  *data = 0;
  ...
  while ((dirent = efs_readdir (dirp)) != NULL)
  {
    ...
    if (fs_strlcpy (pathbuf, entryname, len) >= len ||
        fs_strlcat (pathbuf, "/", len)       >= len ||
        fs_strlcat (pathbuf, dirent->d_name, len) >= len)
    {
        ...
        (S_ISREG (fmode) && disp_pkt->header.op == FS_DIAG_DISP_FILES))
    {
      ...
      (void) fs_strlcpy (data, pathbuf, remain);
      data += len;
      remain -= len;
      num_items++;
    }
...
fs_diag_read_file (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  {
    PACKED void *rsp = diagpkt_alloc (DIAG_FS_OP_F, pkt_len);

    if (rsp != NULL)
      memcpy ((void *)rsp, fsd_last_response, fsd_last_response_len);
  ...
  if (read_pkt->seq_num == 0)
  {
    ...
    if (first_pkt == NULL)
    {
      MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_read.", 0, 0, 0);
...
fs_diag_write_file (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  if (wrt_pkt->seq_num == 0)
  {
    ...
    (void) fs_strlcpy (fs_diag_writepath, w_pkt->filename.file,
...
fs_diag_write_done:
  rsp_pkt = (fs_diag_write_rsp_type *)
            diagpkt_alloc (DIAG_FS_OP_F, sizeof (fs_diag_write_rsp_type));

  if (rsp_pkt == NULL)
  {
    MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_write.", 0, 0, 0);
    return ((PACKED void *) rsp_pkt);
  }
...
fs_diag_remove_file (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  if (rsp_pkt == NULL)
  {
    MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_remove_file.", 0, 0, 0);
    return ((PACKED void *) rsp_pkt);
  }
...
fs_diag_get_attr (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  if (rsp_pkt == NULL)
  {
    MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_get_attr.", 0, 0, 0);
    return ((PACKED void *) rsp_pkt);
  }
...
fs_diag_set_attr (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  {
    MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_set_attr.", 0, 0, 0);
  }
...
fs_diag_iter (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  if (iter_pkt->seq_num == 0)
  {
    ...
    fs_strlcpy (dir_path, iter_pkt->filename.file, sizeof(fs_diag_dirpath));
  ...
  do
  {
    ...
    len = sizeof (fs_diag_pathbuf);
    /* Copy to temporary buffer, if path is too long we quit. */
    if (fs_strlcpy (pathbuf, dir_path, len) >= len ||
        fs_strlcat (pathbuf, "/", len)     >= len ||
        fs_strlcat (pathbuf, dirent->d_name, len) >= len)
      goto fs_diag_iter_err;
  ...
  if (rsp_pkt == NULL)
  {
    MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_iter.", 0, 0, 0);
    return ((PACKED void *) rsp_pkt);
  }
  ...
  memcpy ((void *)rsp_pkt->filename.file, pathbuf, rsp_len);
...
fs_diag_space_avail (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  if (rsp_pkt == NULL)
  {
    MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_space_avail.", 0, 0, 0);
    return ((PACKED void *) rsp_pkt);
  }
...
fs_diag_format (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  if (rsp_pkt == NULL)
  {
    MSG_HIGH ("Diagpkt_alloc returned NULL in fs_diag_format.", 0, 0, 0);
    return ((PACKED void *) rsp_pkt);
  }
...
fs_diag_cmd (PACKED void *req_pkt, uint16 pkt_len)
{
  ...
  if (rsp_pkt == NULL)
    MSG_HIGH ("Response pointer returned to fs_diag_cmd is NULL.", 0, 0, 0);
