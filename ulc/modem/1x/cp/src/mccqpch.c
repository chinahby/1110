...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/mccqpch.c#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $
...
)
{
  ...
  if (mcc_get_bs_info(&bs_info_id, sys_time, &bs))
  {
    ...
    else
    {
      ...
      MSG_MED("QPCH: Tot %d, Hash %d PI walsh %d", bs->csp.esp.num_qpch,
        bs->qpch_num, qpch_info->pi_walsh_code);

      qpch_info->bi_supported = FALSE;

      #if defined(FEATURE_IS2000_REL_A_CC) || defined(FEATURE_IS2000_REL_B)
      #if defined(FEATURE_BROADCAST_SMS) || defined(FEATURE_IS2000_REL_A_CC_BCSMS)
      if (mccbcsms_is_enabled() && (bs->csp.esp.bcast_index != 0))
      {

        #ifdef FEATURE_IS2000_REL_B
        if (OVHD_CHAN_IS_PCH &&
            ((cur_bs_ptr->csp.esp.p_rev >= P_REV_IS2000_REL_B) && 
             bs->csp.esp.qpch_bi_supported))
        {
          qpch_info->bi_supported = TRUE;
          qpch_info->bi.walsh_code = QPCH_BI_WALSH;
          qpch_info->bi.power_level = (int8)bs->csp.esp.qpch_power_level_bcast;
        }
        #endif /* FEATURE_IS2000_REL_B */

        #ifdef FEATURE_IS2000_REL_A_CC_BCSMS
        if (OVHD_CHAN_IS_BCCH && bs->csp.esp.qpch_bi_supported)
        {
          qpch_info->bi_supported = TRUE;
          qpch_info->bi.walsh_code = QPCH_BI_WALSH;
          qpch_info->bi.power_level = (int8)bs->csp.esp.qpch_power_level_bcast;
      ...
      #endif /* FEATURE_BROADCAST_SMS or FEATURE_IS2000_REL_A_CC_BCSMS */
...
)
{
  ...
  qword msg_age;  // Temporary storage for age of message in current bs_info

  if ((OVHD_CHAN_IS_PCH && ESP_MSG_NOT_RECVD)
#ifdef FEATURE_IS2000_REL_A_CC
      || (OVHD_CHAN_IS_BCCH && MCRRP_MSG_NOT_RECVD)
#endif /* FEATURE_IS2000_REL_A_CC */
     )
  {
