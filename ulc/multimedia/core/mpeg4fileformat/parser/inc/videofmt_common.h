...
#define ALBM_TYPE 0x616C626D  /* 'albm' */
#define YRRC_TYPE 0x79727263  /* 'yrcc' */
...
*/
typedef struct {
  uint32  creation_time;        /* creation time of presentation           */
  uint32  mod_time;             /* last time the presentation was modified */
  uint32  movie_timescale;      /* time units per sec for whole movie      */
  uint32  total_movie_duration; /* length (in timescale) of movie ('mvhd') */
  ...
  uint32  fragment_file_total_movie_duration;
...
** for the avcc atom whose size might exceed the maximum size of
** ping pong buffers
*/
typedef struct
{
...
typedef struct {
  ...
  uint16                      frame_count;
  uint16                      depth;
  uint8                       config_ver;     /* configuration version     */
  uint8                       profile_comp;   /* profile compatibility     */
  uint8                       len_minus_one;  /* NAL unit length minus one */
  uint8                       num_seq_param;
  uint8                       num_pic_param;
  video_fmt_h264_param_type  *seq_param_set;
  video_fmt_h264_param_type  *pic_param_set;
  video_fmt_mp4r_avcc_data_type avcc_alloc; /* Dynamic allocation for avcc atom*/
...
typedef struct {
  ...
  uint32           frames;         /* length in frames                     */
  uint32           bytes;          /* length in bytes                      */
  ...
  uint32           media_duration; /* in time units using media_timescale  */
  uint32           creation_time;  /* creation time of the stream          */
  uint32           mod_time;       /* last time the stream was modified    */
  boolean          fixed_size;
  boolean          fixed_rate;
  ...
  uint32                    fragment_offset;  /* Position of the fragment for repositioning */
...
typedef struct {
  ...
  uint32  offset;   /* Offset of atom contents, in MP4 file               */
  uint32  parent;   /* Offset of atom's parent's contents - zero          */
...
typedef struct video_fmt_user_data_type_t {
  ...
  boolean titl_auth_desc_valid; /*if this flag not set then format writer*/
                                  /*will exclude below sections*/
...
typedef void (*video_fmt_sample_info_cb_func_type) (
  uint32                         stream_number,
  uint32                         offset,        /* in samples            */
  uint32                         size,          /* in samples            */
...
typedef struct {
  ...
  uint32                           num_atoms;  /* total number of tx3g atoms */
} video_fmt_tx3g_data_type;
...
typedef union video_fmt_status_cb_info_type_t {
  ...
  video_fmt_tx3g_data_type     tx3g_atom;     /* VIDEO_FMT_TX3G          */
