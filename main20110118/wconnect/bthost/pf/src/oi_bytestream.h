...
$Header: //source/qcom/qct/wconnect/bthost/pf/rel/00.00.26/src/oi_bytestream.h#1 $ 
...
{ \
    OI_ASSERT(((bs).__data != NULL) && ((bs).__trueSize > 0)); \
    (bs).__pos = 0; \
    (bs).__RWFlag = (rw); \
 ...
 */
#define ByteStream_GetCurrentBytePointer(bs) (OI_BYTE*)(&((bs).__data[(bs).__pos]))
 ...
 */
#define ByteStream_GetDataPointer(bs) (OI_BYTE*)((bs).__data)
...
{ \
    ...
    OI_ASSERT(((B) + (N)) <= 8); \
    (i) = (OI_BYTE) (((bs).__data[(bs).__pos] >> (B)) & (0xFF >> (8 - (N)))); \
}
...
{ \
    ...
    OI_BYTE *__q = (OI_BYTE*) ((bs).__data + (bs).__pos); \
...
{ \
    ...
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) { \
        (i) = GetUINT16_BigEndian((bs).__data + (bs).__pos); \
    } else { \
        (i) = GetUINT16_LittleEndian((bs).__data + (bs).__pos); \
    } \
    (bs).__pos += 2; \
}
...
{ \
    ...
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) { \
        (i) = GetUINT24_BigEndian((bs).__data + (bs).__pos); \
    } else { \
        (i) = GetUINT24_LittleEndian((bs).__data + (bs).__pos); \
    } \
    (bs).__pos += 3 ; \
}
...
{ \
    ...
    OI_ASSERT((bs).__RWFlag == BYTESTREAM_READ); \
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) { \
        (i) = GetUINT32_BigEndian((bs).__data + (bs).__pos); \
    } else { \
        (i) = GetUINT32_LittleEndian((bs).__data + (bs).__pos); \
    } \
    (bs).__pos += 4; \
}
...
#ifndef OI_CODE
#define ByteStream_GetINT64 ByteStream_GetUINT64

/**
 * Reads a 64-bit integer from a byte stream, specifying whether the byte
 * order in the stream is big- or little-endian.
 * 
 * @param bs The byte stream from which to get the integer
 * @param i [out] The integer obtained
 * @param bo Byte order (endianness) of the stream
 *
 * void ByteStream_GetUINT64(OI_BYTE_STREAM   bs,
 *                           UINT64           i,
 *                           OI_UINT8         bo);
 */
#define ByteStream_GetUINT64(bs, i, bo) \
{ \
    OI_ASSERT( ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) || ((bo) == OI_LITTLE_ENDIAN_BYTE_ORDER) ); \
    OI_ASSERT(((bs).__pos + 8) <= (bs).__size); \
    OI_ASSERT((bs).__RWFlag == BYTESTREAM_READ); \
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) { \
        (i) = GetUINT64_BigEndian((bs).__data + (bs).__pos); \
    } else { \
        (i) = GetUINT64_LittleEndian((bs).__data + (bs).__pos); \
    } \
    (bs).__pos += 8; \
} 
#endif /* OI_CODE */
...
{ \
    ...
    OI_ASSERT((bs).__RWFlag == BYTESTREAM_READ); \
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) { \
        GetBDADDR_BigEndian((ba).addr, &((bs).__data[(bs).__pos])) \
    } else { \
        GetBDADDR_LittleEndian((ba).addr, &((bs).__data[(bs).__pos])) \
    } \
...
{ \
    ...
    (bs).__data[(bs).__pos] |= ((i) & (0xFF >> (8 - (N)))) << (B); \
}
...
{ \
    OI_BYTE *__p = (OI_BYTE*) ((bs).__data + (bs).__pos); \
...
{ \
    OI_BYTE *__d = (OI_BYTE*) ((dst_bs).__data + (dst_bs).__pos); \
    OI_BYTE *__s = (OI_BYTE*) ((src_bs).__data + (src_bs).__pos); \
...
{ \
    ...
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) { \
        SetUINT16_BigEndian((bs).__data + (bs).__pos, (i)); \
    } else { \
        SetUINT16_LittleEndian((bs).__data + (bs).__pos, (i)); \
    } \
...
{ \
    ...
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) { \
        SetUINT24_BigEndian((bs).__data + (bs).__pos, (i)); \
    } else { \
        SetUINT24_LittleEndian((bs).__data + (bs).__pos, (i)); \
    } \
    (bs).__pos += 3; \
}
...
{ \
    ...
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) { \
        SetUINT32_BigEndian((bs).__data + (bs).__pos, (i)); \
    } else { \
        SetUINT32_LittleEndian((bs).__data + (bs).__pos, (i)); \
    } \
...
#ifndef OI_CODE
/**
 * Writes a 64-bit integer to a byte stream, specifying whether the byte
 * order in the stream is big- or little-endian.
 * 
 * @param bs The byte stream to which to write the integer
 * @param i The integer to write to the byte stream 
 * @param bo The byte order (endianness) of the stream
 *
 * void ByteStream_PutUINT64(OI_BYTE_STREAM   bs,
 *                           UINT64           i,
 *                           OI_UINT8         bo);
 */
#define ByteStream_PutUINT64(bs, i, bo) \
{ \
    OI_ASSERT( ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) || ((bo) == OI_LITTLE_ENDIAN_BYTE_ORDER) ); \
    OI_ASSERT(((bs).__pos + 8) <= (bs).__size); \
    OI_ASSERT((bs).__RWFlag == BYTESTREAM_WRITE); \
    OI_ASSERT(sizeof(i) == 8); \
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) { \
        SetUINT64_BigEndian((bs).__data + (bs).__pos, (i)); \
    } else { \
        SetUINT64_LittleEndian((bs).__data + (bs).__pos, (i)); \
    } \
    (bs).__pos += sizeof(UINT64); \
} 
#endif /* OI_CODE */
...
{ \
    ...
    OI_ASSERT((bs).__RWFlag == BYTESTREAM_WRITE); \
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) { \
        SetBDADDR_BigEndian(&((bs).__data[(bs).__pos]), (ba).addr) \
    } else { \
        SetBDADDR_LittleEndian(&((bs).__data[(bs).__pos]), (ba).addr) \
    } \
    (bs).__pos += OI_BD_ADDR_BYTE_SIZE ; \
}
