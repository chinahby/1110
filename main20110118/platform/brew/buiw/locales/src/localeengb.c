...
// Format variables, strings, characters and patterns for English (United Kingdom)
...
static const AECHAR Currency_NegativePattern []           = { '(',0xa3,'#',')',0 };
static const AECHAR Currency_PositivePattern[]            = { 0xa3,'#',0 };
static const AECHAR Currency_CurrencySymbol[]             = { 0xa3,0 };
...
const AECHAR DateTime_AMDesignator[]               = { 'A','M',0 };
const AECHAR DateTime_DateSeparator[]              = { '/',0 };
const AECHAR DateTime_PMDesignator[]               = { 'P','M',0 };
const AECHAR DateTime_TimeSeparator[]              = { ':',0 };
const int DateTime_FirstWeekDay                    = 1;
...
/* -----------------------------------------------------------------------
    LocaleENGB_Ctor 
   --- */
static void LocaleENGB_Ctor(LocaleENGB *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleENGB_New  
   --- */
int LocaleENGB_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleENGB *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleENGB, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleENGB_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule); 
...
int LocaleENGB_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALEENGB) {
      return LocaleENGB_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
