...
$Header: //depot/asic/msmshared/services/common/intconv.c#2 $
...
static uint64 Power( uint8 i, uint8 power )
{
  uint64 result = 1;
  uint32 p;
...
{
  uint64 result = (uint64)(i & (Power(2,bits)-1));

  // Sign extend the result if it should be negative.
  //
  if ((result >= Power(2,bits-1)) && ((int64)result > 0)) {
...
{
  return (uint32)(i & (Power(2,bits)-1));
}
