...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_mnand_pm.c#1 $ $DateTime: 2008/12/12 11:11:10 $ $Author: richardp $
...
#define FS_MNAND_PM_ADVANCE_BLOCK(pm, pg) \
  do  \
  { \
    (pg) |= (FS_MNAND_CLUSTERS_PER_BLOCK -1); \
    ++(pg); \
    if ((pg) >= (pm)->max_page) \
      (pg) = 0; \
  } while (0);
...
#define FS_MNAND_PM_BLOCK_NBR(pg)  ((pg) / FS_MNAND_CLUSTERS_PER_BLOCK)
...
#define MNAND_MAX_COMBINED_BUFFER_SIZE      \
                      (FS_MAX_IOVEC_ITEMS_COUNT * FS_MNAND_CLUSTER_SIZE)
static FS_ALIGN uint8 mnand_combined_buffer[MNAND_MAX_COMBINED_BUFFER_SIZE];
...
fs_mnand_pm_init (efs_device_t dev)
{
  ...
  fs_paranoia_register (&mnand_pm_inst, sizeof(mnand_pm_inst),
                        "mnand_pm_inst");
  fs_paranoia_register (liquid_trans, sizeof(liquid_trans), "liquid_trans");
  fs_paranoia_register (solid_trans, sizeof(solid_trans), "solid_trans");
  fs_paranoia_register (mnand_combined_buffer, sizeof(mnand_combined_buffer),
                        "mnand_combined_buffer");
  ...
  pm->max_page = (pm->super.data->data.end - pm->super.data->data.start)
                    / FS_MNAND_SECTORS_PER_CLUSTER;
  max_sect = pm->max_page * FS_MNAND_SECTORS_PER_CLUSTER;
  ...
  {
    pm->liquid_alloc = 0;
    pm->solid_alloc = FS_MNAND_CLUSTERS_PER_BLOCK;
...
fs_mnand_pm_start_transaction (fs_pm_t pm_data)
{
  ...
  while ( (jrnl_free_cnt < (FS_JOURNAL_SIZE / 2)) ||
...
{
  uint32 i, j;
  uint32 clust_size = FS_MNAND_SECTORS_PER_CLUSTER;
  ...
  for (i=0; i<old_iovec_cnt; i=j)
  {
    ...
    memcpy (mnand_combined_buffer, iovec[i].buff_base, FS_MNAND_CLUSTER_SIZE);
    EFS_BAB_END (EFS_BAB_MNAND_PM_MEMCPY);
    buff_idx = FS_MNAND_CLUSTER_SIZE;
    ...
    for (j=i+1; j<old_iovec_cnt; ++j)
    {
      ...
      EFS_BAB_START (EFS_BAB_MNAND_PM_MEMCPY);
      memcpy ((mnand_combined_buffer+buff_idx), iovec[j].buff_base,
              FS_MNAND_CLUSTER_SIZE);
      EFS_BAB_END (EFS_BAB_MNAND_PM_MEMCPY);
    ...
    fs_device_write_sectors (pm->dev, iovec[i].sector_id,
        mnand_combined_buffer, combined_sect_cnt, FS_FOP_USER);
                                ...
                                uint32 iovec_cnt)
{
  ...
  if (iovec_cnt == 1)
  {
    fs_device_write_iovec (pm->dev, iovec, iovec_cnt, FS_FOP_USER);
  }
  else
  {
...
{
  fs_sector_id phy_sect;
  phy_sect = page * FS_MNAND_SECTORS_PER_CLUSTER;
