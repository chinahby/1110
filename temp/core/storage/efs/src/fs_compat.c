...
Copyright (C) 2002-2007, 2009 by QUALCOMM Inc. All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_compat.c#2 $ $DateTime: 2009/07/06 18:30:05 $ $Author: ebuller $
  ...
  fs_enum_type file_type)
{
  ...
  while (1) {
    ...
    fs_strlcpy (pathbuf, dirname, sizeof (pathbuf));
    fs_strlcat (pathbuf, "/", sizeof (pathbuf));
    fs_strlcat (pathbuf, result->d_name, sizeof (pathbuf));
...
fs_compat_enum_init (fs_cmd_type *cmd_ptr)
{
  ...
  fs_strlcpy ((char *) it->rootname, cmd_ptr->fs_op.parms.enum_init.dirname,
      sizeof (it->rootname));
...
fs_compat_enum_move (fs_cmd_type *cmd_ptr, int direction)
{
  ...
  do {
          ...
          FS_DIRENT_HAS_ST_SIZE))
    {
      ...
      fs_strlcpy (pathbuf, (char *) iterator->rootname, sizeof (pathbuf));
      fs_strlcat (pathbuf, "/", sizeof (pathbuf));
      fs_strlcat (pathbuf, idata.dirent_ptr->d_name, sizeof (pathbuf));
      if (efs_stat (pathbuf, &stat_buf) == -1)
      {
        // ERR_FATAL ("Figure out what to do here", 0, 0, 0);
        return FS_BAD_FILE_NAME_S;
    ...
    fs_strlcpy (data_ptr->fullname, idata.dirent_ptr->d_name,
