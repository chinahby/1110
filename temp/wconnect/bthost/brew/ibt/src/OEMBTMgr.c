...
$Header: //source/qcom/qct/wconnect/bthost/brew/ibt/rel/00.00.26/src/OEMBTMgr.c#3 $ 
$DateTime: 2009/06/17 17:39:49 $
$Author: c_bhatg $

  when        who  what, where, why
  ----------  ---  ------------------------------------------------------------
  2009-06-12   gb  Appropriate Security mapping of 2.0 specific sec level 
                   to 2.1 service level security.  
...
)
{
   ...
   if ((NULL != pMe) && (pgTheOEMBTMgr == pMe) && (NULL != pSvcId))
   {
                                       ...
                                       i++) == AEE_SUCCESS))
      {
         ...
         stat = bt_cmd_rm_set_sm4 (pMe->appId, &service_ident, sec_level, 
                                   TRUE, FALSE);
#endif /* FEATURE_BT_2_1 */
         result = OEMBT_CmdStatus2AEEResult (stat);
         if (AEE_SUCCESS != result)
