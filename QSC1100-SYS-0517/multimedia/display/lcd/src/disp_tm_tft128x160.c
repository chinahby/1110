...
GENERAL DESCRIPTION
  This module is the driver for the Epson TM QVGA LCD Display panel.

EXTERNALIZED FUNCTIONS
  tm_tft128x160_disp_init
...
#define TM_TFT128x160_DISP_WIDTH             128
#define TM_TFT128x160_DISP_HEIGHT            160

#define TM_TFT128x160_DISP_MIN_CONTRAST      0
#define TM_TFT128x160_DISP_MAX_CONTRAST      255
#define TM_TFT128x160_DISP_DEFAULT_CONTRAST  128

#define TM_TFT128x160_DISP_MIN_BACKLIGHT     0 
#define TM_TFT128x160_DISP_MAX_BACKLIGHT     7
#define TM_TFT128x160_DISP_DEFAULT_BACKLIGHT 3

#define TM_TFT128x160_DISP_ENABLE            TRUE
#define TM_TFT128x160_DISP_DISABLE           FALSE
...
#define LCD_VISIBLE_BACKLIGHT_LEVELS   \
        ( (TM_TFT128x160_DISP_MAX_BACKLIGHT) - (TM_TFT128x160_DISP_MIN_BACKLIGHT) )
...
static void tm_tft128x160_disp_update
(
...
#define TM_TFT_WRITE_CMD(v)  out_byte(lcatDispCfg[0].cmd_addr,(v))
#define TM_TFT_WRITE_DATA(v)  out_byte(lcatDispCfg[0].param_addr,(v))
#define TM_TFT_DELAY(v) clk_busy_wait(v*1000)
...
static lcd_properties_type lptDispInfo[LCD_MAX_DISPLAYS] =
{
    ...
    */
   {
      TM_TFT128x160_DISP_HEIGHT, //320
      TM_TFT128x160_DISP_WIDTH, //240
      8,             // Bits per pixel    
...
static rex_crit_sect_type        tm_tft128x160_crit_sect = {0};
static disp_info_type            tm_tft128x160_disp_info = {0};
static tm_tft128x160_state_type tm_tft128x160_state = {0};
...
int tm_tft128x160_install(char *str);
...
static void tm_tft128x160_reset(void)
{
     /* Reset controller to ensure clean state */
     /* Transfer command to display hardware*/  
     TM_TFT_WRITE_CMD(TM_TFT128x160_SOFT_RESET_C);
     clk_busy_wait(1000);	 
} /*tm_tft128x160_reset */

/*===========================================================================

FUNCTION      tm_tft128x160_HW_INIT
...
===========================================================================*/
static void tm_tft128x160_hw_init(void)
{
	uint32 regval = 0;
...
} /* tm_tft128x160_hw_init() */


/*===========================================================================

FUNCTION      tm_tft128x160_SET_SCREEN_AREA
...
===========================================================================*/
static void tm_tft128x160_set_screen_area(uint32 start_row, uint32 start_col,
                                           uint32 end_row, uint32 end_col)
{
	...
	*/
	    ...
	    (end_row   < lptDispInfo[nDisp].height) )
	{
		...
		/* Transfer command to display hardware*/
		TM_TFT_WRITE_CMD(TM_TFT128x160_SET_START_ADDRESS_C);
		TM_TFT_WRITE_DATA((uint8)0);
		TM_TFT_WRITE_DATA((uint8)(start_col&0xff));
		TM_TFT_WRITE_DATA((uint8)0);
		TM_TFT_WRITE_DATA((uint8)(end_col&0xff));

		/* Set LCD hardware to set start address */
		/* Transfer command to display hardware */
		TM_TFT_WRITE_CMD(TM_TFT128x160_SET_END_ADDRESS_C);
		TM_TFT_WRITE_DATA((uint8)0);
		TM_TFT_WRITE_DATA((uint8)(start_row&0xff));
		TM_TFT_WRITE_DATA((uint8)0);
		TM_TFT_WRITE_DATA((uint8)(end_row&0xff));
	}
} /* tm_tft128x160_set_screen_area() */

/*===========================================================================

FUNCTION      tm_tft128x160_DISP_SET_BACKLIGHT
...
===========================================================================*/
static void tm_tft128x160_disp_set_backlight(byte level)
{
	static uint32 current_level =  TM_TFT128x160_DISP_MIN_BACKLIGHT;
  
	if(tm_tft128x160_state.disp_initialized &&
		tm_tft128x160_state.disp_powered_up  &&
		tm_tft128x160_state.display_on)
	{
		if (level!= current_level)
		{
			rex_enter_crit_sect(&tm_tft128x160_crit_sect);

			/* Bound given intensity */
			if (level > TM_TFT128x160_DISP_MAX_BACKLIGHT )
			{
				level = TM_TFT128x160_DISP_MAX_BACKLIGHT;
			}
			else if (TM_TFT128x160_DISP_MIN_BACKLIGHT - level > 0)
			{
				/* Turn backlight off */
				level = TM_TFT128x160_DISP_MIN_BACKLIGHT;
			}

		{
			...
			*/
			...
			current_level = level;   
			rex_leave_crit_sect(&tm_tft128x160_crit_sect);
		}
	}
} /* tm_tft128x160_disp_set_backlight */

/*===========================================================================

FUNCTION      tm_tft128x160_DISP_CLEAR_SCREEN_AREA
...
===========================================================================*/
static void tm_tft128x160_disp_clear_screen_area(word start_row, word start_col,
                                                  word end_row, word end_col)
{
		...
		tm_tft128x160_state.display_on)
	{
		...
		tm_tft128x160_set_screen_area(start_row, start_col, end_row, end_col);
		/* Transfer command to display hardware */
		TM_TFT_WRITE_CMD(TM_TFT128x160_RAM_WRITE_C);  
		while(i--)
		{
			TM_TFT_WRITE_DATA((uint8)(whitebpp));
			TM_TFT_WRITE_DATA((uint8)(whitebpp));
		}

		rex_leave_crit_sect(&tm_tft128x160_crit_sect);
	}
} /* tm_tft128x160_disp_clear_screen_area() */


/*===========================================================================

FUNCTION      tm_tft128x160_DISP_CLEAR_WHOLE_SCREEN
...
===========================================================================*/
static void tm_tft128x160_disp_clear_whole_screen(void)
{
	tm_tft128x160_disp_clear_screen_area(0, 
	                                    0,
	                                    tm_tft128x160_disp_info.disp_height-1,
	                                    tm_tft128x160_disp_info.disp_width-1);
} /* tm_tft128x160_disp_clear_whole_screen() */


/*===========================================================================

FUNCTION      tm_tft128x160_DISP_OFF
...
===========================================================================*/
static void tm_tft128x160_disp_off(void)
{
	rex_enter_crit_sect(&tm_tft128x160_crit_sect);

	if(tm_tft128x160_state.disp_powered_up && tm_tft128x160_state.display_on)
	{
		/* Add code here */
		/* Transfer command to display hardware */
		TM_TFT_WRITE_CMD(TM_TFT128x160_DISPLAY_OFF_C); 
		clk_busy_wait(1000);
		tm_tft128x160_disp_set_backlight(TM_TFT128x160_DISP_MIN_BACKLIGHT);
		/* Display put to SLEEP state */
		tm_tft128x160_state.display_on = FALSE;
	}

	rex_leave_crit_sect(&tm_tft128x160_crit_sect);
} /* tm_tft128x160_disp_off() */


/*===========================================================================

FUNCTION      tm_tft128x160_DISP_ON
...
===========================================================================*/
static void tm_tft128x160_disp_on(void)
{
	rex_enter_crit_sect(&tm_tft128x160_crit_sect);
	...
	{  
		/* Transfer command to display hardware */
		TM_TFT_WRITE_CMD(TM_TFT128x160_DISPLAY_ON_C); 
		clk_busy_wait(1000);

		/* Display put to ACTIVE state */
		tm_tft128x160_state.display_on = TRUE;
	}

	rex_leave_crit_sect(&tm_tft128x160_crit_sect);
} /* tm_tft128x160_disp_on() */


/*===========================================================================

FUNCTION      tm_tft128x160_DISP_POWERUP
...
===========================================================================*/
static void tm_tft128x160_disp_powerup(void)
{
    rex_enter_crit_sect(&tm_tft128x160_crit_sect);

    if (!tm_tft128x160_state.disp_powered_up && !tm_tft128x160_state.display_on)
    {
    	TM_TFT_WRITE_CMD(0x11); //Exit Sleep
		TM_TFT_DELAY(120);		
		
		TM_TFT_WRITE_CMD(0x26); //Set Default Gamma
		TM_TFT_WRITE_DATA(0x04);
		
		TM_TFT_WRITE_CMD(0xB1);
		TM_TFT_WRITE_DATA(0x08);
		TM_TFT_WRITE_DATA(0x10);
		
		TM_TFT_WRITE_CMD(0xC0); //Set VRH1[4:0] & VC[2:0] for VCI1 & GVDD
		TM_TFT_WRITE_DATA(0x02);
		TM_TFT_WRITE_DATA(0x00);
		
		TM_TFT_WRITE_CMD(0xC1); //Set BT[2:0] for AVDD & VCL & VGH & VGL
		TM_TFT_WRITE_DATA(0x02);
		
		TM_TFT_WRITE_CMD(0xC5); //Set VMH[6:0] & VML[6:0] for VOMH & VCOML
		TM_TFT_WRITE_DATA(0x4C);//0x4C
		TM_TFT_WRITE_DATA(0x5E);
		
		TM_TFT_WRITE_CMD(0xC7); 
		TM_TFT_WRITE_DATA(0x00);
		
		
		
		
		TM_TFT_WRITE_CMD(0x3a); //Set Color Format
		TM_TFT_WRITE_DATA(0x05);
		
		TM_TFT_WRITE_CMD(0x2A); //Set Column Address
		TM_TFT_WRITE_DATA(0x00);
		TM_TFT_WRITE_DATA(0x00);
		TM_TFT_WRITE_DATA(0x00);
		TM_TFT_WRITE_DATA(0x7F);
		TM_TFT_WRITE_CMD(0x2B); //Set Page Address
		TM_TFT_WRITE_DATA(0x00);
		TM_TFT_WRITE_DATA(0x00);
		TM_TFT_WRITE_DATA(0x00);
		TM_TFT_WRITE_DATA(0x9F);
		
		TM_TFT_WRITE_CMD(0x36); //Set Scanning Direction
		TM_TFT_WRITE_DATA(0x60);  //..0x60 //..0x40 //..0x20 //..0x00 //..0x80 //0xa0 //..0xc0 //0xe0
		
		TM_TFT_WRITE_CMD(0xB7); //Set Source Output Direction
		TM_TFT_WRITE_DATA(0x00);
		
		TM_TFT_WRITE_CMD(0xf2); //Enable Gamma bit
		TM_TFT_WRITE_DATA(0x01);
		
		TM_TFT_WRITE_CMD(0xE0);
		TM_TFT_WRITE_DATA(0x3F);//p1
		TM_TFT_WRITE_DATA(0x22);//p2
		TM_TFT_WRITE_DATA(0x20);//p3
		TM_TFT_WRITE_DATA(0x24);//p4
		TM_TFT_WRITE_DATA(0x20);//p5
		TM_TFT_WRITE_DATA(0x0C);//p6
		TM_TFT_WRITE_DATA(0x4E);//p7
		TM_TFT_WRITE_DATA(0xB7);//p8
		TM_TFT_WRITE_DATA(0x3C);//p9
		TM_TFT_WRITE_DATA(0x19);//p10
		TM_TFT_WRITE_DATA(0x22);//p11
		TM_TFT_WRITE_DATA(0x1E);//p12
		TM_TFT_WRITE_DATA(0x02);//p13
		TM_TFT_WRITE_DATA(0x01);//p14
		TM_TFT_WRITE_DATA(0x00);//p15
		
		TM_TFT_WRITE_CMD(0xE1);
		TM_TFT_WRITE_DATA(0x00);//p1
		TM_TFT_WRITE_DATA(0x1B);//p2
		TM_TFT_WRITE_DATA(0x1F);//p3
		TM_TFT_WRITE_DATA(0x0B);//p4
		TM_TFT_WRITE_DATA(0x0F);//p5
		TM_TFT_WRITE_DATA(0x13);//p6
		TM_TFT_WRITE_DATA(0x31);//p7
		TM_TFT_WRITE_DATA(0x84);//p8
		TM_TFT_WRITE_DATA(0x43);//p9
		TM_TFT_WRITE_DATA(0x06);//p10
		TM_TFT_WRITE_DATA(0x1D);//p11
		TM_TFT_WRITE_DATA(0x21);//p12
		TM_TFT_WRITE_DATA(0x3D);//p13
		TM_TFT_WRITE_DATA(0x3E);//p14
		TM_TFT_WRITE_DATA(0x3F);//p15
		
		TM_TFT_WRITE_CMD(0x29); // Display On
        tm_tft128x160_state.disp_powered_up = TRUE;
   }
...
FUNCTION      tm_tft128x160_DISP_POWERDOWN
...
===========================================================================*/
static int tm_tft128x160_disp_powerdown(void)
{
	rex_enter_crit_sect(&tm_tft128x160_crit_sect);
	...
	if (tm_tft128x160_state.disp_powered_up && !tm_tft128x160_state.display_on) 
	{
		...
		*/
		/* Transfer command to display hardware */
		tm_tft128x160_reset();
		TM_TFT_WRITE_CMD(0x10); //Sleep in
		/* Reset controller to ensure clean state */
		/* Transfer command to display hardware*/
		//out_word(lcatDispCfg[0].cmd_addr, DISP_EPSON_CMD9(TM_TFT128x160_VDD_OFF_C));   miaoxiaoming
		clk_busy_wait(100);
		tm_tft128x160_state.disp_powered_up = FALSE;
	}

	rex_leave_crit_sect(&tm_tft128x160_crit_sect);

	return FALSE;
} /* tm_tft128x160_disp_powerdown() */


/*===========================================================================

FUNCTION      tm_tft128x160_DISP_INIT
...
===========================================================================*/
int tm_tft128x160_disp_init(void)
{
	if (tm_tft128x160_state.disp_initialized) 
	{
	...
	tm_tft128x160_disp_info.disp_width         = TM_TFT128x160_DISP_HEIGHT;//TM_TFT128x160_DISP_WIDTH;
	tm_tft128x160_disp_info.disp_height        = TM_TFT128x160_DISP_WIDTH;//TM_TFT128x160_DISP_HEIGHT;
	tm_tft128x160_disp_info.bpp                = DISP_16BPP;
	tm_tft128x160_disp_info.palette_support    = FALSE;
	tm_tft128x160_disp_info.contrast_support   = FALSE;
	tm_tft128x160_disp_info.contrast_min       = TM_TFT128x160_DISP_MIN_CONTRAST;
	tm_tft128x160_disp_info.contrast_max       = TM_TFT128x160_DISP_MAX_CONTRAST;
	tm_tft128x160_disp_info.contrast_default   = TM_TFT128x160_DISP_DEFAULT_CONTRAST;
	tm_tft128x160_disp_info.backlight_support  = TRUE;
	tm_tft128x160_disp_info.backlight_min      = TM_TFT128x160_DISP_MIN_BACKLIGHT;
	tm_tft128x160_disp_info.backlight_max      = TM_TFT128x160_DISP_MAX_BACKLIGHT;
	tm_tft128x160_disp_info.backlight_default  = TM_TFT128x160_DISP_DEFAULT_BACKLIGHT;
  	tm_tft128x160_disp_info.lcd_type           = TM_TFT128X160;
  	tm_tft128x160_disp_info.phys_width         = TMD_QVGA_LCD_DIMENSION_WIDTH;    
  	tm_tft128x160_disp_info.phys_height        = TMD_QVGA_LCD_DIMENSION_HEIGHT; 

	rex_init_crit_sect(&tm_tft128x160_crit_sect);

	tm_tft128x160_hw_init();

	tm_tft128x160_disp_powerup();

	tm_tft128x160_disp_on();

	tm_tft128x160_state.disp_initialized = TRUE;
	

	tm_tft128x160_disp_clear_whole_screen();

	tm_tft128x160_disp_set_backlight(TM_TFT128x160_DISP_MAX_BACKLIGHT);
...
FUNCTION      tm_tft128x160_DISP_GET_INFO
...
===========================================================================*/
static disp_info_type tm_tft128x160_disp_get_info(void)
{
	return tm_tft128x160_disp_info;
} /* tm_tft128x160_disp_get_info() */


/*===========================================================================

FUNCTION      tm_tft128x160_DISP_COPY
...
===========================================================================*/
static void tm_tft128x160_disp_copy(void *src_ptr, dword copy_count)
{  
	int i;
	uint8 *pdata = src_ptr;
	uint8 data1,data2;
    ...
    if(pdata != NULL)
    {
        for ( i = 0; i < copy_count; i++ )
        {
			...
			data2 = *pdata;
			pdata++;
			TM_TFT_WRITE_DATA((uint8)(data2));			
			TM_TFT_WRITE_DATA((uint8)(data1));			
        }        
    }
    else 
    {
...
} /* tm_tft128x160_disp_copy() */


/*===========================================================================

FUNCTION      tm_tft128x160_DISP_UPDATE
...
)
{
	...
	if (!buf_ptr                          || 
		!tm_tft128x160_state.disp_initialized ||
		!tm_tft128x160_state.disp_powered_up  ||
		!tm_tft128x160_state.display_on)
	{
		return;
	...
	if ((dst_starting_row + num_of_rows) > tm_tft128x160_disp_info.disp_height)
	{
		num_of_rows = tm_tft128x160_disp_info.disp_height - dst_starting_row;
	}
	
	if ((dst_starting_column + num_of_columns) > tm_tft128x160_disp_info.disp_width)
	{
		num_of_columns = tm_tft128x160_disp_info.disp_width - dst_starting_column;
	}

	/* Ensure buffer aligned and parameters valid */
	if((src_starting_row    < tm_tft128x160_disp_info.disp_height) &&
		(src_starting_column < tm_tft128x160_disp_info.disp_width)  &&
		(dst_starting_row    < tm_tft128x160_disp_info.disp_height) &&
		(dst_starting_column < tm_tft128x160_disp_info.disp_width)  &&
		(!(((uint32)buf_ptr) & 0x3)))
	{
		rex_enter_crit_sect(&tm_tft128x160_crit_sect);    

		tm_tft128x160_set_screen_area(dst_starting_row, dst_starting_column, 
		...
		if((src_starting_column == 0) && (num_of_columns == src_width)) 
		{
			...
			tm_tft128x160_disp_copy(src_ptr, copy_count);
		} 
		else 
		{
...
===========================================================================*/
static void tm_tft128x160_disp_set_contrast(word contrast)
{
	return;
} /* tm_tft128x160_disp_set_contrast() */


/*===========================================================================

FUNCTION      tm_tft128x160_DISP_IOCTL
...
===========================================================================*/
static int tm_tft128x160_disp_ioctl ( int cmd, void *arg )
{
	...
	disp_update_cmd = (disp_update_type*)arg;
		...
		{
			tm_tft128x160_disp_update(disp_update_cmd->buf_ptr,
	...
	case IOCTL_DISP_GET_INFO:
		*(disp_info_type *)arg = tm_tft128x160_disp_get_info();
		break;
	...
	case IOCTL_DISP_SET_CONTRAST:
		tm_tft128x160_disp_set_contrast(*(word *)arg);
		break;

	case IOCTL_DISP_SET_BACKLIGHT:
		tm_tft128x160_disp_set_backlight(*(byte *)arg);
		break;
	...
	case IOCTL_DISP_CLEAR_SCREEN_AREA:
		tm_tft128x160_disp_clear_screen_area(((disp_cls_type *)arg)->start_row,
	...
	return 1;
} /* tm_tft128x160_disp_ioctl() */


/*===========================================================================

FUNCTION      TM_TFT128x160_INSTALL
