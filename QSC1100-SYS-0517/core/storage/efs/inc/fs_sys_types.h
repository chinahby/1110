 ...
 ***********************************************************************/
 ...
 /*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/inc/fs_sys_types.h#5 $ $DateTime: 2008/12/12 11:04:36 $ $Author: richardp $
...
/**********************************************************************
 * Configuration items have been moved to fs_config.h
 */
#include "fs_config.h"
...
/**
 * Each task has an errno field in its tcb.  When an error happens in
 * any efs call, this field in for the calling task to reflect the
 * error code (listed in fs_errno.h). */
#ifdef FS_STANDALONE
  extern int efs_errno;
#else
  #include "rex.h"
  #define efs_errno (rex_self()->err_num)
#endif
 ...
 * FS_SHORTNAME_THRESHOLD + fs_inode_t prefix. Anything larger than this
 * will be hashed with the resulting hash stored in the db */
#define NAMED_VNODE_SIZE_SYMLINK (FS_SHORTNAME_THRESHOLD + sizeof (fs_inode_t))

#if defined(FEATURE_EFS_EXTFS) || defined(FEATURE_EFS_FDI)
