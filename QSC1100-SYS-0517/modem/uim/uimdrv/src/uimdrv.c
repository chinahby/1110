...
$Header: //source/qcom/qct/modem/uim/su/baselines/qsc1110/rel/3.3/uim/uimdrv/src/uimdrv.c#2 $ $DateTime: 2009/06/17 06:12:33 $ $Author: rmandala $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (sw2 == (int)SW2_NORMAL_END)
  {
    ...
    if(uim_processing_envelope_apdu() == TRUE)
    {
      ...
      {
        resp_buf[uim_drv_slot]->sw1 = (int)SW1_WARNINGS1;
      }
      else if(uim_warning2_indicator == TRUE)
      {
        resp_buf[uim_drv_slot]->sw1 = (int)SW1_WARNINGS2;
      }
...
)
{
  ...
  /* Update the flag to indicate that WARNINGS1 status is returned by the SIM */
...
)
{
  ...
  /* Update the flag to indicate that WARNINGS2 status is returned by the SIM */
...
#ifndef FEATURE_UIM_DONOT_SUPPRESS_GET_RESPONSE_APDU_FOR_SEEK
                       (last_sent_apdu[uim_drv_slot].instrn != SEEK ) &&
#endif /* FEATURE_UIM_DONOT_SUPPRESS_GET_RESPONSE_APDU_FOR_SEEK */
                       (last_sent_apdu[uim_drv_slot].instrn != ENVELOPE)
                     )
                  {
...
/*  UIM_TEST_INT_PROCESSING(UIM_DEBUG_ENTER_UART_ISR); */
...
#endif /* FEATURE_UIM_UART_DM */
  {
    ...
    } /* end if - the receiver detected an error. */
    /* Check for receive overrun condition */
    else if ((uart_status & (MSMU_SR_RXBREAK))
