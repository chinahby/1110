...
#define AEE_SOUNDCTL_CMD_SET_QCONCERT_PLUS_REVERB         (AEE_SOUNDCTL_CMD_BASE + 11)
...
//Audio qconcert plus reverb parameter type
typedef struct AEESoundCtlAudioQconcertPlusReverb
{
  uint16                         nCmd;       //Command for qconcert plus reverb
  uint16                         nData;      //Data specific to the command                                                 
}AEESoundCtlAudioQconcertPlusReverb;
...
AEEINTERFACE(ISoundCtl)
{
   ...
   //Audio Qconcert plus Reverb functions
   int        (*SetQconcertPlusReverbParams)(ISoundCtl* po, AEESoundCtlAudioQconcertPlusReverb* aepType);
...
#define ISOUNDCTL_SetQconcertPlusReverbParams(p, aep)  AEEGETPVTBL(p, ISoundCtl)->SetQconcertPlusReverbParams(p, aep)
