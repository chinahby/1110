...
Notice that changes are listed in reverse chronological order.
  ...
  $Header: //source/qcom/qct/multimedia/audio/6k/ver1/audfmt/main/latest/src/audaac.c#7 $
   
when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/11/09   kkm     Modified audaac_do_file_spec to select ops correctly (seek|time)
                   for adts format
09/11/09   bk      Modified audaac_cleanup_vbr() in order to take care of
                   freeing audaac_parse_ctl.vbr_parse.leftover.                   
06/26/09   kk      Fixed CR 186752 Aberror Observed after performing 
                   FF/rewind operaion on one particular AAC clip.
...
)
{
  ...
  if ((aac_info.song_len_ms > 0) || 
      ((aac_info.config.aac_format ==AUDMAIN_AAC_DATA_FORMAT_ADTS)&&(audaac_parse_ctl.file_len !=0)))
  {
    ...
    case AUDMAIN_AAC_DATA_FORMAT_LOAS:
      snd_spec.supported_ops = SND_SUPPORTED_OPS_MASK_GETTIME | SND_SUPPORTED_OPS_MASK_SEEK;
      break;
  ...
  while ((buf_ptr - buf) < buf_len) {
    ...
    if (IS_SYNC(buf_ptr) &&
      ...
      if (!playback) {
        ...
        MSG_MED(" Notplayback-  framenum = %d, frame_pos = %d, frame_len = %d "
                   ,vbr_parse->frame_num,vbr_parse->vbr_recorded,frame_len);
      ...
      if (playback) {
        ...
        if ((vbr_parse->rrwnd_num == 0) &&
          ...
          if (vbr_parse->frame_pos != NULL) {
            ...
            MSG_MED(" parse_frame-  framenum = %d, frame_pos = %d, frame_len = %d"
                       ,vbr_parse->frame_num,vbr_parse->vbr_recorded,frame_len);
            vbr_parse->vbr_recorded += frame_len;
...
void audaac_cleanup_vbr (void)
{
  if (aac_info.config.aac_format == AUDMAIN_AAC_DATA_FORMAT_ADTS) { 
    ...
    if (audaac_parse_ctl.vbr_parse.leftover_size) {

      free(audaac_parse_ctl.vbr_parse.leftover);
      audaac_parse_ctl.vbr_parse.leftover = NULL;
      audaac_parse_ctl.vbr_parse.leftover_size = 0;
    }
    audaac_parse_ctl.ops = AUDMPEG_PROC_OPS_NONE;
    memset(&(audaac_parse_ctl.vbr_parse), 0, sizeof(audmpeg_vbr_parse_type));
  }
