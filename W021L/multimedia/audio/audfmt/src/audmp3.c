...
Notice that changes are listed in reverse chronological order.
  ...
  $Header: //source/qcom/qct/multimedia/audio/6k/ver1/audfmt/main/latest/src/audmp3.c#6 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/13/09    bk     Modified the audmp3_find_sync() for finding out minimum
                   of two and maximum of four sync words in order to 
                   correctly identifying MP3 file.
...
#define SYNC_MIN_LEN            4
#define FRAME_SYNC_MAX_COUNT	4
#define FRAME_SYNC_MIN_COUNT	2
#define MP3_MIN_FRAME_SIZE	13
...
)
{
  ...
  mp3_header_type  tHdr;
  mp3_info_type    tmp3_info;
  uint32           vbrHdrFlags  = mp3_info.vbr_hdr.flags;

  /* Need a minimum number of bytes in the buffer to have a sync
  */
  if(buf_len < SYNC_MIN_LEN) {
    return(buf_len);
  }

	/* Look through the buffer to find the sync bytes.
	*/
 while (index <= (buf_len - SYNC_MIN_LEN)) {
  frameSyncCount = 0;
  underflow = FALSE;
   ...
   if(audmp3_is_valid_mp3(&mp3_info.hdr)) {
    if(mp3_info.vbr_hdr.flags || !mp3_info.hdr.bitrate) {
    /************************* FREE FORMAT *******************************/
    int      index1	       = index+1;
    byte  *buffer1	       = buffer+1;
    int	framelength = 0;

   while((index1 <= (buf_len - SYNC_MIN_LEN)) 
          && (frameSyncCount < FRAME_SYNC_MAX_COUNT)) {
     if(IS_SYNC(buffer1)) {		
	audmp3_extract_mp3_header(buffer1, &tmp3_info);
  /* check prev frame header is matched with current frame */
     if((framelength >= MP3_MIN_FRAME_SIZE) && IS_VALID_FRAME(buffer1) 
	 	&& audmp3_supported_mp3(&tmp3_info.hdr)) {
	framelength = 0;
	frameSyncCount++;
	}
	}
   ...
   if((index1 >= (buf_len - SYNC_MIN_LEN)) && 
   	 (frameSyncCount >= FRAME_SYNC_MIN_COUNT))
   underflow = TRUE;
  ...
  /******************************* CBR + VBR ***********************/
   ...
   while((frameSyncCount < FRAME_SYNC_MAX_COUNT) 
   	     && (frame_len > MP3_MIN_FRAME_SIZE)) {
	...
	* look for another sync */
	if ((index1 + frame_len) < (buf_len - (SYNC_MIN_LEN - 1))) {
	 next_fr_off = frame_len;
	 next_fr_off_end = next_fr_off +
		MIN(9, buf_len - (index1+frame_len) - (SYNC_MIN_LEN - 1));

         /* Search the next few bytes for the next sync */
     while (next_fr_off < next_fr_off_end)  {
     if (IS_SYNC(buffer1+next_fr_off)) {
      audmp3_extract_mp3_header((buffer1+next_fr_off), &tmp3_info);
     if(IS_VALID_FRAME(buffer1+next_fr_off) 
	  && audmp3_supported_mp3(&tmp3_info.hdr)) {
      frame_len = audmp3_calc_frame_bytes(&tmp3_info.hdr,&tmp3_info.vbr_hdr);
      frameSyncCount++;
      break;
	}
	}
   ...
   if(next_fr_off >= next_fr_off_end) {
  /* not valid sync so search for first valide sync */
   break; 
  }
   index1	+=  next_fr_off;
   buffer1 += next_fr_off;
   }else {
   if(frameSyncCount >= FRAME_SYNC_MIN_COUNT)
    underflow = TRUE;
    break;
    }
   }
   ...
   if(underflow || (frameSyncCount >= FRAME_SYNC_MAX_COUNT)) {
				break;
    }
   }
    buffer++;
   ...
   if(frameSyncCount >= FRAME_SYNC_MIN_COUNT) {
    ...
    if(audmp3_supported_mp3(&mp3_info.hdr)) {
     if(vbrHdrFlags && (vbrHdrFlags != mp3_info.vbr_hdr.flags))
       mp3_info.vbr_hdr.flags = vbrHdrFlags;
     return(index);
     }
    }
	return(buf_len);
...
)
{
  ...
  if(parse_ctl != NULL && ms_forward > 0) {
    ...
    if (mp3_info.is_stream == TRUE ||
      ...
      if(IS_VBR) {
        ...
        if (mp3_info.vbr_hdr.toc_flag) {
          ...
          MSG_ERROR("This is a VBR file and the mp3_ptr->file_location = %d ",mp3_ptr->file_location,0,0);		  
        }
      } else {
        mp3_ptr->file_location =
                               audmp3_calc_byte_offset(ms_forward, *elapsed_ms);
	 MSG_ERROR("This is a CBR file and the mp3_ptr->file_location = %d",mp3_ptr->file_location,0,0);		
      }

      mp3_ptr->resync_state = AUDMPEG_RESYNC_START;
      if(mp3_ptr->file_location >= audmp3_parse_ctl.file_len) {
        *adjusted_ms = *elapsed_ms = mp3_info.song_len_ms;
	MSG_ERROR("In if(mp3_ptr->file_location >= audmp3_parse_ctl.file_len) *adjusted_ms = %d, mp3_info.song_len_ms = %d  ",*adjusted_ms,mp3_info.song_len_ms,0);	
      } else {
        if(IS_VBR) {
	...
	MSG_ERROR("This is a VBR file and the mp3_ptr->file_location = %d, mp3_info.song_len_ms = %d",*adjusted_ms,mp3_info.song_len_ms,0);			
          }
        } else {
          *adjusted_ms = *elapsed_ms = *elapsed_ms + ms_forward;
	MSG_ERROR("This is a CBR file and the mp3_ptr->file_location = %d  mp3_info.song_len_ms = %d",*adjusted_ms,mp3_info.song_len_ms,0);	  
        }
      }
    }
  }
