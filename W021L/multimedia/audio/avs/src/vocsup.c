...
$Header: //source/qcom/qct/multimedia/audio/6k/ver1/avs/main/latest/src/vocsup.c#11 $ $DateTime: 2009/10/23 01:37:24 $ $Author: nrchella $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/23/09   nr      Corrected abnormal code in vocsup_validate_frame_control_byte().
10/14/09   yp      CMI changes for SCMM
10/07/09   rm      Added condition to handle frame rate for VoIP
08/31/09   kk      Removed reseting the recording frame count during
...
word vocsup_rate_to_size (
  ...
  else if (so == VOC_CAP_IS127)
  {
    ...
    return(vocsup_frame_sizes_8k[rate]);
           ...
           || so == VOC_CAP_4GV_WB || so == VOC_CAP_4GV_WB_VOIP)
  {
	/* Avoid out of bound array access */
	if(rate >=  (sizeof(vocsup_frame_sizes_8k) / sizeof(vocsup_frame_sizes_8k[0])))
	{
