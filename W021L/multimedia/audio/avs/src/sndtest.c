...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/multimedia/audio/6k/ver1/avs/main/latest/src/sndtest.c#21 $ $DateTime: 2009/10/23 05:48:44 $ $Author: kmodak $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/23/09   knm     Added adie read/write support for marimba codec
...
static cmx_handle_type        file_pb_handle;
static boolean file_pb_is_playing = FALSE;                
static boolean file_pb_is_paused = FALSE;                

static char pb_test_file[2][CMX_AF_SAMPLE_RATE_48000+1]
                   [80] =
{
  {
    {"none"},
    {"brew/mod/mediaplayer/media/test_m_8k.mp3"},
    {"brew/mod/mediaplayer/media/test_m_11P025k.mp3"},
    {"brew/mod/mediaplayer/media/test_m_12k.mp3"},
    {"brew/mod/mediaplayer/media/test_m_16k.mp3"},
    {"brew/mod/mediaplayer/media/test_m_22P050k.mp3"},
    {"brew/mod/mediaplayer/media/test_m_24k.mp3"},
    {"brew/mod/mediaplayer/media/test_m_32k.mp3"},
    {"brew/mod/mediaplayer/media/test_m_44P10k.mp3"},
    {"brew/mod/mediaplayer/media/test_m_48k.mp3"}
  },

  {
    {"none"},
	{"brew/mod/mediaplayer/media/test_s_8k.mp3"},
	{"brew/mod/mediaplayer/media/test_s_11P025k.mp3"},
	{"brew/mod/mediaplayer/media/test_s_12k.mp3"},
	{"brew/mod/mediaplayer/media/test_s_16k.mp3"},
	{"brew/mod/mediaplayer/media/test_s_22P050k.mp3"},
	{"brew/mod/mediaplayer/media/test_s_24k.mp3"},
	{"brew/mod/mediaplayer/media/test_s_32k.mp3"},
	{"brew/mod/mediaplayer/media/test_s_44P10k.mp3"},
	{"brew/mod/mediaplayer/media/test_s_48k.mp3"}
  }
};
...
cmx_handle_type qcp_rec_handle;
cmx_qcp_rec_para_type cmx_qcp_rec_params;
...
/***************************************************************************/
/* Declarations for Marimba I2C Reads/Writes                        */
/***************************************************************************/
#ifdef MSMAUD_SCMM
#error code not present
#endif /* MSMAUD_SCMM */
...
#endif
    /* by default select 8Khz mono file */
    snd_test_block.file_pb_chnl_mode = 0;
    snd_test_block.file_pb_smpl_rate = 1;
  }
...
#error code not present
  ...
  if (snd_test_block.file_pb_op != 0xFF) 
  {
    switch (snd_test_block.file_pb_op) 
    {
      /* start playback */
      case 1:
        if (snd_test_block.file_pb_chnl_mode > 2)
        {
          MSG_ERROR("Can't start playback. Invalid file_pb_chnl_mode = %d", 
                    snd_test_block.file_pb_chnl_mode, 0,0 );
        }
        else if (snd_test_block.file_pb_smpl_rate > CMX_AF_SAMPLE_RATE_48000)
        {
          MSG_ERROR("Can't start playback. Invalid file_pb_smpl_rate = %d", 
                    snd_test_block.file_pb_smpl_rate, 0,0 );
        }

        file_pb_handle.source_type = CMX_SOURCE_EFS;
        file_pb_handle.efs.source_type = CMX_SOURCE_EFS;    
        memcpy(file_pb_handle.efs.filename,
               pb_test_file[snd_test_block.file_pb_chnl_mode][snd_test_block.file_pb_smpl_rate],
               sizeof(file_pb_handle.efs.filename));

        MSG_HIGH("Starting file playback with sampling rate=%d, chnl=%d", 
                 snd_test_block.file_pb_smpl_rate, snd_test_block.file_pb_chnl_mode, 0);
        cmx_audfmt_play(&file_pb_handle, CMX_AF_CALLBACK_NONE, CMX_AUDFMT_START_NORMAL, NULL, NULL);
        file_pb_is_playing = TRUE;
        file_pb_is_paused = FALSE;
        break;

      /* pause playback */
      case 2:
        if (file_pb_is_playing == TRUE)
        {
          cmx_audfmt_pause(NULL, NULL);
          file_pb_is_paused = TRUE;
        }
        break;

      /* resume playback */
      case 3:
        if ((file_pb_is_playing == TRUE) && (file_pb_is_paused == TRUE))
        {
          cmx_audfmt_resume(NULL, NULL);
          file_pb_is_paused = FALSE;
        }
        break;
      ...
      case 4:
        if (file_pb_is_playing == TRUE)
        {
          ...
          file_pb_is_playing = FALSE;
        }
        break;
    }

    snd_test_block.file_pb_op = 0xFF;
    snd_test_block.file_pb_chnl_mode = 0xFF;
    snd_test_block.file_pb_smpl_rate = 0xFF;
  }
  ...
  if ( snd_test_block.qcp_file_rec_op != 0xFF )
  {
    switch ( snd_test_block.qcp_file_rec_op )
    {
      /*----------------------------------------------------------*/
      ...
      case 1:
        /* Setup EFS handle */
        qcp_rec_handle.efs.source_type = CMX_SOURCE_EFS; 
        memcpy(qcp_rec_handle.efs.filename,
               "brew/mod/mediaplayer/media/test_reverse.qcp",
               sizeof("brew/mod/mediaplayer/media/test_reverse.qcp"));

        if (snd_test_block.qcp_file_rec_format == CMX_QCP_FORMAT_INVALID) 
        {
          MSG_ERROR("In valid qcp_file_rec_format = %d", 
                    snd_test_block.qcp_file_rec_format, 0, 0);
          break;
        }

        cmx_qcp_rec_params.format = snd_test_block.qcp_file_rec_format;
        cmx_qcp_rec_params.report_ms = 0;
        cmx_qcp_rec_params.auto_stop_ms = 0;

        /* Start recording */
        cmx_qcp_record_reverse(&qcp_rec_handle,
                               &cmx_qcp_rec_params,
                               NULL,
                               NULL);
        break;
      ...
      case 2:
        ...
        qcp_rec_handle.efs.source_type = CMX_SOURCE_EFS; 
        memcpy(qcp_rec_handle.efs.filename,
               "brew/mod/mediaplayer/media/test_forward.qcp",
               sizeof("brew/mod/mediaplayer/media/test_forward.qcp"));
        ...
        /* Start recording */
        cmx_qcp_record_forward(&qcp_rec_handle,
                               &cmx_qcp_rec_params,
                               NULL,
                               NULL);
        break;
      ...
      default:
        MSG_ERROR("QCP Record Test: invalid evb_rec value.", 0, 0, 0);

    }

    snd_test_block.qcp_file_rec_op = 0xFF;
    snd_test_block.qcp_file_rec_format = 0xFF;
  }
