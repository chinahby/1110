...
/*===========================================================================
 ...
 $Header: //depot/asic/msmshared/services/diag/MSMSHARED_DIAG_1.2/rbq.c#3 $
...
typedef PACK(uint16) rbq_blk_ovrn_type;

#define RBQ_INIT_PATTERN (0x06108626)
#define RBQ_BLK_OVRN_PATTERN (0xDEAD)
...
{
  rbq_blk_ovrn_type *pattern = NULL;
  boolean success = FALSE;

  if (blk_hdr)
  {
    pattern = (rbq_blk_ovrn_type *) 
      (((uint8 *) blk_hdr) + blk_hdr->length + sizeof (*blk_hdr));

    *pattern = RBQ_BLK_OVRN_PATTERN;
...
rbq_check_ovrn_pattern (rbq_header_type *blk_hdr)
{
  ...
  if (blk_hdr)
  {
    pattern = (PACK(uint16 *)) (((byte *) blk_hdr) + blk_hdr->length +
                                 sizeof (rbq_header_type));
    //TODO: Allow custom FPs to handle ovrns and other failures.
    if (*pattern == RBQ_BLK_OVRN_PATTERN)
    {
      success = TRUE;
    }
    else
