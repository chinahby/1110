...
/*=============================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_ftl.c#6 $
  $DateTime: 2009/04/06 18:41:29 $
  $Author: richardp $
...
ftl_raw_read (ftl_raw_device_t dev, uint32 sector, byte* buffer, uint16 count)
{
  ...
  for ( ; sector < last_page_to_read; ++sector)
  {
    if (device_page != LOGICAL_TO_DEVICE_PAGE (sector))
    {
      ...
      {
        ERR_FATAL ("phy_blk[%d] & LBN_MASK) < MAX_BLOCK_COUNT", phy_blk,0,0);
      }
     ...
     *   complexity
     */
    memcpy (buffer, page_buff_ptr, DEFAULT_MS_SECTOR_SIZE);
  ...
  while (src_data_buffer)
  {
        ...
        && (src_data_buffer->sector_num < logical_page_end))
    {
      ...
      if (dest_page_buffer != 0)
      {
        ...
        memcpy (dst_ptr, src_data_buffer->data_ptr, DEFAULT_MS_SECTOR_SIZE);
      }
