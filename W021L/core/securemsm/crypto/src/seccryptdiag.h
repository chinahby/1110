...
Notice that changes are listed in reverse chronological order.
  ...
  $PVCSPath: L:/src/asw/COMMON/vcs/seccryptdiag.h_v   1.3   14 Oct 2003 17:08:46   rhummel  $
  $Header: //source/qcom/qct/core/securemsm/crypto/main/latest/src/seccryptdiag.h#1 $ $DateTime: 2007/06/26 13:17:46 $ $Author: jmiera $
...
/*** Macros to make code more readable ***/
#ifdef FEATURE_SECCRYPT_DIAG_EVENTS

  #define SECCRYPT_DIAG_EVENT_ALLOC( payload ) \
    seccrypt_event_payload_type  (payload)
     
  #define SECCRYPT_CMD_DIAG_EVENT_ALLOC( payload ) \
    seccrypt_cmd_event_payload_type  (payload)
     
  #define SECCRYPT_DIAG_EVENT_SET( payload_ptr, ev, stat, cmd, inst_ptr)\
    (payload_ptr)->specific_event = (ev);                               \
    (payload_ptr)->status         = (stat);                             \
    (payload_ptr)->command        = (uint32)(cmd);                      \
    (payload_ptr)->instance_id    = (uint32)(inst_ptr);                 \
  ...
  #define SECCRYPT_DIAG_EVENT_REPORT( payload_ptr )                  \
    if( payload_ptr) event_report_payload( EVENT_SECCRYPT,           \
                                           sizeof( *(payload_ptr) ), \
                                           (payload_ptr) )

  #define SECCRYPT_CMD_DIAG_EVENT_REPORT( payload_ptr )              \
    if( payload_ptr) event_report_payload( EVENT_SECCRYPT_CMD,       \
                                           sizeof( *(payload_ptr) ), \
                                           (payload_ptr) )

#else /* FEATURE_SECCRYPT_DIAG_EVENTS */
...
#endif /* FEATURE_SECCRYPT_DIAG_EVENTS */
...
/*** Macros to make code more readable ***/
  ...
  #define SEC_LOG_LOCAL( ptr, lcode ) \
    lcode##_C_type*  (ptr) = NULL
  #define SEC_LOG_ALLOC( ptr, lcode ) \
    (ptr) = log_alloc( lcode##_C, sizeof( lcode##_C_type ) )
  #define SEC_LOG_SET( lvalue_ptr, lvalue_offset, data ) \
    if ( lvalue_ptr ) lvalue_ptr##->##lvalue_offset = (data)
  #define SEC_LOG_MEMCP( lvalue_ptr, lvalue_offset, data_ptr, len )    \
    if ( (lvalue_ptr) && (data_ptr) )                                  \
      memcpy( ((uint8*) &(lvalue_ptr##->##lvalue_offset)),             \
              ((uint8*) data_ptr), (len) )
  #define SEC_LOG_MEMSET( lvalue_ptr, lvalue_offset, datum, len )      \
    if ( (lvalue_ptr) )                                                \
      memset( ((uint8*) &(lvalue_ptr##->##lvalue_offset)),             \
              ((uint8) datum), (len) )
  #define SEC_LOG_COMMIT( ptr ) \
      if ( ptr ) log_commit( ptr )
#else
  #define SEC_LOG_LOCAL( ptr, lcode )
  #define SEC_LOG_ALLOC( ptr, lcode )
  #define SEC_LOG_SET( lvalue_ptr, lvalue_offset, data )
