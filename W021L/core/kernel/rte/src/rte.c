...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/kernel/rte/main/latest/src/rte.c#1 $
...
)
{
   ...
   memset( &piptr->os_info.tcb_ptr->ipc_info,
           0,
           sizeof(piptr->os_info.tcb_ptr->ipc_info));

#if 0
   /* Set up the space for the priority queue */
   piptr->os_info.tcb_ptr->ipc_info.mq_ptr = (struct pq_msg_queue_type*) piptr->os_info.tcb_ptr->ipc_info.mq;
#endif
   
   /* Create the thread representing the process */
   rex_def_task_internal( piptr->os_info.tcb_ptr,
...
{
   return (rte_process_id_type)rex_self()->ipc_info.node_id;
}
...
)
{
   ...
   memset( &tiptr->os_info.tcb_ptr->ipc_info,
           0,
           sizeof(tiptr->os_info.tcb_ptr->ipc_info));

#if 0   
   /* Set up the space for the priority queue */
   tiptr->os_info.tcb_ptr->ipc_info.mq_ptr = (struct pq_msg_queue_type*) tiptr->os_info.tcb_ptr->ipc_info.mq;
#endif
   
   /* Create the thread representing the service */
   rex_def_task_internal( tiptr->os_info.tcb_ptr,
   ...
   if (ipcns_service_register(service_id, tiptr->os_info.tcb_ptr))
   {
      MSG_LOW( "Thread (%s) has Node ID (%d) and provides Service (%d)",
               name,
               tiptr->os_info.tcb_ptr->ipc_info.node_id,
               service_id );
   }
   else
   {
  ...
  return tiptr->os_info.tcb_ptr->ipc_info.node_id;
}
