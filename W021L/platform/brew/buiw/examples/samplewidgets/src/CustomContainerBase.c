...
boolean ContainerBase_SetProperty(ContainerBase* me, uint16 wParam, uint32 dwParam)
{
   ...
   case PROP_FLAGS:
      // this isn't like your normal PROP_FLAGS block.  We need to 
      // check the flag before we set it.  Otherwise, we will catch
      // PROP_FLAGS meant for the derived container.
      if(dwParam & CWF_BROADCAST) {
         me->fBroadcast = (dwParam & CWF_BROADCAST);
         return TRUE;
      }
      break;
   }
...
boolean ContainerBase_DefHandleEvent(IWidget *po, AEEEvent evt, uint16 wParam, uint32 dwParam)
{
   ...
   // if you have a border member, this is where you'd ask it to
   // handle this event.
...
void ContainerBase_Draw(IWidget *po, ICanvas *piCanvas, int x, int y)
{
   ...
   // If your container supports a border, this would
   // be a good time to draw and account for it ...
...
boolean ContainerBase_IntersectOpaque(IWidget *po, AEERect *prcResult, const AEERect *prcIn)
{
   ...
   // if your container base class support a border, you
   // can use it to determine intersections here.
...
{
   // Tell widget it's losing focus if it had it
