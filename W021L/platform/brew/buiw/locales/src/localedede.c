...
/* -----------------------------------------------------------------------
    LocaleDEDE_Ctor 
   --- */
static void LocaleDEDE_Ctor(LocaleDEDE *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleDEDE_New  
   --- */
int LocaleDEDE_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleDEDE *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleDEDE, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleDEDE_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule); 
...
int LocaleDEDE_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALEDEDE) {
      return LocaleDEDE_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
