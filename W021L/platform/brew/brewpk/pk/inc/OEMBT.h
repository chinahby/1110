...
extern "C" {
   ...
   AEE_EXPORTS int OEMBT_GetSPPServer( OEMBT* pme, BTServiceClassUUID uuid, 
                                       const uint8* pszServiceName,
                                       const uint16 version, void** ppo );
   AEE_EXPORTS int OEMBT_GetSPPServerClone(OEMBT* pme,
                            int16 streamID,
                            void** ppo
                            );

   AEE_EXPORTS int OEMBT_GetSPPClient( OEMBT* pme, void** ppo );
   AEE_EXPORTS int OEMBT_GetDevDB( OEMBT* pme, void** ppo );
