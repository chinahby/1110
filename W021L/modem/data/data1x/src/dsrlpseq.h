...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/data/1x/1xrlp/main/lite/src/dsrlpseq.h#1 $ $DateTime: 2007/11/06 01:21:33 $ $Author: nsivakum $
...
#define DSRLPSEQ_GET_12BIT_FRAME_SEQ(seq) \
     ((((uint32)seq) >> 16) & 0x00000FFF)

/*===========================================================================

MACRO          DSRLPSEQ_GET_HDR_SEQ

DESCRIPTION    Gets the 22-bit HDR sequence number from the internal 32-bit
               representation.
...
RETURN VALUE   the 22-bit value, properly shifted into the lower three
               bytes.

SIDE EFFECTS   None.

===========================================================================*/
#define DSRLPSEQ_GET_HDR_SEQ(seq) \
     ((((uint32)seq) >> 4) & 0x003FFFFF)

/*===========================================================================
...
MACRO DSRLPSEQ_PUT_HDR_SEQ

DESCRIPTION
  This macro will take a 22-bit HDR sequence number and store it in the 
  appropriate bit position location of the internal representation of 32-bit
  sequence numbers.  The 32-bit value has the following struct:

         MSB                                    LSB
          |                                      |
          0000  00aa  aaaaaaaa  aaaaaaaaaaaa  0000          

  where:
  

    0000                          = fixed bit values
    aaaaaaaaaaaaaaaaaaaaa         = 22-bit HDR SEQ

PARAMETERS
  seq_32   - the 32-bit sequence number that is being generated.
  seq_22   - the 22-bit sequence number that is to be shifted in.

DEPENDENCIES
  None

RETURN VALUE
  the 32-bit sequence number with the 22-bit parameter bit-shifted in

SIDE EFFECTS
  None
===========================================================================*/
#define DSRLPSEQ_PUT_HDR_SEQ( seq_32, seq_22 ) \
  (seq_32 = ((((uint32)seq_32) & 0x0FFFFFF0) | \
             (((uint32)seq_22) << 4)))

/*===========================================================================
