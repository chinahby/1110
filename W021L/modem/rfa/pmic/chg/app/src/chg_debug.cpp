...
/* =======================================================================
  ...
  $Header: //source/qcom/qct/modem/rfa/pmic/chg_task/main/latest/app/src/chg_debug.cpp#14 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/07/09   hs      Fixed LINT errors.
09/23/09   hs      Modified charger type related messages.
...
void ChargingQxdmDebugger::outputAutoChargingDebugMessages(chg_auto_charging_debug_info_type information)
{
    if( chg_get_debug_state() >= CHG_DEBUG_INF_MINIMUM )
    {
        ...
        if(information.charger_irq_triggered == TRUE)
        {
            ...
            if(information.charger_type == CHARGER_TYPE__NONE)
            {
                MSG_FATAL("Charging Debug: auto charging -- charger removed; ", 0, 0, 0);
            }
            else if(information.charger_type == CHARGER_TYPE__WALL)
            {
                MSG_FATAL("Charging Debug: auto charging -- A regular wall charger attached; ", 0, 0, 0);
            }
            else if(information.charger_type == CHARGER_TYPE__USB_PC)
            {
                MSG_FATAL("Charging Debug: auto charging -- A PC USB attached; ", 0, 0, 0);
            }
            else if(information.charger_type == CHARGER_TYPE__USB_WALL)
            {
                MSG_FATAL("Charging Debug: auto charging -- A USB wall charger attached; ", 0, 0, 0);
            }
        }

        if(information.charger_type != CHARGER_TYPE__NONE)
        {
            if(information.charger_type == CHARGER_TYPE__WALL)
            {
                MSG_FATAL("Charging Debug: auto charging -- charger type: regular wall charger ; ", 0, 0, 0);

            }
            else if(information.charger_type == CHARGER_TYPE__USB_PC)
            {
                MSG_FATAL("Charging Debug: auto charging -- charger type: PC USB ; ", 0, 0, 0);
            }
            else if(information.charger_type == CHARGER_TYPE__USB_WALL)
            {
                MSG_FATAL("Charging Debug: auto charging -- charger type: USB wall charger ; ", 0, 0, 0);
            }
        ...
        if(information.battery_bad == TRUE)
        {
            MSG_FATAL("Charging Debug: battery is bad!! ", 0, 0, 0); 
        }
