...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/rfa/pmic/chg_task/main/latest/app/src/chg_common.h#16 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/07/09   hs      Removed unnecessary header file inclusion.
09/17/09   hs      Removed chg_general_status.
                   Added USB valid/invalid system event IDs.
                   Removed chg_auto_charging_charger_type.
08/07/09   hs      Added support for bad battery.
07/30/09   hs      Added support for providing general charging status.
                   Removed chg_update_charging_status()
...
#include "vbatt_pmic_comparator.h"  /* For pm_batt_index                   */
...
typedef enum
{
    ...
    CHG_SYSTEM__USB_VALID,
    /*! USB device is removed                        */
    CHG_SYSTEM__USB_INVALID,
...
/*! \var chg_handle_lwr
 *  \brief This variable is a registration handle for the charger task as a client
 *  to the vbatt task for the battery alarm lower threshold. It is assigned uniquely
 *  when the charger task registers to the vbatt task during its task initialization. 
 *  \note This variable is modified in the following function(s):
 *  \li chg_task_init()
 */
extern pm_batt_index                chg_handle_lwr ;
/*! \var chg_handle_upr
 *  \brief This variable is a registration handle for the charger task as a client
 *  to the vbatt task for the battery alarm upper threshold. It is assigned uniquely
 *  when the charger task registers to the vbatt task during its task initialization. 
 *  \note This variable is modified in the following function(s):
 *  \li chg_task_init()
 */
extern pm_batt_index                chg_handle_upr ; 
#endif /* FEATURE_VBATT_TASK */ 
/*===========================================================================
...
#ifdef FEATURE_VBATT_TASK 
/*! \brief This function is the call back function for battery alarm upper threshold 
 *  status
 *  \details This call back function is called by the vbatt task when the battery
 *  alarm upper threshold reached.  
 *  \param void* A void pointer. Not used in the charger application.
 *  \return void.
 *  \note This function should be registered to the vbatt task during the charger task
 *  initialization in order to become the call back for the battery alarm upper
 *  threshold status. If regiser successfully, it should be called when the battery 
 *  alarm upper threshold reached. 
 *  This function is used in the following two APIs from the VBATT task interface:
 *  \li pm_vbatt_register()
 *  \li pm_vbatt_modify_client()
 */ 
extern void chg_vbatt_upr_thr_cb ( void* ) ;
/*! \brief This function is the call back function for battery alarm lower threshold 
 *  status
 *  \details This call back function is called by the vbatt task when the battery
 *  alarm lower threshold reached.  
 *  \param void* A void pointer. Not used in the charger application.
 *  \return void.
 *  \note This function should be registered to the vbatt task during the charger task
 *  initialization in order to become the call back for the battery alarm lower
 *  threshold status. If regiser successfully, it should be called when the battery 
 *  alarm lower threshold reached. 
 *  This function is used in the following two APIs from the VBATT task interface:
 *  \li pm_vbatt_register()
 *  \li pm_vbatt_modify_client()
 */ 
extern void chg_vbatt_lwr_thr_cb ( void* ) ;
#endif /* FEATURE_VBATT_TASK */ 
