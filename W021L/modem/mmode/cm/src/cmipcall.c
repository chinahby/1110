...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/mmode/cm/rel/08H1/src/cmipcall.c#4 $
...
)
{
  ...
  */
  ...
  if (app_cb_ptr->cmipapp_cmd_answer_call_waiting_func_ptr != NULL)
  {
  ...
  else if (app_cb_ptr->cmipapp_cmd_simple_flash_func_ptr != NULL)
  {
    call_info_ptr = cmcall_info_alloc();
    CM_ASSERT( call_info_ptr != NULL );
    (void) cmcall_info_get( call_ptr, call_info_ptr );
    if (!app_cb_ptr->cmipapp_cmd_simple_flash_func_ptr (
                                            call_ptr->call_id, 
                                            call_info_ptr, 
                                            call_ptr->ipapp_data_block))
    {
      CM_ERR ("Answer to Call wait or remove last party to app failed", 0, 0, 0);
      return;
    }
  }
