...
$Header: //source/qcom/qct/modem/uim/mmgsdi/rel/07H1_2/src/mmgsdi_cnf.c#6 $$ $DateTime: 2009/07/08 09:29:13 $ $Author: ykang $
...
#if defined(FEATURE_MMGSDI_MFLO)
    if ( (card_pup_cnf_ptr->option == MMGSDI_CARD_POWER_UP_SWITCH_UIM_PASSIVE) &&
         (mmgsdi_data_slot1.mmgsdi_state == MMGSDI_STATE_NO_CARD))
    {
      /* Flip the MMGSDI_STATE from NO CARD to CARD INSERTED
       * to allow the MMGSDI to process GET_ATR, CARD_STATUS and
       * SEND APDU Commands for MFLO Devices that need to
       * communicate with a UICC
       * The last protocol with MFLO cards on Multimode builds is ICC
       * so we need to switch back to UICC protocol since MFLO cards are UICC.
       */
