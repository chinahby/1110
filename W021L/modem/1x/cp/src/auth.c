...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/auth.c#5 $ $DateTime: 2010/02/05 04:12:52 $ $Author: anishg $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/05/10   ag      Fix to read NV_COUNT_I only when subscription is available.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  { 
     MSG_ERROR("Message buffer overflow",0,0,0);
     return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (num_bytes > CAI_REV_TC_MAX_MSG_BYTES)
  { 
     MSG_ERROR("Message overflow",0,0,0);
     return;
  }
  for (i = 0; i < num_bytes; i++)
  {
