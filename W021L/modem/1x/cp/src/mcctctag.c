...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/mcctctag.c#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $
...
void mcctctag_initialize(void)
{
  ...
  */ 
  ...
  MSG_MED("Tag manager initialized.", 0, 0, 0);
  ...
  if (i < MAX_TAG_ITEM_NUM)
  {
    ...
    */
    for (i=0; i<=MAX_TAG_ID; i++)    /* Only iterate for MAX_TAG_ID times */
    {
      ...
      */
      ...
      if (!bad_tag)
      {
        ...
        tag_mgr.next_tag = (++tag_mgr.next_tag) % NUM_OF_TAG;
                                           /* increment next_tag */
        break;           
      }
      else
    ...
    if (tag_found)
    {
      ...
      MSG_MED("Tag %d available", tag_id, 0, 0);
    }
    else
    {
      /* This is impossible if tag item is less than 16 */
      MSG_ERROR("No valid tag!!!", 0, 0, 0);
    }
  }
  else
  {
    /* No tag is free! */
    MSG_ERROR("Out of free TAG item!!!", 0, 0, 0);
  }
...
)
{
  ...
  {
    MSG_HIGH("Bad Tag Manager timer ID received %x", timer_id, 0,0);
  }
  else
  {
    /* clear timer */
...
)
{
  ...
  for( i=0; i<MAX_TAG_ITEM_NUM; i++)
  {
    if (tag_mgr.tags[i].active && (tag_mgr.tags[i].tag_id == id ))
    {
      ...
      MSG_MED("L2 ack rxed for EOM", 0, 0, 0);
      break;
    }
  }
