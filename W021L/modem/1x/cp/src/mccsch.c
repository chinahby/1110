...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/mccsch.c#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $
...
#if defined(FEATURE_MCLK_DYNAMIC_SPEED_CTRL)
 /* Function that decides if clk freq needs to step up or down
 based on the current fwd and rev SCH rates */
static void mccsch_adjust_clk_freq(void);
...
)
{
  ...
  switch ( trans_type )
  {
    ...
    default :
      ERR( "Txn not supported by this function", 0, 0, 0 );
      return;
  }
    ...
    switch ( server_token )
    {
      case MCTRANS_SERVER_RXC :
      {
		...
		// check for valid range of sch_rate value
		    ...
		    (packet->action.rxc_f_sch.sch_rate < NUM_DEFINED_RATE))
                {
...
#error code not present
            ...
            {
              ERR("Missing SCCL entry:%d", sch_assign.forward[0].sccl_index, 0, 0 );
            }

            break;
          } // of case MCTRANS_F_SCH_ACTION

        } //lint !e744 !e787: No default needed for this switch (action)
        break;
        ...
        switch(action)
        {
          ...
          case MCTRANS_FFPC_ACTION:
          {
            ...
            if ( pending_msg != NULL )
            {
              ...
              {
                ERR(" No data available for FFPC action",0,0,0);
              }
            }
            else
            {
...
#endif
        } //lint !e744 !e787: No default needed for this switch
        break;
      }
    } //lint !e744 !e787: No default needed for this switch
  }
...
)
{
  ...
  // If not infinite duration or stop, need to schedule a "stop" transaction
...
#if defined(FEATURE_MCLK_DYNAMIC_SPEED_CTRL)
 /* Need to check if clk freq needs to be adjusted */
 mccsch_adjust_clk_freq();
#endif
...
void mccsch_stop_f_sch0_trans( void )
{
  ...
  MSG_LOW("Preparing F-SCH0 stop txn", 0, 0, 0 );
...
void mccsch_stop_f_sch0_trans_now( void )
{
  ...
  ts_get_20ms_frame_offset_time( cur_time );
...
)
{
      ...
      switch (action)
      {
        case MCTRANS_F_SCH_ACTION:
          ...
          if (packet->action.srch_f_sch.sch[0].num_pilots)
          {
            ...
            */
            MSG_HIGH("B to B burst: Stop action pkt Found >0 SCH pilots",
                      0,0,0);
  ...
  } //lint !e744 !e787: No default needed for this switch (server)
    ...
    switch ( server_token )
    {
      case MCTRANS_SERVER_TXC :
      {
        switch ( action )
        {
		  ...
		  (packet->action.txc_r_sch.sch_rate < NUM_DEFINED_RATE))
              {
                //Look up tables for number of LTUs and LTU length.
...
)
{
  ...
  // If USE_T_ADD_ABORT changed, need to schedule a "update" transaction.
  // If not infinite duration or stop, need to schedule a "stop" transaction
...
void mccsch_begin_r_sch0_extend_trans( void )
{
  ...
  {
    ts_get_20ms_frame_offset_time( trans_time );
  }
  else
  {
    qword trigger_t;
  ...
  MSG_HIGH("Despatch R-SCH0 extension at %lx", trans_time[0], 0, 0 );
...
} //lint !e715 mccsch_update_r_sch_parm_action_packet
/*lint +e818*/
...
void mccsch_update_r_sch_parm_trans( void )
{
  ...
  // Start rightaway
  ts_get_20ms_frame_offset_time(sch_assign.reverse[0].start_time);
  ...
  MSG_MED("ESCAM:Upd RSCH (SRCH) at %lx", sch_assign.reverse[0].start_time[0],
    0, 0);
...
{
  qword   abs_start_time, cur_time, tmp_time = { 0, 0 };
  dword   wait_time;
  uint8   mod32_time;
  ...
  if ( start_time != MCCTCHO_IMPLICIT_ACTION_TIME )
  {
    ...
    mod32_time = qw_lo( tmp_time ) & 0x1f;
  ...
  if ( dir == MCCSCH_FORWARD )
  {
         ...
         sch_assign.forward[0].for_sch_duration != 0 )
    {
      ...
      MSG_ERROR( "Delayed burst by 1 frames.", 0, 0, 0 );
...
{
  qword actual_duration, stop_time;
  uint8 shift;
  ...
  qw_set( actual_duration, 0UL, duration_table[duration] - shift );
...
)
{
  ...
  {
    MSG_HIGH("ESCAM received in wrong P_REV",0,0,0);
    error = CAI_REJ_CODE;
  ...
  {
    MSG_HIGH( "F-SCH config with no assignment!", 0, 0, 0 );
    error=CAI_REJ_FIELD;
    goto function_end; /*lint !e801 */
  }
  ...
  {
    MSG_HIGH( "R-SCH config with no assignment!", 0, 0, 0 );
    error=CAI_REJ_FIELD;
    goto function_end; /*lint !e801 */
  }
#endif
...
#error code not present
  ...
  if ( escam->num_for_sch > 0 )
  {
...
#endif
      {
        MSG_HIGH( "ESCAM conflicts with fsch config!", 0, 0, 0 );
        error=CAI_REJ_CFG;
        goto function_end;  /*lint !e801 */
      }
    }
    ...
    if (P_REV_IS_8_OR_GREATER && escam->for_sch_cc_incl)
    {
      ...
      {
        MSG_HIGH( " Bad ESCAM fsch config!", 0, 0, 0 );
        error=CAI_REJ_FIELD;
        goto function_end;  /*lint !e801 */
      }
    }
    ...
    if ( sccl_entry->num_pilots == 0 )
    {
      ...
      if ( error )
      {
        MSG_HIGH( "SCCL entry %d not valid!", escam->for_sch[0].sccl_index, 0, 0 );
        goto function_end;  /*lint !e801 */
      }
    }
  }
  ...
  if ( escam->num_rev_sch > 0 )
  {
...
#endif
      {
        MSG_HIGH( "ESCAM conflicts with rsch config!", 0, 0, 0 );
        error=CAI_REJ_CFG;
        goto function_end;  /*lint !e801 */
      }
    }
    ...
    if (P_REV_IS_8_OR_GREATER && escam->rev_sch_cc_incl)
    {
      ...
      {
        MSG_HIGH( " Bad ESCAM rsch config!", 0, 0, 0 );
        error=CAI_REJ_FIELD;
        goto function_end;  /*lint !e801 */
      }
    }
  ...
  if ( escam->for_cfg_included )
  {
    ...
    for ( bn = 0; bn <= escam->num_for_cfg_recs; ++bn )
    {
      ...
      {
        MSG_HIGH( "SCH %d not supported!", escam->for_cfg[bn].for_sch_id, 0, 0 );
        error = CAI_REJ_CAP;
        goto function_end;   /*lint !e801 */
      }
      ...
      {
        MSG_HIGH( "F-Rate %d is > than MAX_RATE %d", escam->for_cfg[bn].for_sch_rate,
                  max_rate, 0 );
        error = CAI_REJ_FIELD;
        goto function_end;   /*lint !e801 */
      }
      ...
      {
        MSG_HIGH( "Rate %d not allowed by for_sch_mux 0x%x", escam->for_cfg[bn].for_sch_rate,
                  config->for_sch[0].for_sch_mux, 0 );
        error = CAI_REJ_FIELD;
        goto function_end;
      }
#endif /* FEATURE_IS2000_REL_A */
      ...
      for ( pilot = 0; pilot <= escam->for_cfg[bn].num_sup_sho; ++pilot )
      {
        ...
        {
          MSG_HIGH( "SCH pilot %d not in active set!",
                    escam->for_cfg[bn].sup_sho[pilot].pilot_pn, 0, 0 );
          error = CAI_REJ_FIELD;
          goto function_end;  /*lint !e801 */
        }
      }
  ...
  if ( escam->rev_cfg_included )
  {
    ...
    for ( bn = 0; bn <= escam->num_rev_cfg_recs; ++bn )
    {
      ...
      {
        MSG_HIGH( "R-Rate %d is > than MAX_RATE %d", escam->rev_cfg[bn].rev_sch_rate,
                max_rate, 0 );
        error = CAI_REJ_FIELD;
        goto function_end;  /*lint !e801 */
      }
    ...
    for ( bn = 0; bn <= escam->num_rev_cfg_recs; ++bn )
    {
      ...
      {
        MSG_HIGH( "Rate %d not allowed by rev_sch_mux 0x%x", escam->rev_cfg[bn].rev_sch_rate,
                  config->rev_sch[0].rev_sch_mux, 0 );
        error = CAI_REJ_FIELD;
        goto function_end;
      }
    }
  ...
  if ( escam->add_info_incl )
  {
    ...
    {
      MSG_HIGH( "Inconsistent FPC primary channel %d!", escam->fpc_pri_chan, 0, 0 );
      error = CAI_REJ_CAP;
      goto function_end;   /*lint !e801 */
    }
  }
  ...
  if ( escam->rev_cfg_included && num_r_sch_supported < CAI_MAX_NUM_REV_SCH )
  {
    ...
    for ( bn = 0; bn <= escam->num_rev_cfg_recs; ++bn )
    {
      ...
      {
        MSG_HIGH( "SCH %d not supported!", escam->rev_cfg[bn].rev_sch_id, 0, 0 );
        error = CAI_REJ_CAP;
        goto function_end;   /*lint !e801 */
      }
    }
  ...
  if ( escam->fpc_incl )
  {
    ...
    {
      MSG_HIGH( "SCH FPC config error!", 0, 0, 0 );
      error = CAI_REJ_CAP;
      goto function_end;   /*lint !e801 */
    }
  }
  ...
  if (sch_assign.rev_parms.ignore_escam)
  {
      ...
      mccds_get_scrm_seq_num()))
    {
      ...
      MSG_MED("SCRM seq # mismatch, ignore R-SCH assign", 0, 0, 0);
      escam->num_rev_sch = 0;
    }
    else
    {
  ...
  if (P_REV_IS_7_OR_GREATER)
  {
    ...
    {
      MSG_HIGH( "3X configuration for pilots!", 0, 0, 0 );
      error = CAI_REJ_CAP;
      goto function_end;   /*lint !e801 */
    }
  }
        ...
        sch_assign.rev_parms.current_use_t_add_abort)))
  {
    ...
    MSG_MED("ESCAM:Upd RSCH at %lx", sch_assign.reverse[0].start_time[0], 0, 0);
  }
  else
  {
    // Store REV_SCH_DTX_DURATION
  ...
  if ( escam->num_for_sch > 0 ) // Assignment included
  {
    ...
    {
      MSG_HIGH( "ESCAM:Stop FSCH at %lx", sch_assign.forward[0].start_time[0], 0, 0 );
    }
    else
    {
      ...
      MSG_HIGH( "ESCAM:FSCH %lx--%lx", sch_assign.forward[0].start_time[0], stop, 0 );
    }
    ...
    // Now despatch the transaction.
    (void)mctrans_despatch_transaction( sch_assign.forward[0].start_time, &despatch_info, msg_to_save );
  ...
  if ( escam->num_rev_sch > 0 )
  {
    ...
    {
      MSG_HIGH( "ESCAM:Stop RSCH at %lx", sch_assign.reverse[0].start_time[0], 0, 0 );
    }
    else
    {
      ...
      MSG_HIGH( "ESCAM:RSCH %lx--%lx", sch_assign.reverse[0].start_time[0], stop, 0 );
    ...
    // Now despatch the transaction.
    (void)mctrans_despatch_transaction( sch_assign.reverse[0].start_time, &despatch_info, msg_to_save );
...
)
{
  if (sch_assign.rev_parms.ignore_escam != ignore_escam)
  {
    ...
    MSG_MED("IGNORE_ESCAM = %d", sch_assign.rev_parms.ignore_escam, 0, 0);
  }
...
FUNCTION mccsch_adjust_clk_freq

DESCRIPTION
  We are supposed to run at a higher MCLK speed only if our SCH rates warrants
  that due to high CPU loading. Whenever we do not need it we should go back to
  nominal MCLK speed. So every time our SCH assignment changes (starts or stops), we
  need to call this function, which figures out the rates we need for current
  operation and will set this value in boot module accordingly.
...
void mccsch_adjust_clk_freq
(
  void
)
{
  cai_sch_rate_type for_sch_rate, rev_sch_rate;

  const mccccl_sccl_entry_type* sccl_entry;
  /* Need to figure out if we need a fast clock, So based on current
  fwd and rev rates, figure out if clk needs to go up or down */

  /* If the forward sch is active then get the channel rate*/
  if (sch_assign.forward[0].active)
  {
    sccl_entry = mccccl_code_channel_list.get_sccl_entry( 0, sch_assign.forward[0].sccl_index );
    for_sch_rate = (cai_sch_rate_type)sccl_entry->sch_rate;
  }
  else
  {
    for_sch_rate = CAI_SCH_RATE_NULL;
  }

  /* If the reverse sch is active then get the channel rate*/
  if (sch_assign.reverse[0].active)
  {
    rev_sch_rate = (cai_sch_rate_type)sch_assign.reverse[0].new_rate;

  }
  else
  {
    rev_sch_rate = CAI_SCH_RATE_NULL;
  }

  /* Now that we have the rate information, set the msm to the appropiate mclk speed*/

  mclk_convert_sch_rate_to_speed((byte)for_sch_rate,(byte)rev_sch_rate);
...
)
{
  ...
  if (mccsch_sch_is_active(MCCSCH_REVERSE, 0))
  {
    ...
    MSG_MED("r_sch0_stop transaction dispatched",0,0,0);
  }
#endif /* FEATURE_IS2000_R_SCH */
  ...
  if (mccsch_sch_is_active(MCCSCH_FORWARD, 0))
  {
    ...
    (void)mctrans_despatch_transaction( action_time, &despatch_info, 0 );
    MSG_MED("f_sch0_stop transaction dispatched",0,0,0);
  }
