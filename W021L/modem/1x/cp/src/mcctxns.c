...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/mcctxns.c#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $
...
)
{
  ...
  MSG_MED("Long Code Mask transaction despatched, trans_num %d", trans_num, 0, 0 );
...
)
{
  ...
  MSG_MED("SCR transaction despatched, trans_num %d", trans_num, 0, 0 );
} 
...
)
{
  ...
  trans_num = mctrans_despatch_transaction( trans_time, &despatch_info, 0 );
  MSG_MED("HO transaction despatched, trans_num %d ", trans_num, 0, 0 );
  ...
  */
  ...
  if ( ho_data->ho_msg.extra_parms )
  {
    ...
    if ( ho_data->ho_msg.extra.scr_included &&
         ((cur_fwd_chan_mask & MCC_F_FCH) && ho_data->ho_msg.extra.cfg.fch_cc_incl
...
)
{
  ...
  MSG_MED("FPC_SUBCHAN transaction despatched, trans_num %d", trans_num, 0, 0 );
} 
...
)
{
  ...
  MSG_MED("PWR_CTRL transaction despatched, trans_num %d", trans_num, 0, 0 );
...
#error code not present
  ...
  {
    MSG_HIGH("No actions added for %d due to no mesg found", 
      curr_trans->trans_num, 0, 0);
  }
...
)
{
  ...
  MSG_MED("CFS transaction despatched, trans_num %d, trans_type %d", 
                         trans_num, 
...
)
{
  ...
  {
    MSG_HIGH("No actions added for %d due to no mesg found", curr_trans->trans_num, 
              0,0);
  }
  ...
  switch ( server_token )
  {
    case MCTRANS_SERVER_RXC :
    {
      ...
      if (fwd_chan_mask & MCC_F_FCH)
      {
        ...
        MSG_HIGH("Fwd FCH RC %d ", packet->action.rxc_scr.fch.rc,0,0);
      }
      else
      {
      ...
      if (fwd_chan_mask & MCC_F_DCCH)   
      {
        ...
        MSG_HIGH("Fwd DCCH RC %d ", packet->action.rxc_scr.dcch.rc,0,0);
      }
      else
      {
...
#error code not present
           ...
           )
        {
          packet->action.rxc_scr.rev_pwr_cntl_delay.value =
            #ifdef FEATURE_IS2000_P2
            (rev_chan_mask & MCC_R_PICH || (nn_config->pilot_gate_rate == CAI_GATING_RATE_ONE) ) ? /*lint !e641 */
...
#error code not present
      ...
      {
        /* Diagnostics only */
        MSG_MED("LPM: SR_ID %d LOG %d PHY %d", nn_config->lpm_entries[i].sr_id,
          nn_config->lpm_entries[i].logical_resource, nn_config->lpm_entries[i].physical_resource);
        MSG_MED("                             FWD %d REV %d",
          nn_config->lpm_entries[i].forward_flag, nn_config->lpm_entries[i].reverse_flag, 0);
      ...
      if (rev_chan_mask & MCC_R_FCH)
      {
        ...
        MSG_HIGH("Rev FCH RC %d ", packet->action.txc_scr.fch.rc,0,0);
      }
      else
      {
      ...
      if (rev_chan_mask & MCC_R_DCCH)
      {
        ...
        MSG_HIGH("Rev DCCH RC %d ", packet->action.txc_scr.dcch.rc,0,0);
      }
      else
      {
...
#error code not present
            ...
            & MCTRANS_PHY_CHAN_FCH)
        {
          if ((so_chan_mask & SNM_FCH) == 0) /*lint !e641 */
          {
            ...
            MSG_HIGH("Excluding FCH for sr_id %d given to mux", config->con[i].sr_id, 0, 0);
            packet->action.txc_scr.so_cfgs[packet->action.txc_scr.num_of_so_recs].phy_channel
              &= (~MCTRANS_PHY_CHAN_FCH);
          }
        }
            ...
            & MCTRANS_PHY_CHAN_DCCH)
        {
          ...
          {
            /* Should exclude from what is being given to mux */
            MSG_HIGH("Excluding DCCH for sr_id %d given to mux", config->con[i].sr_id, 0, 0);
            packet->action.txc_scr.so_cfgs[packet->action.txc_scr.num_of_so_recs].phy_channel
              &= (~MCTRANS_PHY_CHAN_DCCH);
          }
        }
...
#error code not present
            ...
            & MCTRANS_PHY_CHAN_SCH0)
        {
          ...
          {
            /* Should exclude from what is being given to mux */
            MSG_HIGH("Excluding SCH0 for sr_id %d given to mux", config->con[i].sr_id, 0, 0);
            packet->action.txc_scr.so_cfgs[packet->action.txc_scr.num_of_so_recs].phy_channel
              &= (~MCTRANS_PHY_CHAN_SCH0);
          }
        }
...
#error code not present
  ...
  } /*lint !e744 !e787 no default necessary */
...
#error code not present
  ...
  switch (server_token)
  {
    case MCTRANS_SERVER_RXC :
    {
      ...
      } /*lint !e744 !e787 switch default not used */
      break;
...
#error code not present
        ...
        case MCTRANS_ASET_ACTION:
        {
          ...
          packet->action.srch_aset.tc_aset.cdma_freq = cdma.cdmach;
          packet->action.srch_aset.tc_aset.band_class = cdma.band_class;
          break;
        } 
      ...
      switch ( action )
      {
        ...
        case MCTRANS_HO_ACTION :
        {
          ...
          if (ho_data->ho_msg.extra_parms)
          {
            ...
            if ( ho_data->high_rc_after_ho )
            {
              // For RC 3 and above, the preambles is expressed in number of PCGs
              if ( hw_version() == MSM_50_A0 )
              {
                // But for Tiger2, the chip does not support preamble in PCGs, so always use
                // 1 frame
                packet->action.txc_ho.num_preamble.unit = MCTRANS_PREAMBLE_FRAME;
                packet->action.txc_ho.num_preamble.value = 1;
              }
              else
              {
  ...
  switch ( server_token )
  {
    case MCTRANS_SERVER_SRCH :
    {
      ...
      */
      ...
      if (cfs_req == NULL)
      {
        MSG_ERROR("Pending msg NULL and no defaults available for CFS NSET",0,0,0);
...
#error code not present
  ...
  MSG_MED("Extended Release transaction despatched, trans_num %d", trans_num, 0, 0 );
...
)
{
  ...
  if (msg == NULL)
  {
    MSG_ERROR("No ERM found for the trans %d; no action added",
      curr_trans->trans_num, 0, 0);
    return;
  }
...
#error code not present
  ...
  */
  ...
  {
    MSG_HIGH("F_PDCH must get released", 0, 0, 0);
...
#error code not present
#else
    MSG_HIGH("R_PDCH not supported", 0, 0, 0);
#endif /* FEATURE_IS2000_REL_D_DV */
  }
...
#error code not present
  ...
  switch ( server_token )
  {
    case MCTRANS_SERVER_RXC:
    {
      switch( action)
      {
        ...
        case MCTRANS_CTRL_HLD_GATING_ACTION :
        {
          ...
          packet->action.rxc_ctrl_hld_gating.rev_pwr_control_delay =
            ( nnscr->pilot_gate_rate == CAI_GATING_RATE_ONE ) ? /*lint !e641 */
    ...
    case MCTRANS_SERVER_SRCH :
    {
      ...
      } /*lint !e744 !e787 no default */
      break;
    }
#endif
    default:
      ERR ("Incorrect server token %d", server_token, 0, 0);
  }
...
#endif //defined(FEATURE_IS2000_CHS) || (defined(FEATURE_IS2000_P2) && defined(FEATURE_IS2000_REL_A_SVD))
...
{ 
  ...
  MSG_MED("Control Hold to active transaction despatched, trans_num %d", trans_num, 0, 0 );
...
)
{
  ...
  if (mccccl_code_channel_list.get_fch_to_be_restored())
  {
    MSG_HIGH("FCH is being restored",0,0,0);
    restore_fch = TRUE;
    mccccl_code_channel_list.set_fch_to_be_restored(FALSE);
  }
#endif
...
)
{
  ...
  MSG_MED("HHO Begin transaction despatched", trans_num, 0, 0 );
} 
...
)
{
  ...
  switch ( server_token )
  {
    case MCTRANS_SERVER_RXC:
    {
      ...
      {
        ERR_FATAL ("Wrong action type %d", action,0,0);
      }
    ...
    case MCTRANS_SERVER_SRCH:
    {
      ...
      switch (action)
      {
        case MCTRANS_ASET_ACTION:
        {
          packet->action.srch_aset.tc_aset.cdma_freq = ho_data->ho_msg.extra.cdma_freq;
          packet->action.srch_aset.tc_aset.band_class = ho_data->ho_msg.extra.band_class;
...
)
{
  ...
  MSG_MED("TX on/off txn despatched %d", trans_num, 0, 0 );
} 
...
#error code not present
  ...
  MSG_MED("Return transaction despatched %d", trans_num, 0, 0 );
} 
...
#if (defined FEATURE_IS2000_REL_C_DV || defined FEATURE_IS2000_P2)
...
#error code not present
  ...
  MSG_MED("CFS/GPS Prep transaction despatched %d", trans_num, 0, 0 );
...
#error code not present
        ...
        default:
        {
          MSG_ERROR("Unknown Action %d", action, 0,0);
        }
      }
      break;
    }
