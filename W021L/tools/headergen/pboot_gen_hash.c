...
//         (data_len   <  BLOCK_SIZE)    ||
 ...
 /* Hash the input data*/
      Prog_Boot_SHA_1_Hash(data_addr,blk_size,table_ptr);
      data_len  -= blk_size;
      data_addr += blk_size;

          if(i==0 || i==1 || i==(num_hashes-1))            
          {
            uint32_t* ptr = (uint32_t*)table_ptr;
#ifdef DEBUG_HASHES
            printf("Hash is: 0x%x 0x%x 0x%x 0x%x 0x%x\n",ptr[0],ptr[1],ptr[2],ptr[3],ptr[4]);
#endif
          }
