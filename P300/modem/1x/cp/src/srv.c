...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/srv.c#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (e_ptr->ec == SRV_CON_MSG_EC)
  {
       ...
       )
    {
      ...
      MSG_HIGH("Rej SCM - use_old_srv_cfg %d not supported",
        e_ptr->ep.con_msg.msg_ptr->use_old_srv_cfg, 0, 0);
    }
    else
        ...
        (pre_process_cc_info(&(e_ptr->ep.con_msg.msg_ptr->cc), &rej_parms) != 0))
    {
      ...
      continue_cfg_eval = FALSE;
      MSG_FATAL("CC Info in SCM not acceptable",0,0,0);
    }
    ...
    if (continue_cfg_eval) /*lint !e774 */
    {
          ...
          fwd_chan_mask, rev_chan_mask) == SNM_ACCEPT_CFG )
      {
        ...
        else
        {
          ...
          {
            MSG_HIGH("NNSCR not acceptable, rejecting SCM", 0,0,0);
          }
          else
          {
            cfg_okay = TRUE;
  ...
  {
    /* Should never happen */
    MSG_ERROR("Invalid EC %d", e_ptr->ec, 0, 0);
    return ( subf );
  }
        ...
        (!e_ptr->ep.ho_msg.msg_ptr->extra.scr_included))
    {
      if (subf == SRV_WF_TIM_SUBF)  /*lint !e774 */
      {
        /* Note this means we already have a pending SCR/NNSCR but this is going to
           ...
           subf back to normal */
        subf = SRV_NORM_SUBF;
        MSG_HIGH("Changing SUBF becos SCR trans getting voided",0,0,0);
      }
      else
      {
  ...
  switch ( e_ptr->ec )
  {
    ...
    case SRV_TC_CON_EC:
      ...
      if ( srv_neg_needed )
      {
        ...
        {
          e_ptr->status = SRV_ERR_S;
          ERR( "Cfg not built - so: %x", srv_saved_so, 0, 0 );
        }
    ...
    /*-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -*/
    case SRV_RIF_HHO_EC:
      MSG_HIGH( "Rxed SRV_RIF_HHO_EC in normal subf ", 0, 0, 0 );
      e_ptr->status = SRV_DONE_S;
      break;

    /*-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -*/
    default:
      ERR( "Unknown event - ec: %x", e_ptr->ec, 0, 0 );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch ( e_ptr->ec )
  {
    ...
    case SRV_SCR_CMP_EC:
      ...
      MSG_HIGH("SCR AT arrives in SRV_WF_REQ", 0, 0, 0);
    ...
    /*-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -*/
    case SRV_RIF_HHO_EC:
      MSG_HIGH( "Rxed SRV_RIF_HHO_EC in Wait for Req subf ", 0, 0, 0 );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch ( e_ptr->ec )
  {
    ...
    case SRV_SCR_CMP_EC:
      ...
      MSG_ERROR("SCR AT arrives in SRV_WF_RSP", 0, 0, 0);
    ...
    /*-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -*/
    case SRV_RIF_HHO_EC:
      MSG_HIGH( "Rxed SRV_RIF_HHO_EC in Wait for Rsp subf ", 0, 0, 0 );
      e_ptr->status = SRV_DONE_S;
      subf = SRV_NORM_SUBF;
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch ( e_ptr->ec )
  {
    ...
    case SRV_SCR_CMP_EC:
      ...
      MSG_HIGH("SCR AT arrives in SRV_WF_CON", 0, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch ( e_ptr->ec )
  {
    ...
    /*-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -*/
    case SRV_RIF_HHO_EC:
      MSG_HIGH( "Rxed SRV_RIF_HHO_EC. Going back to normal subf ", 0, 0, 0 );
      e_ptr->status = SRV_DONE_S;
      subf = SRV_NORM_SUBF;
      break;
...
)
{
  ...
  switch ( srv_subf )  /* Invoke current service subfunction */
  {
    ...
    default:
      ERR ( "Unknown subf - subf: %x", srv_subf, 0, 0 );
      break;

  } /* switch */


  MSG_MED( "Event (out) - subf: %x  status: %x",
           srv_subf, e_ptr->status, 0 );
    ...
    case CAI_SO_RS1_OTAPA:
      ...
      if (P_REV_IS_6_OR_GREATER)
      {
        if (user_pref_rc_valid)
        {
          ...
          {
            MSG_MED("Ignoring NV Pref %d", user_pref_fundicated_rc, 0, 0);
            *fwd_rc_ptr = CAI_RC_3;
            *rev_rc_ptr = CAI_RC_3;
          }
        }
...
#error code not present
      ...
      !snm_so_is_enabled(so))
  {
    /* Unsupported service option specified */
    ERR( "Unsupported or non-functioning so - so: %x", so, 0, 0 );
    return SRV_NOTSUP_S;
  }
...
#error code not present
  ...
  {
    MSG_MED("Init - mob_orig: %x so: %x gm: %x", mob_orig, so, 
      ...
      switch (chn_assign->mode.am0.granted_mode)
      {
        case CAI_GRANT_USE_DC:  /* Use DEFAULT_CONFIG field */
          ...
          else
          {
             ...
             * this moment */
            /* Unsupported service option specified */
            ERR("Unsupported so and default_config - so: %x, dc: %x", so,
              chn_assign->mode.am0.default_config, 0);
            status = SRV_NOTSUP_S;
          }
...
#error code not present
        ...
        /*-   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -*/
        default:
          ERR("Unknown grant mode - gm: %x",chn_assign->mode.am0.granted_mode , 0, 0 );
