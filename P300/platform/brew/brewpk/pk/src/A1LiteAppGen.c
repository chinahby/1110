...
SERVICES: IApplet implementation file for A1Lite
...
#include "A1LiteAppGen.h"  // AEEApplet definitions
#include "A1LiteModGen.h"  // AEEMod definitions and utility function
#include "nmdef.h"
...
static uint32 AEEApplet_Release(IApplet * po)
{
   ...
   // Release the interfaces created in A1LiteApplet_New()
   if (pme->m_pIDisplay)
      IDISPLAY_Release(pme->m_pIDisplay);   


   IEnv_Release(pme->m_piEnv);
...
DESCRIPTION
   ...
   invoked by the app from within the A1LiteClsCreateInstance() function

PROTOTYPE:
   boolean A1LiteApplet_New(int16 nSize,
                         AEECLSID clsID,
                         IShell * pIShell, 
                         IModule * pIModule, 
                         IApplet **ppobj, 
...
SIDE EFFECTS
  none
  ===========================================================================*/
boolean A1LiteApplet_New(int16 nIn,
                      AEECLSID clsID,
                      IShell * pIShell, 
                      IModule * pIModule, 
                      IEnv * piEnv,
