        ...
        case EVT_COMMAND:
        {
            ...
            if(wParam == DOWNLOAD_MENU)
            {
				...
				if(pMe->m_RingerType == SET_RINGER)
				{
					pMe->m_RingID[pMe->m_CurProfile] = OEMNV_STARTUP_MUSIC; //OEMNV_ALARM_RINGER;
                    ICONFIG_SetItem(pMe->m_pConfig,CFGI_PROFILE_CALL_RINGER,pMe->m_RingID,sizeof(pMe->m_RingID));
                	//nNewConfigRinger[pMe->m_CurProfile] = 0;
                ...
                {
                	//nNewConfigRinger[pMe->m_CurProfile]= OEMNV_STARTUP_MUSIC;
                	//nNewConfigRinger[pMe->m_CurProfile].ringType = OEMNV_MID_RINGER;
                	//ICONFIG_SetItem(pMe->m_pConfig,CFGI_PROFILE_STARTUP_MUSIC,(void*)nNewConfigRinger,sizeof(nNewConfigRinger));
                	pMe->m_RingID[pMe->m_CurProfile] = OEMNV_STARTUP_MUSIC; //OEMNV_ALARM_RINGER;
                    ICONFIG_SetItem(pMe->m_pConfig,CFGI_PROFILE_STARTUP_MUSIC,pMe->m_RingID,sizeof(pMe->m_RingID));
                }
                else if(pMe->m_RingerType == SET_SHUTDOWN)
                {
                	//nNewConfigRinger[pMe->m_CurProfile] = OEMNV_SHUTDOWN_MUSIC;
                	//nNewConfigRinger[pMe->m_CurProfile].ringType = OEMNV_MID_RINGER;
                	//ICONFIG_SetItem(pMe->m_pConfig,CFGI_PROFILE_SHUTDOWN_MUSIC,(void*)nNewConfigRinger,sizeof(nNewConfigRinger));
                	pMe->m_RingID[pMe->m_CurProfile] = OEMNV_STARTUP_MUSIC; //OEMNV_ALARM_RINGER;
                    ICONFIG_SetItem(pMe->m_pConfig,CFGI_PROFILE_SHUTDOWN_MUSIC,pMe->m_RingID,sizeof(pMe->m_RingID));
                }
                CLOSE_DIALOG(DLGRET_MESSAGE)
        ...
        case EVT_DIALOG_START:
        {
            ...
            switch(smsID[pMe->m_CurProfile].midID)
            {
                ...
                default:
                	...
                	smsID[pMe->m_CurProfile].ringType = OEMNV_MP3_RINGER;
                	smsID[pMe->m_CurProfile].midID = IDS_QUIETMODE;
                	#else
                    ICONFIG_GetItem(pMe->m_pConfig,CFGI_PROFILE_SMS_RINGER_ID,(void*)smsID,sizeof(smsID));
            ...
            {
            	#ifdef FEATURE_USES_LOWMEM
            	pMe->m_lastSmsRingPlayed = IDS_QUIETMODE;
            	#else
