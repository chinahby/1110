 ...
 *                             EDIT HISTORY
 *
 * $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_realpath.c#3 $ $DateTime: 2009/04/23 17:30:14 $ $Author: richardp $
 *
  ...
  while (*pbase) {
    ...
    if (pend - pbase == 1 && *pbase == '.') {
      ...
      if (resolve_last_link || *pend != 0) {
        ...
        memcpy (pend - result, rl_buf, result);
        pend -= result;
