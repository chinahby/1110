...
{
	m_bDrawDashedFocusCircle = FALSE;
	iPhoto = IMAG_CONNECT;
}
...
BEGIN_MESSAGE_MAP(CRoundButton, CColorBtn)
...
//	pDC->FillSolidRect(rect, ::GetSysColor(COLOR_BTNFACE));
	...
	/*
	提示:FillSolidRect是在整块矩形区域,应该在有效区域作图才对。
	*/
	CRgn rgn;
	rgn.CreateEllipticRgnIndirect(rect);
	...
	pDC->FillRgn(&rgn,&br);
		...
		CBitmap m_BkBmp ;
	if(iPhoto == IMAG_CONNECT)
		m_BkBmp.LoadBitmap(IDB_BITMAP_CONNECT);
	if(iPhoto == IMAG_DISABLECON)
		m_BkBmp.LoadBitmap(IDB_BITMAP_DISCON);
	if(iPhoto == IMAG_DISABLECUT)
		m_BkBmp.LoadBitmap(IDB_BITMAP_DISCUT);
	if(iPhoto == IMAG_DISCONNECT)
		m_BkBmp.LoadBitmap(IDB_BITMAP_DISCONNECT);
...
//	CColorBtn button.Create();
//	button.SetBitmapID(IDB_BITMAP_CALL, IDB_BITMAP_CALL);


	// Draw the focus circle around the button
	if ((state & ODS_FOCUS) && m_bDrawDashedFocusCircle)
		DrawCircle(pDC, m_ptCentre, nRadius--, RGB(207,215,220));
	...
	if ((state & ODS_FOCUS) && m_bDrawDashedFocusCircle)
		DrawCircle(pDC, m_ptCentre, nRadius-2, RGB(207,215,220), TRUE);
...
BOOL CRoundButton::OnEraseBkgnd(CDC* pDC) 
{
	...
	CRect rect;
	GetClientRect(rect);
	dc.FillSolidRect(rect, RGB(207,215,220));
//	return CButton::OnEraseBkgnd(pDC);

	/*
	重载WM_ERASEBKGND消息函数，禁止它进行任何画图动作
	当然也可以把DrawItem()函数中画背景的代码放到这里执行
	最重要的是不要让OnEraseBkgnd()执行CButton类的默认操作
	*/
