...
CSetPinDlg::CSetPinDlg(CWnd* pParent /*=NULL*/)
...
void CSetPinDlg::DoDataExchange(CDataExchange* pDX)
{
...
void CSetPinDlg::OnButtonSetcallring() 
{
...
void CSetPinDlg::OnButtonSetsmsring() 
{
	// TODO: Add your control notification handler code here
/*	CString strFilter = LoadStringEx(IDS_WAVEFILE_TYPE);
	CString strFilename;
...
BOOL CSetPinDlg::OnInitDialog() 
{
	CBmpBkDialog::OnInitDialog();
	
	GetDlgItem(IDC_TEXT_PINCODE)->SetWindowText(LoadStringEx(IDC_STATIC_PINCODE));
	GetDlgItem(IDC_PIN_REMAINING)->SetWindowText(LoadStringEx(IDC_STATIC_PINREMAINING));
	GetDlgItem(IDC_BUTTON_PINOK)->SetWindowText(LoadStringEx(IDS_OK));
	GetDlgItem(IDC_BUTTON_PINCANCEL)->SetWindowText(LoadStringEx(IDS_CANCEL));
