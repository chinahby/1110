					  ...
					  const CString& strPasswrod)
{
	...
	strcpy(dialParam.szEntryName, strConnection);
	strcpy(dialParam.szPhoneNumber, strPhoneNum);
	strcpy(dialParam.szUserName, strUser);
	strcpy(dialParam.szPassword, strPasswrod);
...
{
	return ::RasValidateEntryName("", m_ConnectionName) == ERROR_ALREADY_EXISTS;
}
					   ...
					   const CString& strPassword)
{
	...
	if(dwRV == 0)
	{
		...
		strncpy(dailParam.szEntryName, m_ConnectionName, sizeof(dailParam.szEntryName));
		strncpy(dailParam.szPhoneNumber, strPhoneNum, sizeof(dailParam.szPhoneNumber));
		strncpy(dailParam.szCallbackNumber, "", sizeof(dailParam.szCallbackNumber));
		strncpy(dailParam.szUserName, strUserName, sizeof(dailParam.szUserName));
		strncpy(dailParam.szPassword, strPassword, sizeof(dailParam.szPassword));
