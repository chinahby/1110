...
void CSplitterControl::Create(DWORD dwStyle, const CRect &rect, CWnd *pParent, UINT nID)
{
	...
	CStatic::Create(TEXT(""), dwStyle, rc, pParent, nID);
...
void CSplitterControl::DrawLine(CDC* pDC, int x, int y)
{
	...
	if (m_nType == SPS_VERTICAL)
	{
		...
		pDC->LineTo(m_nX - d, rcWnd.bottom);//rcWnd.bottom-500rcWnd.top+m_nMy
...
BOOL CSplitterControl::SetWindowRect(int nType, int nCoordinate,int nOffset)
{
	if(nType == SPS_VERTICAL)
	{
		m_nMy = nCoordinate - nOffset;
	}
	else if(nType == SPS_HORIZONTAL)
	{
		m_nMx = nCoordinate - nOffset	;
	}
	return TRUE;
}