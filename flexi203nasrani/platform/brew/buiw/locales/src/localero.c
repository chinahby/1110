...
// Format variables, strings, characters and patterns for Romanian (Romania)
...
static const AECHAR Currency_NegativePattern []           = { '-','#',' ','l','e','i',0 }; 
static const AECHAR Currency_PositivePattern[]            = { '#',' ','l','e','i',0 }; 
static const AECHAR Currency_CurrencySymbol[]             = { 'l','e','i',0 }; 
...
static const AECHAR DateTime_DayNames[] = {
   ...
   'j','o','i', 0,
   'v','i','n','e','r','i', 0,
   's',0xee,'m','b',0x103,'t',0x103, 0,
   'd','u','m','i','n','i','c',0x103, 0,
   0 };
...
/* -----------------------------------------------------------------------
    LocaleRO_Ctor 
   --- */
static void LocaleRO_Ctor(LocaleRO *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleRO_New  
   --- */
int LocaleRO_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleRO *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleRO, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleRO_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule); 
...
int LocaleRO_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALERO) {
      return LocaleRO_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
