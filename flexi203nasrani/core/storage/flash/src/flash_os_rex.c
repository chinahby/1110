...
/*===========================================================================
  ...
  $Header: //depot/asic/msmshared/drivers/flash/MSM_FLASH.01.04/flash_os_rex.c#8 $ $DateTime: 2009/06/02 02:12:02 $ $Author: rpulyala $

when         who     what, where, why
--------     ---     ----------------------------------------------------------
2009-05-28   rk      Block rex_set_sigs if caller task tcb value is zero
...
{ 
  ...
  /* Make sure to send caller task TCB as NULL for polled transfers.
       DM callback with check to see if the TCB value is not zero before
       notifying the caller task. */
  xfer_ptr->caller_task = NULL;
  ...
  void *user_ptr)
{
  ...
  /* Block rex_set_sigs if caller task tcb value is zero. */
  if (0 != xfer_ptr->caller_task)
  {
