...
static NAMED *
lookup(HASH_TABLE *table, KEY name, jsize_t createSize);
...
void * XMLCALL
XML_MemMalloc(XML_Parser parser, jsize_t size)
{
  return MALLOC(size);
}

void * XMLCALL
XML_MemRealloc(XML_Parser parser, void *ptr, jsize_t size)
{
...
lookup(HASH_TABLE *table, KEY name, jsize_t createSize)
{
  ...
  if (table->size == 0) {
    ...
    table->size = (jsize_t)1 << INIT_POWER;
    ...
    if (table->used >> (table->power - 1)) {
      ...
      jsize_t newSize = (jsize_t)1 << newPower;
      unsigned long newMask = (unsigned long)newSize - 1;
      jsize_t tsize = newSize * sizeof(NAMED *);
...
poolGrow(STRING_POOL *pool)
{
  ...
  if (pool->blocks && pool->start == pool->blocks->s) {
    ...
    if (blockSize < INIT_BLOCK_SIZE)
      ...
      blockSize *= 2;
    tem = (BLOCK *)pool->mem->malloc_fcn(j_offsetof(*tem, s)
                                        + blockSize * sizeof(XML_Char));
    if (!tem)
      return XML_FALSE;
    tem->size = blockSize;
