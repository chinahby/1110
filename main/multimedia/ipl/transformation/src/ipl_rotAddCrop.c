...
$Header: //source/qcom/qct/multimedia/ipl/transformation/main/latest/src/ipl_rotAddCrop.c#7 $
...
)
{
    ...
    (input_frame_ptr->dy != i_img_ptr->dx)) )
  {
    ...
    IPL2_MSG_FATAL( "ipl2_rot_add_crop failed for invalid Frame setting /\
               input_frame_ptr= %p, /\
               rotate = %d, input_frame_ptr->dx = %d", input_frame_ptr,
               rotate, input_frame_ptr->dx );

    IPL2_MSG_FATAL( "ipl2_rot_add_crop failed for invalid Frame setting /\
               i_img_ptr->dy= %d, /\
               input_frame_ptr->dy = %d, i_img_ptr->dx) = %d",i_img_ptr->dy,
               input_frame_ptr->dy, i_img_ptr->dx );
      ...
      switch ( o_img_ptr->cFormat )
      {
        ...
        default:
          IPL2_MSG_FATAL( "ipl2_handle_YCbCrImg :: /\
                  unknown o/p color format = %d", o_img_ptr->cFormat, 0, 0);
    ...
    switch ( o_img_ptr->cFormat )
    {
      ...
      default:
        IPL2_MSG_FATAL( "ipl2_hanle_YCbCrImg :: /\
                unknown o/p = %d ", o_img_ptr->cFormat, 0, 0);
    ...
    default :
      {
        IPL2_MSG_FATAL( "ipl2_Rot000Frame :: /\
                unknown rotation option = %d", rotate, 0, 0);
...
)
{
  ...
  {
    IPL2_MSG_ERROR( "ipl2_handle_YCrCb420lpImg failed i_img_ptr->dx = %x, /\
               i_img_ptr->dy  = %x", (uint32) i_img_ptr->dx,
               (uint32)i_img_ptr->dy, 0 );
...
)
{
  ...
  if( (i_img_ptr->dx & 0x1) || (i_img_ptr->dy & 0x1) )
  {
    IPL2_MSG_ERROR( "ipl2_handle_YCbCr420Img failed i_img_ptr->dx = %x, /\
               i_img_ptr->dy  = %x", (uint32) i_img_ptr->dx,
               (uint32)i_img_ptr->dy, 0 );
  ...
  if ( !i_frame_ptr )
  {
    switch ( rotate )
    {
      ...
      default:
        ...
        IPL2_MSG_FATAL( "ipl2_Rot000Frame :: /\
           unknown o/p rotation = %d ", rotate, 0, 0);
...
FUNCTION    ipl4_Rot000Frame_CropYCrCb420lpToRGB()

DESCRIPTION
  This function will handle cropping, color converting of
  YCbCr420 based images to RGB565 using colorconversion closure to MDP flavors. This function is more
  optimalif you are not cropping . This function is specifically designed
  for the favorable input condition where - there is no framing going on. So
  transparent value check is not needed. 
    ...
    for ( row = render_row_size/2; row; row-- )
    {
      ...
      for ( col = render_col_size/2; col; col-- )
      {
       ...
       bc = (ycbcr_convert[5]*(cr-128) + ycbcr_convert[6]*(cb-128));
        ...
        luma = *inputImgPtr;  /* corresponds to Y1 in the pic */
        luma -= 16;
        ...
        *outputImgPtr = pack_rgb565(r,g,b);
        ...
        luma = *(inputImgPtr + 1);  /* corresponds to Y2 in the pic */
        luma -= 16;
        luma *= ycbcr_convert[0];
        r = (luma + rc)>>16;
        g = (luma + gc)>>16;
        ...
        *(outputImgPtr + 1) = pack_rgb565(r,g,b);
        ...
        g = CLIPIT(g);
        b = CLIPIT(b);
       *(outputImgPtr++ + rowSize) = pack_rgb565(r,g,b);
...
)
{
  ...
  if (i_frame_ptr == NULL)
  {
    ...
    for ( row = 0; row < render_row_size /2 ; row++ )
    {
      ...
      ----------------------------------------------------------------------*/
      inputImgPtr = i_img_ptr->imgPtr + src_index + 2*(row+1);  
      cr_ptr = i_img_ptr->clrPtr + cr_src_index + 2*(row+1);  
    }
  }
  else
  {
...
FUNCTION ipl4_Rot090Frame_CropYCrCb420lpToRGB

DESCRIPTION
  This function rotates crops and color convert a YUV420 based
  frame into RGB565 format using conversion closer to MDP The degree of rotation is 90 degree.
...
)
{
  ...
  inputImgPtr += i_img_ptr->dx * (iicrop.y + iicrop.dy-1) + iicrop.x + iicrop.dx-1;
  ...
  ------------------------------------------------------------------------*/
  cr_ptr  = i_img_ptr->clrPtr;
  cr_ptr += i_img_ptr->dx * (iicrop.y/2 + iicrop.dy/2 - 1) + iicrop.x + iicrop.dx - 2;
...
)
{
  ...
  ------------------------------------------------------------------------*/
  cr_src_index = iicrop.dx + iicrop.x + i_img_ptr->dx*iicrop.y/2;
...
0 Copyright (c) 2004 by QUALCOMM, Incorporated.  All Rights Reserved.
  QUALCOMM Proprietary.  Export of this technology or software is
 regulated by the U.S. Government. Diversion contrary to U.S. law prohibited.
===========================================================================*/
      ...
      default :
        {
          IPL2_MSG_FATAL( "ipl2_RotFrame :: /\
                          uknonw to rotaion param = %d", rotate, 0, 0);
  ...
  MSG_LOW("inside ipl2_RotFrame_YCrCb420lpToRGB\n",0,0,0);
    ...
    switch ( rotate )
    {
      case IPL_NOROT :
        ...
        ------------------------------------------------------------------*/
        if(o_img_ptr->cFormat == IPL_RGB565)
          status = ipl4_Rot000Frame_CropYCrCb420lpToRGB(
                       i_img_ptr,
                       i_frame_ptr,
                       transparentValue,
                       o_img_ptr,
      ...
      case IPL_ROT90 :
        ...
        ------------------------------------------------------------------*/
        if(o_img_ptr->cFormat == IPL_RGB565)
          status = ipl4_Rot090Frame_CropYCrCb420lpToRGB(
                       i_img_ptr,
                       i_frame_ptr,
                       transparentValue,
                       o_img_ptr,
