...
$Header: //source/qcom/qct/multimedia/ipl/colorconversion/main/latest/src/ipl_convert.c#14 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/07/09   SV	   added ipl_convert_pixel_rgb565_ycrcb() to convert rgb565 pixel values to y,cr,cb values.. 
10/03/08   babakf  Added swap support when going from frame pack to line pack
12/19/07   babakf  Added support to ipl3_convert_ycrcb420lp_to_rgb565 to 
                   support single line image.
...
)
{
  ...
  if (o_img_ptr->dx != i_img_ptr->dx || o_img_ptr->dy != i_img_ptr->dy)
  {
    MSG_LOW("ipl_convert_to_luma_only marker_201\n",0,0,0);
    return IPL_FAILURE;
  }
  ...
  else if (i_img_ptr->cFormat == IPL_YCbCr420_FRAME_PK ||
           i_img_ptr->cFormat == IPL_YCbCr420_LINE_PK || 
           i_img_ptr->cFormat == IPL_YCbCr422_LINE_PK ||
           i_img_ptr->cFormat == IPL_YCbCr444_LINE_PK || 
           i_img_ptr->cFormat == IPL_YCrCb420_LINE_PK ||
           i_img_ptr->cFormat == IPL_YCrCb422_LINE_PK || 
           i_img_ptr->cFormat == IPL_YCrCb444_LINE_PK)
  {
    MSG_LOW("ipl_convert_to_luma_only marker_2\n",0,0,0);
    memcpy(o_img_ptr->imgPtr, i_img_ptr->imgPtr, i_img_ptr->dx*i_img_ptr->dy);
  }
  else if (i_img_ptr->cFormat == IPL_YCbCr)
  {
    uint8 * iptr;

    iptr = i_img_ptr->imgPtr;
    outImgPtr = o_img_ptr->imgPtr;

    MSG_LOW("ipl_convert_to_luma_only marker_3\n",0,0,0);
  ...
  {
    MSG_LOW("ipl_convert_to_luma_only marker_202\n",0,0,0);
    return IPL_FAILURE;
  }
...
)
{
  ...
  if((!oddWidth)&&(!oddHeight))
  {
    ...
    {
      MSG_LOW("ipl_convert_rgb565_to_ycrcb420lp marker_203\n",0,0,0);
      return IPL_FAILURE;
    }
  }
...
FUNCTION    ipl4_convert_ycrcb420lp_to_rgb565
...
==========================================================================*/
extern ipl_status_type ipl4_convert_ycrcb420lp_to_rgb565
(
  ipl_image_type* input_img_ptr,       /* Pointer to the input image  */
  ipl_image_type* output_img_ptr       /* Pointer to the output image */
)
{
  ...
  MSG_LOW("ipl4_convert_ycrcb420lp_to_rgb565 marker_0\n",0,0,0);

  if (!input_img_ptr  || !input_img_ptr->imgPtr ||
      !output_img_ptr || !output_img_ptr->imgPtr)
  {
    MSG_LOW("ipl4_convert_ycrcb420lp_to_rgb565 marker_200\n",0,0,0);
    return IPL_FAILURE;
  }
  ...
  if (pitchx < 0)
  {
    MSG_LOW("ipl4_convert_ycrcb420lp_to_rgb565 marker_201\n",0,0,0);
    return IPL_FAILURE;
  }
  ...
  MSG_LOW("ipl4_convert_ycrcb420lp_to_rgb565 marker_1\n",0,0,0);
  ...
  MSG_LOW("ipl4_convert_ycrcb420lp_to_rgb565 marker_100\n",0,0,0);

  return IPL_SUCCESS;
} /* end of function ipl4_convert_ycrcb420lp_to_rgb565 */
...
)
{
  ...
  for (row = input_img_ptr->dy; row; row--)
  {
    for (col = (output_img_ptr->dx - oddWidth)/2; col; col--)
    {
      ...
      luma = *inputImgPtr++;        /* corresponds to Y1 */
      r = (uint8)CLIPIT((luma + ipl_crr[cr]));
      g = (uint8)CLIPIT((luma + ((ipl_crg[cr] + ipl_cbg[cb]) >> 16)));
      b = (uint8)CLIPIT((luma + ipl_cbb[cb]));
      *outputImgPtr++   = r;
      *outputImgPtr++ = g;
      *outputImgPtr++ = b;
...
)
{
  ...
  /* fill in last row since we skipped it earliear */
  if (oddWidth && input_img_ptr->dy > 1)
  {
    /* leave this outer for loop in for debugging purposes */
    for (col = 0; col < 1; col++) 
    {
  ...
  if (input_img_ptr->dy % 2 && output_img_ptr->dy >= 3)
  {
    /* CR 135220 <--- */
    outputImgPtr = (uint16*) output_img_ptr->imgPtr + 
    (output_row_size * ((input_img_ptr->dy / 2) * 2));
    inputImgPtr = input_img_ptr->imgPtr + 
    (input_row_size * ((input_img_ptr->dy / 2) * 2));
    cb_ptr = input_img_ptr->clrPtr + 
    (input_row_size * (input_img_ptr->dy / 2));
    /* CR 135220 <--- */
    ...
    /* CR 135220 <--- */
    /* fill in last col since we skipped it earliear */
    if (oddWidth)
    {
      cr = *cb_ptr++;
      cb = *cb_ptr++;
  ...
  // special case if jpeg calls us with 1 lin 420line pack which is against
  // standard, but can happen
  else if (input_img_ptr->dy == 1)
  {
    for (col = (output_row_size - oddWidth)/2; col; col--)
    {
      /*--------------------------------------------------------------------
...
)
{
  ...
  if (!input_img_ptr  || !input_img_ptr->imgPtr ||
      !output_img_ptr || !output_img_ptr->imgPtr ||
      !input_img_ptr->clrPtr || !output_img_ptr->clrPtr)
  {
...
)
{
  ...
  else if (input_img_ptr->cFormat == IPL_RGB888_PAD) 
  {
    ...
    else if (output_img_ptr->cFormat == IPL_YCbCr) 
    {
      ...
      /* Loop through input image */
      for (y = input_img_ptr->dy; y; y--) 
      {
        for (x = input_img_ptr->dx; x; x--) 
        {
          /* Pixel 1 */
          r = *data_in++;
          g = *data_in++;
          ...
          *data_out++ = (uint8)cb;
          *data_out++ = (uint8)luma1;
          *data_out++ = (uint8)cr;
          *data_out++ = (uint8)luma2;
        }
      }
...
FUNCTION ipl_convert_dither

DESCRIPTION
  This function performs color conversion from RGB888 to RGB565 with error 
  diffusion dithering.  Floyd Steinberg's algorithm disperses the quantization
  error to neighbouring pixels to the right and bottom of current pixel - 
  7/16 of error to the right, 3/16 of error to the next row &
  previous column, 5/16 of error to next row & same column, 
  1/16 of error to next row & next column.
...
ARGUMENTS IN
  input_img_ptr     pointer to input image 
...
===========================================================================*/
extern ipl_status_type ipl_convert_dither
(
  ipl_image_type* input_img_ptr,        /* Points to the input image  */
  ipl_image_type* output_img_ptr        /* Points to the output image */
)
{
  ...
  MSG_LOW("ipl_convert_dither marker_0\n",0,0,0);

  if (!input_img_ptr  || !input_img_ptr->imgPtr  || 
      !output_img_ptr || !output_img_ptr->imgPtr)
  {
    MSG_LOW("ipl_convert_dither marker_200\n",0,0,0);
    return IPL_FAILURE;
  }
  ...
  if (input_img_ptr->dx != output_img_ptr->dx || 
      input_img_ptr->dy != output_img_ptr->dy)
  {
    MSG_LOW("ipl_convert_dither marker_201\n",0,0,0);
    return IPL_FAILURE;
  }
  ...
  if ( (input_img_ptr->cFormat != IPL_RGB888) ||
       (output_img_ptr->cFormat != IPL_RGB565))
  {
    MSG_LOW("ipl_convert_dither marker_202\n",0,0,0);
    return IPL_FAILURE;
  }
  ...
  MSG_LOW("ipl_convert_dither marker_1\n",0,0,0);
 

  width3 = output_img_ptr->dx * 3;

  for(i = 0; i < (output_img_ptr->dy - 1); i++)
  {
    index = i * width3;
    odx   = i * output_img_ptr->dx;
  
    for(j = 1; j < (output_img_ptr->dx - 1); j++)
    {
      
      data_out[odx] = pack_rgb565(data_in[index], data_in[index+1], data_in[index+2]);
      
      /* R's error */
      cp = data_in + index;
      error = *cp - rgb565_table[*cp];
      /* Distribute the error according to Floyd Steinberg algorithm */
      /* Pixel in row, col+1 */
      *(cp + 3)         = ipl_clamp(*(cp + 3)   +  (( 7 * error) >> 4)); /*7/16 */
      /* Pixels in the row+1 */
      {
        /* Pixel in row+1, col-1 */
        *(cp + width3 - 3) = ipl_clamp(*(cp + width3 - 3) + ((3 * error) >> 4)); /*3/16*/
        /* Pixel in row+1, col */
        *(cp + width3)     = ipl_clamp(*(cp + width3)     + ((5 * error) >> 4)); /*5/16*/
        /* Pixel in row+1, col+1 */
        *(cp + width3 + 3) = ipl_clamp(*(cp + width3 + 3) + ((1 * error) >> 4)); /*1/16*/
      }
      index++;
           
      /* G's error */
      cp = data_in + index;
      error = *cp - rgb565_table[*cp+256];
      /* Distribute the error according to Floyd Steinberg algorithm */
      /* Pixel in row, col+1 */
      *(cp + 3)         = ipl_clamp(*(cp + 3)         + (( 7 * error) >> 4));/*7/16 */
      /* Pixels in the row+1 */
  ...
  MSG_LOW("ipl_convert_dither marker_100\n",0,0,0);
    ...
    for (row = 0; row < i_frame_ptr->dy; row += 2)
    {
      for (col = 0; col < i_frame_ptr->dx; col += 2)
      {
            ...
            v3 == transparentValue || v4 == transparentValue)
        {
          ...
          *(outImgPtr )         = (uint8)tY;  //107
          *(outImgPtr + 1)      = (uint8)tY;  //107
          *(outImgPtr + dx)     = (uint8)tY;  //107
          *(outImgPtr + dx + 1) = (uint8)tY;  //107

          outImgPtr += 2;

          // set cr and cb to transparent
          *outClrPtr++ = (uint8)tCr;  // 222;
          *outClrPtr++ = (uint8)tCb;  // 202;
        } 
        else 
        {
          /* Pixel 1 */
...
)
{
  ...
  else if (input_img_ptr->cFormat == IPL_YCrCb420_LINE_PK)
  {
    if (output_img_ptr->cFormat == IPL_YCbCr) 
      ...
      retval = ipl3_convert_ycrcb420lp_to_rgb565(input_img_ptr,output_img_ptr);
    else if (output_img_ptr->cFormat == IPL_RGB888) 
    ...
    else if (output_img_ptr->cFormat == IPL_YCbCr422_LINE_PK) 
    {
      ...
      retval=ipl_convert_ycbcr420lp_to_ycbcr444lp(input_img_ptr,output_img_ptr);
             ...
             (output_img_ptr->cFormat == IPL_PAD_RGB888))
      retval = ipl_convert_ycrcb420lp_to_rgb888pad(input_img_ptr, output_img_ptr, 0);
    else if (output_img_ptr->cFormat == IPL_LUMA_ONLY)
      retval = ipl_convert_to_luma_only(input_img_ptr, output_img_ptr);
    else
      retval = IPL_FAILURE;
...
FUNCTION    ipl_convert_ycrcb420lp_to_rgb888pad

DESCRIPTION
  This function converts from YCrCb 4:2:0 line packed format to RGB888 pad (0 being MSB)
  and pad RGB888 (0 being LSB). 
...
==========================================================================*/
extern ipl_status_type ipl_convert_ycrcb420lp_to_rgb888pad
(
  ipl_image_type* input_img_ptr,       /* Pointer to the input image  */
  ipl_image_type* output_img_ptr,      /* Pointer to the output image */
  uint8           pad                  /* Alpha Channel Value */
)
{
  register unsigned char* inputImgPtr;
  register uint8* outputImgPtr;
  ...
  MSG_LOW("ipl_convert_ycrcb420lp_to_rgb888pad marker_0\n",0,0,0);

  if (!input_img_ptr  || !input_img_ptr->imgPtr ||
      !output_img_ptr || !output_img_ptr->imgPtr)
  {
    MSG_LOW("ipl_convert_ycrcb420lp_to_rgb888pad marker_200\n",0,0,0);
    return IPL_FAILURE;
  }
  ...
  input_row_size = input_img_ptr->dx;
  output_row_size = 4*output_img_ptr->dx;
  cb_ptr = input_img_ptr->clrPtr;
  ...
  if (pitchx < 0)
  {
    MSG_LOW("ipl_convert_ycrcb420lp_to_rgb888pad marker_201\n",0,0,0);
    return IPL_FAILURE;
  }
  ...
  MSG_LOW("ipl_convert_ycrcb420lp_to_rgb888pad marker_1\n",0,0,0);

  if (output_img_ptr->cFormat == IPL_RGB888_PAD)
  {
    for (row = input_img_ptr->dy/2; row; row--)
    {
      for (col = (output_img_ptr->dx - oddWidth)/2; col; col--)
      {
        ...
        *(outputImgPtr++ + output_row_size) = g;
        *(outputImgPtr++ + output_row_size) = b;
        *(outputImgPtr++ + output_row_size) = pad;
      ...
      /*----------------------------------------------------------------------
          Add on row increments
      ----------------------------------------------------------------------*/
      outputImgPtr += (4*oddWidth) + output_row_size;
      inputImgPtr += (input_row_size + oddWidth + pitchx);
      cb_ptr += (pitchx + oddWidth);
    } 
    ...
    if (oddWidth)
    {
      ...
      for (col = 0; col < 1; col++) 
      {
        outputImgPtr = output_img_ptr->imgPtr + (output_row_size - 4 + col);
        inputImgPtr = input_img_ptr->imgPtr + input_row_size - pitchx - 1 + col;
        cb_ptr = input_img_ptr->clrPtr + input_row_size - pitchx - 1 + 2*(col/2);
  ...
  } /* end of output format RGB888_PAD */
  else if (output_img_ptr->cFormat == IPL_PAD_RGB888)
  {
    for (row = input_img_ptr->dy/2; row; row--)
    {
      for (col = (output_img_ptr->dx - oddWidth)/2; col; col--)
  ...
  } /* end of output format PAD_RGB888 */
  else
  {
    MSG_LOW("ipl_convert_ycrcb420lp_to_rgb888pad marker_202\n",0,0,0);
    return IPL_FAILURE;
  }

  MSG_LOW("ipl_convert_ycrcb420lp_to_rgb888pad marker_100\n",0,0,0);

  return IPL_SUCCESS;
} /* end of function ipl_convert_ycrcb420lp_to_rgb888pad */


/* <EJECT> */
/*===========================================================================

FUNCTION ipl_convert_pixel_rgb565_ycrcb

DESCRIPTION
  This function converts rgb565 pixels to ycrcb.

DEPENDENCIES
  None

ARGUMENTS IN
  rgb565      Input rgb565 value.
  y           Pointer to output luma(Y) value
  cr          Pointer to output cr value
  cb          Pointer to output cb value
...
ipl_status_type ipl_convert_pixel_rgb565_ycrcb
(
 uint16 rgb565,
 uint8 *y,
 uint8 *cr,
...
)
{
   ...
   r = (unsigned char)((rgb565&0xF800)>>8); //lint !e613
   g = (unsigned char)((rgb565&0x07E0)>>3); //lint !e613
   b = (unsigned char)((rgb565&0x001F)<<3); //lint !e613
    //printf("r %d, g %d, b %d\n", r,g,b);
    tY = (ycbcr_convert[0]*r + ycbcr_convert[1]*g +
          ycbcr_convert[2]*b)*4+0x8000;
    tY = (tY >> 16) + 16;
  ...
  return IPL_SUCCESS;
}/* End ipl_convert_pixel_rgb565_ycrcb */
