...
#include "disp_st7735r_128x128.h"
#include "gpio_1100.h"
#include "pm.h"
#include "pmapp.h"
#ifdef T_FFA
...
#define ST7735R_128x128_DISP_WIDTH             128
#define ST7735R_128x128_DISP_HEIGHT            128

#define ST7735R_128x128_DISP_MIN_CONTRAST      0
#define ST7735R_128x128_DISP_MAX_CONTRAST      255
#define ST7735R_128x128_DISP_DEFAULT_CONTRAST  128

#define ST7735R_128x128_DISP_MIN_BACKLIGHT     0 
#define ST7735R_128x128_DISP_MAX_BACKLIGHT     15
#define ST7735R_128x128_DISP_DEFAULT_BACKLIGHT 3

#define ST7735R_128x128_DISP_ENABLE            TRUE
#define ST7735R_128x128_DISP_DISABLE           FALSE
...
#define LCD_VISIBLE_BACKLIGHT_LEVELS   \
        ( (ST7735R_128x128_DISP_MAX_BACKLIGHT) - (ST7735R_128x128_DISP_MIN_BACKLIGHT) )
...
static void st7735r_128x128_disp_update
(
...
static rex_crit_sect_type        st7735r_128x128_crit_sect = {0};
static disp_info_type            st7735r_128x128_disp_info = {0};
static st7735r_128x128_state_type st7735r_128x128_state = {0};
...
int st7735r_128x128_install(char *str);
...
FUNCTION      st7735r_128x128_reset
...
static void st7735r_128x128_reset(void)
{
...
FUNCTION      st7735r_128x128_HW_INIT
...
===========================================================================*/
static void st7735r_128x128_hw_init(void)
{
	uint32 regval = 0;
...
} /* st7735r_128x128_hw_init() */


/*===========================================================================

FUNCTION      st7735r_128x128_SET_SCREEN_AREA
...
===========================================================================*/
static void st7735r_128x128_set_screen_area(uint32 start_row, uint32 start_col,
...
static void st7735r_128x128_disp_set_backlight(byte level)
{
	static uint32 current_level =  ST7735R_128x128_DISP_MIN_BACKLIGHT;
    
	if(st7735r_128x128_state.disp_initialized &&
		st7735r_128x128_state.disp_powered_up  &&
		st7735r_128x128_state.display_on)
	{
		if (level!= current_level)
		{
			...
			if (level > ST7735R_128x128_DISP_MAX_BACKLIGHT )
			{
				level = ST7735R_128x128_DISP_MAX_BACKLIGHT;
			}
			else if (ST7735R_128x128_DISP_MIN_BACKLIGHT - level > 0)
			{
				/* Turn backlight off */
				level = ST7735R_128x128_DISP_MIN_BACKLIGHT;
			}
...
} /* st7735r_128x128_disp_set_backlight */

/*===========================================================================

FUNCTION      st7735r_128x128_DISP_CLEAR_SCREEN_AREA
...
===========================================================================*/
static void st7735r_128x128_disp_clear_screen_area(word start_row, word start_col,
...
} /* st7735r_128x128_disp_clear_screen_area() */


/*===========================================================================

FUNCTION      st7735r_128x128_DISP_CLEAR_WHOLE_SCREEN
...
===========================================================================*/
static void st7735r_128x128_disp_clear_whole_screen(void)
{
	st7735r_128x128_disp_clear_screen_area(0, 
	                                    0,
	                                    st7735r_128x128_disp_info.disp_height-1,
	                                    st7735r_128x128_disp_info.disp_width-1);
} /* st7735r_128x128_disp_clear_whole_screen() */


/*===========================================================================

FUNCTION      st7735r_128x128_DISP_OFF
...
===========================================================================*/
static void st7735r_128x128_disp_off(void)
{
	rex_enter_crit_sect(&st7735r_128x128_crit_sect);
	...
	rex_leave_crit_sect(&st7735r_128x128_crit_sect);
} /* st7735r_128x128_disp_off() */


/*===========================================================================

FUNCTION      st7735r_128x128_DISP_ON
...
===========================================================================*/
static void st7735r_128x128_disp_on(void)
{
	rex_enter_crit_sect(&st7735r_128x128_crit_sect);
	...
	{  
		...
		st7735r_128x128_state.display_on = TRUE;
	}

	rex_leave_crit_sect(&st7735r_128x128_crit_sect);
} /* st7735r_128x128_disp_on() */


/*===========================================================================

FUNCTION      st7735r_128x128_DISP_POWERUP
...
FUNCTION      st7735r_128x128_DISP_POWERDOWN
...
===========================================================================*/
static int st7735r_128x128_disp_powerdown(void)
{
	rex_enter_crit_sect(&st7735r_128x128_crit_sect);
	...
	if (st7735r_128x128_state.disp_powered_up && !st7735r_128x128_state.display_on) 
	{
		...
		*/
		...
		/* Reset controller to ensure clean state */
		/* Transfer command to display hardware*/
		//out_word(lcatDispCfg[0].cmd_addr, DISP_EPSON_CMD9(ST7735R_128x128_VDD_OFF_C));   miaoxiaoming
		clk_busy_wait(100);
		st7735r_128x128_state.disp_powered_up = FALSE;
	}

	rex_leave_crit_sect(&st7735r_128x128_crit_sect);

	return FALSE;
} /* st7735r_128x128_disp_powerdown() */


/*===========================================================================

FUNCTION      st7735r_128x128_DISP_INIT
...
===========================================================================*/
int st7735r_128x128_disp_init(void)
{
	if (st7735r_128x128_state.disp_initialized) 
	{
	...
	st7735r_128x128_disp_info.palette_support    = FALSE;
	st7735r_128x128_disp_info.contrast_support   = FALSE;
	st7735r_128x128_disp_info.contrast_min       = ST7735R_128x128_DISP_MIN_CONTRAST;
	st7735r_128x128_disp_info.contrast_max       = ST7735R_128x128_DISP_MAX_CONTRAST;
	st7735r_128x128_disp_info.contrast_default   = ST7735R_128x128_DISP_DEFAULT_CONTRAST;
	st7735r_128x128_disp_info.backlight_support  = TRUE;
	st7735r_128x128_disp_info.backlight_min      = ST7735R_128x128_DISP_MIN_BACKLIGHT;
	st7735r_128x128_disp_info.backlight_max      = ST7735R_128x128_DISP_MAX_BACKLIGHT;
	st7735r_128x128_disp_info.backlight_default  = ST7735R_128x128_DISP_DEFAULT_BACKLIGHT;
    st7735r_128x128_disp_info.lcd_type           = TM_SCTN128;
    st7735r_128x128_disp_info.phys_width         = ST7735R_128x128_DISP_WIDTH;    
    st7735r_128x128_disp_info.phys_height        = ST7735R_128x128_DISP_HEIGHT; 

	rex_init_crit_sect(&st7735r_128x128_crit_sect);

	st7735r_128x128_hw_init();

	st7735r_128x128_disp_powerup();

	st7735r_128x128_disp_on();

	st7735r_128x128_state.disp_initialized = TRUE;
	
	st7735r_128x128_disp_clear_whole_screen();

	st7735r_128x128_disp_set_backlight(ST7735R_128x128_DISP_DEFAULT_BACKLIGHT);
...
FUNCTION      st7735r_128x128_DISP_GET_INFO
...
===========================================================================*/
static disp_info_type st7735r_128x128_disp_get_info(void)
{
	return st7735r_128x128_disp_info;
} /* st7735r_128x128_disp_get_info() */


/*===========================================================================

FUNCTION      st7735r_128x128_DISP_COPY
...
===========================================================================*/
static void st7735r_128x128_disp_copy(void *src_ptr, dword copy_count)
{  
	int i;
...
} /* st7735r_128x128_disp_copy() */


/*===========================================================================

FUNCTION      st7735r_128x128_DISP_UPDATE
...
)
{
	...
	if (!buf_ptr                          || 
		!st7735r_128x128_state.disp_initialized ||
		!st7735r_128x128_state.disp_powered_up  ||
		!st7735r_128x128_state.display_on)
	{
		return;
	...
	if ((dst_starting_row + num_of_rows) > st7735r_128x128_disp_info.disp_height)
	{
		num_of_rows = st7735r_128x128_disp_info.disp_height - dst_starting_row;
	}
	
	if ((dst_starting_column + num_of_columns) > st7735r_128x128_disp_info.disp_width)
	{
		num_of_columns = st7735r_128x128_disp_info.disp_width - dst_starting_column;
	}

	/* Ensure buffer aligned and parameters valid */
	if((src_starting_row    < st7735r_128x128_disp_info.disp_height) &&
		(src_starting_column < st7735r_128x128_disp_info.disp_width)  &&
		(dst_starting_row    < st7735r_128x128_disp_info.disp_height) &&
		(dst_starting_column < st7735r_128x128_disp_info.disp_width)  &&
		(!(((uint32)buf_ptr) & 0x3)))
	{
		rex_enter_crit_sect(&st7735r_128x128_crit_sect);    

		st7735r_128x128_set_screen_area(dst_starting_row, dst_starting_column, 
		...
		if((src_starting_column == 0) && (num_of_columns == src_width)) 
		{
			...
			st7735r_128x128_disp_copy(src_ptr, copy_count);
		} 
		else 
		{
...
===========================================================================*/
static void st7735r_128x128_disp_set_contrast(word contrast)
{
	return;
} /* st7735r_128x128_disp_set_contrast() */


/*===========================================================================

FUNCTION      st7735r_128x128_DISP_IOCTL
...
===========================================================================*/
static int st7735r_128x128_disp_ioctl ( int cmd, void *arg )
{
	...
	disp_update_cmd = (disp_update_type*)arg;
		...
		{
			st7735r_128x128_disp_update(disp_update_cmd->buf_ptr,
	...
	case IOCTL_DISP_GET_INFO:
		*(disp_info_type *)arg = st7735r_128x128_disp_get_info();
		break;
	...
	case IOCTL_DISP_SET_CONTRAST:
		st7735r_128x128_disp_set_contrast(*(word *)arg);
		break;
	...
	case IOCTL_DISP_SET_BACKLIGHT:
        ...
        #else
		st7735r_128x128_disp_set_backlight(*(byte *)arg);
        #endif
		break;
	...
	case IOCTL_DISP_CLEAR_SCREEN_AREA:
		st7735r_128x128_disp_clear_screen_area(((disp_cls_type *)arg)->start_row,
	...
	return 1;
} /* st7735r_128x128_disp_ioctl() */


/*===========================================================================

FUNCTION      ST7735R_128x128_INSTALL
