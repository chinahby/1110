...
$Header: //depot/asic/msmshared/services/graph/graph.c#76 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/14/09   st      Fix for multiple start and stop operations for 
                   pmd file having svg content
...
#error code not present
    ...
    case GRAPH_SVG_CMD_ABORT:
	  ...
	  {
		  /*svg handler not available to cmx.svg will provide it's handler after 
		  processing the data and before rendering.If cmx receives any abort command 
		  before svg  handle receiving,then cmx will send abort command to graph task
		  with svg handle NULL.0xDEADBEAF used as identifier to handle this special case.
		  */
		  svg_decode_abort((void*)0xDEADBEAF);
	  }
      break;
