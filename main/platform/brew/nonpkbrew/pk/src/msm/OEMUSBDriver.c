...
#define INTPORT_TO_USBDRIVER(p)  (((IUSBIntPort*)(void *) p)->pMe)
...
struct IUSBDriver
{
   ...
   byte          *pResponse;
   AEECallback    cbResponseComplete;    // Response has been completed
};
...
static AEEDSMBuf* AEEDSMMgr_DequeueByDSMPtr(AEEDSMBuf **ppList, 
                                            dsm_item_type *pFind);
static void free_brew_dsm_buffer(void *pBuf);
...
#ifdef FEATURE_HSU_MTP
static void USBDriver_USBDCRxCB(mtpfd_pipe_type_t pipe_type);
...
static boolean    gAbortRead = FALSE;
...
FUNCTION      USBDriver_ClearResponseBuffer

DESCRIPTION   
    This function is responsible for cleaning up the allocated response buffer.
...
==============================================================================*/
static void USBDriver_ClearResponseBuffer(void *po)
{
   IUSBDriver *pme = (IUSBDriver *)po;
   if(NULL != pme->pResponse)
   {
      FREE(pme->pResponse);
      pme->pResponse = NULL;
   }
...
#endif
   CALLBACK_Init(&pme->cbResponseComplete, USBDriver_ClearResponseBuffer, (void*)pme);
 
   pme->intPort.pMe =  pme;
   pme->intPort.pvt = &gUSBDriverIntVtblFuncs;
...
#ifdef FEATURE_HSU_MTP
      /*
        The buffer requirements from USB core (Jungo s/w) is that virtual memory 
        address matches the physical memory and the buffer is non-cachable.
        This change fixed Teleca MTP app not working with log message turned off issue.
      */
      pNewBuf->pBuf = pmem_malloc(AEEDSMBUFMGR_ALLOCSIZE, PMEM_USB_ID); 
#else                                  
      ...
      {
         free_brew_dsm_buffer(pNewBuf->pBuf);
         FREE(pNewBuf);
      }
      else
      {
...
static void AEEDSMBufMgr_Free(AEEDSMBuf *pFree)
{
   ...
   free_brew_dsm_buffer(pFree->pBuf);
  
   FREE(pFree);
}

/* This function is created to handle different FREE cases. One is for HSU using PMEM and
   the other is for FS USB using BREW FREE funcation. 
*/
static void free_brew_dsm_buffer(void *pBuf)
{
    if (pBuf != NULL)
...
static int32 IUSBDriver_Read(IUSBDriver *pme, char *pcBuf, int32 cbBuf)
{
          ...
          USBDriver_TransferQSize(pSubPort->pRxQ) > 0)
   {
      ...
      MEMCPY(pcBuf+dwRead, pSubPort->pRxQ->pBuf, (unsigned int)dwCnt);
...
static IPort* IUSBDriver_GetInterruptEndpoint(IUSBDriver *pme, int nLatency)
{
   ...
   pRet = (IPort*)(void *)(&pme->intPort);
   (void)IUSBDRIVER_AddRef(pme);
   return pRet;
}
                                     ...
                                     int nSize)
{
   ...
   if(NULL != pme->pResponse)
   {
      CALLBACK_Cancel(&pme->cbResponseComplete);
      FREE(pme->pResponse);
      pme->pResponse = NULL;
   }
...
#if defined (FEATURE_HSU_MTP) || (defined(FEATURE_USB) && !defined(FEATURE_USBDRIVER_NO_USBDC_API))
   if((0 == (pme->SetupMsg.wRequestAndType & 0x0080)) ||
      (0 == pme->SetupMsg.wLength))
   {
      if(((unsigned int)nSize != sizeof(int)) || 
         (pResponse == NULL) ||
...
#else  /* HS USB */
      hsu_al_brewext_send_setup_msg_response(0, NULL);
#endif /* FEATURE_USB */
      return SUCCESS;
   }
   ...
   pme->pResponse = MALLOC((dword)nSize);
   if (pme->pResponse != NULL)
      {
      (void)MEMCPY(pme->pResponse, pResponse, (unsigned int)nSize);
   }

   if (pme->pResponse != NULL)
   {
#ifdef FEATURE_HSU_MTP
      hsu_al_brewext_send_setup_msg_response(nSize, pme->pResponse);
#else
  #ifdef FEATURE_USBDRIVER_NO_USBDC_API
      usbdc_send_setup_msg_response(nSize, pme->pResponse);
  #else
      usbdc_api_send_setup_msg_response(USBDC_BREW_EXTENDED_DEVICE, 
                                        (uint16)nSize, pme->pResponse);
  #endif /* FEATURE_USBDRIVER_NO_USBDC_API */
#endif /* FEATURE_HSU_MTP */

      AEE_SetTimerCallback(500, &pme->cbResponseComplete, NULL);
   }
   return SUCCESS;
}
...
static int IUSBDriver_Int_QueryInterface(IPort *po, AEECLSID cls, void **ppif)
{
   ...
   switch (cls)
   {
      ...
      default:
         pme->subports[SUBPORT_INT].nLastErr = ECLASSNOTSUPPORT;
                                          ...
                                          int nIOCtl, uint32 dwParam)
{
   ...
   pSubPort->bStreamMode = (boolean)dwParam;
   return SUCCESS;
}
                                        ...
                                        int nIOCtl, uint32 dwParam) 
{
   ...
   if(dwParam & AEEUSBDRIVER_BM_OUTENDPOINT)
   {
      ...
      #ifdef FEATURE_USBDRIVER_NO_USBDC_API
            usbdc_ioctl(usbdc_iface, USBDC_FLUSH_READ, NULL);
      #else
            usbdc_api_ioctl(usbdc_iface, USBDC_FLUSH_READ, NULL);
      #endif /* FEATURE_USBDRIVER_NO_USBDC_API */
   ...
   return nErr;
#elif defined(FEATURE_HSU_MTP) 
   if(dwParam & AEEUSBDRIVER_BM_INENDPOINT)
   {
      /* Flush Tx Data */
      mtpfd_pipe_type_t pipe_type;
  
      pipe_type = ((pSubPort->nPort == SUBPORT_BULK) ?
      ...
      hsu_al_brewext_ioctl(pipe_type, MTPFD_ABORT_WRITE, 0);
   ...
   if(dwParam & AEEUSBDRIVER_BM_OUTENDPOINT)
   {
      ...
      hsu_al_brewext_ioctl(pipe_type, MTPFD_ABORT_READ, 0);
                                        ...
                                        int nIOCtl, uint32 dwParam) 
{
   ...
   if(dwParam & AEEUSBDRIVER_BM_INENDPOINT)
   {
      ...
      hsu_al_brewext_stall_in_pipe(pipe_type);
      nErr = SUCCESS;
   }
...
#if defined(FEATURE_USB) && !defined(FEATURE_USBDRIVER_NO_USBDC_API)
   MEMCPY(pReadStruct->pData, pme->ctrlRequestOutData, (unsigned int)pReadStruct->nSize);
   return SUCCESS;
#else  /* FEATURE_HSU_MTP */
   /* place some dummy string for now */
   MEMCPY(pReadStruct->pData, "01234567890123456789", 20);
   return SUCCESS;
#endif
}
...
static void USBDriver_Destroy(IUSBDriver *pme, boolean bIgnoreCB)
{
   ...
   while ((pDel = pme->bufmgr.pUsedBufs) != NULL)
   {
      ...
      free_brew_dsm_buffer(pDel->pBuf);
      FREE(pDel);
   }
...
{
   IUSBDriver *pme = &gUSBDriver;
   PARAM_COULD_BE_CONST(pSetupMsg);
...
static int USBDriver_Open(SubPort *pSubPort)
{
   ...
   if (pSubPort->nPort == SUBPORT_BULK) { // Bulk Port
      ...
      USBDriver_USBDCIoctl(SUBPORT_BULK, (int)AEEUSBDRIVER_IOCTL_NTFYCNXNSTATUS,
...
static int USBDriver_ScheduleWrite(SubPort *pSubPort, const char *pcBuf, int nLen )
{
      ...
      ((pSubPort->nCachedSize + nLen) < USBDRIVER_MIN_WRITE_SIZE))
   {
      ...
      pNewBuffer = REALLOC(pSubPort->pCachedData,
                           ALLOC_NO_ZMEM | ((unsigned int)(pSubPort->nCachedSize + nLen)));
   ...
   while((nLen > 0) || (bTerminate && (pSubPort->nCachedSize != 0)))
   {
      ...
      if(pSubPort->nCachedSize != 0)
      {
         ...
         {
            MEMCPY(item_ptr->data_ptr, pSubPort->pCachedData, (unsigned int)nWrote);
            FREE(pSubPort->pCachedData);
            pSubPort->pCachedData = NULL;
            pSubPort->nCachedSize = 0;
         }
         else
         {
            ...
            MEMCPY(item_ptr->data_ptr, pSubPort->pCachedData, (unsigned int)nWrote);
            pNewBuff = MALLOC((unsigned int)(pSubPort->nCachedSize - nWrote));
            if(pNewBuff == NULL)
            {
               pSubPort->nLastErr = ENOMEMORY;
               brewdsm_free_buffer(item_ptr);
            ...
            else
            {
               MEMCPY(pNewBuff, 
                      ...
                      (unsigned int)(pSubPort->nCachedSize - nWrote));
               FREE(pSubPort->pCachedData);
      ...
      if(!bTerminate && (nSize < (item_ptr->size - nWrote)))
      {
         ...
         if(pSubPort->nCachedSize != 0)
         {
            ...
            pSubPort->pCachedData = MALLOC((unsigned int)pSubPort->nCachedSize);
            if(NULL == pSubPort->pCachedData)
            {
            ...
            else
            {
               MEMCPY(pSubPort->pCachedData, 
                      ...
                      (unsigned int)pSubPort->nCachedSize);
            }
         }
      }
      MEMCPY(item_ptr->data_ptr + nWrote, pcBuf+nWritten, (unsigned int)nSize);
      item_ptr->used = (uint16) (nSize + nWrote);
                                      ...
                                      word *rx_data_size)
{
      ...
      {
         //This should always be TRUE as there is no interrupt OUT endpoint. 
...
static void USBDriver_USBDCFlowControl(boolean bEnable)
{
  ...
  #endif /* FEATURE_USBDRIVER_NO_USBDC_API */
#else
   //Flow control is not needed on HS-USB
   USE_PARAM(bEnable);
#endif /* FEATURE_USB */
}
...
#ifdef FEATURE_HSU_MTP
      mtpfd_pipe_type_t pipe_type;
      static boolean bShortPacketSent = FALSE;
      
      pipe_type = ((nPort == SUBPORT_BULK) ?
                  MTPFD_PIPE_BULK : MTPFD_PIPE_INTR);

      /* If we are sending NULL packet and previous write is short package,
         then don't need to send it down to the lower layer USB core, just drop it.
         512 is the current HS USB package size, use this number for now.
      */
      if ((NULL == pcBuf) && (0 == nLen) && bShortPacketSent)
      {
        /* Invoke the callback function even though we don't send this 
           packet down to USB core. 
         */
        USBDriver_USBDCTxCB(); 
        return;
      }
...
#else
   usbdc_api_write_data(usbdc_iface, (byte *)pcBuf, (uint16)nLen);
#endif
                                 ...
                                 uint32 dwParam)
{
   ...
   pipe = (nPort == SUBPORT_BULK) ? MTPFD_PIPE_BULK : MTPFD_PIPE_INTR;

   if(nIoctl == ((int)AEEUSBDRIVER_IOCTL_GETCNXNSTATUS))
   {
      if (dwParam)
      {
         hsu_al_brewext_ioctl(pipe, MTPFD_GET_CONNECT_STATUS, (void *)dwParam);
      }
   }
   else if(nIoctl == ((int)AEEUSBDRIVER_IOCTL_NTFYCNXNSTATUS))
   {
      hsu_al_brewext_ioctl(pipe, MTPFD_SET_CONNECT_STATUS_CB, (void *)dwParam);
   }
   else
   {
   if (nPort == SUBPORT_BULK)
...
{
#ifdef FEATURE_HSU_MTP
   (void)hsu_al_brewext_register_mtp_app_cb((mtpfd_mtp_app_cb)USBDriver_MTPAppCB);
#else
