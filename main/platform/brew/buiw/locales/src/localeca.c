...
// Format variables, strings, characters and patterns for Catalan
...
/* -----------------------------------------------------------------------
    LocaleCA_Ctor 
   --- */
static void LocaleCA_Ctor(LocaleCA *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleCA_New  
   --- */
int LocaleCA_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleCA *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleCA, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleCA_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule); 
...
int LocaleCA_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALECA) {
      return LocaleCA_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
