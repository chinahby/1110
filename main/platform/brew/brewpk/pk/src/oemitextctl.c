...
#ifdef FEATURE_FUNCS_THEME   //¸ù¾Ýfeature¿ØÖÆ£¬ÒÔÏÂËùÓÐÓÃµ½´ËfeatureµÄµØ·½ÓÃ#else´¦Àí   
    {
        // »ñÈ¡Ö÷Ìâ²ÎÊý
                                    ...
                                    AEEEvent eCode, uint16 wParam, uint32 dwParam )
{
   ...
   //if( eCode >= EVT_PEN_UP  && eCode <= EVT_PEN_STALE_MOVE ){   modi by ydc  090520
    ...
    switch(eCode)
    {
        ...
        case EVT_COMMAND:          // SoftKey ²Ëµ¥µÄÃüÁî
            if (!pme->m_pSoftKey)
            {
                return FALSE;
            }
...
if ((!pme->m_pSoftKey) && 
            ...
            if ( pme->m_bShowSyms )
            {
                switch (wParam)
                {
                    ...
                    case AVK_SELECT:
                    case AVK_INFO:
                        return TRUE;//ÔÚÊäÈë±êµã·ûºÅÊ±£¬ÕâÁ½¸ö°´¼üÎÞÐ§
            ...
            if ( pme->m_bShowFaceSyms )
            {
                switch (wParam)
                {
                    ...
                    case AVK_SELECT:
                    case AVK_INFO:
                        return TRUE;//Q1 ÒªÇóÔÚÊäÈë·ûºÅÊ± select ¸úinfo ¼üÎÞÐ§
...
#endif
					//Add By zzg 2010_08_14 ·ÀÖ¹ÖØ¸´ÔØÈëSYMBOLÊäÈë·¨				
...
static boolean CTextCtl_Redraw(ITextCtl * pITextCtl)
{
    ...
    if (!(pme->m_dwProps & TP_NODRAW)) 
    {
        ...
        if (pme->m_pTitle) 
        {
...
#endif //FEATURE_FUNCS_THEME
            IDISPLAY_SetColor(pme->m_pIDisplay, CLR_USER_TEXT, RGB_WHITE);//ÁÙÊ±¸Ä±äÎÄ±¾ÑÕÉ«  
            ...
            IDISPLAY_SetColor(pme->m_pIDisplay, CLR_USER_TEXT, RGB_BLACK);//»Ö¸´ÎÄ±¾ÏÔÊ¾ÑÕÉ«            
            if ((!pme->m_pSoftKey)&&(pme->m_dwProps&TP_STARKEY_SWITCH))
            {
...
static AEETextInputMode CTextCtl_SetInputMode(ITextCtl * po, AEETextInputMode m)
{
    ...
    if (tmSetMode != OEM_TextGetCurrentMode(pme->m_pText))
    {
        ...
        switch(tmSetMode)
        {
            case TEXT_MODE_MULTITAP://×ÖÄ¸ÊäÈëÄ£Ê½
...
#if 0    
    //Draw title  //ÕâÀï½«Í³Ò»µÄtitlebar»­ÉÏÈ¥
    {
        MEMSET(&TitleBar, 0, sizeof(TitleBar_Param_type));
...
#endif
#if 0 //È¥µôÔ­À´Ìî³äÑÕÉ«µÄtitlebar
...
//   AEERect        bottomrect;
   ...
   ISHELL_GetDeviceInfo(pme->m_pIShell, &dm);
...
#endif
#if 0  //È¥µôÔ­À´Ìî³äÑÕÉ«µÄ»­titlebarµÄ·½Ê½  
    SETAEERECT(&titlerect, 0, 0, dm.cxScreen, pme->m_nFontHeight);   
#ifdef FEATURE_FUNCS_THEME     
    IDISPLAY_FillRect(pme->m_pIDisplay, &titlerect,  pme->m_themeColor);
#endif //FEATURE_FUNCS_THEME
    ...
    //Draw bottom bar Ê¹ÓÃÍ³Ò»µÄµ×Ìõ
    {
        BottomBar_Param_type BottomBar;
...
static void TextCtl_ShowSymbolDialog(CTextCtl * pme)
{
   ...
   SetArrowFlagonIM(TRUE);//Ôö¼ÓÈí¼üÀ¸·­Ò³Í¼±ê
...
ËµÃ÷£º
       ÎÄ±¾¿Ø¼þÊäÈë·¨ÉèÖÃº¯Êý¡£
       ÓÃÓÚ²»Í¬ÓïÑÔ°æ±¾µÄÊäÈë·¨ÇÐ»»

²ÎÊý£º
       pme [in]£ºCTextCtl t¶ÔÏóÖ¸Õë¡£
...
static void OEM_SetInputMode(CTextCtl * pme)
{
    ...
    {   //  Ö»ÓÐÔÚÉèÖÃÁË TP_STARKEY_SWITCH ÊôÐÔÇÒÃ»Ê¹ÓÃÈí¼üµÄÌõ¼þÏÂÇÐ»»ÊäÈë·¨
        return;
    }
