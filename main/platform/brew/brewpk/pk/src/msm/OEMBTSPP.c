                  ...
                  boolean bServer)
{
   ...
   if ( bServer )
   {
      ...
      len = ( STRLEN((char*)pszServiceName) <= BT_SD_MAX_SERVICE_NAME_LEN ) ?
            STRLEN((char*)pszServiceName) : BT_SD_MAX_SERVICE_NAME_LEN;

      STRNCPY( (char*)(pMe->role.serv.svcName), (const char*)(pszServiceName), len );
   }
   else
   {
      pMe->bClientApp = TRUE;
...
static void OEMBTSPP_InitTxRxBuffer( OEMBTSPP* pMe )
{
   ...
   pMe->rxWaterMark.q_ptr                = &pMe->rxQueue;
...
static void OEMBTSPP_DevNameResp( OEMBTSPP* pMe, bt_ev_msg_type* bt_ev_ptr )
{
   ...
   {
      int len = STRLEN( (char *)pEvt->device_name_str );
      ...
      {
         STRNCPY( (char*)pDevInfo->bdi.szName,(char*) pEvt->device_name_str, len );
         // just to avoid any problems in case AEEBT_MAX_LEN_DEVICE_NAME is
         // lesser than the BT string len 
         pDevInfo->bdi.szName[len] = 0;
      }
   }
