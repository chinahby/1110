...
/*============================================================================
 ...
 $Header: //depot/asic/msmshared/apps/brew_3.1.5/pk/src/msm/OEMCamera.c#59 $ $DateTime: 2007/06/01 11:08:01 $ $Author: jdas $
...
//            nRet = EBADPARM;
     ...
     case CAM_PARM_TRANSITION:
      ...
      case CAM_PARM_BESTSHOT:
        eParm = CAMERA_PARM_BESTSHOT_MODE;
        break;
#endif //FEATURE_CAMERA_BESTSHOT_MODE

#ifdef FEATURE_CAMERA_BURST_MODE
      case CAM_PARM_BRACKETING:
#ifdef FEATURE_CAMERA7500
        if (p1 < (int32)CAMERA_BRACKETING_MAX)
          pme->m_nBracketingMode = p1;
        else
          nRet = EBADPARM;
#else
        nRet = EUNSUPPORTED;
      ...
      case CAM_PARM_ENCODESNAPSHOT_EX:
        if (p1 < pme->m_multishotInfo.nCount)
        {
          pme->m_nShotIndex = p1;
          nRet = OEMCamera_EncodeSnapshot(pme);
        }
        else
          nRet = EBADPARM;
        break;
...
int OEMCamera_GetParm(OEMINSTANCE h, int16 nParmID, int32 * pP1, int32 * pP2)
{
   ...
   switch (nParmID)
   {
     ...
     case CAM_PARM_FRAME_TIMESTAMP:
      ...
      case CAM_PARM_FRAME_EX:
        if (pP2)
        {
          int nIndex = (int)pP1;
          if (nIndex <= pme->m_nCurrIndex)
          {
            camera_frame_type camFrame;
            nRet = (int)camera_get_picture((uint32)nIndex, &camFrame);
            if (nRet == SUCCESS)
            {
              if (OEMCamera_SaveFrameInfo(pme, &camFrame))
              {
                IBITMAP_AddRef((IBitmap *)pme->m_pFrame);
                *pP2 = (int32)pme->m_pFrame;
              }
              else
                nRet = EFAILED;
            }
      ...
      case CAM_PARM_PIXEL_COUNT:  // Fall through...
      case CAM_PARM_VIDEO_ENCODE_LIST:  // will be supported in BREW 3.1.6
      case CAM_PARM_AUDIO_ENCODE_LIST:  // will be supported in BREW 3.1.6
      default:
         nRet = EUNSUPPORTED;
         break;
   }
   ...
   if ((pP1 || pP2) && eParm != CAMERA_PARM_MAX && SUCCESS == nRet)
   {
      ...
      if (!nCamRet)
      {
         ...
         case CAM_PARM_ISO:
            ...
            if (OEMCamera_ConverISO(&pi.nCurrent, FALSE) == SUCCESS)
            {
               if (pP1)
                  *pP1 = pi.nCurrent;

               if (pP2 && (OEMCamera_ConverISO(&pi.nMax, FALSE) == SUCCESS))
               {
                  MEMCPY(pP2, &pi, sizeof(AEEParmInfo));
               }
             }
...
int OEMCamera_EncodeSnapshot(OEMINSTANCE hInstance)
{
   ...
   {
      nRet = camera_encode_picture_ext((uint32)pme->m_nShotIndex, &h,
                                       OEMCamera_CameraLayerCB,
                                       (void *)pRsp->hObject);
...
static int OEMCamera_RecordSnapshot(OEMCamera * pme, CameraRsp * pRsp)
{
   ...
   if (pme->m_multishotInfo.nCount > 1)
   {
      nRet = (int)camera_take_multiple_pictures(OEMCamera_CameraLayerCB,
                                                (void *)pRsp->hObject,
                                                (uint32)pme->m_multishotInfo.nCount,
                                                (camera_bracketing_mode_type)pme->m_nBracketingMode,
                                                pme->m_multishotInfo.dwInterval);
      return OEMCamera_AEEError(nRet);
   }
#endif //FEATURE_CAMERA_BURST_MODE
...
#ifdef FEATURE_CAMERA_BURST_MODE
      case CAMERA_FUNC_TAKE_MULTIPLE_PICTURES:
      {
        switch (cb)
        {
           case CAMERA_RSP_CB_SUCCESS:
           ...
           case CAMERA_EXIT_CB_DONE:
             ...
             dwData = TRUE;
             pme->m_nCurrIndex = -1;
             if (pme->m_multishotInfo.nCount != parm4)
             {
               nStatus = CAM_STATUS_FAIL;
               dwData = CAM_ESNAPSHOTFAIL;
             }
           ...
           case CAMERA_EVT_CB_PICTURE:
             pme->m_nCurrIndex++;
             nStatus = CAM_STATUS_FRAME;
             dwData = pme->m_nCurrIndex;
             break;
...
CBExit:
   ...
   if (pme->m_bDirectMode && (cb == CAMERA_EVT_CB_FRAME))
     return;
#endif //FEATURE_CAMERA_LCD_DIRECT_MODE
