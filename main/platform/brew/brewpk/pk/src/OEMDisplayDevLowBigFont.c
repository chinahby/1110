...
extern int OEMFont_NewFromBMP(IFont **ppif,
...
#if 1
static IBitmap *gpDevBitmap2 = NULL;
static IBitmap *gpDevBitmap3 = NULL;
static IBitmap *gpDevBitmap4 = NULL;
uint8 current_bitmap= 0;
#endif
...
//Oct262009 tyj. indicate wheather MMOVERLAY is enabled.
boolean g_bOEMDisplayDevMMOverlayIsEnabled = FALSE;

#if defined(FEATURE_QTM10) || defined(FEATURE_QTM11N) || defined(FEATURE_QTM13) || defined(FEATURE_QTM16)//Nov072009FEATURE_CARRIER_INDONESIA_SMART
#ifndef FEATURE_CARRIER_INDONESIA_CERIA
#undef FEATURE_MMOVERLAY
#endif
#endif /* FEATURE_CARRIER_INDONESIA_SMART */
...
//zgm test
   ...
   if (gpDevBitmap3) {
      IBITMAP_Release(gpDevBitmap3);
      gpDevBitmap3 = NULL;
   }
      if (gpDevBitmap4) {
      IBITMAP_Release(gpDevBitmap4);
      gpDevBitmap4 = NULL;
   }   
#endif
#ifdef FEATURE_UIONE_HDK
...
//zgm test start   
	&&(AEECLSID_DISPLAYDEV3 != cls)&&(AEECLSID_DISPLAYDEV4 != cls)
//zgmtest end
...
#if 1
       && (AEECLSID_DEVBITMAP2 != cls)
       && (AEECLSID_DEVBITMAP3 != cls)
       && (AEECLSID_DEVBITMAP4 != cls)
#endif
#ifdef FEATURE_UIONE_HDK
...
//zgm test
      ...
      case AEECLSID_DEVBITMAP2:
         ...
         IBITMAP_AddRef((IBitmap*)*ppif);
         break;
      case AEECLSID_DEVBITMAP3:
         if (!gpDevBitmap3) {
            nErr = AEE_CreateInstanceSys(AEECLSID_DISPLAYDEV3,
                                         (void**)&pDispDev);
            if (SUCCESS != nErr) {
               return nErr;
            }
         ...
         IBITMAP_AddRef((IBitmap*)*ppif);
         break;		 
	case AEECLSID_DEVBITMAP4:
         if (!gpDevBitmap4) {
            nErr = AEE_CreateInstanceSys(AEECLSID_DISPLAYDEV4,
                                         (void**)&pDispDev);
            if (SUCCESS != nErr) {
               return nErr;
            }
...
#if 1
        && (AEECLSID_DEVBITMAP2_CHILD != cls)
        && (AEECLSID_DEVBITMAP3_CHILD != cls)
        && (AEECLSID_DEVBITMAP4_CHILD != cls)
#endif
       ) {
      return ECLASSNOTSUPPORT;
   }
...
//zgm test
      ...
      case AEECLSID_DEVBITMAP2_CHILD:
         ...
         break;
	case AEECLSID_DEVBITMAP3_CHILD:
         pac = AEE_EnterAppContext(NULL);
         nErr = AEE_CreateInstanceSys(AEECLSID_DEVBITMAP3, (void**)&pDevBitmap);
         AEE_LeaveAppContext(pac);
         if (SUCCESS != nErr) {
            return nErr;
         }
         ...
         break;
      case AEECLSID_DEVBITMAP4_CHILD:
         pac = AEE_EnterAppContext(NULL);
         nErr = AEE_CreateInstanceSys(AEECLSID_DEVBITMAP4, (void**)&pDevBitmap);
         AEE_LeaveAppContext(pac);
         if (SUCCESS != nErr) {
            return nErr;
         }
...
{
#ifdef FONT_THIN_14
   extern boolean gEisBrewAppActive;
   
	if(gEisBrewAppActive && AEECLSID_FONTSYSNORMAL == cls)
   		cls = AEECLSID_FONT12X12;
#endif    
   ...
   case AEECLSID_FONTSYSBOLD:
   case AEECLSID_FONTSYSLARGE:    
      return OEMFont_NewFromBMP((IFont **)ppif, NULL, FONTCHAR_COUNT, fontbitmap, sizeof(fontbitmap), 
                                FONTCHAR_WIDTH, FONTCHAR_HEIGHT, 0, 0, HALF_WIDTH_UNICODE, cls);
#endif
   default:
      return ECLASSNOTSUPPORT;
...
static int OEMDisplayDev_Update(IDisplayDev *pMe, IBitmap *pbmSrc, AEERect *prc)
{
   ...
   if((0 != pMe->displayClsID) && (NULL != pMe->pSysMMOverlayRegistry)
        && g_bOEMDisplayDevMMOverlayIsEnabled)
   {
      boolean bRegistered;
      if(0 == pMe->mdpOvID)
      {
         ...
         pMe->mdpImg.mdpOp = MDPOP_VSYNC_OFF;
