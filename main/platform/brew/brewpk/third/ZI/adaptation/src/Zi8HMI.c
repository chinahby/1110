...
static ZI8UCHAR IMEScratchBuffer[1536];    //chester occupy 1.5k ram can be removed with share
#endif
...
#ifdef __POPO_SUPPORT__
extern BOOL popo_is_running(void);
extern UI_string_type popo_inline_full_screen_edit_saved_LSK_label;
#endif
/*****************************************************************************
 * FUNCTION
 *  ZiUpdateLSKString
...
void ZiUpdateLSKString(void)
{
	...
	if (popo_is_running())
	{
		temp_left_softkey_text = popo_inline_full_screen_edit_saved_LSK_label;
	}
	else
#endif
    ...
    switch (keyID)
    {
        ...
        case KEYUP:
            ...
            if (IMEbPhoneticComfirmed || !IMEbValidPhonetic)
            {
                ...
                /* move focus to the first candidate when no more previous page */
                IMEcandIndex = 0;
                goto IME_EXIT;
            }
...
/* under construction !*/
    ...
    {
        PRINT_ERROR(("<<** ZI ZI ZI ZI ZI ZI Successfully **>>\n"));

    }
    else
    {
        PRINT_ERROR(("<<** ZI ZI ZI ZI ZI ZI ZI failed **>>\n"));
    }
...
/* under construction !*/
        ...
        if ((ziInputMethodCategoryFlag & ZI_IN_CATEGORY_5_FLAG))
        {
            ...
            #else /* __MMI_UI_STYLE_14x14__ */ 
#if defined(__MMI_LOW_MEMORY_PHONE__)
                h = ziMultilineEditorHeight; //- MMI_multitap_height - 3; Ritesh
#else
...
ZI8UCHAR ZiMaxLengthCheck(void)
{
        ...
        (ziInputMethodCategoryFlag & ZI_IN_CATEGORY_INLINE_MULTILINE_INPUTBOX_FLAG))
    {
        ...
        if (MMI_current_input_ext_type & INPUT_TYPE_EXT_USE_0x81_ENCODING)
        {
            ...
            if (temp_0x81 != 0)
            {
                if ((MMI_multiline_inputbox.text_length >> 1) - 1 >= MMI_multiline_inputbox.available_length - 3)
                {
                    return ZI8FALSE;
                }
                else
