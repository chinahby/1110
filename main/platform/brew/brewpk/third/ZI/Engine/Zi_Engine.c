...
struct _CIMMgr{
   ...
   ZI8WCHAR             elementBuffer[SIZE_OF_ELEMENT_BUF];
...
#ifdef FEATURE_ZICORP_PINYIN // {
static ZI8WCHAR           IMMgr_PYNMapKeytoElement(AVKType key);
#endif // } FEATURE_ZICORP_SPINYIN
#ifdef FEATURE_ZICORP_STROKE // {
static ZI8WCHAR           IMMgr_STKMapKeytoElement(AVKType key);
#endif // } FEATURE_ZICORP_STROKE
#endif // } FEATURE_ZICORP_CHINESE || FEATURE_ZICORP_EZITEXT
#ifdef FEATURE_ZICORP_EZITEXT // {
static ZI8WCHAR           IMMgr_EZIMapKeytoElement(AVKType key);
#endif // } FEATURE_ZICORP_EZITEXT
//add by yangdecai 2011-07-11
static boolean          IMMgr_Zi8Init(IIMMgr *po);
...
#endif // } FEATURE_ZICORP_CHINESE || FEATURE_ZICORP_EZITEXT
	MSG_FATAL("OEMIMMgr_HandleEvent.....pme->bInit=%d,pme->tmCurrent=%d",pme->bInit,pme->tmCurrent,0);
   if( !pme->bInit ){
      return FALSE;
   }
   ...
   if( eCode == EVT_KEY && ((wParam >= AVK_0 && wParam <= AVK_9) || wParam == AVK_CLR) ){
      ...
      case AEE_TM_EZTEXT:
#endif // } FEATURE_ZICORP_EZITEXT
         {
            ...
            if( wParam == AVK_CLR && pme->nelementcount<= 1 )
			{
               if( pme->nelementcount )
			   {
                  pme->elementBuffer[0] = 0;
				  pme->nelementcount = 0;
                  return TRUE;
               }
			   else
			...
			MSG_FATAL("pme->tmCurrent===========%d",pme->tmCurrent,0,0);
			 if( wParam == AVK_CLR )
			 {
               ...
               pme->elementBuffer[pme->nelementcount-1]= 0;
			   pme->nelementcount--;
			 }
			 else
			 {
			 	 if( (pme->nelementcount+1) < SIZE_OF_ELEMENT_BUF)
				 {
				 	// We start with 0, they start with 1, so special case it.
                  if( pme->tmCurrent == AEE_TM_PINYIN )
				  {
#ifdef FEATURE_ZICORP_PINYIN 
                     pme->elementBuffer[pme->nelementcount] = IMMgr_PYNMapKeytoElement((AVKType)wParam);
					 pme->nelementcount++;
					 MSG_FATAL("pziGP->pElements[0]====%x",pme->elementBuffer[pme->nelementcount-1],0,0);
#endif
                  }else if( pme->tmCurrent == AEE_TM_STROKE )
                  {
#ifdef FEATURE_ZICORP_STROKE 
                     pme->elementBuffer[pme->nelementcount] = IMMgr_STKMapKeytoElement((AVKType)wParam);
					 pme->nelementcount++;
#endif 
                  }else if( pme->tmCurrent == AEE_TM_EZTEXT )
                  {
#ifdef FEATURE_ZICORP_EZITEXT 
                     pme->elementBuffer[pme->nelementcount] = IMMgr_EZIMapKeytoElement((AVKType)wParam);
					 pme->nelementcount++;
#endif 
                  }
                  if( !pme->elementBuffer[pme->nelementcount-1]){
                     return FALSE;
                  }
			 	 }
			 }
			MSG_FATAL("pme->nelementcount======%d",pme->nelementcount,0,0);
			for(i=0;i<pme->nelementcount;i++)
			{
				MSG_FATAL("pme->elementBuffer[%d]=%x",i,pme->elementBuffer[i],0);
			}
			
			MSG_FATAL("GetMode=%d,GetOptions=%d,MaxCandidates=%d",pziGP->GetMode,pziGP->GetOptions,pziGP->MaxCandidates);
			pziGP->pElements = pme->elementBuffer;
			pziGP->ElementCount = pme->nelementcount;
            nMatches = Zi8GetCandidates(pziGP);
			for(i=0;i<pme->nelementcount;i++)
			{
				MSG_FATAL("111->elementBuffer[%d]=%x",i,pme->elementBuffer[i],0);
				MSG_FATAL("111->pElements[%d]=%x",i,pziGP->pElements[i],0);
			}
			MSG_FATAL("pziGP->count==============%d=%d",pziGP->count,pziGP->pCandidates[0],0);
...
static void OEMIMMgr_Reset(IIMMgr * po)
{
   ...
   //add by yangdecai 2011-07-11
   MEMSET((void *)pme->elementBuffer,0,sizeof(pme->elementBuffer));
   pme->nelementcount = 0;
...
static int OEMIMMgr_Initialise(IIMMgr * po)
{
   ...
   if(IMMgr_Zi8Init(po))
   {
       MSG_FATAL("mmi_ime_word_init is success!!!!!!!!!!!!!!!",0,0,0);
   }
#if defined(FEATURE_ZICORP_CHINESE) || defined(FEATURE_ZICORP_EZITEXT) // {
   //if( !Zi8Initialize() ){
   //   return EFAILED;
...
static boolean IMMgr_Zi8Init(IIMMgr * po)
{
	...
	{
		pme->bzi8init = /*Zi8Initialize();*/Zi8InitializeDynamic((PZI8VOID)gIMELDBArray); /* Initialize LDB table */
		MSG_FATAL("g_engine.is_inited========%d",pme->bzi8init,0,0);
	}
