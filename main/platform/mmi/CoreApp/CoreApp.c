...
void CoreApp_FreeAppData(IApplet* po)
{
	...
	if(pMe->m_Idle_Icon)
	{
	if(pMe->m_dial_Image !=NULL)
	{
		IIMAGE_Release(pMe->m_dial_Image);
		pMe->m_dial_Image = NULL;
	}
	if(pMe->m_time_Image !=NULL)
	{
		IIMAGE_Release(pMe->m_time_Image);
		pMe->m_time_Image = NULL;
	}
	if(pMe->m_Idle_Icon->m_iconback_Image !=NULL)
	{
		IIMAGE_Release(pMe->m_Idle_Icon->m_iconback_Image);
		pMe->m_Idle_Icon->m_iconback_Image = NULL;
	}
	if(pMe->m_Idle_Icon->m_arrow_uncheck !=NULL)
	{
		IIMAGE_Release(pMe->m_Idle_Icon->m_arrow_uncheck);
		pMe->m_Idle_Icon->m_arrow_uncheck = NULL;
	}
	if(pMe->m_Idle_Icon->m_arrow_check !=NULL)
	{
		IIMAGE_Release(pMe->m_Idle_Icon->m_arrow_check);
		pMe->m_Idle_Icon->m_arrow_check = NULL;
	}
	if(pMe->m_Idle_Icon->m_arrowleft_Image !=NULL)
	{
		IIMAGE_Release(pMe->m_Idle_Icon->m_arrowleft_Image);
		pMe->m_Idle_Icon->m_arrowleft_Image = NULL;
	}
	if(pMe->m_Idle_Icon->m_arrowright_Image !=NULL)
	{
		IIMAGE_Release(pMe->m_Idle_Icon->m_arrowright_Image);
		pMe->m_Idle_Icon->m_arrowright_Image = NULL;
	}
	for(i = 0;i<11;i++)
	{
		if(pMe->m_Idle_Icon->m_IconList[i]->m_ImageBig !=NULL)
		{
			IIMAGE_Release(pMe->m_Idle_Icon->m_IconList[i]->m_ImageBig);
			pMe->m_Idle_Icon->m_IconList[i]->m_ImageBig = NULL;
		}
	}
	for(i = 0;i<11;i++)
	{
		if(pMe->m_Idle_Icon->m_IconList[i]->m_ImageSmall !=NULL)
		{
			IIMAGE_Release(pMe->m_Idle_Icon->m_IconList[i]->m_ImageSmall);
			pMe->m_Idle_Icon->m_IconList[i]->m_ImageSmall = NULL;
		}
	}
	for(i = 0;i<11;i++)
	{
		if(pMe->m_Idle_Icon->m_IconList[i]->m_ImageSmallCheck !=NULL)
		{
			IIMAGE_Release(pMe->m_Idle_Icon->m_IconList[i]->m_ImageSmallCheck);
			pMe->m_Idle_Icon->m_IconList[i]->m_ImageSmallCheck = NULL;
		}
	}
...
//wlh 20090427 add start 时间表盘
	pMe->m_b_dialflag = FALSE;//wlh 20090427 add 时间表盘
	//pMe->m_b_dialmove = FALSE;//ydc 20090523 add 是否移动
	/*
	pMe->m_dial_rc.x = 0;//wlh 20090427 add 时间表盘
	pMe->m_dial_rc.y = 14;//wlh 20090427 add 时间表盘
...
//wlh 20090427 add end 时间表盘
	...
	if(pMe->m_Idle_Icon)
	{
		MEMSET(pMe->m_Idle_Icon, 0, sizeof(IdleIcon));

		for(i = 0; i < 11;i++)
			pMe->m_Idle_Icon->m_IconList[i] = MALLOC(sizeof(xIcon_AppList *));

		pMe->m_Idle_Icon->m_iconback_Image = ISHELL_LoadResImage( AEE_GetShell(),
                                                                  AEE_APPSCOMMONRES_IMAGESFILE,
                                                                  IDI_TIMEBACK
                                                                   );
		pMe->m_Idle_Icon->m_IconList[0]->xIconAppNum = 0;
		pMe->m_Idle_Icon->m_fisticon = 1;
		pMe->m_Idle_Icon->m_checkicon = 1;
		pMe->m_Idle_Icon->m_icon_rc.x = 162;//刚开始只显示箭头
		pMe->m_Idle_Icon->m_icon_rc.y = 14;
		pMe->m_Idle_Icon->m_icon_rc.dx = 176;
		pMe->m_Idle_Icon->m_icon_rc.dy = 34;

		pMe->m_Idle_Icon->m_b_leftflag = FALSE;
		pMe->m_Idle_Icon->m_b_rightflag = TRUE;
		pMe->m_Idle_Icon->m_b_show = FALSE;
		pMe->m_now_Num = 0;

		pMe->m_Idle_Icon->m_arrow_uncheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_ARROW_UNCHECK
																	);
		pMe->m_Idle_Icon->m_arrow_check =   ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_ARROW_CHECK
																	);

		pMe->m_Idle_Icon->m_arrowleft_Image = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_ARROW_LEFT
																	);
		pMe->m_Idle_Icon->m_arrowright_Image = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_ARROW_RIGHT
																	);
		//blue															
		pMe->m_Idle_Icon->m_IconList[0]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_BLUE_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[0]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_BLUE_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[0]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_BLUE_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[0]->m_rc.dx = 63;
		pMe->m_Idle_Icon->m_IconList[0]->m_rc.dy = 65;
		pMe->m_Idle_Icon->m_IconList[0]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[0]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[0]->xIconAppNum = 0;
		//browser										
		
		pMe->m_Idle_Icon->m_IconList[1]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_BROWSER_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[1]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_BROWSER_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[1]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_BROWSER_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[1]->m_rc.dx = 58;
		pMe->m_Idle_Icon->m_IconList[1]->m_rc.dy = 54;
		pMe->m_Idle_Icon->m_IconList[1]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[1]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[1]->xIconAppNum = 0;
		//calc															
		pMe->m_Idle_Icon->m_IconList[2]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_CALC_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[2]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_CALC_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[2]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_CALC_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[2]->m_rc.dx = 46;
		pMe->m_Idle_Icon->m_IconList[2]->m_rc.dy = 56;
		pMe->m_Idle_Icon->m_IconList[2]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[2]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[2]->xIconAppNum = 0;
		//date															);
		pMe->m_Idle_Icon->m_IconList[3]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_DATE_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[3]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_DATE_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[3]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_DATE_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[3]->m_rc.dx = 64;
		pMe->m_Idle_Icon->m_IconList[3]->m_rc.dy = 63;
		pMe->m_Idle_Icon->m_IconList[3]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[3]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[3]->xIconAppNum = 0;
		//file															);
		pMe->m_Idle_Icon->m_IconList[4]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_FILE_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[4]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_FILE_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[4]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_FILE_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[4]->m_rc.dx = 60;
		pMe->m_Idle_Icon->m_IconList[4]->m_rc.dy = 60;
		pMe->m_Idle_Icon->m_IconList[4]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[4]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[4]->xIconAppNum = 0;
		//health															);
		pMe->m_Idle_Icon->m_IconList[5]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_HEALTH_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[5]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_HEALTH_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[5]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_HEALTH_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[5]->m_rc.dx = 60;
		pMe->m_Idle_Icon->m_IconList[5]->m_rc.dy = 54;
		pMe->m_Idle_Icon->m_IconList[5]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[5]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[5]->xIconAppNum = 0;
		//mediaplay															);
		pMe->m_Idle_Icon->m_IconList[6]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_MEDIAPLAY_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[6]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_MEDIAPLAY_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[6]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_MEDIAPLAY_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[6]->m_rc.dx = 47;
		pMe->m_Idle_Icon->m_IconList[6]->m_rc.dy = 53;
		pMe->m_Idle_Icon->m_IconList[6]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[6]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[6]->xIconAppNum = 0;
		//music															);
		pMe->m_Idle_Icon->m_IconList[7]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_MUSIC_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[7]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_MUSIC_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[7]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_MUSIC_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[7]->m_rc.dx = 61;
		pMe->m_Idle_Icon->m_IconList[7]->m_rc.dy = 58;
		pMe->m_Idle_Icon->m_IconList[7]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[7]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[7]->xIconAppNum = 0;
		//sms															);
		pMe->m_Idle_Icon->m_IconList[8]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_SMS_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[8]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_SMS_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[8]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_SMS_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[8]->m_rc.dx = 59;
		pMe->m_Idle_Icon->m_IconList[8]->m_rc.dy = 52;
		pMe->m_Idle_Icon->m_IconList[8]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[8]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[8]->xIconAppNum = 0;
		//sports															);
		pMe->m_Idle_Icon->m_IconList[9]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_SPORTS_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[9]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_SPORTS_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[9]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_SPORTS_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[9]->m_rc.dx = 58;
		pMe->m_Idle_Icon->m_IconList[9]->m_rc.dy = 57;
		pMe->m_Idle_Icon->m_IconList[9]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[9]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[9]->xIconAppNum = 0;
		//usb
		pMe->m_Idle_Icon->m_IconList[10]->m_ImageBig = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_USB_BIG
																	);
		pMe->m_Idle_Icon->m_IconList[10]->m_ImageSmall = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_USB_SMALL
																	);
		pMe->m_Idle_Icon->m_IconList[10]->m_ImageSmallCheck = ISHELL_LoadResImage(  AEE_GetShell(),
																	AEE_APPSCOMMONRES_IMAGESFILE,
																	IDI_USB_SMALL_CHECK
																	);
		pMe->m_Idle_Icon->m_IconList[10]->m_rc.dx = 64;
		pMe->m_Idle_Icon->m_IconList[10]->m_rc.dy = 63;
		pMe->m_Idle_Icon->m_IconList[10]->m_rc.x = 0;
		pMe->m_Idle_Icon->m_IconList[10]->m_rc.y = 0;
		pMe->m_Idle_Icon->m_IconList[10]->xIconAppNum = 0;
...
static void CoreAppLoadTimepImage(CCoreApp *pMe)
{
	...
	pMe->m_dial_minue_image[58] = ISHELL_LoadResImage(AEE_GetShell(),AEE_APPSCOMMONRES_IMAGESFILE,IDI_MINUTE_348);
