...
==============================================================================*/
#ifdef FEATURE_CONTACT_APP
// ÓÃÓÚ´ÓÍ¨ÐÅ²¾È¡ºÅÂëµÄ»Øµ÷º¯Êý
static boolean GetNumCallback(sSelectFieldListNode* pNode);

// ÓÃÓÚ´ÓÍ¨ÐÅ²¾È¡ÁªÏµÈËÐÅÏ¢²¢²åÈëµ½µ±Ç°±à¼­µÄÏûÏ¢ÖÐµÄ»Øµ÷º¯Êý
static boolean InsertNumCallback(sSelectFieldListNode* pNode);
#endif

// ÓÃÓÚ´ÓÍ¨»°ÀúÊ·¼ÇÂ¼È¡ÁªÏµÈËÐÅÏ¢²¢²åÈëµ½µ±Ç°±à¼­µÄÏûÏ¢ÖÐµÄ»Øµ÷º¯Êý
...
ËµÃ÷: 
    º¯ÊýÓÃÀ´½«¶ÌÐÅÊ±¼ä´Á¸ñÊ½×ª»»ÎªÃë¡£
       
²ÎÊý: 
    pTime [in]: wms_timestamp_s_type ½á¹¹Êý¾ÝÖ¸Õë
...
ËµÃ÷: 
    ½«ÒÔÃë±íÊ¾µÄÊ±¼ä×ª»»Îª×Ö·û´®ÐÎÊ½¡£

²ÎÊý: 
    pMe [in]: Ö¸ÏòWMS Applet¶ÔÏó½á¹¹µÄÖ¸Õë¡£¸Ã½á¹¹°üº¬Ð¡³ÌÐòµÄÌØ¶¨ÐÅÏ¢¡£
    dwTime [in]: ÒÔ¼ÆÃëµÄÊ±¼ä¡£
    pBuf [out]: ÓÃÓÚ´æ·ÅÊ±¼ä×Ö·û´®µÄ»º³åÇø¡£
    nSize [in]: pBuf µÄ¿Õ¼ä´óÐ¡(µ¥Î»:×Ö½Ú)¡£
...
²ÎÊý: 
    pWstr [in]: Ö¸Ïò¿í×Ö·û´®¡£
    nPostion [out]: ·ÇASCII×Ö·ûµÚÒ»´Î³öÏÖµÄÎ»ÖÃ¡£

·µ»ØÖµ: 
    Èôº¬ÓÐ·ÇASCII×Ö·û£¬·µ»ØTRUE£»·ñÔò£¬FALSE¡£
...
boolean WMSUtil_HaveNoneASCIIChar ( AECHAR * pWstr, int * pPostion)
{
    ...
    // ¶ÔÓÚÒÔÉ«ÁÐÔËÓªÉÌ¹Ì¶¨Ê¹ÓÃ WMS_ENCODING_LATIN_HEBREW ±àÂë
    return FALSE;
#else
    int i ,len;
...
ËµÃ÷: 
    ÅÐ¶Ï·¢ËÍ¸ø¶¨×Ö·û´®ÐèÒªµÄ¶ÌÐÅÌõÊý¡£

²ÎÊý: 
    pWstr [in]: Ö¸Ïò¿í×Ö·û´®¡£
    pencoding [in/out]: ÓÃÓÚ·µ»Øµ±Ç°Ê¹ÓÃµÄ±àÂë·½Ê½¡£
    pnMaxcharsOneitem [in/out]: ÓÃÓÚ·µ»Øµ¥Ìõ¶ÌÐÅ×î¶àÈÝÄÉµÄ×Ö·ûÊýµÄÕûÊýÖ¸Õë¡£

·µ»ØÖµ: 
    ·¢ËÍ¸ø¶¨×Ö·û´®ÐèÒªµÄ¶ÌÐÅÌõÊý¡£
...
{
    int nItems = 1;// ½«ÏûÏ¢»®·ÖºóµÃµ½µÄ×ÜÏîÊý
    int nMaxItemChars = 0;     // Ã¿Ïî×î´ó×Ö·ûÊý
    int nLen = 0;
...
ËµÃ÷: 
    ¸ù¾Ýµç»°ºÅÂë´Óµç»°±¾ÖÐ²éÑ¯ÈËÃû¡£

²ÎÊý: 
    pMe [in]: Ö¸ÏòWMS Applet¶ÔÏó½á¹¹µÄÖ¸Õë¡£¸Ã½á¹¹°üº¬Ð¡³ÌÐòµÄÌØ¶¨ÐÅÏ¢¡£
    pwszNum [in]: ºÅÂë×Ö·û´®¡£
    pwszName [in/out]: ÓÃÓÚ·µ»ØÈËÃûµÄ×Ö·û´®Buf¡£
    wBufLen [in]: pwszName ×î¶àÈÝÄÉµÄ×Ö·ûÊý(AECHAR)
...
ËµÃ÷: 
    ½«°´¼üÊäÈë´ú±ÊµÄ×Ö·û×ª»»³ÉÏàÓ¦µÄ¿í×Ö·û¡£
...
ËµÃ÷:
    º¯ÊýÖØÐÂÉèÖÃÏûÏ¢½á¹¹¸÷³ÉÔ±±äÁ¿ÎªÈ±Ê¡Öµ¡£

²ÎÊý:
    pMsg [in/out]: WMSMessageStruct Êý¾Ý½á¹¹Ö¸Õë¡£
...
²ÎÊý:
    pMsg [in/out]: WMSMessageStruct_Free Êý¾Ý½á¹¹Ö¸Õë¡£
...
ËµÃ÷:
    º¯Êý½« wms_client_message_s_type ÏûÏ¢½âÎöµ½ÄÚ²¿½á¹¹ WMSMessageStruct ¡£

²ÎÊý:
    pClMsg [in]: wms_client_message_s_type Êý¾Ý½á¹¹Ö¸Õë¡£
    pOutMs [in/out]: WMSMessageStruct Êý¾Ý½á¹¹Ö¸Õë¡£
...
{
    wms_client_ts_data_s_type   *cl_data = NULL;    // Ðè·ÖÅäÄÚ´æ
    wms_client_bd_s_type        *info_data = NULL;  // Ðè·ÖÅäÄÚ´æ
    uint8                       *buf = NULL;        // Ðè·ÖÅäÄÚ´æ
    ...
    {// CDMA ÏûÏ¢
        ...
        // ÏÈ¼ì²éÏûÏ¢ÓÐÎÞµØÖ·(MT:Ê¼·¢µØÖ· MO:½ÓÊÕµØÖ·)
        if ((pClMsg->u.cdma_message.address.number_of_digits) > 0)
        {
            ...
            if (nlen>0)
            {
                     ...
                     pClMsg->msg_hdr.tag == WMS_TAG_MT_NOT_READ))
                {
                    ...
                    // ½«"158"±ä³É"0416"
                    // ½«"199"±ä³É"0419"
                    // ½«"199"±ä³É"0426" guoys modified for movilnet change 2008.10.28
...
ËµÃ÷:
    ±¾º¯ÊýÓÃÓÚ·¢Æð¶ÔÖ¸¶¨ºÅÂëµÄºô½Ð¡£

²ÎÊý:
    pMe [in]: Ö¸ÏòWMS Applet¶ÔÏó½á¹¹µÄÖ¸Õë¡£¸Ã½á¹¹°üº¬Ð¡³ÌÐòµÄÌØ¶¨ÐÅÏ¢¡£
    szVMNumber [in/out]: ²¦³öºÅÂë×Ö·û´®¡£
...
ËµÃ÷:
    Í¨¹ýContact Applet½«µç»°ºÅÂë±£´æµ½Í¨ÐÅ²¾Êý¾Ý¿â¡£

²ÎÊý:
    pMe [in]: Ö¸ÏòWMS Applet¶ÔÏó½á¹¹µÄÖ¸Õë¡£¸Ã½á¹¹°üº¬Ð¡³ÌÐòµÄÌØ¶¨ÐÅÏ¢¡£
    wstrNum [in]: ´ý±£´æµÄºÅÂë×Ö·û´®¡£
    wstrURL [in]: ´ý±£´æµÄ URL ×Ö·û´®¡£
    wstrEmail [in]: ´ý±£´æµÄ e-mail µØÖ·×Ö·û´®¡£

·µ»ØÖµ:
    none

±¸×¢:
    µ÷ÓÃContact Applet£¬WMS applet±»¹ÒÆð¡£
...
ËµÃ÷:
    ±¾º¯ÊýÓÃÓÚ´ÓÍ¨ÐÅ²¾ÊýÈ¡ºÅÂë¡£

²ÎÊý:
    pMe [in]: Ö¸ÏòWMS Applet¶ÔÏó½á¹¹µÄÖ¸Õë¡£¸Ã½á¹¹°üº¬Ð¡³ÌÐòµÄÌØ¶¨ÐÅÏ¢¡£
    eType [in]: ´Óµç»°±¾È¡ºÅÂëÊ±²Ù×÷ÀàÐÍ
...
²ÎÊý:
    selRec [in]: Ñ¡¶¨µÄ¼ÇÂ¼¡£
    selItem [in]: Ñ¡¶¨¼ÇÂ¼µÄ×Ö¶Î¡£

·µ»ØÖµ:
    TRUE: Contact applet Ó¦ÍË³ö¡£
    FALSE: Contact applet ²»ÍË³ö¡£

±¸×¢:
    º¯Êý»áÐÞ¸ÄWMS¶ÔÏóµÄ m_pSendList ÖÐµÄÄ³¸ö³ÉÔ±Ïî¡£
...
ËµÃ÷:
    ...
    ÓÃÓÚ´ÓÍ¨ÐÅ²¾È¡ºÅÂë²¢²åÈëµ½µ±Ç°±à¼­µÄÏûÏ¢ÖÐµÄ»Øµ÷º¯Êý¡£
        ...
        if (gpWMSApp->m_eInsertType == INSERT_EDITWMS)
        {
            ...
            else
            {
                ...
                else if (WMSUtil_HaveNoneASCIIChar(wszPack, &nPos))
                {
                    ...
                    {// Ö»ÄÜ²¿·Ö²åÈë
                        // ÏûÏ¢±¾Éí²»º¬¿í×Ö·û, Ö»ÄÜ²åÈëÇ°ÃæµÄ·Ç¿í×Ö·ûÐòÁÐ
...
ËµÃ÷:
    ±£´æµ±Ç° Applet ÅäÖÃÊý¾Ý¡£
       
²ÎÊý:
    pMe [in]:Ö¸Ïò WMS Applet ¶ÔÏó½á¹¹µÄÖ¸Õë¡£¸Ã½á¹¹°üº¬Ð¡³ÌÐòµÄÌØ¶¨ÐÅÏ¢¡£
...
ËµÃ÷:
    ½«ÒÔÃëÎªµ¥Î»µÄÊ±¼ä×ª»»Îª wms_timestamp_s_type ¸ñÊ½¡£
       
²ÎÊý:
    sec [in]: Ê±¼ä(µ¥Î»: Ãë)
    pmc_time [in]:Ö¸Ïò wms_timestamp_s_type ¶ÔÏó½á¹¹µÄÖ¸Õë¡£
...
ËµÃ÷:
    º¯Êý¸ù¾Ý WMSMessageStruct ½á¹¹ÏûÏ¢´´½¨ wms_client_message_s_type ½á¹¹Ä£°åÏûÏ¢¡£

²ÎÊý:
    pMe [in]: Ö¸Ïò WMS Applet¶ÔÏó½á¹¹µÄÖ¸Õë¡£¸Ã½á¹¹°üº¬Ð¡³ÌÐòµÄÌØ¶¨ÐÅÏ¢¡£
    pMs [in]: WMSMessageStruct Êý¾Ý½á¹¹Ö¸Õë¡£
    bOutTemplate [in]: TRUE--Êä³ö cdma_template FALSE--Êä³ö cdma_message
                                             ...
                                             boolean bOutTemplate)
{
    ...
    // ×ª»»µ½ wms_cdma_user_data_s_type ½á¹¹
...
²ÎÊý:
    pMe [in]: Ö¸Ïò WMS Applet ¶ÔÏó½á¹¹µÄÖ¸Õë¡£¸Ã½á¹¹°üº¬Ð¡³ÌÐòµÄÌØ¶¨ÐÅÏ¢¡£
    pmsMsg [in]:Ö¸Ïò WMSMessageStruct ½á¹¹µÄÖ¸Õë¡£

·µ»ØÖµ:
    none

±¸×¢:
    ±¾º¯Êý¼Ù¶¨´«ÈëµÄÏûÏ¢½á¹¹±£´æµÄÊÇ½ÓÊÕµ½µÄÏûÏ¢¡£
==============================================================================*/
void WmsApp_BuildNumList(WmsApp *pMe, WMSMessageStruct *pmsMsg)
{
    ...
    for (i=0; i<nLen; i++)
    {
        ...
        {// µ±Ç°×Ö·ûÎªÊý×Ö
            ...
            // ÐèÒª´ËÅÐ¶Ï£¬·ñÔòºÅÂëÎ»ÓÚÏûÏ¢ÌåÎ²²¿Ê±½«È¡²»³ö
            if (i != nLen-1)
            {
                continue;
            }
        ...
        { // Á¬ÐøÊý×Ö´®³¤¶ÈÐ¡ÓÚ3»ò²»µÍÓÚMAX_PH_DIGITS£¬´ËºÅÂëÎÞÐ§
            nStartPos = -1;
            continue;
        }
...
ËµÃ÷:
    ·ÖÎö¸ø¶¨×Ö·û´®£¬½¨Á¢µç×ÓÓÊ¼þµØÖ·Á´±í¡£

²ÎÊý:
    pMe [in]: Ö¸Ïò WMS Applet ¶ÔÏó½á¹¹µÄÖ¸Õë¡£¸Ã½á¹¹°üº¬Ð¡³ÌÐòµÄÌØ¶¨ÐÅÏ¢¡£
    wstrText [in]: ´ý·ÖÎö×Ö·û´®Ö¸Õë¡£

·µ»ØÖµ:
    none

±¸×¢:
    ÓÊ¼þµØÖ·±ØÐëÊÇÖ»º¬³ý "(),:;<>[\] Ö®ÍâµÄÇÒÖµ´óÓÚ0x20¡¢Ð¡ÓÚ0x7FµÄASCII ×Ö·û,
    ÓÊ¼þµØÖ·×î´ó³¤¶ÈÎª64¸ö×Ö·û¡£
    ¼ûÁªÍ¨<<¶àÃ½ÌåÓÊ¼þÖÕ¶Ë¹¦ÄÜ¹æ·¶>>
    
    Óë WmsApp_BuildNumList º¯Êý¹²ÓÃ m_pSaveNumList Á´±í¡£µ÷ÓÃ´Ëº¯ÊýÇ°Ó¦½«´ËÁ´±í
    Çå¿Õ¡£
==============================================================================*/
void WmsApp_BuildMailAddList(WmsApp *pMe, AECHAR *wstrText)
{
    ...
    while (NULL != wstrAt)
    {
        ...
        {
            wstrAt = NULL;// ÉèÖÃÖÕÖ¹ while Ñ­»·µÄÌõ¼þ
        }
    }
}
...
±¸×¢:
    URL±ØÐëÊÇÖ»º¬³ý "<> Ö®ÍâµÄÇÒÖµ´óÓÚ0x20¡¢Ð¡ÓÚ0x7FµÄASCII ×Ö·û, ÇÒ×î´ó³¤¶ÈµÍÓÚ
    256¸ö×Ö·û¡£
    ¼ûÁªÍ¨<<¶àÃ½ÌåÓÊ¼þÖÕ¶Ë¹¦ÄÜ¹æ·¶>>
==============================================================================*/
...
ËµÃ÷: 
    ÓÃÓÚÐÞÕýÑÇÌ«¿íÆµ·ÛºìÉ«¿¨¸üÐÂ³É¹¦ºóÌáÊ¾×Ö·û´®²»ÍêÕûµÄÎÊÌâ¡£
...
·µ»ØÖµ: 
    TRUE:  Ðèµ÷ÓÃÕß¸üÐÂ´ËÏûÏ¢
    FALSE: µ÷ÓÃÕß²»Ðè¸üÐÂ´ËÏûÏ¢
...
²ÎÊý:
    pIText, µ±Ç°µÄtext control¡£

·µ»ØÖµ:
    µ±Ç°ÎÄ±¾¿Ø¼þÔÊÐíÊäÈëµÄ×î´ó×Ö·ûÊý¡£
±¸×¢:
...
int WmsApp_SetMaxSize(ITextCtl *pIText)
{
...
#if defined FEATURE_CARRIER_THAILAND_HUTCH || defined FEATURE_CARRIER_THAILAND_CAT        
    {
        ...
        {// ¶ÔÓÚ³¤¶È³¬¹ý MAX_UNICODMSG_PAYLOAD µÄÎÄ±¾£¬Èô±àÂëÎª WMS_ENCODING_ASCII
         // Ôò²»Óè½Ø¶Ï£¬µ«×î´óÊýÎª MAX_ASCIIMSG_PAYLOAD £¬¶àÓà²¿·ÝÈÔ½Ø¶Ï£¬ÇÒÐè½«ÊäÈë
         // ·¨µ÷ÖÁ²»ÄÜÊäÈëUNICODE µÄÊäÈë·¨; ·ñÔòÇÐ¶ÏÏûÏ¢
...
ËµÃ÷:
    Ó¦ÒÔÉ«ÁÐÒªÇó¸ñÊ½»¯·¢³öµÄ¶ÌÐÅÎÄ±¾¡£

²ÎÊý:
    pclt_msg [in/out]: wms_client_message_s_type ÐÍÖ¸Õë¡£
...
void wmsutil_formatmessage(wms_client_message_s_type *pclt_msg)
{
    ...
    wms_cdma_user_data_s_type *puserdata = NULL;
    int              nOpt; // 0-ÎÞ²Ù×÷ 1-²åÈë 2-Ìæ»» ÆäÓàÖµ-ÎÞ¶¨Òå
        ...
        if (i < nLen)
        {
            ...
            {// ·Ç¿Õ¸ñ£¬»ØËÝ¿´ÄÜ·ñÕÒµ½Ìõ¼þ×Ö·û
                ...
                if ((NULL != wtrP) && (wtrP != &pMs->m_szMessage[nPos]))
                {
                    ...
                    {// ¼ì²éµØÖ· wtrP Ç°ÊÇ·ñÈ«Îª CONDTION_AECHAR
...
±¸×¢:
    ±¾º¯ÊýÊÇº¯Êý wmsutil_formatmessage µÄÄæ²Ù×÷¡£
...
ËµÃ÷:
    ÓÃÓÚ´ÓÍ¨»°ÀúÊ·¼ÇÂ¼È¡ºÅÂëµÄ»Øµ÷º¯Êý¡£

²ÎÊý:
    pNode [in]: Ñ¡¶¨¼ÇÂ¼µÄ×Ö¶Î¡£

·µ»ØÖµ:
    TRUE: recentcalls applet Ó¦ÍË³ö¡£
    FALSE: recentcalls applet ²»ÍË³ö¡£
...
±¸×¢:
    µ÷ÓÃRecentCalls Applet£¬WMS applet±»¹ÒÆð¡£
...
ËµÃ÷£º
    º¯ÊýÓÃÓÚ¼ÆËã×Ö·û´®µÄ CRC32 Ð£ÑéÂë¡£
...
²ÎÊý£º
    szRegInfo [in]£ºÖ¸ÏòÓÃÓÚ·µ»Ø×¢²áÐÅÏ¢µÄcharÖ¸Õë¡£
    nSize [in]£º    »º´æÇøµÄ´óÐ¡¡£(µ¥Î»£ºbyte) 

·µ»ØÖµ£º
    >=0:    ³É¹¦,Êý×ÖÎªÏûÏ¢Õ¼ÓÃµÄ×Ö½ÚÊý
    <0:    Ê§°Ü
...
int GetRegisterInfo(char *szRegInfo, int nSize)
{
    ...
    //  ÃüÁîÀàÐÍ--ÒÆ¶¯Ì¨×Ô¶¯×¢²áÏûÏ¢
    szRegInfo[1] = 3;
    
    // Ìî³äÎ»
    szRegInfo[3] = 0;
    
    // ¸ñÊ½»¯Êý¾Ý£º»úÐÍ¡¢ÊÖ»úESN¡¢IMSIºÅÂë¡¢µ±Ç°Èí¼þ°æ±¾
...
int GetSeamlessSMSInfo(char *szInfoEx, int nSize)
{
    ...
    // ¸ñÊ½»¯Êý¾Ý£ºÆ·ÅÆ¡¢»úÐÍ¡¢ÊÖ»úESN¡¢MDN¡¢MIN¡¢µ±Ç°Èí¼þ°æ±¾¡¢Ê±¼ä
...
ËµÃ÷:
    ±¾º¯ÊýÓÃÓÚ¸ù¾ÝÌØ¶¨µÄÏûÏ¢ÀàÐÍ·µ»ØÏàÓ¦µÄ wms_client_message_s_type ÐÍ MO ÏûÏ¢¡£

²ÎÊý:
    pwstrType [in]: Çø·Ö»ñÈ¡ÀàÐÍ¡£

·µ»ØÖµ:
    ÎÞ¡£

±¸×¢:
    ·µ»ØBufÏµ¶¯Ì¬·ÖÅä£¬µ÷ÓÃÕß¸ºÔðÊÍ·Å¡£
