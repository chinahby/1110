...
//状态 STATE_CVIDEO 处理函数
static NextFSMAction CameraApp_StateVideoHandle(CCameraApp *pMe);

//状态STATE_CVIDEOCFG 处理函数
static NextFSMAction CameraApp_StateVideoCFGHandle(CCameraApp *pMe);

//状态 STATE_CVIDEOSAVE 处理函数
static NextFSMAction CameraApp_StateVideoSaveHandle(CCameraApp *pMe);

//状态 STATE_CFUN 处理函数
static NextFSMAction CameraApp_StateFunHandle(CCameraApp *pMe);
...
NextFSMAction CameraApp_ProcessState(CCameraApp *pMe)
{
    ...
    switch(pMe->m_eCurState)
    {
        ...
        case STATE_CVIDEOCFG:
            retVal = CameraApp_StateVideoCFGHandle(pMe);
            break;
      
        case STATE_CVIDEOSAVE:
            retVal = CameraApp_StateVideoSaveHandle(pMe);
            break;

        case STATE_CFUN:
            retVal = CameraApp_StateFunHandle(pMe);
            break;
...
static NextFSMAction CameraApp_StateVideoHandle(CCameraApp *pMe)
{
    ...
    switch(pMe->m_eDlgRet)
    {
        ...
        case DLGRET_VIDEOCFG:
            MOVE_TO_STATE(STATE_CVIDEOCFG)
            return NFSMACTION_CONTINUE;
        ...
        case DLGRET_VIDEOSAVE:
            MOVE_TO_STATE(STATE_CVIDEOSAVE)
            return NFSMACTION_CONTINUE;            
...
/*==============================================================================
函数：
       CameraApp_StateVideoCFGHandle
说明：
       STATE_VIDEOCFG 状态处理函数
...
static NextFSMAction CameraApp_StateVideoCFGHandle(CCameraApp *pMe)
{
    ...
    switch (pMe->m_eDlgRet)
    {
        case DLGRET_CREATE:
            pMe->m_bNotOverwriteDlgRet = FALSE;
            CameraApp_ShowDialog(pMe, IDD_CVIDEOCFG);
            return NFSMACTION_WAIT;
...
/*==============================================================================
函数：
       CameraApp_StateVideoSaveHandle
说明：
       STATE_CPIC 状态处理函数
...
static NextFSMAction CameraApp_StateVideoSaveHandle(CCameraApp *pMe)
{
    ...
    switch(pMe->m_eDlgRet)
    {
        case DLGRET_CREATE:
            pMe->m_bNotOverwriteDlgRet = FALSE;
            CameraApp_ShowDialog(pMe, IDD_CVIDEOSAVE);
            return NFSMACTION_WAIT;  
...
static NextFSMAction CameraApp_StateFunHandle(CCameraApp *pMe)
{
    ...
    switch(pMe->m_eDlgRet)
    {
        case DLGRET_CREATE:
            pMe->m_bNotOverwriteDlgRet = FALSE;
            CameraApp_ShowDialog(pMe, IDD_CFUN);
            return NFSMACTION_WAIT;  
...
{    
    ...
    switch(pMe->m_eDlgRet)
    {
        ...
        case DLGRET_CANCELED:
            if((pMe->m_ePreState == STATE_CPIC) && (pMe->m_nSnapShotPicNum == 0))
            {
                pMe->m_bRePreview =  TRUE;
                MOVE_TO_STATE(STATE_CPREVIEW);
            }
            else if(pMe->m_ePreState == STATE_CVIDEOSAVE)
            {
                pMe->m_bRePreview =  TRUE;
                MOVE_TO_STATE(STATE_CVIDEO);
            }
