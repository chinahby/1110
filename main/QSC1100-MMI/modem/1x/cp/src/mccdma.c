...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/mccdma.c#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $
...
#ifdef FEATURE_MEID_SUPPORT
  if (cdma.is_cdma_subs_avail && read_ok) 
  {
    /* Read the MEID */
  ...
  ** ------------------------------------------------------------ */
...

  {
    ...
    if (cdma.ruim_ota_features.num_features != 0)
    {
       ...
       if (cdma.ruim_ota_features.num_features > OTASPI_FEATURE_MAX )
            cdma.ruim_ota_features.num_features = OTASPI_FEATURE_MAX ;
          ...
          ** ---------------------------------- */
          ...
          if ( uim_cmd_ptr->hdr.status == UIM_RECEIVED_S )
          {
                 ...
                 (uim_rpt_ptr->rpt_status == UIM_PASS))
             {
                ...
                for (i = 0, j = 0; i < cdma.ruim_ota_features.num_features; i++)
                {
...
)
{
  ...
  /* Inform system determination of call connected */
  if ((cmd_ptr->hdr.cmd == CM_CALL_CONNECTED_F) ||
      (cmd_ptr->hdr.cmd == CM_CALL_EMERGENCY_FLASHED_F)
     )
  {
