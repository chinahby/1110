...
$Header: //source/qcom/qct/modem/uim/nvruim/rel/07H1_2/src/nvruim.c#15 $ $DateTime: 2009/05/27 11:46:41 $ $Author: shanmugh $
...
#define NVRUIM_CHINA_UNICOM_AN_CAVE_NAI_LENGTH 33
...
#ifndef FEATURE_UIM_3GPD_FALLBACK
  if(!nvruim_3gpd_sip_svc || !nvruim_3gpd_ext_support)
  {
    *op_status = NV_DONE_S;
    return(NV_RUIM_SUPPORTS_ITEM);
  } /* end !nvruim_3gpd_sip_svc ||  !nvruim_3gpd_ext_support*/
#endif /* !FEATURE_UIM_3GPD_FALLBACK */
    ...
    case NV_HDR_AN_AUTH_USER_ID_LONG_I:
      if(NV_DONE_S == *op_status)
      {
        ...
        /* --------------------------------------------------------------------
           ...
           Copy the item from the cache buffer. 
           ------------------------------------------------------------------*/
          nv_cmd_ptr->data_ptr->hdr_an_auth_user_id_long.user_id_len = 33;
    ...
    case NV_HDR_AN_AUTH_NAI_I:
      if(NV_DONE_S == *op_status)
      {
        ...
        /*---------------------------------------------------------------------
         ...
         Copy the item from the cache buffer. 
         --------------------------------------------------------------------*/
          nv_cmd_ptr->data_ptr->hdr_an_auth_nai.nai_len = 33;
          min1 = (nvruim_imsi_m_cache_buf.imsi_s1[2] << 16) |
                 (nvruim_imsi_m_cache_buf.imsi_s1[1] << 8) |
                 (nvruim_imsi_m_cache_buf.imsi_s1[0]);
...
)
{
  ...
  switch( nv_cmd_ptr->item )
  {
    ...
    case NV_SMS_MO_ON_TRAFFIC_CHANNEL_I:
      nv_cmd_ptr->data_ptr->sms_mo_on_traffic_channel = nvruim_smscap_ef_buf[2] 
           & NVRUIM_SMSCAP_ON_TRAFFIC_CH_MASK;
        break;
...
FUNCTION NVRUIM_DATA_3GPD_EXTENSIONS_SUPPORT
