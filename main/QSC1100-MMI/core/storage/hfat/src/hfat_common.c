...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/hfat/main/latest/src/hfat_common.c#11 $ $DateTime: 2009/01/30 12:47:45 $ $Author: richardp $
...
_f_prepareformat (F_VOLUME * vi, long fattype)
{
  ...
  {
    f_bootrecord->number_of_hidden_sectors = boundary_unit;
  ...
  if (f_bootrecord->sector_per_cluster)
  {
    ...
    { 
      /* Max limit for SDHC cards is 32GB */
      if (vi->sectornum > 0x4000000)
        return F_ERR_MEDIATOOLARGE;
    }
    else
...
_f_check_handle (FN_FILE * filehandle)
{
  ...
  if (((int) filehandle & a) >= F_MAXFILES)
...
int
fn_seek (F_MULTI * fm, FN_FILE * filehandle, long offset, long whence)
{
  FN_FILEINT *f = _f_check_handle (filehandle);
  F_VOLUME *vi;
  int ret;
...
rex_crit_sect_type hfat_crit_sect;
...
fnSetTask (F_MULTI ** fm)
{
  ...
  rex_init_crit_sect (&hfat_crit_sect);
  ...
  ERR_FATAL ("fnSetTask failed",0,0,0);
  return 1;
}
#endif
...
{
  /* fn_seek sets the last error code */
  int ret = fn_seek (fm, filehandle, length, FN_SEEK_SET);
  if (ret)
    return ret;
