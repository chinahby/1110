...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/hfat/main/latest/src/hfat_lfn.c#9 $ $DateTime: 2009/01/30 12:47:45 $ $Author: richardp $
...
static int
fn_wsettimedate_h (F_MULTI * fm, const wchar * filename, unsigned short ctime,
                   unsigned short cdate, F_NAME *fsname, F_LFNINT *lfn);

static int
fn_wgettimedate_h (F_MULTI * fm, const wchar * filename, unsigned short *pctime,
                   unsigned short *pcdate, F_NAME *fsname, F_LFNINT *lfn);
...
_towchar (wchar * nconv, const char *s)
{
  ...
  ERR_FATAL ("Some called _towchar",0,0,0);
...
_fromwchar (char *s, wchar * nconv)
{
  ...
  ERR_FATAL ("Someone called _fromwchar",0,0,0);
  for (a = 0; a < FN_MAXPATH - 1; a++)
  {
...
static void
_f_strcpytolower (char *dst, const char *src, int len, int to_lower)
{
  int i;
                ...
                const char *ext, unsigned char ntres)
{
  ...
  char temp_name[8], temp_ext[4];

  _f_strcpytolower (temp_name, name, 8, (ntres & NTRES_NAME_LOW));
  _f_strcpytolower (temp_ext, ext, 3, (ntres & NTRES_EXT_LOW));
...
_f_addlfn (const F_DIRENTRY * de, F_LFNINT * plfn, unsigned long num)
{
  ...
  {                             /* check sum check */
    ...
    _f_getfilename ((W_CHAR *) plfn->name, de->name, de->ext, de->ntres);
  ...
  if (f->relpos == F_SECTOR_SIZE)
  {
...
#ifdef DIRCACHE_ENABLE
    {
      ...
      if ((f->pos.sector) - (vi->dircache_start) >= n)
      {
        ...
        {
          if (_f_readsector (vi, vi->dircache, f->pos.sector, (int) n))
            return 0;
  ...
  while (pos->cluster < F_CLUSTER_RESERVED)
  {
    for (; pos->sector < pos->sectorend; pos->sector++)
    {
      ...
      for (; pos->pos < F_SECTOR_SIZE / sizeof (F_DIRENTRY); de++, pos->pos++)
      {
        ...
        _f_getfilename (sname, (char *) de->name, (char *) de->ext, de->ntres);
...
static int
_f_findlfilewc (F_VOLUME * vi, W_CHAR * lname, F_POS * pos, F_DIRENTRY ** pde,
                F_LFNINT * lfn)
{
  unsigned long denum = 0;
  W_CHAR sname[8 + 3 + 2];
...
_f_findpath (F_VOLUME * vi, F_NAME * fsname, F_POS * pos)
{
  ...
  for (; *path;)
  {
    ...
    {
      if (!_f_findfile (vi, currname, pos, &de, lfn))
        goto Error;
...
fn_findfirst (F_MULTI * fm, const char *filename, FN_FIND * find)
{
  ...
  if (find2_inuse == TRUE)
  {
    ERR_FATAL ("Global find2 Buffer in use",0,0,0);
  }
  else
  {
    find2_inuse = TRUE;
  ...
  /* Check these are the same file */
  {
    ...
    for (i=0; ; i++)
    {
      ...
      {  
        ...
        if (_f_findfile (vi, fsnewname->lname, &pos, 0, lfn))
               ...
               F_NAME *fsname, F_NAME *fsnamenew, F_LFNINT *lfn, F_LFNINT *lfn2)
{
  ...
  if (de->attr & F_ATTR_DIR)
  {
    ...
    if (_f_findfile (vi, fsnamenew->lname, &pos2, &de2, lfn2))
    {
...
int
fn_settimedate (F_MULTI * fm, const char *filename, unsigned short ctime,
                unsigned short cdate)
{
  ...
  hpb = hfat_allocate_path_buffer (NULL);
  ret_val = fn_wsettimedate (fm, _towchar (hpb->buf, filename), ctime, cdate);
  hfat_free_path_buffer (hpb);
  return ret_val;
}

int
fn_wsettimedate (F_MULTI * fm, const wchar * filename, unsigned short ctime,
                 unsigned short cdate)
{
  int ret_val;
  struct hfat_f_name_buffer *f_name_hnb;
  ...
  ret_val = fn_wsettimedate_h (fm, filename, ctime, cdate,
...
int
fn_gettimedate (F_MULTI * fm, const char *filename, unsigned short *pctime,
                unsigned short *pcdate)
{
  ...
  hpb = hfat_allocate_path_buffer (NULL);
  ret_val = fn_wgettimedate (fm, _towchar (hpb->buf, filename), pctime, pcdate);
  hfat_free_path_buffer (hpb);
  return ret_val;
}

int
fn_wgettimedate (F_MULTI * fm, const wchar * filename, unsigned short *pctime,
                 unsigned short *pcdate)
{
  int ret_val;
  struct hfat_f_name_buffer *f_name_hnb;
  ...
  ret_val = fn_wgettimedate_h (fm, filename, pctime, pcdate,
                                  &f_name_hnb->f_name, &f_lfn_hnb->f_lfnint);
                 ...
                 unsigned short *pcdate, F_NAME *fsname, F_LFNINT *lfn)
{
  ...
  if (pctime)
    *pctime = _f_get16bitl (de->ctime);
  if (pcdate)
    *pcdate = _f_get16bitl (de->cdate);
                ...
                F_LFNINT *lfn)
{
  ...
  if (!_f_findfile (vi, fsname->lname, &pos, &pde, lfn))
    return F_SETLASTERROR (F_ERR_NOTFOUND);
 ...
 */
static void
w_char_strlcpy (W_CHAR *dest, const W_CHAR *src, size_t size)
{
  unsigned int i;

  /* If buffer size is zero just return. */
  if (size == 0)
    return;

  /* The loop is intentionally run to (size-2) to ensure the NULL
   * termination.
   */
  for (i = 0; i < size-1; i++) {
    /* When src terminates break the loop. */
    if (!src[i]) {
      break;
    }
    /* Copy src to destination until NULL-termination.*/
    dest[i] = src[i];
  }
  /* Ensure destination NULL termination. */
  dest[i] = 0;
                      ...
                      F_DIRENTRY *dep, F_POS *pos)
{
  ...
  w_char_strlcpy (lf->path, path, FN_MAXPATH);
  w_char_strlcpy (lf->lname, name, F_MAXLNAME);
  lf->pos = *pos;
