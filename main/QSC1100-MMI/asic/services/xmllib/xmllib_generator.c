...
/*===========================================================================
  ...
  $Header: //depot/asic/msmshared/services/xmllib/xmllib/xmllib_generator.c#5 $
...
)
{
  ...
  if( NULL != xmldecl_node->payload.element.child )
  {
    *error_code = XMLLIB_ERROR_MALFORMED_TREE;
    ERR("xmldecl node with child %p",
         xmldecl_node->payload.element.child,0,0 );
    ret = XMLLIB_ERROR;
  }
...
)
{
  ...
  if( XMLLIB_PARSETREE_NODE_MAX <= node->nodetype )
  {
    *error_code = XMLLIB_ERROR_MALFORMED_TREE;
    ret = XMLLIB_ERROR;
    ERR("Invalid node type %d",node->nodetype, 0, 0);   
  }
