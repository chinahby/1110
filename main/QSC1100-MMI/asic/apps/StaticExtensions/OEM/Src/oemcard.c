...
$Header: //depot/asic/qsc60x0/apps/StaticExtensions/OEM/Src/oemcard.c#5 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/19/08   jk      Added Application Label File for OMH
01/17/08   jk      Added OMH rev C files
10/15/07   vs      Added OMH feature support
...
static OEMCardFileChangeNotify sTempFileChangeNotify = { NULL };
  ...
  if ( sim_data->message_header.gsdi_status == GSDI_SUCCESS )
  {
    switch(sim_data->pin_status_cnf.status) {
      ...
      case GSDI_PIN_ENABLED:
        /* PIN is required. */
        if(pin1_status.lock_state != AEECARD_PIN_VERIFIED)
        {
...
#error code not present
  ...
  {
    ERR("FAILED GET SIM Capability with reason %d",
...
#error code not present
    ...
    case GSDI_PIN1_BLOCKED:
      pin1_status.lock_state = AEECARD_PIN_BLOCKED;
      pin1_status.tries_remaining = AEECARD_UNKNOWN_TRIES;
      /* set cache pin status */
      gsdi_status = gsdi_get_pin_status(GSDI_PIN1,
                          &pin_status_dummy,
    ...
    case GSDI_PIN2_BLOCKED:
      pin2_status.lock_state = AEECARD_PIN_BLOCKED;
      pin2_status.tries_remaining = AEECARD_UNKNOWN_TRIES;
      gsdi_status = gsdi_get_pin_status(GSDI_PIN2,
                          &pin_status_dummy,
                          &num_retries_dummy,
    ...
    case GSDI_PIN2_UNBLOCKED:
      pin2_status.lock_state = AEECARD_PIN_VERIFIED;
      pin2_status.tries_remaining = AEECARD_UNKNOWN_TRIES;
      gsdi_status = gsdi_get_pin_status(GSDI_PIN2,
                          &pin_status_dummy,
                          &num_retries_dummy,
    ...
    case GSDI_PIN1_VERIFIED:
      ...
      {
        pin1_status.lock_state = AEECARD_PIN_VERIFIED;
        pin1_status.tries_remaining = OEMCARD_MAX_PIN_RETRIES;
      }
      gsdi_status = gsdi_get_pin_status(GSDI_PIN1,
                          &pin_status_dummy,
                          &num_retries_dummy,
      ...
      OEMCardNewPinStatus(AEECARD_PIN1, &pin1_status);
      break;

    case GSDI_PIN2_VERIFIED:
      if (pin2_status.lock_state != AEECARD_PIN_VERIFIED)
      {
        pin2_status.lock_state = AEECARD_PIN_VERIFIED;
        pin2_status.tries_remaining = OEMCARD_MAX_PIN_RETRIES;
      }
      gsdi_status = gsdi_get_pin_status(GSDI_PIN2,
                          &pin_status_dummy,
                          &num_retries_dummy,
      ...
      OEMCardNewPinStatus(AEECARD_PIN2, &pin2_status);
      break;

    case GSDI_PIN1_ENABLED:
      if ((pin1_status.lock_state == AEECARD_PIN_DISABLED) ||
          (pin1_status.lock_state == AEECARD_PIN_VERIFIED)) {
        pin1_status.lock_state = AEECARD_PIN_VERIFIED;
      }
      else {
        pin1_status.lock_state = AEECARD_PIN_ENABLED;
      }
      pin1_status.tries_remaining = AEECARD_UNKNOWN_TRIES;
      gsdi_status = gsdi_get_pin_status(GSDI_PIN1,
                          &pin_status_dummy,
    ...
    case GSDI_PIN2_ENABLED:
      if ((pin2_status.lock_state == AEECARD_PIN_DISABLED) ||
          (pin2_status.lock_state == AEECARD_PIN_VERIFIED)) {
        pin2_status.lock_state = AEECARD_PIN_VERIFIED;
      }
      else {
        pin2_status.lock_state = AEECARD_PIN_ENABLED;
      } 
      pin2_status.tries_remaining = AEECARD_UNKNOWN_TRIES;
      gsdi_status = gsdi_get_pin_status(GSDI_PIN2,
                          &pin_status_dummy,
    ...
    case GSDI_PIN1_DISABLED:
      pin1_status.lock_state = AEECARD_PIN_DISABLED;
      pin1_status.tries_remaining = AEECARD_UNKNOWN_TRIES;
      gsdi_status = gsdi_get_pin_status(GSDI_PIN1,
                          &pin_status_dummy,
                          &num_retries_dummy,
    ...
    case GSDI_PIN2_DISABLED:
      pin2_status.lock_state = AEECARD_PIN_DISABLED;
      pin2_status.tries_remaining = AEECARD_UNKNOWN_TRIES;
      gsdi_status = gsdi_get_pin_status(GSDI_PIN2,
                          &pin_status_dummy,
                          &num_retries_dummy,
    ...
    case GSDI_PIN1_PERM_BLOCKED:
      pin1_status.lock_state = AEECARD_PIN_PERM_DISABLED;
      pin1_status.tries_remaining = AEECARD_UNKNOWN_TRIES;
      gsdi_status = gsdi_get_pin_status(GSDI_PIN1,
                          &pin_status_dummy,
                          &num_retries_dummy,
    ...
    case GSDI_PIN2_PERM_BLOCKED:
      pin2_status.lock_state = AEECARD_PIN_PERM_DISABLED;
      pin2_status.tries_remaining = AEECARD_UNKNOWN_TRIES;
      gsdi_status = gsdi_get_pin_status(GSDI_PIN2,
                          &pin_status_dummy,
                          &num_retries_dummy,
   ...
   const mmgsdi_cnf_type  *cnf_ptr)
{
  ...
  if (status == MMGSDI_SUCCESS && ICardobj != NULL )
  {
    if (cnf_ptr == NULL)
    {
      MSG_ERROR("Null Cnf Ptr for SUCCESS status, cnf 0x%x", cnf, 0, 0);
      return;
    }
...
static void OEMCard_FreeSequenceNumber(uint32 dwSeqNum)
{
  ...
  if (sCardRspTable[dwSeqNum].sRsp != NULL)
  {
...
#error code not present
    ...
    default:
      ...
      */
      *pbyCardStatus = AEECARD_INVALID_CARD;
      DBGPRINTF("Unknown status value %d received from get card status()",
          CardStatus);
      break;
...
SIDE EFFECTS
===========================================================================*/
static int OEMCard_ParseUSIMProtectionInfo (AEECardAccessConditionType* pAccessStruct,
                                     uint8                       byAccessValue)
{
  if (pAccessStruct == NULL) {
    return EFAILED;
  }
...
SIDE EFFECTS
===========================================================================*/
static int OEMCard_ParseSIMProtectionInfo (AEECardAccessConditionType* pAccessStruct,
                                    uint8                       byAccessValue)
{
  if (pAccessStruct == NULL) {
    return EFAILED;
...
SIDE EFFECTS
===========================================================================*/
static int OEMCard_MapFileProtection(AEECardFileProtectionType *pCardFileProtection,
                              gsdi_file_protection_T nGSDIFileProtection)
{
  gsdi_card_apps_T  sCardApp;
  int               nReturnStatus;
  if (pCardFileProtection == NULL) {
    return EFAILED;
  }
...
SIDE EFFECTS
===========================================================================*/
static int OEMCard_PopulateFileAttribBuf(AEECardFileAttribStatus *pFileAttrib,
                                  gsdi_file_attributes_T *pGSDIFileAttrib)
{
  int nReturnStatus = SUCCESS;
  if ((pFileAttrib == NULL) || (pGSDIFileAttrib == NULL)){
    return EBADPARM;
  }
...
static void OEMCardGSDIcmdCB(gsdi_cnf_T *gsdi_info)
{
  ...
  if ((gsdi_info == NULL) || (ICardobj == NULL)) {
    return;
  }
  ...
  switch(gsdi_info->message_header.resp_type)
  {
    ...
    case GSDI_PERSO_SET_PHONE_CODE_RSP:
      break;
    default:
      ERR("Unknown command to OEMCardGSDIcmdCB %d",
          gsdi_info->message_header.resp_type, 0,
          0);
      OEMCard_CleanUpAsynObject(Handler);
      return;
  }
...
#error code not present
#endif /*FEATURE_MMGSDI_DUAL_SLOT */
      {
        ...
        {
          ERR("Unknown PIN to OEMCardGSDIcmdCB %d", pin_id, 0, 0);
          OEMCard_CleanUpAsynObject(Handler);
          return;
        }
        /* Status word */
...
static void OEMCardGSDIPersocmdCB(gsdi_cnf_T *gsdi_info)
{
  ...
  case GSDI_PERSO_UNBLOCK_IND_RSP:
    ...
    OEMCard_MapGSDIFeatureToCardFeature(gsdi_info->dact_ind_cnf.perso_feature,
                                        &pDeactivateCmdStatus->nPersoFeatureID);
...
{
  if ((pGSDIPinId == NULL) || (pGSDISlotID == NULL)) {
    return EBADPARM;
  }
...
{
  if (sFilePath.nPathLen == 0) {
    return EFAILED;
  }
  if (sFilePath.pPath == NULL) {
    return EFAILED;
  }
  return SUCCESS;
}
                    ...
                    gsdi_file_enum_type *gsdi_file_item )
{
  ...
  if (sFilePath.nFileIndication == AEECARD_FILE_NAME) {
    ...
    MSG_ERROR("Unknown file IndType 0x%x", sFilePath.nFileIndication, 0, 0);
    return EBADPARM;
  }
  return SUCCESS;
}
                              ...
                              AEECallback *pUserCB)
{
  ...
  if (gsdi_sim_read(uim_file_item, GSDI_INDEX_ABSOLUTE, nRecordNum,
                    NULL, 0, &dummy_data_p, 0, 0, &dummy_rec_num_accessed, (uint32)Handler,
      OEMCardGSDIGenericcmdCB) != GSDI_SUCCESS)
  {
    OEMCard_CleanUpAsynObject(Handler);
    return EFAILED;
...
#error code not present
    ...
    case AEECARD_SAP_PIN:
      ...
      break;
    default:
      ERR("PIN id %d not supported", pin_id, 0, 0);
      return;
  }
...
int OEMCardModel_New(IShell *pIShell, AEECLSID cls, void **ppif)
{
   ...
   *ppif = NULL;
   ...
   if (ICardModel_obj == NULL)
   {
     ...
     if (!ICardModel_obj->m_pcbExit)
     {
       SysModelBase_Release(SYSMODELBASE_TO_IMODEL(&ICardModel_obj->base));
       return ENOMEMORY;
     }
...
int OEMCardConnection_New(IShell *pIShell, AEECLSID cls, void **ppif)
{
   ...
   *ppif = NULL;
   ...
   if (cls != AEECLSID_CARDCONNECTION) {
      return EUNSUPPORTED;
   }
...
SIDE EFFECTS
===========================================================================*/
static int OEMCardConnection_FindNextAvailRspSlot(uint32 *dwSeqNum,
                                            int     nSize,
                                            uint8 *byData,
                                            uint32  index)
{
  ...
  for (i = 0; i< OEMCARDCONNECTION_MAX_RSP; i++) {
    if (sCardChannelRspTable[i].sRsp == NULL) {
...
static void OEMCardConnectionGSDIcmdCB(gsdi_cnf_T *gsdi_info)
{
  ...
  if (bIsRelease) {
    if (gsdi_info->message_header.resp_type == GSDI_CLOSE_CHANNEL_RSP) {
      ...
      MSG_ERROR("Release for resp type 0x%x", gsdi_info->message_header.resp_type, 0, 0);
    }
    FREE( pClientData );
    pClientData = NULL;
#if !defined(FEATURE_ICARD_NO_UI_BASE) && defined(FEATURE_UIM)
  ...
  case GSDI_PERSO_CHANGE_DCK_RSP:
  case GSDI_PERSO_GET_DCK_NUM_RETRIES_RSP:
  ...
  case GSDI_ACTIVATE_ONCHIP_SIM_RSP:
  case GSDI_MAX_RSP:
    break;
  }
...
===========================================================================*/
static int OEMCard_MapPersoEnum(int nCardPersoEnum, gsdi_perso_enum_type *pGSDIPersoEnum)
{
  if (pGSDIPersoEnum == NULL) {
    return EBADPARM;
  }
   ...
   switch(gsdi_info->message_header.resp_type)
   {
     case GSDI_SIM_READ_RSP:
       ...
       if(gsdi_info->message_header.gsdi_status == GSDI_SUCCESS)
       {
         ...
         if (pFileContents != NULL)
         {
           FREEIF(pFileContents->data);
           pFileContents->data = NULL;
           FREE(pFileContents);
           pFileContents = NULL;
         }
       }
