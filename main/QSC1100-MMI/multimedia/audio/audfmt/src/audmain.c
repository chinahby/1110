...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/multimedia/audio/6k/ver1/audfmt/main/latest/src/audmain.c#5 $
...
audmain_status_type audmain_format_play_init (
  ...
  if(ret_val == AUDMAIN_STATUS_FAILURE || ret_val == AUDMAIN_STATUS_ERROR) {
    ...
    if(cmd_buf != NULL) {
      *cmd_buf = &(audmain_parse_ctl.current_buf);
    }
...
#error code not present
    ...
    else if( audmain_adpcm_status == AUDMAIN_STATUS_DATA_ERR )
    {
      /* do nothing as audmain_parse_ctl.type still would be unkown */
    }
    else
#endif /* FEATURE_WAVE_PB */
#ifdef FEATURE_AAC
