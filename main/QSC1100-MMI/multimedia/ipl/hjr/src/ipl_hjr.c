...
$Header: //source/qcom/qct/multimedia/ipl/hjr/main/latest/src/ipl_hjr.c#5 $
...
/*===========================================================================
===========================================================================*/
static uint16 hjr_get_scaler(uint32 first, uint32 second)
{
  uint16 scale_out;
  uint8 scale_first, scale_second, scale;
  uint32 size;
  ...
  scale = scale_first*2+scale_second+2;

  size= first*second;
  scale_out = (uint16)((1<<(scale+10))/size);
  scale_out= (uint16)((scale_out<<12)/first);
  return scale_out;
}
                      ...
                      uint32 *pass_corr)
{
  ...
  for (i = 0; i < (int32) (2*(int32)max_lag+1); i++)
  {
    ...
    for (j = 0; j < N; j++)
    {
      tmp32 = (*ptr1++)-(*ptr2++);
      tmp32 = tmp32>>1;
      tmp = tmp + ((tmp32*tmp32)>>scale); //lint !e704
    }
    corr[i] = tmp;
    ...
    int32 scaleQ1)
{
  ...
  if (maxStrips == 0)
  {
    return(hjr_add_frames(in1, in2, out, jitter, 0, scaleQ1));
  }
    ...
    boolean *pass_corr)
{
  ...
  uint32 passV,passH;
  uint16 hjr_dimension_scale_col, hjr_dimension_scale_row; 
  uint32 final_pass_q7;
  ...
  *jitter     = -hjr_corr1D(row_sum1, row_sum2, in1->dx, (int32) searchX, &passV);
  *(jitter+1) = -hjr_corr1D(col_sum1, col_sum2, in1->dy, (int32) searchY, &passH);
  ...
  hjr_dimension_scale_col = hjr_get_scaler (in1->dx, in1->dy);
  hjr_dimension_scale_row = hjr_get_scaler (in1->dy, in1->dx);
  final_pass_q7 = ((uint32) (passV * (uint32) hjr_dimension_scale_row) + 
                   (uint32) (passH * (uint32) hjr_dimension_scale_col)) >> 15;
  ...
  {
    *pass_corr = (final_pass_q7 < passDiff);
  }
...
{//lint !e508
  ...
  int strip;
  int maxStrips;
  uint8 row_sum_scale = 0;
  uint16 hjr_dimension_scale_row, hjr_dimension_scale_col; 
...
#define cleanUP_2() \
  ...
  numFrames = 0;
  while (in_img_array[numFrames] != NULL && numFrames <= HJR_MAX_IMAGES+1)
    numFrames++;
  ...
  if (strips == 1)
  {
    ...
    else
    {
      ...
      if (tmp == NULL)
      {
        IPL2_MSG_MED("### IPL HJR, not enough free memory to use strips.###\n", 0,0,0);
        strips = 0;
      }
      ipl_free(tmp);
    }
  ...
  if (strips)
  {
    ...
    for (i = 0; i < numFrames; i++)
    {
      ...
      {
        if (row_sum_scale >= 1)
          row_sum[i][j] = (row_sum[i][j] + (1 << (row_sum_scale - 1))) >> 
                          row_sum_scale; 
      }
      ...
      if (i > 0)
      {
        ...
        hjr_dimension_scale_col = hjr_get_scaler (dx, dy);
        hjr_dimension_scale_row = hjr_get_scaler (dy, dx);
        final_pass_q7 = ((uint32) (passV * (uint32) hjr_dimension_scale_row) + 
                         (uint32) (passH * (uint32) hjr_dimension_scale_col)) >> 15;
        ...
        {
        pass_corr[i-1] = (final_pass_q7 < passDiff);
        }
      }
      else
      {
    ...
    // free up the memory we dont need
    ...
    for (i = 1; i < numFrames; i++)
    {
      if (pass_corr[i-1])
      {
        ...
        IPL2_MSG_MED ("HJR Frame Registration: i = %d", i, 0, 0);
        ...
        if (retval == IPL_NO_MEMORY)
        {
          ...
          if (hjr_add_frames(out_img_ptr, in_img_array[i], out_img_ptr, 
              jitter[i], 0, (int32) blendWeights[i]) != IPL_SUCCESS)
            return IPL_FAILURE;
      ...
      {
        IPL2_MSG_MED ("HJR Frame Registration-frames too different: i = %d", i, 0, 0);
      }
    }
    // cleanup
  ...
  else
  {
    ...
    for (i = 1; i < numFrames; i++)
    {
      ...
      if (hjr_correlate_frames(in_img_array[0], in_img_array[i], 
          searchX, searchY, &jitter[i][0], &pass_corr[i-1]) != IPL_SUCCESS)
      return IPL_FAILURE;
    }
