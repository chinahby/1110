...
$Header: //source/qcom/qct/wconnect/bthost/brew/statapp/rel/00.00.26/src/BTAppOPP.c#5 $
$DateTime: 2009/04/17 12:08:59 $
$Author: ganeshs $
...
void BTApp_OPPBuildMainMenu( CBTApp* pMe )
{
  ...
  // set the title
  IMENUCTL_SetTitle( pMe->m_pIMenu, BTAPP_RES_FILE, IDS_OPP_TESTS, NULL );
...
void BTApp_OPPBuildServerMenu( CBTApp* pMe )
{
  ...
  {
    szStatus[ 3 ] = 'C';
    ISHELL_LoadResString( pMe->a.m_pIShell, BTAPP_RES_FILE, IDS_OPP_SERVER, 
  ...
  {
    IMENUCTL_SetTitle( pMe->m_pIMenu, BTAPP_RES_FILE, IDS_OPP_SERVER, NULL );
  }
...
void BTApp_OPPHandleUserEvents( CBTApp* pMe, uint32 dwParam )
{
  ...
  switch ( dwParam )
  {
    ...
    case EVT_OPP_PROG_INFO:
    {
      ...
      {
        MSG_HIGH("BTAppOPP: GetProgInfo num_bytes = %d, obj_size = %d",
                 progInfo.numBytes, progInfo.objSize, 0);
      }
      else
      {
