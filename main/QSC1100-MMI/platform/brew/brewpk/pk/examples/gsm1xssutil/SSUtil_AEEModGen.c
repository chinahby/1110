...
FILE: SSUtil_AEEModGen.c // singleton-change
...
#include "SSUtil_AEEModGen.h"  // singleton-change // Contains Sample AEEModule declarations
#include "SSUtil_AEEAppGen.h"  // singleton-change 
#include "SSUtil.bid" // singleton-change
...
uint32 SSUtilCls_Release(ISSUtilCls * po); // singleton-change
int SSUtilCls_New(AEECLSID clsID, 
                            IShell   *ps, 
                            IModule  *pIModule, 
                            ISSUtilCls **ppObj); // singleton-change
    ...
    int AEEMod_Load(IShell *pIShell, void *ph, IModule **ppMod)
{
   ...
   return SSUtil_AEEStaticMod_New(sizeof(AEEMod),pIShell,ph,ppMod,NULL,NULL);
}
...
==============================================================================*/
int SSUtil_AEEStaticMod_New(int16 nSize, IShell *pIShell, void *ph, IModule **ppMod,
                     PFNMODCREATEINST pfnMC,PFNFREEMODDATA pfnMF)
{
   ...
   *ppMod = NULL;
   ...
   pMe->pISSUtilCls = NULL; // singleton-change
   SSUtilCls_New(AEECLSID_SSUTIL, pIShell, (IModule*)pMe, 
                &(pMe->pISSUtilCls)); // singleton-change
