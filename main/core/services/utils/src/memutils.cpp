...
$Header: //depot/asic/msmshared/services/common/memutils.cpp#10 $ 
$DateTime: 2006/12/11 10:06:26 $ $Author: sleahy $
...
============================================================================== */
#if defined (FEATURE_MEMUTILS_MEMORY_TRACKING) || defined (FEATURE_MEMUTILS_HEAP_CACHE_ENHANCED_DEBUGGING)
void *HeapCacheGetBuffer(size_t size, const char *file, unsigned long line)
{
   return HeapCache::GetBuffer(size, file, line);
}
#else
void *HeapCacheGetBuffer(size_t size)
{
...
void HeapCacheLeakReport(void)
{
   ...
   sprintf(fname, "heapcacheleaks%d.txt", heapcache_report_num);
   ...
   if (fd != -1)
   {
      ...
      sprintf(str, "Net   - Memory usage minus heap cache overhead\r\n"); 
      sprintf(str, "Total - Memory usage including heap cache overhead\r\n"); 
      sprintf(str, "     Peak Count  Peak Bytes\r\n"); 
      efs_write(fd, str, strlen(str));
      sprintf(str, "Net:   %8ld    %8ld\r\n", 
         heap_cache_peak_count, heap_cache_peak_usage_net);
      efs_write(fd, str, strlen(str));
      sprintf(str, "Total: %8ld    %8ld\r\n", 
         heap_cache_peak_count, heap_cache_peak_usage_total);
      efs_write(fd, str, strlen(str));

      sprintf(str, "      Time\t      Addr\t Size\tFile                                     \t Line\r\n");
      efs_write(fd, str, strlen(str));
...
#endif
         {
            sprintf(str, "%10ld\t0x%08lx\t%5ld\t%-40s\t%5ld\r\n", 
...
#endif
      if (m_refCnt > 0 && bin < m_cacheBinCount)
      {
#ifdef FEATURE_MEMUTILS_HEAP_CACHE_ENHANCED_DEBUGGING
...
void HeapCache::UsageStatistics(void)
{
   ...
   heapcache_report_num++;
   sprintf(fname, "heapcacheusage%d.txt", heapcache_report_num);
   int fd = efs_open(fname, O_WRONLY | O_TRUNC | O_CREAT, S_IREAD | S_IWRITE);
   if (fd != -1)
   {
...
void memory_tracker_leak_report(void)
{
   ...
   memleaks_report_num++;
   sprintf(fname, "memleaks%d.txt", memleaks_report_num);
   int fd = efs_open(fname, O_WRONLY | O_TRUNC | O_CREAT, S_IREAD | S_IWRITE);
   if (fd != -1)
   {
      ...
      sprintf(str, "                  Usage Count  Usage Bytes  Peak Count   Peak Bytes\r\n"); 
      efs_write(fd, str, strlen(str));
      sprintf(str, "Starting Statistics: %8ld     %8ld    %8ld    %8ld\r\n", 
         memory_init_inst_count, memory_init_inst_usage, memory_init_peak_count, memory_init_peak_usage);
      efs_write(fd, str, strlen(str));
      sprintf(str, "Ending Statistics:   %8ld     %8ld    %8ld    %8ld\r\n", 
         memory_inst_count, memory_inst_usage, memory_peak_count, memory_peak_usage);
      efs_write(fd, str, strlen(str));
      sprintf(str, "  Num\t      Addr\t Size\t TID\tPri\tFile                                     \tLine\r\n");
      efs_write(fd, str, strlen(str));
      INTLOCK()
      ...
      while (mt)
      {
         sprintf(str, "%5ld\t0x%08lx\t%5ld\t%4ld\t%3ld\t%-40s\t%5d\r\n", 
...
# endif /* FEATURE_MEMUTILS_MEMORY_TRACKING */
