...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/data/1x/1xrlp/main/lite/src/dsrlp.c#4 $ $DateTime: 2010/04/19 00:33:20 $ $Author: nsivakum $

when       who        what, where, why
--------   ---        -------------------------------------------------------
03/31/10   sn         Fixed compilation errors.
...
/*===========================================================================
  FUNCTION   DSRLP_IS_HDR   

  DESCRIPTION   
    This function looks at whether the RLP associated with the particular
    sr_id has been initialized for HDR. This is needed since both 1X and HDR
    share the same control blocks
  
  DEPENDENCIES  
    None.

  RETURN VALUE
    TRUE:  If RLP has been initialized for HDR
    FALSE: Otherwise

  SIDE EFFECTS  None
===========================================================================*/
boolean dsrlp_is_hdr( uint8 sr_id )
{
  dsrlpi_sess_ctl_blk_type       *rscb_ptr;  /* rlp-specific control block */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(rscb_index >= DSRLPI_INVALID_RSCB_INDEX)
  {
    MSG_HIGH("dsrlp_is_hdr called with invalid sr_id",0,0,0);
    return FALSE;
  }
