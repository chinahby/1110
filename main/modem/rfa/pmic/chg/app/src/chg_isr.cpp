...
/* =======================================================================
  ...
  $Header: //source/qcom/qct/modem/rfa/pmic/chg_task/main/latest/app/src/chg_isr.cpp#11 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/23/09   hs      Stopped listening for USB valid/invalid IRQs.
                                                     ...
                                                     unsigned short        vmin)
{
    ...
    err = pm_vbatt_modify_client (chg_handle_lwr ,             \
                                  (pm_battery_voltage_type) vmin , \
                                   VBATT_VOLTAGE_DOWN , \
                                    chg_vbatt_lwr_thr_cb,\
                                     NULL ) ; 

    err |= pm_vbatt_modify_client (chg_handle_upr , \
                                    (pm_battery_voltage_type) vmax , \
                                     VBATT_VOLTAGE_UP , \
...
#ifdef FEATURE_VBATT_TASK
void IrqEventGenerator::VbattUpperThresholdCallback(void)
{
    /* Battery voltage above threshold. */
    mBatteryAlarmStatus = CHG_VBAT_TH_UPPER;
...
void IrqEventGenerator::VbattLowerThresholdCallback(void)
{
    /* Battery voltage below threshold. */
    mBatteryAlarmStatus = CHG_VBAT_TH_LOWER;
