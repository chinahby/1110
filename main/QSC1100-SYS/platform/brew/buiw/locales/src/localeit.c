...
// Format variables, strings, characters and patterns for Italian (Italy)
...
/* -----------------------------------------------------------------------
    Locale_Ctor 
   --- */
static void LocaleIT_Ctor(LocaleIT *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    Locale_New  
   --- */
int LocaleIT_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleIT *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleIT, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleIT_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule);
...
int LocaleIT_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALEIT) {
      return LocaleIT_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
