...
// Format variables, strings, characters and patterns for Hungarian (Hungary)
...
static const AECHAR Currency_NegativePattern []           = { '-','#',' ','F','t',0 }; 
static const AECHAR Currency_PositivePattern[]            = { '#',' ','F','t',0 }; 
static const AECHAR Currency_CurrencySymbol[]             = { 'F','t',0 }; 
...
static const AECHAR DateTime_AMDesignator[]               = { 'd','e','.',0 };
static const AECHAR DateTime_DateSeparator[]              = { '.',0 };
static const AECHAR DateTime_PMDesignator[]               = { 'd','u','.',0 };
...
static const AECHAR DateTime_LongDatePattern[]            = { 'y','y','y','y','.',' ','M','M','M','M',' ','d','.', 0 };
static const AECHAR DateTime_LongTimePattern[]            = { 'H',':','m','m',':','s','s',0 };
static const AECHAR DateTime_MonthDayPattern[]            = { 'd','.',' ','M','M','M',0 };
static const AECHAR DateTime_ShortDatePattern[]           = { 'y','y','y','y','.','M','M','.','d','.', 0 };
...
/* -----------------------------------------------------------------------
    LocaleHU_Ctor 
   --- */
static void LocaleHU_Ctor(LocaleHU *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleHU_New  
   --- */
int LocaleHU_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleHU *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleHU, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleHU_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule); 
...
int LocaleHU_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALEHU) {
      return LocaleHU_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
