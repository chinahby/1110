...
// Format variables, strings, characters and patterns for Swedish (Sweden)
...
static const AECHAR DateTime_LongDatePattern[]            = { '\'','d','e','n',' ','\'','d',' ','M','M','M','M',' ','y','y','y','y', 0 };
...
/* -----------------------------------------------------------------------
    LocaleSR_Ctor 
   --- */
static void LocaleSR_Ctor(LocaleSR *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleSR_New  
   --- */
int LocaleSR_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleSR *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleSR, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleSR_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule); 
...
int LocaleSR_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALESR) {
      return LocaleSR_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
