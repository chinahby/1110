...
// Format variables, strings, characters and patterns for Dutch (Netherlands)
...
/* -----------------------------------------------------------------------
    LocaleNL_Ctor 
   --- */
static void LocaleNL_Ctor(LocaleNL *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleNL_New  
   --- */
int LocaleNL_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleNL *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleNL, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleNL_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule);
...
int LocaleNL_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALENL) {
      return LocaleNL_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
