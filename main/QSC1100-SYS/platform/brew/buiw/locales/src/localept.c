...
// Format variables, strings, characters and patterns for Portuguese (Portugal)
...
/* -----------------------------------------------------------------------
    LocalePT_Ctor 
   --- */
static void LocalePT_Ctor(LocalePT *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocalePT_New  
   --- */
int LocalePT_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocalePT *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocalePT, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocalePT_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule);
...
int LocalePT_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALEPT) {
      return LocalePT_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
