...
// Format variables, strings, characters and patterns for Indonasian (Indonasia)
...
static const AECHAR Currency_NegativePattern []           = { '(','R','p','#',')',0 };
static const AECHAR Currency_PositivePattern[]            = { 'R','p','#',0 }; 
static const AECHAR Currency_CurrencySymbol[]             = { 'R','p',0 }; 
...
/* -----------------------------------------------------------------------
    LocaleID_Ctor 
   --- */
static void LocaleID_Ctor(LocaleID *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleID_New  
   --- */
int LocaleID_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleID *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleID, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleID_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule); 
...
int LocaleID_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALEID) {
      return LocaleID_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
