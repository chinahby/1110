...
static void tm_sctn128x128_reset(void)
{
     ...
     TM_SCTN_WRITE_CMD(TM_SCTN128x128_SOFT_RESET_C);
     clk_busy_wait(1000);	 
                                           ...
                                           uint32 end_row, uint32 end_col)
{
	...
	*/
	    ...
	    (end_row   < lptDispInfo[nDisp].height) )
	{
		...
		/* Transfer command to display hardware*/
		TM_SCTN_WRITE_CMD(TM_SCTN128x128_SET_START_ADDRESS_C);
		TM_SCTN_WRITE_DATA((uint8)start_col);
		TM_SCTN_WRITE_DATA((uint8)end_col);

		/* Set LCD hardware to set start address */
		/* Transfer command to display hardware */
		TM_SCTN_WRITE_CMD(TM_SCTN128x128_SET_END_ADDRESS_C);
...
static void tm_sctn128x128_disp_off(void)
{
	...
	if(tm_sctn128x128_state.disp_powered_up && tm_sctn128x128_state.display_on)
	{
		/* Add code here */
		/* Transfer command to display hardware */
		TM_SCTN_WRITE_CMD(TM_SCTN128x128_DISPLAY_OFF_C); 
		clk_busy_wait(1000);
...
static void tm_sctn128x128_disp_on(void)
{
	...
	{  
		/* Transfer command to display hardware */
		TM_SCTN_WRITE_CMD(TM_SCTN128x128_DISPLAY_ON_C); 
		clk_busy_wait(1000);
