...
Notice that changes are listed in reverse chronological order.
  ...
  $Header: //source/qcom/qct/multimedia/audio/6k/ver1/audfmt/main/latest/src/audmpeg.c#6 $
...
)
{
  ...
  /* copy the start of ID3 tag */
    mpeg_parse_ctl->id3v2_tag_start = id3v2_tag_length;
    ...
    id3_tag->v2_tag_length = id3v2_tag_length;
    MSG_LOW("id3v2_tag_length = %d",id3v2_tag_length,0,0);
    if(id3v2_tag_length) {
      if (id3v2_tag_length < mpeg_parse_ctl->file_len) {
        ...
        if (audmain_get_sync_data(handle, id3v2_tag_length, &buffer, &data_len)
            != AUDMAIN_STATUS_SUCCESS) {
          MSG_LOW("Got buffer position = %d",id3v2_tag_length,0,0);
          return(FALSE);
        }
        if (data_len == 0) {
      ...
      MSG_ERROR("Corrupt ID3V2 tag/taglength - Unable to play the file",0,0,0);
      return(FALSE);
    }
  }
  ...
  mpeg_parse_ctl->first_sync = id3v2_tag_length + sync_start;
