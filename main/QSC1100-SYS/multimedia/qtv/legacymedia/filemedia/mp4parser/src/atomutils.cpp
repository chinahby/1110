...
$Header: //source/qcom/qct/multimedia/qtv/legacymedia/filemedia/mp4parser/main/latest/src/atomutils.cpp#9 $
...
========================================================================== */
bool 
AtomUtils::readNullTerminatedString(OSCL_FILE *fp, OSCL_STRING &data)
{
  uint8 buf[256];
  int32 index = 0;
  ...
  OSCL_STRING temp((const OSCL_TCHAR *)outbuf);

#else
  // ZString is a char string - no conversion needed
  OSCL_STRING temp((const OSCL_TCHAR *)buf);
#endif
...
========================================================================== */
bool 
AtomUtils::readNullTerminatedUnicodeString(OSCL_FILE *fp, OSCL_STRING &data)
{
  ...
  *wptr = (uint16) (firstbyte << 8 | (uint16) secondbyte); 
  ...
  // String (wchar_t) buffer filled - now create OSCL_STRING

#ifdef UNICODE 
  // OSCL_STRING is a wchar_t string so no conversion needed
  OSCL_STRING temp((const OSCL_TCHAR *)buf);
  data = temp;
#else
  // OSCL_STRING is a char string - conversion needed
  uint8 outbuf[256];
  (void)UnicodeToUTF8(buf, index, (int8 *)outbuf, 256); 
...
========================================================================== */
bool 
AtomUtils::readNullTerminatedAsciiString(OSCL_FILE *fp, OSCL_STRING &data)
{
...
{
  int32 filePointer = OSCL_FileTell(fp);

  if ( filePointer >= AtomUtils::fileSize )
  {
    return 0;
  }
  ...
  (void)OSCL_FileSeek(fp,-8,SEEK_CUR);
...
int32 AtomUtils::getNextAtomSize(OSCL_FILE *fp)
{
  ...
  (void)OSCL_FileSeek(fp,-4,SEEK_CUR);
  return (int32)size;
}
...
uint32 AtomUtils::peekNextNthBytes(OSCL_FILE *fp, int32 n)
{
  ...
  (void)OSCL_FileSeek(fp, -4*n, SEEK_CUR);
  return tag;
}
...
uint8 AtomUtils::peekNextByte(OSCL_FILE *fp)
{
  ...
  (void)OSCL_FileSeek(fp, -1, SEEK_CUR);
...
AtomUtils::getNextUUIDAtomType(OSCL_FILE *fp)
{
  ...
  // Rewinding the stream back to atom start
  (void)OSCL_FileSeek(fp,-12,SEEK_CUR);
