...
/**********************************************************************************
  $Revision: #1 $
  Copyright 2005 Open Interface North America, Inc. All rights reserved.
***********************************************************************************/
...
typedef struct {
    ...
    OI_BYTE *           __data;     // Pointer to an array of bytes of size trueSize
...
{                                                                           \
    ...
    (ch) = (OI_UINT32) (bs).__data[(bs).__pos++];                           \
    if ((ch) >= 0xc0) {                                                     \
        ...
        (ch) &= ~mask;                                                      \
        while (count--) {                                                   \
            (ch) = ((ch) << 6) | ((bs).__data[(bs).__pos++] & 0x3f);        \
        }                                                                   \
    }                                                                       \
}
...
{                                                                                               \
    ...
    if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) {                                                     \
        (ch) = GetUINT16_BigEndian((bs).__data + (bs).__pos);                                   \
    } else {                                                                                    \
        (ch) = GetUINT16_LittleEndian((bs).__data + (bs).__pos);                                \
    }                                                                                           \
    (bs).__pos += 2;                                                                            \
    if ((0xd800 <= (ch)) && ((ch) <= 0xdbff)) {                                                 \
        ...
        OI_ASSERT(((bs).__pos + 2) <= (bs).__size);                                             \
        if ((bo) == OI_BIG_ENDIAN_BYTE_ORDER) {                                                 \
            (ch) |= (GetUINT16_BigEndian((bs).__data + (bs).__pos) & 0x3ff);                    \
        } else {                                                                                \
            (ch) |= (GetUINT16_LittleEndian((bs).__data + (bs).__pos) & 0x3ff);                 \
        }                                                                                       \
        (ch) += 0x10000;                                                                        \
        (bs).__pos += 2;                                                                        \
    }                                                                                           \
