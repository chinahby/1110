...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_device_efs_mnand.c#3 $ $DateTime: 2008/12/16 16:11:39 $ $Author: richardp $
...
fs_device_efs_mnand_open (uint32 which_device)
{
  ...
  hdev = hotplug_disable_dev_by_path (FS_DEVICE_EFS_MNAND_DEV_PATH);
  if (hdev == NULL)
  {
    return NULL;
  }

  result = hotplug_dev_reset (hdev);
  if (result != 0)
  {
    return NULL;
  }

  result = hotplug_dev_open (hdev);
  if (result != 0)
  {
    return NULL;
  }
...
fs_device_efs_mnand_close (efs_device_t base_dev)
{
  ...
  ASSERT (base_dev->priv.mnand_hdev != NULL);

  result = hotplug_dev_close (base_dev->priv.mnand_hdev);
  ASSERT (result == 0);
  base_dev->priv.mnand_hdev = NULL;
  return 0;
}
                                ...
                                uint32 iovec_cnt)
{
  ...
  ASSERT (base_dev->priv.mnand_hdev != NULL);
  result = hotplug_dev_read_iovec (base_dev->priv.mnand_hdev,
                                   iovec, iovec_cnt);
  if (result != 0)
    FS_ERR_FATAL ("hotplug_read_iovec() failed...",0,0,0);
                                 ...
                                 uint32 iovec_cnt)
{
  ...
  ASSERT (base_dev->priv.mnand_hdev != NULL);
  result = hotplug_dev_write_iovec (base_dev->priv.mnand_hdev, iovec,
                                    iovec_cnt, 1);
  if (result != 0)
    FS_ERR_FATAL ("hotplug_write_iovec() failed...",0,0,0);
                                  ...
                                  void *data, uint32 num_sectors)
{
  ...
  ASSERT (base_dev->priv.mnand_hdev != NULL);
  result = hotplug_dev_read (base_dev->priv.mnand_hdev, sect,
                                   ...
                                   void *data, uint32 num_sectors)
{
  ...
  ASSERT (base_dev->priv.mnand_hdev != NULL);
  result = hotplug_dev_write (base_dev->priv.mnand_hdev, sect,
                              data, (uint16)num_sectors);
  if (result != 0)
                            ...
                            fs_sector_id sect, void *data, uint32 num_sectors)
{
  ...
  ASSERT (base_dev->priv.mnand_hdev != NULL);
  result = hotplug_dev_reliable_write (base_dev->priv.mnand_hdev, sect,
                                       data, num_sectors);
  if (result != 0)
    FS_ERR_FATAL ("hotplug_reliable_write() failed...",0,0,0);
                               ...
                               void *data)
{
  ...
  ASSERT (base_dev->priv.mnand_hdev != NULL);
  result = hotplug_dev_read (base_dev->priv.mnand_hdev, page,
                                ...
                                void *data)
{
  ...
  ASSERT (base_dev->priv.mnand_hdev != NULL);
  result = hotplug_dev_write (base_dev->priv.mnand_hdev, page,
                              data, FS_MNAND_SECTORS_PER_CLUSTER);
  if (result != 0)
    FS_ERR_FATAL ("hotplug_dev_write() failed...",0,0,0);
