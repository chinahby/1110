...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/hfat/main/latest/src/hfat_port_f.c#1 $ $DateTime: 2008/05/29 19:19:11 $ $Author: richardp $
...
f_getdate (void)
{
  ...
  /*
   * Julian time API is calacuated with respect to 6 JAN 1980 and EPOC is 1st
   * Jan 1970
   */
  gmttime -= CDMA_TO_FS_TIME_DELTA;
  fs_time_jul_from_secs (gmttime, &julian);

  pcdate =
    (unsigned
     short) ((((julian.year -
                1980) << F_CDATE_YEAR_SHIFT) & F_CDATE_YEAR_MASK) | ((julian.month <<
                                                                      F_CDATE_MONTH_SHIFT)
                                                                     &
                                                                     F_CDATE_MONTH_MASK)
             | ((julian.day << F_CDATE_DAY_SHIFT) & F_CDATE_DAY_MASK));
  return pcdate;
}
...
f_gettime (void)
{
  ...
  pctime =
    (unsigned short) (((hour << F_CTIME_HOUR_SHIFT) & F_CTIME_HOUR_MASK) |
...
hfat_allocate_path_buffer (W_CHAR** buf_ptr)
{
  ...
  if (i >= MAX_HFAT_PATH_BUFFERS)
  {
    ...
    ERR_FATAL ("Out of hfat Path buffers", 0, 0, 0);
  }
...
hfat_free_path_buffer (struct hfat_path_buffer *buf)
{
  if (!buf || (buf->in_use == 0) )
  {
    ERR_FATAL ("Attempt to free bad hfat path buffer", 0, 0, 0);
  }
  buf->in_use = 0;
...
hfat_allocate_f_name_buffer (F_NAME** buf_ptr)
{
  ...
  {
    /* No unused buffers found -- bail out */
    ERR_FATAL ("Out of hfat f_name buffers", 0, 0, 0);
  }
...
hfat_free_f_name_buffer (struct hfat_f_name_buffer *buf)
{
  if (!buf || (buf->in_use == 0) )
  {
    ERR_FATAL ("Attempt to free bad hfat f_name buffer", 0, 0, 0);
  }
  buf->in_use = 0;
...
hfat_allocate_f_lfnint_buffer (F_LFNINT** buf_ptr)
{
  ...
  {
    /* No unused buffers found -- bail out */
    ERR_FATAL ("Out of hfat f_lfnint buffers", 0, 0, 0);
  }
...
hfat_free_f_lfnint_buffer (struct hfat_f_lfnint_buffer *buf)
{
  if (!buf || (buf->in_use == 0) )
  {
    ERR_FATAL ("Attempt to free bad hfat f_lfnint buffer", 0, 0, 0);
  }
  buf->in_use = 0;
