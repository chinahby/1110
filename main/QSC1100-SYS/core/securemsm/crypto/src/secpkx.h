...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/securemsm/crypto/main/latest/src/secpkx.h#2 $ 
  $DateTime: 2007/07/30 17:44:34 $ 
  $Author: hvimadal $
...
struct secpkx_instance_s
{
  struct __pkx_common_info_s
  {
    ...
    union
    {
      ...
      {
        uint8  base    [SECCRYPT_PKX_MAX_BASE_SIZE];
        uint8  modulus [SECCRYPT_PKX_MAX_MOD_SIZE];
        uint8  exponent[SECCRYPT_PKX_MAX_EXP_SIZE];
        uint8  result  [SECCRYPT_PKX_MAX_EXP_SIZE];		
		uint8  priv_key_iv[SECCRYPT_ENC_AES_IV_SIZE];
		uint8  priv_key_devkey[SECCRYPT_ENC_AES_IV_SIZE];
      } mod_exp;
      ...
      struct __pkx_dh_storage_s
      {
        ...
        uint8  my_private [SECCRYPT_PKX_MAX_EXP_SIZE];
        uint8  my_public  [SECCRYPT_PKX_MAX_MOD_SIZE];
        uint8  peer_public[SECCRYPT_PKX_MAX_MOD_SIZE];
        uint8  secret     [SECCRYPT_PKX_MAX_MOD_SIZE];
      } dh;
      ...
      {
        uint8  modulus    [SECCRYPT_PKX_MAX_MOD_SIZE];
        uint8  padded_data[SECCRYPT_PKX_MAX_BASE_SIZE];
        uint8  private_key[SECCRYPT_PKX_MAX_EXP_SIZE];
        uint8  public_key [SECCRYPT_PKX_MAX_EXP_SIZE];
        uint8  cipher_data[SECCRYPT_PKX_MAX_MOD_SIZE];
		uint8  priv_key_iv[SECCRYPT_ENC_AES_IV_SIZE];
		uint8  priv_key_devkey[SECCRYPT_ENC_AES_IV_SIZE];
	  } rsa;
  ...
  union
  {
    ...
    secpkxdsp_instance_type  dsp;  /* contains DME info */
...
FUNCTION SECPKX_LOG_MODMATH_STATS

DESCRIPTION
  This function logs the modmath stats.
