...
EXTERNALIZED FUNCTIONS
  ...
  void dsrlprsq_remove_chain(dsrlpi_sess_ctl_blk_type     *rscb_ptr,      
                             uint32                        max_chain_cnt 
                            );
...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/data/1x/1xrlp/main/lite/src/dsrlprsq.c#2 $ $DateTime: 2008/06/24 01:30:38 $ $Author: nsivakum $
...
---------------------------------------------------------------------------*/
#define DSRLPRSQ_MAX_OVERLAP_ITEMS   10
...
DESCRIPTION
  This macro will calculate the last sequence # + 1 of the range for the rlp 
  frame in hand. It will do this by adding the length of the frame to the
  very first sequence #  of that frame. It returns the calculated number. If
  the LST bit is set to 1 this is the last segmented frame and thus you need
  to calculate 1 past the range of sequence numbers used for the segment. 
...
PARAMETERS
  ...
  lseq:     To be SET to the last sequence number of the range.
  fr_len:   This is the length of the frame
  lst_bit:  This shows whether this is the last frame of a segment.
  seq_mod:  The modulus for the version of rlp
...
SIDE EFFECTS
  None
===========================================================================*/
#define DSRLPRSQ_GET_LAST_PLUS_SEQ_NUM(fseq, lseq, fr_len, lst_bit,seq_mod)\
{\
  DSRLPRSQ_GET_LAST_SEQ_NUM(fseq, lseq, fr_len, seq_mod);\
  if (lst_bit)\
  {\
    lseq = DSRLPSEQ_GET_NEXT_SEQ(lseq,seq_mod);\
  }\
  else\
  {\
    lseq = DSRLPSEQ_INCREMENT_32BIT_SEQ(lseq,seq_mod);\
  }\
}

...
===========================================================================*/
#define DSRLPRSQ_REESTABLISH_FAST_PTRS(rscb_ptr, reseq_q_ptr)\
{\
  rscb_ptr->state.reseq_last_chain_ptr =\
    (dsm_item_type *)q_last_check( reseq_q_ptr );\
  DSRLPRSQ_GET_LAST_CHAIN_ITEM(rscb_ptr->state.reseq_last_item_ptr,\
                               rscb_ptr->state.reseq_last_chain_ptr);\
}
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*-------------------------------------------------------------------------
    initialize the resequencing queue in the dsrlpi_state_info_type struct
  -------------------------------------------------------------------------*/
  q_ptr = q_init( &(rscb_ptr->state.reseq_q) );
  
  ASSERT( q_ptr == &(rscb_ptr->state.reseq_q));

  /*-------------------------------------------------------------------------
    initialize the boolean to show no data in the resequencing queue 
  -------------------------------------------------------------------------*/
  rscb_ptr->state.reseq_q_more_data = FALSE;

  rscb_ptr->state.reseq_last_chain_ptr = NULL;
  rscb_ptr->state.reseq_last_item_ptr  = NULL;

  rscb_ptr->state.reseq_last_insert_chain_ptr = NULL;
  rscb_ptr->state.reseq_last_insert_item_ptr  = NULL;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  reseq_q_ptr = &(rscb_ptr->state.reseq_q);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  -------------------------------------------------------------------------*/
  reseq_q_ptr     = &(rscb_ptr->state.reseq_q);/* pointer to the reseq Q   */
  seq_mod         =  (rscb_ptr->ver_ptr)->seq_modulus;
...
#error code not present
  ...
  -------------------------------------------------------------------------*/
  if ((rscb_ptr->state.reseq_last_chain_ptr     == 
       rscb_ptr->state.reseq_last_insert_chain_ptr ) &&
      (rscb_ptr->state.reseq_last_chain_ptr != NULL)
     ) 
  {
    MSG_HIGH("Fast pointers pointing to same item found. RLP: %d", 
             rscb_ptr->sr_id, 0, 0);
    rscb_ptr->state.reseq_last_insert_chain_ptr = NULL;
    rscb_ptr->state.reseq_last_insert_item_ptr  = NULL;
  }
  ...
  if ( (walk_item_ptr = q_check(reseq_q_ptr)) != NULL) 
  {
     if ( rscb_ptr->state.reseq_last_chain_ptr != 
          (dsm_item_type *)q_last_check( reseq_q_ptr ))
     {
       DSRLPRSQ_REESTABLISH_FAST_PTRS(rscb_ptr, reseq_q_ptr);
       ERR("Fast pointers found not pointing to last element.", 0, 0, 0);
     }
  }
  ...
  -------------------------------------------------------------------------*/
  needed_seq_num  = rscb_ptr->state.needed_seq_num;

  /*-------------------------------------------------------------------------
    read the LV(R), it will help with optimized fast insert
  -------------------------------------------------------------------------*/
  expected_seq_num= rscb_ptr->state.expected_seq_num;
  ...
  -------------------------------------------------------------------------*/
  new_item.seq_num= fr_ptr->app_field = frame_info_ptr->seq_num;   
  ...
  -------------------------------------------------------------------------*/
  DSRLPRSQ_GET_LAST_PLUS_SEQ_NUM(new_item.seq_num,
                                 new_item.last_plus_seq_num,
                                 new_item.length,
                                 new_item.lst_set,
                                 seq_mod); 
  ...
  if (DSRLPSEQ_GT_SEQ_CHECK(needed_seq_num, new_item.last_seq_num, seq_mod))
  {
    ...
    -----------------------------------------------------------------------*/
    ERR("LV(N)= %d > than last seq.= %d. Frame discarded ", 
                                 needed_seq_num, new_item.last_seq_num, 0);
  ...
  if ( rsq_item_count == 0 )
  {
    ...
    -----------------------------------------------------------------------*/
    ASSERT(rscb_ptr->state.reseq_last_item_ptr == NULL);
    ASSERT(rscb_ptr->state.reseq_last_chain_ptr== NULL);
    ASSERT(rscb_ptr->state.reseq_last_insert_item_ptr == NULL);
    ASSERT(rscb_ptr->state.reseq_last_insert_chain_ptr== NULL);
    ...
    rscb_ptr->state.reseq_last_item_ptr  = last_item_ptr;
    rscb_ptr->state.reseq_last_chain_ptr = new_item.ptr;
    
#ifdef FEATURE_DSM_MEM_CHK
    new_item.ptr->tracer = DSM_RLP_RESEQ;
  ...
  else
  {
    ...
    ASSERT( rscb_ptr->state.reseq_last_chain_ptr != NULL);
    ASSERT( rscb_ptr->state.reseq_last_item_ptr  != NULL);
    ASSERT( (rscb_ptr->state.reseq_last_item_ptr)->pkt_ptr == NULL);
    ...
    -------------------------------------------------------------------------*/
    fast_item.ptr     = rscb_ptr->state.reseq_last_chain_ptr; 
    ...
    DSRLPRSQ_GET_LAST_PLUS_SEQ_NUM( fast_item.seq_num, 
                                    fast_item.last_plus_seq_num,
                                    fast_item.length,
                                    fast_item.lst_set,
                                    seq_mod
                                  );
    ...
    if(DSRLPSEQ_GT_SEQ_CHECK(expected_seq_num, fast_item.last_seq_num,seq_mod))
    {
         ...
         ) 
      {
        ...
        -------------------------------------------------------------------*/
        (rscb_ptr->state.reseq_last_item_ptr)->pkt_ptr = new_item.ptr;
        rscb_ptr->state.reseq_last_item_ptr            = last_item_ptr;
    ...
    else if (rscb_ptr->state.reseq_last_insert_chain_ptr != NULL) 
    {
      /*-------------------------------------------------------------------------
      ...
      -----------------------------------------------------------------------*/
      fast_item.ptr     = rscb_ptr->state.reseq_last_insert_chain_ptr; 
      fast_item.seq_num = fast_item.ptr->app_field;
      fast_item.lst_set = DSRLPRSQ_GET_LST_STATUS(fast_item.ptr);
      fast_item.length  = dsm_length_packet(fast_item.ptr);
        ...
        )
      {
        ...
        /*-------------------------------------------------------------------
          This is the fast APPEND
        -------------------------------------------------------------------*/
          (rscb_ptr->state.reseq_last_insert_item_ptr)->pkt_ptr = new_item.ptr;
           rscb_ptr->state.reseq_last_insert_item_ptr           = last_item_ptr;
        ...
        else 
        {
          ...
          -------------------------------------------------------------------*/
          rscb_ptr->state.reseq_last_insert_item_ptr  = last_item_ptr;
          rscb_ptr->state.reseq_last_insert_chain_ptr = new_item.ptr;
...
SIDE EFFECTS   NONE   
===========================================================================*/
dsm_item_type *dsrlprsq_fast_walk(
  dsrlpi_sess_ctl_blk_type   *rscb_ptr,      /* curr RLP session ctl blk    */
...
)
{
  ...
  -------------------------------------------------------------------------*/
  if ((rscb_ptr->state.reseq_last_insert_chain_ptr != NULL) &&
      (DSRLPSEQ_GT_SEQ_CHECK(
          new_item_ptr->seq_num, 
		  DSRLPRSQ_GET_SEQ_NUM(rscb_ptr->state.reseq_last_insert_chain_ptr), 
		  seq_mod)
      )
     )
  {
    walk_item_ptr = rscb_ptr->state.reseq_last_insert_chain_ptr;   
  }
  else
  {
    /*-------------------------------------------------------------------------
...
)
{
  ...
  if (DSRLPSEQ_GT_SEQ_CHECK(needed_seq_num, new_item.seq_num, seq_mod)) 
  {
    ...
    dsm_pullup( &(new_item.ptr), NULL, pullup_bytes);
    ASSERT(new_item.ptr != NULL);
  
    DSRLPRSQ_ADD_32BIT_SEQ( (new_item.ptr)->app_field, 
                             pullup_bytes, 
                             seq_mod); 

    new_item.seq_num = (new_item.ptr)->app_field;
  ...
  (void) dsrlprsq_fast_walk
              (rscb_ptr, &new_item, &first_item, &last_item);
  ...
  if ((truelast_item.found) && (truelast_distance > 1)) 
  {
    ...
    for ( i=0; i < truelast_distance-1; i++)
    {
      ...
      ---------------------------------------------------------------------*/
      if(overlap_items.overlap_arr[i] ==rscb_ptr->state.reseq_last_chain_ptr) 
      {
        rscb_ptr->state.reseq_last_item_ptr  = NULL;
        rscb_ptr->state.reseq_last_chain_ptr = NULL;
        establish_ptrs                       = TRUE;
      ...
      if(overlap_items.overlap_arr[i] ==rscb_ptr->state.reseq_last_insert_chain_ptr) 
      {
        rscb_ptr->state.reseq_last_insert_item_ptr  = NULL;
        rscb_ptr->state.reseq_last_insert_chain_ptr = NULL;
      }/* if */
    ...
    do 
    {
      ...
      if (delete_item_ptr != NULL)
      {
        /*-------------------------------------------------------------------
          ...
          pointer is pointing to. If we are, set the pointer as NULL
        -------------------------------------------------------------------*/
        if (delete_item_ptr == rscb_ptr->state.reseq_last_chain_ptr) 
        {
          rscb_ptr->state.reseq_last_item_ptr  = NULL;
          rscb_ptr->state.reseq_last_chain_ptr = NULL;
          establish_ptrs                       = TRUE;
        ...
        if (delete_item_ptr == rscb_ptr->state.reseq_last_insert_chain_ptr) 
        {
          rscb_ptr->state.reseq_last_insert_item_ptr  = NULL;
          rscb_ptr->state.reseq_last_insert_chain_ptr = NULL;
        }/* if */
  ...
  if (first_item.found) 
  {
    ...
    DSRLPRSQ_GET_LAST_PLUS_SEQ_NUM( first_item.seq_num,
                                    first_item.last_plus_seq_num,
                                    first_item.length,
                                    first_item.lst_set,
                                    seq_mod); 
    ...
    {                     /* we have an overlap, so calculate some numbers */
      ...
      ---------------------------------------------------------------------*/
      ASSERT(first_item.overlap_bytes < first_item.length);
    }
  } 
  ...
  if (last_item.found) 
  {
    ...
    DSRLPRSQ_GET_LAST_PLUS_SEQ_NUM( last_item.seq_num,
                                    last_item.last_plus_seq_num,
                                    last_item.length,
                                    last_item.lst_set,
                                    seq_mod);     
  ...
  if (truelast_item.found) 
  {
    ...
    DSRLPRSQ_GET_LAST_PLUS_SEQ_NUM( truelast_item.seq_num,
                                    truelast_item.last_plus_seq_num,
                                    truelast_item.length,
                                    truelast_item.lst_set,
                                    seq_mod);     
  ...
  if (first_item.found) 
  {
    ...
    if (first_item.overlap_bytes > 0)
    {
      ...
      else 
      {
        ...
        -------------------------------------------------------------*/
        pullup_bytes = dsm_pullup
          ( &(new_item.ptr), NULL, (word)first_item.overlap_bytes);
    ...
    if (truelast_item.overlap_bytes > 0)
    {
      ...
      else 
      {
        ...
        /*-------------------------------------------------------------
          ...
          it has to be CHOP new frame from the head with dsm_pullup()
        --------------------------------------------------------------*/
  ...
  if (last_item.found && (last_item.ptr!=truelast_item.ptr) && !discard_flag) 
  {
    ...
    if (last_item.overlap_bytes > 0)
    {
      ...
      else 
      {
        ...
        -------------------------------------------------------------------*/
        ASSERT(last_item.overlap_bytes <= new_item.length);
  ...
  { 
    ...
    if (first_item.found && !last_item.found) 
    {
      ...
      ---------------------------------------------------------------------*/
      ASSERT( first_item.ptr == rscb_ptr->state.reseq_last_chain_ptr);
    ...
    else if (last_item.found && (!first_item.found)) 
    {
          ...
          (!new_item.lst_set))
      {
        ...
        if (temp_ptr != NULL) 
        {
          if( rscb_ptr->state.reseq_last_insert_chain_ptr == last_item.ptr)
          {
            /*---------------------------------------------------------------
    ...
    else 
    {
      ...
      ASSERT(first_item.ptr   != NULL);
      ASSERT(last_item.ptr    != NULL);
      ASSERT(last_item.found  == TRUE);
      ASSERT(first_item.found == TRUE);
    ...
    if (first_item.replace) 
    {
      ...
      if (first_item.ptr == rscb_ptr->state.reseq_last_chain_ptr) 
      {
        establish_ptrs = TRUE;
      }
      
      if (first_item.ptr == rscb_ptr->state.reseq_last_insert_chain_ptr) 
      {
        rscb_ptr->state.reseq_last_insert_chain_ptr = NULL;
        rscb_ptr->state.reseq_last_insert_item_ptr  = NULL;
      }
    ...
    { 
      ...
      if (last_item.ptr == rscb_ptr->state.reseq_last_chain_ptr) 
      {
        establish_ptrs = TRUE;
      }
      
      if (last_item.ptr == rscb_ptr->state.reseq_last_insert_chain_ptr) 
      {
        rscb_ptr->state.reseq_last_insert_chain_ptr = NULL;
        rscb_ptr->state.reseq_last_insert_item_ptr  = NULL;
      }
    ...
    if (truelast_item.replace)  
    {
      ...
      if (truelast_item.ptr == rscb_ptr->state.reseq_last_chain_ptr) 
      {
        establish_ptrs = TRUE;
      }
      
      if (truelast_item.ptr == rscb_ptr->state.reseq_last_insert_chain_ptr) 
      {
         rscb_ptr->state.reseq_last_insert_chain_ptr = NULL;
         rscb_ptr->state.reseq_last_insert_item_ptr  = NULL;
      }
...
)
{
  ...
  -------------------------------------------------------------------------*/
  ASSERT( rscb_ptr       != NULL );
  ASSERT( frame_info_ptr != NULL );
  ASSERT( fr_ptr         != NULL );

  if (dsrlprsq_fast_insert(rscb_ptr, frame_info_ptr, fr_ptr)) 
  {
    return;
  }
  else
  {
    ...
    -------------------------------------------------------------------------*/
    dsrlprsq_regular_insert(rscb_ptr, frame_info_ptr, fr_ptr);
  }
}
#endif /* MSM5000_IRAM_FWD */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  do 
  {
    ...
    if (DSRLPSEQ_GE_SEQ_CHECK(needed_seq_num, walk_seq_num, seq_mod))
    {
      ...
      ---------------------------------------------------------------------*/
      if (insert_item_ptr == rscb_ptr->state.reseq_last_insert_chain_ptr) 
      {
        rscb_ptr->state.reseq_last_insert_chain_ptr = NULL;
        rscb_ptr->state.reseq_last_insert_item_ptr  = NULL;
      }
      ...
      rscb_ptr->stats.rx_total_bytes += dsm_length_packet(insert_item_ptr);
...
#error code not present
      ...
      (LVN_exceeded  == FALSE))
  {
    ...
    if ( DSRLPSEQ_GT_SEQ_CHECK(needed_seq_num, walk_seq_num, seq_mod))
    {
      ...
      ---------------------------------------------------------------------*/
      rscb_ptr->state.reseq_q_more_data = TRUE;
    }
  } 
  else 
  {    
