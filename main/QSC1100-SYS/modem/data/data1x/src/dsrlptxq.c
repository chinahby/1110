...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/data/1x/1xrlp/main/lite/src/dsrlptxq.c#3 $ $DateTime: 2009/02/01 22:28:06 $ $Author: ssaha $
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*-------------------------------------------------------------------------
    initialize the Transmit queue in the dsrlpi_state_info_type struct
  -------------------------------------------------------------------------*/
  q_ptr = q_init( &(rscb_ptr->state.tx_q) );
  
  ASSERT( q_ptr == &(rscb_ptr->state.tx_q));

  rscb_ptr->state.tx_q_recent = NULL;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- -*/
  ...
  if ( q_items >= rscb_ptr->ver_ptr->txq_size ) 
  {
    /*-----------------------------------------------------------------------
    ...
    if (free_item_ptr != NULL)
    {
      ...
      if (free_item_ptr == rscb_ptr->state.tx_q_recent)
      {
        rscb_ptr->state.tx_q_recent = NULL;
      }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- -*/
  ...
  do
  {
                             ...
                             ))
    {
      ...
      payload_len = dsm_length_packet(walk_item_ptr);
      ...
      {
        ERR("Transmitted a zero length rlp frame?",0,0,0);
        end_seq_num = seq_num;
      }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- -*/
  ...
  -------------------------------------------------------------------------*/
  if (rscb_ptr->state.tx_q_recent != NULL)
  {
    if (DSRLPSEQ_GE_SEQ_CHECK(req_seq_num,
                              RLPTXQ_GET_SEQ(rscb_ptr->state.tx_q_recent->app_field),
                              rscb_ptr->ver_ptr->seq_modulus))
    {
      walk_item_ptr = rscb_ptr->state.tx_q_recent;
    }
  }
  ...
  do 
  {
    ...
    if ( next_item_ptr == NULL )
    {
      ...
      {
        ERR("Accessing a zero length rlp frame?",0,0,0);
  ...
  MSG_LOW("TXQ Examined = %d RLP: %d",examined,rscb_ptr->sr_id,0);
  ...
  {
    rscb_ptr->state.tx_q_recent = fr_info_ptr->fr_ptr;
  }
