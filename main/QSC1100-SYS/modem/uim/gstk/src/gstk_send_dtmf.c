...
$Header: //source/qcom/qct/modem/uim/gstk/rel/07H1_2/src/gstk_send_dtmf.c#1 $
...
gstk_status_enum_type gstk_send_dtmf_req  (gstk_proactive_cmd_type* command_ptr)
{
  ...
  MSG_HIGH(" IN GSTK_SEND_DTMF_REQ ",0,0,0);
  ...
  */
  ...
  if(!error_already_raised) {
    ...
    ERR("send send dtmf parsing err", 0, 0, 0);
    /* Send Terminal Response to Card */
  ...
  gstk_send_dtmf_cnf_type* send_dtmf_cnf_ptr )
{
  ...
  MSG_HIGH("IN GSTK send dtmf cnf ",0,0,0);

  if(send_dtmf_cnf_ptr == NULL)
  {
    MSG_ERROR("send_dtmf_cnf_ptr ERR:NULL",0,0,0);
    return GSTK_BAD_PARAM;
  }
      ...
      if (uim_cmd_ptr != NULL) {
        ...
        if(STK_response.data.result.result_tag_length > 1 ) { /* has additional info */
          gstk_byte_offset_memcpy(uim_cmd_ptr->terminal_response.data,
                            ...
                            UIM_MAX_CHARS,
                            (send_dtmf_cnf_ptr->result_additional_info.length * 
                             sizeof(byte)));
          offset = (int)(offset + STK_response.data.result.result_tag_length - 1);
        }

        MSG_MED("DTMF: TR 0x%x bytes", uim_cmd_ptr->terminal_response.num_bytes, 0, 0);
