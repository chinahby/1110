...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/otaspx.c#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $
      ...
      ** --------------------------- */
      switch (int_ptr->dload_req.block[i].block_id)
      {
        ...
        case OTASP_CDMA_ANALOG_NAM_BLK:
          ...
          for (j = 0; j < int_ptr->dload_req.block[i].blk.ftc_cell.n_sid_nid;
                 j++)
          {
        ...
        case OTASP_CDMA_NAM_BLK:
          ...
          for (j = 0; j < int_ptr->dload_req.block[i].blk.ftc_pcs.n_sid_nid;
               j++)
          {
            /* Unpack system identification */
        ...
        case OTASP_PREF_ROAM_LIST_BLK:
          ...
          ** ------------------------------------------------------------- */
          ...
          if ((block_pos % 8) != 0)
          {
            ERR( "Unexpected offset while unpacking Roaming List",0,0,0);
          }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (i = 0 ; i < (int_ptr->ms_key_req.param_p_len/sizeof(dword)); i++)
  {
  ...
  for (i = 0 ; i < (int_ptr->ms_key_req.param_g_len / sizeof(dword)) ; i++)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (i = 0; i < int_ptr->prot_cap_req.num_cap_records; i++) 
  {
       ...
       ** ------------------------------- */
...
#endif /* (!defined( FEATURE_NSOTASP) || defined( FEATURE_ACP )) */
          break;
        ...
        ** --------------------------------------- */
        case OTASP_PREF_ROAM_LIST_SIZE_BLK:
          ...
          /* Pack max list size */
          b_packw( int_ptr->config_rsp.block[i].blk.roam_siz.max_pr_list_size,
                   otasp_ptr, buffer_pos,
                   FSIZ( otaspe_roam_siz_type, max_pr_list_size ));
          buffer_pos += FSIZ( otaspe_roam_siz_type, max_pr_list_size );

          /* Pack current list size */
          b_packw( int_ptr->config_rsp.block[i].blk.roam_siz.cur_pr_list_size,
                   otasp_ptr, buffer_pos,
                   FSIZ( otaspe_roam_siz_type, cur_pr_list_size ));
          buffer_pos += FSIZ( otaspe_roam_siz_type, cur_pr_list_size );

          /* Pack number of Acquisition Records */
          b_packb( int_ptr->config_rsp.block[i].blk.roam_siz.num_acq_recs,
                   otasp_ptr, buffer_pos,
                   FSIZ( otaspe_roam_siz_type, num_acq_recs ));
          buffer_pos += FSIZ( otaspe_roam_siz_type, num_acq_recs );

          /* Pack number of System Records */
          b_packb( int_ptr->config_rsp.block[i].blk.roam_siz.num_sys_recs,
                   otasp_ptr, buffer_pos,
                   FSIZ( otaspe_roam_siz_type, num_sys_recs ));
          buffer_pos += FSIZ( otaspe_roam_siz_type, num_sys_recs );
#endif /* FEATURE_NSOTASP */
          break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (int_ptr->cap_rsp.msg_type == OTASP_EXT_PROT_CAP_RSP_MSG) 
  {
    ...
    for (i=0; i < int_ptr->cap_rsp.num_cap_records; i++)
    {
      ...
      default:
        MSG_ERROR("Unsupported cap rec type %d",
                  int_ptr->cap_rsp.cap_rec[i].cap_record_type,0,0);
        break;
        ...
        ** ------------------------------------- */
        ...
        case OTASP_CFG_EXT_PRL_DIMENSIONS_BLK:
          ...
          /*lint +e641*/
          {
            /* Pack the reserved bits  */
            /*lint -e734*/
            b_packb(int_ptr->sspr_cfg_rsp.block.blk.prl_size.num_comm_subnet_recs, 
