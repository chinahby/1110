...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/wconnect/bthost/pf/rel/00.00.26/src/btpf.c#3 $
  $DateTime: 2009/04/17 12:03:36 $
  $Author: ganeshs $
  ...
  switch (status_ptr->spp_state)
  {
    ...
    case BT_SPP_ST_OPEN: /* only applicable to server */
      ...
      else if (bt_pf_sio_table[i].state == BT_PF_SIO_SRV_NO_CONNECT)
      {
        ...
        /* notify OI_RFCOMM_RegisterServer to return OK */
        rex_set_sigs( bt_pf_app_tcb_ptr, BT_PF_WAIT_SIG );
      }
      break;
    case BT_SPP_ST_OPEN_ERROR:
      ...
      else if (bt_pf_sio_table[i].state == BT_PF_SIO_SRV_NO_CONNECT)
      {
        ...
        /* notify OI_RFCOMM_RegisterServer to return failure */
        rex_set_sigs( bt_pf_app_tcb_ptr, BT_PF_WAIT_SIG );
      }
      break;
  }
...
#error code not present
  ...
  /* pend on signal, with timeout, indicating response received
     (status change callback received) */
  rex_timed_wait( BT_PF_WAIT_SIG | BT_PF_TMO_SIG, &bt_pf_tmo_timer, 100 );
  rex_clr_timer( &bt_pf_tmo_timer );
  ...
  else
  {
    ...
    /* SPP open error or timeout */
    rex_clr_sigs( rex_self(), BT_PF_TMO_SIG );
    return(OI_RFCOMM_OUT_OF_SERVER_CHANNELS);
  }
}
