...
$Header: //source/qcom/qct/wconnect/bthost/brew/statapp/rel/00.00.26/src/BTApp.c#4 $
...
#define BTAPP_AG_CLRBIT( x ) ( pMe->mAG.uFlags &= ~(x) )
#define BTAPP_AG_SETBIT( x ) ( pMe->mAG.uFlags |= (x) )
...
static void BTApp_HandleCallSetup( CBTApp* pMe );
          ...
          case NMASK_BT_A2DP:
            ...
            switch ( evt )
            {
              ...
              case AEEBT_A2DP_EVT_SUSPEND:
                ...
                {
                  /* inband ringtone was interrupted before,
                   * let's rock again
                   */
...
static void BTApp_BuildBondMenu( CBTApp* pMe )
{
   ...
   else
   {
     ...
     ISHELL_CreateDialog( pMe->a.m_pIShell, BTAPP_RES_FILE, 
...
#error code not present
    ...
    case BT_APP_MENU_PASSKEY:
      ...
      {
        MSG_LOW( "Local HC LMPVersion = %d",
                                    pMe->mRM.myInfo.uLMPVersion, 0, 0 );
...
void BTApp_EnableBT( CBTApp* pMe )
{
  ...
  switch(pMe->mEnablingType)
  {
    ...
    {
      if ( BTApp_EnableOPP(pMe, &bSettingBondable, &bSettingDiscoverable) 
                           == TRUE)
      {
        break;
      }
    ...
    {
      if( BTApp_EnableFTP(pMe, &bSettingBondable, &bSettingDiscoverable)
                          == TRUE)
      {
        break;
      }
    ...
    {
      if( BTApp_EnableBIP( pMe, &bSettingBondable, &bSettingDiscoverable )
                           == TRUE)
      {
        break;
      }
    ...
    {
      if ( BTApp_EnableBPP( pMe, &bSettingBondable ,&bSettingDiscoverable )
                           == TRUE)
      {
        break;
      }
    ...
    {
      if( BTApp_EnablePBAP(pMe, &bSettingBondable, &bSettingDiscoverable) 
                            == TRUE )
      {
        break;
      }
...
static void BTApp_BuildListMenu( CBTApp* pMe, BTAppMenuType menu )
{
  ...
  if ( val != -1 )
  {
    ...
    pImage = ISHELL_LoadResImage( pMe->a.m_pIShell, BTAPP_RES_FILE, IDB_UP_DOWN );
...
static void BTApp_BuildVRCapableMenu( CBTApp* pMe )
{
  ...
  BTApp_AddMenuItem( pMe, pMe->m_pIMenu, &ai, IDS_YES, 
                     pMe->mAG.bVREnabled ? 
                     IDB_RADIO_FILLED : IDB_RADIO_UNFILLED );
  BTApp_AddMenuItem( pMe, pMe->m_pIMenu, &ai, IDS_NO,
                     pMe->mAG.bVREnabled ? 
                     IDB_RADIO_UNFILLED : IDB_RADIO_FILLED );
...
static void BTApp_BuildPrompt( CBTApp* pMe, BTAppMenuType menu )
{
  ...
  // Add individual entries to the Menu
  BTApp_AddMenuItem( pMe, pMe->m_pISoftMenu, &ai, IDS_NO, 0 );
  BTApp_AddMenuItem( pMe, pMe->m_pISoftMenu, &ai, IDS_YES, 0 );
...
{  
  ...
  {    
    ...
    MSG_MED( "BldDevRespMenu - nameLen=%d, uValue1=%d, bEIRReqName=%d",
             len, pDev->uValue1, bEIRReqName );
...
static void BTApp_BuildDevOptions( CBTApp* pMe )
{
  ...
  // form the title
  ISHELL_LoadResString( pMe->a.m_pIShell, BTAPP_RES_FILE, IDS_LABEL_OPTIONS, 
...
static void BTApp_BuildSearchRespScreen( CBTApp* pMe )
{
  ...
  // label
  ISHELL_LoadResString( pMe->a.m_pIShell, BTAPP_RES_FILE, IDS_SRCH_SVC_RESULTS, 
                        &pMe->pText1[ uLen], 
                        (LONG_TEXT_BUF_LEN - uLen) * sizeof(AECHAR) );
  uLen = WSTRLEN( pMe->pText1 );
  pMe->pText1[ uLen++ ] = (AECHAR)(unsigned char) ('\n');
...
static void BTApp_BuildSvcSecMenu( CBTApp* pMe )
{
  ...
  switch( pMe->mAG.srvSecType )
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_AUDIO_GATEWAY, BTAPP_RES_FILE,
...
#error code not present
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_NETWORK_ACCESS, BTAPP_RES_FILE,
...
#error code not present
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_ENABLE_OPP, BTAPP_RES_FILE,
...
#error code not present
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_ENABLE_FTP, BTAPP_RES_FILE,
...
#error code not present
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_ENABLE_BIP, BTAPP_RES_FILE,
...
#error code not present
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_ENABLE_BPP, BTAPP_RES_FILE,
...
#error code not present
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_ENABLE_PBAP, BTAPP_RES_FILE,
...
/*
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_ENABLE_OBEX, BTAPP_RES_FILE,
...
#error code not present
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_ENABLE_A2DP, BTAPP_RES_FILE,
...
#error code not present
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_ENABLE_AVRCP, BTAPP_RES_FILE,
...
#error code not present
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_ENABLE_L2, BTAPP_RES_FILE,
...
#error code not present
  ...
  {
    case AEEBT_RM_SVC_SEC_HIGH:
      IMENUCTL_SetItemText( pMe->m_pIMenu,IDS_ENABLE_SPP, BTAPP_RES_FILE,
...
static boolean BTApp_ConnectAudio( CBTApp* pMe, boolean bForceUnmute )
{
  ...
  if ( IS_AG_CONNECTABLE() != FALSE )
  {
    ...
    if ( pMe->mAG.bAudioConnected == TRUE )
    {
      ...
      {
        MSG_LOW( "AG - Setup PCM path, fum=%d", bForceUnmute, 0, 0 );
        bDone = TRUE;
      }
      else
      {
...
static boolean BTApp_DisconnectAudio( CBTApp* pMe, boolean bForceUnmute )
{
  ...
  {
    MSG_LOW( "AG - Clear PCM path, fum=%d", bForceUnmute, 0, 0 );
...
/* ==========================================================================
FUNCTION BTApp_HandleCallSetup
DESCRIPTION
============================================================================= */
static void BTApp_HandleCallSetup( CBTApp* pMe )
{
  ...
  MSG_HIGH( "AG - HndlCallSetup c=%x i=%x", callPresent, callIncoming, 0 );
  ...
  else
  {
    ...
    if ( (pMe->mAG.devType == AEEBT_AG_AUDIO_DEVICE_HEADSET) &&
         ((callPresent != BT_APP_CALL_NONE) ||
          BTAPP_AG_ISBITSET( BTAPP_AG_FLAG_IGNORE_BUTTON_AT_ACL_B )) )
    {
      MSG_LOW( "HndlCallSetup - start ignoring button press", 0, 0, 0 );
...
#error code not present
    ...
    case AEEBT_RM_EVT_BONDABLE_MODE:   // response to SetBondable()
    {
      ...
      if ( pData->uError == AEEBT_RM_ERR_NONE )
      {
        ...
        {
           BTApp_ShowMessage( pMe, pMe->mRM.bBondable ? IDS_MSG_BONDABLE_SET : 
               IDS_MSG_BONDABLE_CLEARED, NULL, 2 );
        }
      }
      else
      {
        msgID = pMe->mRM.bBondable ? IDS_MSG_BONDABLE_NOT_CLEARED : 
                  IDS_MSG_BONDABLE_NOT_SET;
        BTApp_ShowMessage( pMe, msgID, NULL, 2 );
      }
      break;
    }
...
#error code not present
    ...
    case AEEBT_RM_EVT_DEVICE_UPDATED:  // response to DeviceUpdate()
      MSG_MED( "RM - DevUpdated err=%d bm=0x%x", pData->pDevUpdateStatus->error,
               pData->pDevUpdateStatus->updateBitmap, 0 );
      break;
    ...
    {
      MSG_MED( "RM - AuthReq sim=%d", 
               pData->pAuthorizeReq->serviceId.serviceIdMethod, 0, 0 );
      if ( (pData->pAuthorizeReq->serviceId.serviceIdMethod ==
            pMe->mRM.svcId.serviceIdMethod) &&
