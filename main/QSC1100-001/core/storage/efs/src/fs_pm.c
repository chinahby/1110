...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_pm.c#11 $ $DateTime: 2009/01/29 17:35:24 $ $Author: richardp $
 ...
 * EFS_PAGE_SIZE - HEADERS_SIZE - LOG_PAD => EFS_PAGE_SIZE - 8 - 4
...
static void gc_shred_update (fs_pm_flash_t gc);
...
/* Aligned buffers that will be hooked up with the member-pointers in
   the above union fs_ptable_combined_data. */
static FS_ALIGN uint32 \
  fs_ptable_data_nand_pcache_data[FS_MAX_CACHED_PTABLE_PAGES][FS_PTE_PER_PAGE];
static FS_ALIGN uint8 fs_gc_data_realloc_buffer[EFS_PAGE_SIZE];
static FS_ALIGN struct superblock_data super_block_data;
...
static void
fs_ptable_init_aligned_buff ( void )
{
  int i;
  ...
  for (i=0; i<FS_MAX_CACHED_PTABLE_PAGES; ++i) {
    ptable_pool[0].nand.pcache_data[i] = \
      fs_ptable_data_nand_pcache_data[i];
    ASSERT (FS_OKAY_FOR_DM (ptable_pool[0].nand.pcache_data[i]));
  }

  ptable_pool[0].nand.ptable_gc.realloc_buffer = fs_gc_data_realloc_buffer;
  ASSERT (FS_OKAY_FOR_DM (ptable_pool[0].nand.ptable_gc.realloc_buffer));

  ptable_pool[0].nand.parent.super.data = &super_block_data;
  ASSERT (FS_OKAY_FOR_DM (ptable_pool[0].nand.parent.super.data));
}

/* Register with paranoia all the data that paranoia should watch. */
void
fs_ptable_register_with_paranoia (void)
{
  fs_paranoia_register (fs_ptable_data_nand_pcache_data,
    sizeof (fs_ptable_data_nand_pcache_data), "ptable_data_nand_pcache_data");

  fs_paranoia_register (fs_gc_data_realloc_buffer,
    sizeof (fs_gc_data_realloc_buffer), "fs_gc_data_realloc_buffer");

  fs_paranoia_register (&super_block_data, sizeof (super_block_data),
    "super_block_data");
}
    ...
    fs_logr_t           logr)
{
  ...
  fs_ptable_init_aligned_buff ();

  /* Make sure that the data-buffer is aligned as per DMA's expectations. */
  ASSERT (FS_OKAY_FOR_DM (pt->super.data));
...
/* Search for the erased blocks from a given starting block till gc_next
 * and queue these into erased block list. If the region of search has
 * more erased blocks than the queue can hold, it stops searching.
 */
static void
...
fs_pm_log_page_write_fail_handler (void *priv_data, fs_page_id failed_pg)
{
   ...
   * are here.
   */
  if (recurse_cnt++ > FS_MAX_CONSECUTIVE_LOG_WRITE_FAIL-1)
  {
    ...
    efs_device_t   dev)
{
  int fresh_start;
  fs_block_id block;
  fs_page_id tmp_page, tmp_stop;
  fs_page_id page_state;
  fs_page_id ptmp;
  int result;
  uint32 args[3];
  ...
  /* Make sure that size of superblock_data is equal to page size. This check
   * is needed to validate the pound define of SUPERBLOCK_PAGE_ENTRIES
   */
  ASSERT (sizeof(struct superblock_data) == EFS_PAGE_SIZE);
  ...
  if (fresh_start) {
    ...
    {
      /* For NAND it would be easy, our erased block array already
       * has how many blocks are erased. */
      gc->free_count = fs_pm_get_erased_blk_count () *
                          gc->ptable->super.data->block_size;
  ...
  /* fs_pm_do_urgent call above would have topped off the log so
   * its ok to flush now. */
  fs_log_flush (&gc->log);
    ...
    case FS_LOG_ENTRY_GARBAGE:
      ...
      if (page >= gc->min_page && page < gc->max_page) {
        gc->ptable->set_reverse (gc->ptable, page, FS_RMAP_PAGE_GARBAGE,
                                 FS_PM_NULL_TAG);
        /* For NAND moving a wrong allocator would cause conflicts,
         * so only move the allocator in case of NOR */
        if (FS_ISNOR (&gc->ptable->super))
  ...
  while (1) {
    switch (gc->state) {
      case FS_PM_HEAD:
        ...
        /* Update the shredding routine that gc has some changes. */
        gc_shred_update (gc);
        ...
        switch (pstate) {
          ...
          case FS_RMAP_BLOCK_BAD:
            ...
            if (gc->bad_block_acc > gc->max_bad_blocks_allowed)
              FS_ERR_FATAL ("Number of bad blocks allowed exceeded %d < %d",
...
/* Update shred routine with the updated value of gc_next so that shredding
 * can be stopped if its about to cross its stop point. Do not update the
 * state since this gc_next crossing the stop point can happen in the normal
 * GC process outside shred routine. In that case we stop shredding from
 * the shredding routine. */
static void
gc_shred_update (fs_pm_flash_t gc)
{
 ...
 * -1 : Shreding is not supported.
 */
static int
fs_pm_shred (fs_pm_t gen_gc)
{
  fs_pm_flash_t gc = (fs_pm_flash_t) gen_gc;
  int log_pages = fs_pm_top_off_log(gc);
  fs_page_id temp_shred_stop;
  ...
  {
    /* Determine the shred stop point. Do not assign it to shred variables yet
     * since it might stop shredding even before it started. */
     temp_shred_stop = gc->gc_next & gc->ptable->super.block_mask;
  ...
  gc->ptable->do_gc (gc->ptable);

  /* Update trans_start_free_count to keep NOR incremental GC informed */
  gc->trans_start_free_count = gc->free_count + log_pages;
...
fs_pm_space_limit (fs_pm_t gen_gc)
{
  ...
  if (FS_ISNOR (&gc->ptable->super))
  {
    ...
    size = (size * 217) >> 8;   /* and use 85% of what's left */
  }
  else
  {
  ...
  if( gc->erase_state == FS_PM_ERASE_BUSY)
  {
    ...
    if(fs_device_erase_status (gc->dev) == FS_DEVICE_DONE)
    {
      ...
      if(! finish_erase )
      {
        ...
        status = fs_device_suspend_erase( gc->dev );
