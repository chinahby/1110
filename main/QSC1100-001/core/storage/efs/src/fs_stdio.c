 ...
 * Copyright (C) 2004--2007, 2009, Qualcomm, Inc.
 */
 ...
 *
 *  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_stdio.c#2 $ $DateTime: 2009/01/30 16:38:40 $ $Author: richardp $
 *
...
/* The block, chunk and buffer sizes below must all be powers of two. There
 * is code in this module that makes this assumption.
 */

/* EFS_BLK_SIZE: filesystem block size (not device block size) */
#define EFS_BLK_SIZE           EFS_PAGE_SIZE

/* EFS_CHUNK_SIZE: best size for EFS writes                    */
#define EFS_CHUNK_SIZE        FS_MAX_TRANSACTION

/* EFS_BUF_SIZE: default buffer size                           */
#define EFS_BUF_SIZE          EFS_CHUNK_SIZE
...
struct _efs_file_io {
  ...
  struct rex_tcb_struct *caller_tcb; /* TCB to signal after buffer flush. */
  unsigned long caller_sigs;  /* signals to set after buffer flush.       */
  struct rex_tcb_struct *writer_tcb; /* TCB to signal to flush buffer.    */
...
#define EFS_BUF_COPY(buf, data, start, count, bufsize) \
  do { \
    ...
    if ((start) + (count) > (bufsize)) { \
      memcpy ((buf) + (start), (char *) (data), (bufsize) - (start)); \
      memcpy ((buf), (char *) (data) + ((bufsize) - (start)), \
        (count) - ((bufsize) - (start))); \
    } else { \
      memcpy ((buf) + (start), (char *) (data), (count)); \
    } \
  } while (0)
...
efs_fwrite (void *ptr, fs_size_t size, fs_size_t nitems, EFS_FILE *stream)
{
  ...
  if (is_async_stream && stream->caller_tcb != rex_self ()) {
    /* XXX: Is this the right return code? */
    efs_errno = EPERM;
    return 0;
  }
  ...
  if (is_async_stream)
    rex_clr_sigs (stream->caller_tcb, stream->caller_sigs);
  ...
  while (offset < full_size) {
    ...
    if (EFS_BUF_FULL (stream->head, tail, stream->bufsize)) {
      if (is_async_stream) {
        rex_set_sigs (stream->writer_tcb, stream->writer_sigs);
        while (EFS_BUF_FULL (stream->head, tail, stream->bufsize)) {
          rex_wait (stream->caller_sigs);
          rex_clr_sigs (stream->caller_tcb, stream->caller_sigs);
          tail = stream->tail;
          if (stream->error)
            break;
...
efs_fseek (EFS_FILE *stream, long offset, int whence)
{
  ...
  if (result == (fs_off_t) -1) {
    ...
    stream->pos = result;
    stream->head = stream->tail = stream->pos & (EFS_BLK_SIZE-1);
    return 0;
  }
}
...
flush_write_buffer (EFS_FILE *stream)
{
  ...
  while (head != tail) {
    ...
    if (count > EFS_CHUNK_SIZE)
      count = EFS_CHUNK_SIZE;
    if (count > 0) {
    ...
    if (is_async_stream)
      rex_set_sigs (stream->caller_tcb, stream->caller_sigs);
  }
...
efs_setvbuf (EFS_FILE *stream, char *buf, int mode, fs_size_t size)
{
   ...
   * of two.
   */
  if ((size > EFS_MAX_BUF_SIZE) || (size < EFS_BLK_SIZE) ||
      ((size & (size - 1)) != 0)) {
    efs_errno = EINVAL;
    return -1;
  }
 ...
 */
int
efs_set_async_params (EFS_FILE *stream, struct rex_tcb_struct *caller_tcb,
  unsigned long caller_sigs, struct rex_tcb_struct *writer_tcb,
...
{
  uint32 dog_autokick;
  ASSERT (async_write_enabled (stream));
