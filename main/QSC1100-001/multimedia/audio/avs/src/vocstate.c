...
$Header: //source/qcom/qct/multimedia/audio/6k/ver1/avs/main/latest/src/vocstate.c#75 $ $DateTime: 2009/05/31 23:34:43 $ $Author: kmodak $
          ...
          switch (voc_state_data.module_status) {
            ...
            case VOC_STATE_MOD_STATUS_ENABLED:
              ...
              if (qtv_notify_func_ptr != NULL){
                MSG_LOW("Notifying QTV abt mod_disable evt",0,0,0);
...
{
  qdsp_cmd_status_type           qdsp_ret[7];
...
if(voc_state_set_vp3_parms() != VOC_STATE_SUCCESS){
  MSG_HIGH("VP3 params set: FAILED",0,0,0);
}
  ...
  switch (voc_state_data.current_image)
  {
#if defined(QDSP_IMAGE_AMR_WB_DEFINED) || defined(QDSP_IMAGE_4GV_WB_DEFINED)
#ifdef QDSP_IMAGE_AMR_WB_DEFINED
    case QDSP_IMAGE_AMR_WB:
#endif
#ifdef QDSP_IMAGE_4GV_WB_DEFINED
    case QDSP_IMAGE_4GV_WB:
      ...
      rx_iir_filter_ptr = voc_data_get_rx_iir_filter(5);
      break;
/*IF FEATURE_IIR_FILTER_5S is not defined then it will use the 3 Stage*/
#endif /*FEATURE_IIR_FILTER_5S*/
#endif /*QDSP_IMAGE_AMR_WB_DEFINED  || QDSP_IMAGE_4GV_WB_DEFINED */

    default:
      rx_iir_filter_ptr = voc_data_get_rx_iir_filter(3);
      break;
  }
#endif /* FEATURE_IIR_FILTER */
...
#endif
#ifdef FEATURE_IIR_FILTER_5S
        tx_iir_filter_ptr = voc_data_get_tx_iir_filter(5);
        break;
/*IF FEATURE_IIR_FILTER_5S is not defined then it will use the 3 Stage*/
#endif /*FEATURE_IIR_FILTER_5S*/
#endif /*QDSP_IMAGE_AMR_WB_DEFINED || QDSP_IMAGE_4GV_WB_DEFINED*/
      ...
      default:
        ...
        tx_iir_filter_ptr = voc_data_get_tx_iir_filter(5);
#else
        tx_iir_filter_ptr = voc_data_get_tx_iir_filter(3);
#endif
        break;
    }
...
#ifdef FEATURE_AVS_CRYSTAL_SPEECH
    if(qdsp_cmd_get_ec_gen_id()== QDSP_CMD_EC_GEN_CRYSTAL_SPEECH)
      	tx_second_mic_iir_filter_ptr = voc_data_get_tx_second_mic_iir_filter(5);
#endif /*FEATURE_AVS_CRYSTAL_SPEECH*/

#endif /* FEATURE_IIR_FILTER */
  }
  pre_ec_tx_pcm_filter = voc_data_get_pre_ec_tx_pcm_filter();
  qdsp_ret[0] = qdsp_cmd_set_pre_ec_tx_pcm_filter(pre_ec_tx_pcm_filter);
  if ( qdsp_ret[0] != QDSP_CMD_SUCCESS )
  {
     ERR("Pre EC PCM filter set command failed",0,0,0);
  }

  qdsp_ret[1] = qdsp_cmd_set_rx_pcm_filter(rx_filter_ptr);
...
#ifdef FEATURE_AVS_CRYSTAL_SPEECH
  if(tx_second_mic_filter_ptr != NULL)
  {
    qdsp_ret[2] = qdsp_cmd_set_tx_pcm_secondmic_filter(tx_second_mic_filter_ptr);
    if ( qdsp_ret[2] != QDSP_CMD_SUCCESS )
    {
      ERR("Tx PCM secondMIC filter load command failed",0,0,0);
    }
  }
#endif /* FEATURE_AVS_CRYSTAL_SPEECH */

  qdsp_ret[3] = qdsp_cmd_set_tx_pcm_filter(tx_filter_ptr);
...
#ifdef FEATURE_IIR_FILTER
  qdsp_ret[4] = qdsp_cmd_set_rx_pcm_iir_filter(rx_iir_filter_ptr);
  if ( qdsp_ret[4] != QDSP_CMD_SUCCESS )
  {
  ...
  {
    qdsp_ret[5] = qdsp_cmd_set_tx_pcm_secondmic_iir_filter(tx_second_mic_iir_filter_ptr);
    if ( qdsp_ret[5] != QDSP_CMD_SUCCESS )
    {
      ERR("Tx PCM secondMIC IIR filter load command failed",0,0,0);
    }
  }
#endif /* FEATURE_AVS_CRYSTAL_SPEECH */

  qdsp_ret[6] = qdsp_cmd_set_tx_pcm_iir_filter(tx_iir_filter_ptr);
  if ( qdsp_ret[6] != QDSP_CMD_SUCCESS )
  {
     ERR("Tx PCM IIR filter load command failed",0,0,0);
  }
#endif /* FEATURE_IIR_FILTER */
...
void voc_inform (voc_inform_type inform_val)
{
  ...
  */
  ...
  /*Inform clients if the module required by them cannot be fetched on current image*/
  if(inform_val == VOC_INFORM_MODULE_UNFETCHED)
  {
	  ...
	  for(; key_index < VOC_MAX_ACQUIRE_TYPE ; ++key_index)
	  {
			...
			)
		  {
			  ...
			  state->config_pending = VOC_CAP_NONE;
		  }
	  }
	  return;
  }
...
void voc_state_adie_set_final (
  ...
  */
  ...
  if (voc_check_aux_line_in() == TRUE) {
    ...
    if (codec == VOC_CODEC_STEREO_HEADSET) {
  ...
  if (qdsp_cmd_codec_offset_inuse() == TRUE)
  {
    clk_busy_wait(5000);    /* wait for 5ms for PA saturated with PCM offset */
  }
    ...
    */
	module_select_status = voc_state_get_high_module(&module_select);
      ...
      return(VOC_STATE_CONTINUE);
	  	...
	  	case VOC_CMD_BUSY_S:
	  		{
	  			MSG_HIGH("Module = %d Not Supported on Present Image. Continuing with next best match",module_select,0,0);
    ...
    if (voc_state_data.module_reqd == module_select) {
      ...
      if(voc_state_get_service_cfg_required()!=VOC_SERVICE_MAX )
      {
        if(voc_state_get_service_cfg_required() == voc_data_get_service_option()){
          MSG_HIGH("service config is NOT changed",0,0,0);
          return (VOC_STATE_CONTINUE);
        }
      ...
      MSG_HIGH("service config changed",0,0,0);
      go_to_sleep = TRUE;
    }
  }
