...
$Header: //depot/asic/msmshared/apps/StaticExtensions/OEM/Src/OEMSTK.c#44 $
...
#define OEMSTK_MAX_CACHE_ENTRIES 6
#define OEMSTK_MAX_BT_CAUSE      2
...
static gstk_profile_dl_support_type value_list[] = 
{
  {1, GSTK_MENU_SEL_SUPPORT}, 
  {1, GSTK_CC_SUPPORT} ,
  {1, GSTK_MO_SMS_CONTROL_SUPPORT}, 
  {0, GSTK_UCS2_ENTRY_SUPPORT}, 
  {0, GSTK_UCS2_DISPLAY_SUPPORT}, 
  {1, GSTK_DISPLAY_TEXT_SUPPORT}, 
  {1, GSTK_GET_INKEY_SUPPORT}, 
  {1, GSTK_GET_INPUT_SUPPORT}, 
  {1, GSTK_PLAY_TONE_SUPPORT}, 
  {1, GSTK_SELECT_ITEM_SUPPORT},

  {1, GSTK_SEND_SHORT_MSG_SUPPORT}, 
  {1, GSTK_SEND_SS_SUPPORT}, 
  {1, GSTK_SEND_USSD_SUPPORT}, 
  {1, GSTK_SET_UP_CALL_SUPPORT}, 
  {1, GSTK_SET_UP_MENU_SUPPORT}, 
  {1, GSTK_USER_ACT_EVT_SUPPORT}, 
  {1, GSTK_IDLE_SCRN_AVAIL_EVT_SUPPORT}, 
  {1, GSTK_LANG_SEL_EVT_SUPPORT}, 
  {1, GSTK_BROWSER_TERM_EVT_SUPPORT}, 
  {1, GSTK_SET_UP_IDLE_MODE_TEXT_SUPPORT}, 
  
  {1, GSTK_LANGUAGE_NOTIFICATION_SUPPORT}, 
  {1, GSTK_LAUNCH_BROWSER_SUPPORT}, 
  {1, GSTK_SK_SUPPORT_IN_SEL_ITEM}, 
  {1, GSTK_SK_SUPPORT_IN_SET_UP_MENU}, 
  {7, GSTK_MAX_SK_SUPPORT}, 
  {0, GSTK_CSD_SUPPORT}, 
  {0, GSTK_GPRS_SUPPORT}, 
  {8, GSTK_NUM_CHAR_SUPPORT_DOWN_ME}, 
  {0, GSTK_SCREEN_SIZE_SUPPORT}, 
  {12,GSTK_NUM_CHAR_SUPPORT_ACROSS_ME}, 
  
  {0, GSTK_VARIABLE_FONT_SIZE_SUPPORT}, 
  {0, GSTK_DISPLAY_RESIZED_SUPPORT}, 
  {1, GSTK_TEXT_WRAPPING_SUPPORT}, 
  {1, GSTK_TEXT_SCROLLING_SUPPORT}, 
  {0, GSTK_WIDTH_REDUCE_IN_MENU_SUPPORT}, 
  {0, GSTK_TCP_SUPPORT}, 
  {0, GSTK_UDP_SUPPORT}, 
  {1, GSTK_PROVIDE_LOCAL_INFO_LANG_SUPPORT}, 
  {1, GSTK_CELL_BROADCAST_DATA_DL_SUPPORT}, 
  {1, GSTK_REFRESH_SUPPORT},

  {0, GSTK_CARD_READER_ST_EVT_SUPPORT}, 
  {1, GSTK_DATA_AVAIL_EVT_SUPPORT}, 
  {1, GSTK_CH_STATUS_EVT_SUPPORT}, 
  {0, GSTK_POWER_ON_CARD_SUPPORT}, 
  {0, GSTK_PERFORM_CARD_APDU_SUPPORT}, 
  {0, GSTK_GET_READER_ST_SUPPORT}, 
  {0, GSTK_GET_READER_ST_CARD_ID_SUPPORT}, 
  {0, GSTK_RUN_AT_CMD_SUPPORT}, 
  {1, GSTK_SEND_DTMF_SUPPORT}, 
  {1, GSTK_OPEN_CH_SUPPORT}, 
  
  {1, GSTK_CLOSE_CH_SUPPORT}, 
  {1, GSTK_RECEIVE_DATA_SUPPORT}, 
  {1, GSTK_SEND_DATA_SUPPORT}, 
  {1, GSTK_GET_CH_STATUS_SUPPORT}, 
  {0, GSTK_NUM_CH_SUPPORT}, 
  {1, GSTK_CDMA_SEND_SMS_SUPPORT}, 
  {1, GSTK_CDMA_SMS_PP_SUPPORT}, 
  {14,GSTK_PROTOCOL_VERSION}, 
#ifdef FEATURE_CAT_REL6
  {1, GSTK_DISPLAY_TXT_VARIABLE_TIMEOUT_SUPPORT},
  {1, GSTK_INKEY_VARIABLE_TIMEOUT_SUPPORT},
  {1, GSTK_INKEY_HELP_FOR_VARIABLE_TIMEOUT_AND_IMMEDIATE_DIGIT_RSP},
  {1, GSTK_PROVIDE_LOCAL_INFO_BATTERY_STATE_SUPPORT},
...
int OEMSTK_New(IShell *pIShell, OEMSTK **ppif)
{
  ...
  *ppif = NULL;
  ...
  if (gpSTK == NULL)
  {
    ...
    gpSTK->m_pcbExit =(AEECallback*) sys_malloc(sizeof(AEECallback));
    if (!gpSTK->m_pcbExit)
    {
      *ppif = NULL;
      sys_free(gpSTK);
                                                   ...
                                                   AEESTKString *pOut)
{
  ...
  if((pIn->length > 0) && (pIn->text != NULL))
  {
    if (pIn->dcs == 0x08)
    {
      ...
      switch(pIn->text[0]) {
      case 0x80:
        (void)gstk_decode_h80_usc2_alphabet(orig_alpha_len, &pIn->text[1], pOut->pwszText);
        ...
        break;
      case 0x81:
        (void)gstk_decode_h81_usc2_alphabet(orig_alpha_len, &pIn->text[1], pOut->pwszText);
        MSG_MED("============ Raw Data", 0, 0, 0);
        for (i = 0; i< orig_alpha_len; i++) {
          MSG_MED("orig_cmd_data[%d]: %d", i, pIn->text[i], 0);
        }
        ...
        break;
      case 0x82:
        (void)gstk_decode_h82_usc2_alphabet(orig_alpha_len, &pIn->text[1], pOut->pwszText);
        MSG_MED("============ Raw Data", 0, 0, 0);
        for (i = 0; i< orig_alpha_len; i++) {
          MSG_MED("orig_cmd_data[%d]: %d", i, pIn->text[i], 0);
        }
...
)
{
  ...
  case AEESTK_UCS2_ALPHABET:
    (void)MEMCPY(pOut->text,pIn->pString.pwszText,pIn->pString.nLength *sizeof(AECHAR));
...
SIDE EFFECTS
===========================================================================*/
static void OEMSTK_DCSSpc_AEEStringToGSTKString(AEESTKString *pIn,
                                                gstk_string_type *pOut)
{
  uint32 i = 0;
  ...
  {
   (void)MEMCPY(pOut->text,pIn->pwszText,pIn->nLength);
    pOut->length = uint32touint8(pIn->nLength);
    pOut->dcs = AEESTK_UCS2_DCS;
  }
}
  ...
  AEESTKAdditionalInfo *pIn)
{
  ...
  */
  ...
  (void)MEMSET(pOut->additional_info_ptr, 0x00, sizeof(AECHAR)*(pIn->nLength));
                        ...
                        int32 sz)
{
  ...
  case AEESTK_GET_INPUT_REQ:
    {
      AEESTKString *pString = (AEESTKString*) pIn;
      OEMSTK_DCSSpc_AEEStringToGSTKString(pString,
...
void OEMSTKGSTKevtCb(gstk_cmd_from_card_type * req_rsp )
{
  ...
  if(OEMSTK_IsValidEnvelopeCommand(req_rsp))
  {
    ...
    OEMSTK_MUTEX_LOCK();
    /* The last one is reserved for Proactive Command */
    for(i = 0; i < (OEMSTK_MAX_CACHE_ENTRIES-1); i++)
    {
    ...
    if(dwTransactionID == OEMSTK_MAX_CACHE_ENTRIES-1)
    {
  ...
  else if(OEMSTK_IsValidProactiveCommand(req_rsp))
  {
    ...
    if ((gpSTK->cmdCache[dwTransactionID].state != OEMSTK_CACHE_STATE_FREE) && 
        ((gpSTK->cmdCache[dwTransactionID].state != OEMSTK_CACHE_STATE_DISPATCHED) || 
        ((gpSTK->cmdCache[dwTransactionID].commandID != AEESTK_END_PROACTIVE_CMD_SES) && 
        (gpSTK->cmdCache[dwTransactionID].commandID != AEESTK_PROACTIVE_CMD_SES_WITHOUT_TR)))) {     
      //Recovery.. App is not responding ... start removing the current entries.
...
===========================================================================*/
void OEMSTK_GetNextCmd(OEMSTK *pMe, uint32 *pCommandType, uint32 *pdwTransactionID)
{
  uint32 i;
  if((pMe == NULL) || (pCommandType == NULL) || (pdwTransactionID == NULL) )
  {
    MSG_ERROR("Bad parameters", 0, 0, 0);
    return;
  }
        ...
        gstk_string_type *str_data) 
{
  ...
  {
    OEMSTK_DCSSpc_AEEStringToGSTKString(data,str_data);
  }
  FREEIF(tmp_str);
}
/*===========================================================================
