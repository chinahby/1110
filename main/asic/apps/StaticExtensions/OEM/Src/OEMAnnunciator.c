...
// ÓÃÓÚ±£´æ´ý»ú½çÃæÇ½Ö½¶¥²ãÍ¼±êÀ¸¸²¸Ç²¿·ÖÇ½Ö½µÄÍ¼Æ¬Êý¾Ý
...
static int ClearField (IAnnunciator *pMe, uint32 nAnnunID)
{
    ...
    if (pMe->m_coreObj->m_bAnnunciatorOn)
    {// Çå³ýÏÔÊ¾É«±¸ÉÏµÄÍ¼±ê--´ËÊ±ÐèÅÐ¶ÏÏÔÊ¾Éè±¸Í¼±ê×´Ì¬
        if (need_capture.b_capture == DB_CAPTURE_INIDLE)
        {
            // ´ý»ú½çÃæÏÂÒÔ¿½±´ÏÂÀ´µÄÇ½Ö½Êý¾Ý¸üÐÂÏÔÊ¾Éè±¸
...
#endif
            // ´ËÊ±Ó¦ÓÃÌî³äÉ«£¬¶ø·Ç±³¾°É«, ÇÒÌî³äµÄÊÇÏÔÊ¾Éè±¸
            IDISPLAY_FillRect(pMe->m_coreObj->m_piDisplay, &Rect, nBgColor);
            IDISPLAY_Update(pMe->m_coreObj->m_piDisplay);
        }
    }
    
    // ÒÔÍ¼±ê²ã±³¾°É«Çå³ýÍ¼±ê²ãÎ»Í¼ÏàÓ¦Î»ÖÃµÄÍ¼±êÊý¾Ý--´ËÊ±ÎÞÐèÅÐ¶ÏÏÔÊ¾ÉèÅäÍ¼±ê×´Ì¬£¬²Ù×÷±ØÐë½øÐÐ
