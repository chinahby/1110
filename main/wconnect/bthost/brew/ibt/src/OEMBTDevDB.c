...
$Header: //source/qcom/qct/wconnect/bthost/brew/ibt/rel/00.00.26/src/OEMBTDevDB.c#6 $ 
$DateTime: 2009/07/31 12:27:00 $
$Author: c_bhatg $

when        who  what, where, why
----------  ---  ------------------------------------------------------------
2009-07-31  gb   Fixed compile errors  
2009-07-15  gb   Validating BDAddress before sending CancelBond  
                 cmd to driver 
2009-07-13  gb   Custom interface to support cancel bond
2009-06-02  gb   Fixing indentation problem 
2009-06-02  gb   Update device security will now return Cmd Not allowed in 
                 BT 2.1 targets.
2008-10-02  rs   Removing system level privilage checks, and lint/compiler
                 warnings.
=============================================================================*/
...
struct OEMBTDEVDB
{
   ...
   uint32              nRefs;
   // to keep track of the bonding bd address for cancel bonding
   BDAddress           bondBdAddr;
            ...
            if ( stat == BT_CS_GN_SUCCESS )
            {
               ...
               if ( stat == BT_CS_GN_SUCCESS )
               {
                  ...
                  if ( stat == BT_CS_GN_SUCCESS )
                  {
                     ...
                     /* Set IO capability to "No IO" */
                     (void) bt_cmd_rm_set_io_cap ( 
                        gpOEMBTDevDB->appId, BT_RM_IOC_NO_IO );
                  }
#endif
               }
...
/*======================================================================= 
 Cancel bond
=======================================================================*/
int OEMBTDEVDB_CancelBond( BDAddress* pBdAddr )
{
   int nErr = EBADPARM;   

   MSG_HIGH("OEMBTDEVDB_CancelBond -> ", 0, 0, 0);

   if ( ( gpOEMBTDevDB != NULL ) && ( pBdAddr != NULL ) &&
        ( 0 == ( MEMCMP (pBdAddr, &(gpOEMBTDevDB->bondBdAddr), 
                                          sizeof(BDAddress) ) ) ) )
   {
      //cancel the bond for which bonding is in progress
      bt_cmd_status_type stat = BT_CS_GN_SUCCESS;

      stat = 
      bt_cmd_rm_bond_cancel(gpOEMBTDevDB->appId, (bt_bd_addr_type*) pBdAddr);

      nErr = OEMBTDEVDB_CmdStatusToAEEError(stat);
   }
   MSG_HIGH("OEMBTDEVDB_CancelBond - Err = %d", nErr, 0, 0);
                                         ...
                                         bt_device_type* pDevice)
{
   ...
   pDevInfo->COD.SubDevCls -= pDevInfo->COD.DevCls;
                                     ...
                                     BTDBEntry*      pDbEntry)
{
   ...
   pDev->version.lmp_version       = (uint8)pDevInfo->LMPVersion.VersNum;
   pDev->version.manufacturer_name = (uint16)pDevInfo->LMPVersion.CompID;
   pDev->version.lmp_subversion    = (uint16)pDevInfo->LMPVersion.SubVersNum;
...

{
   ...
   MEMCPY( (void*)&(gpOEMBTDevDB->bondBdAddr), (void*)pBDAddr, 
                                             sizeof(BDAddress) );
...
static void OEMBTDB_EventCallback(bt_ev_msg_type* bt_ev_msg_ptr)
{
   ...
   case BT_EV_RM_USER_CONFIRMATION_REQUEST:
    {
         MSG_MED( "IBT DB User Conf Req", 0, 0, 0 );
         (void) bt_cmd_rm_user_confirmation_reply( 
            gpOEMBTDevDB->appId, 
            &bt_ev_msg_ptr->ev_msg.ev_rm_ucr.bd_addr, 
            TRUE );
         break;
    }
   ...
   case BT_EV_RM_REBOND_REQUEST:
     {
         MSG_MED( "IBT DB ReBond Req", 0, 0, 0 );
         (void) bt_cmd_rm_authorize_rebond( 
            gpOEMBTDevDB->appId, 
            &bt_ev_msg_ptr->ev_msg.ev_rm_rebreq.bd_addr, 
            TRUE,  // bAuthorized
            BT_RM_AB_DONT_CARE );
         break;
     }
