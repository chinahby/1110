...
$Header: //source/qcom/qct/multimedia/ipl/compositing/main/latest/src/ipl_compose.c#7 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/05/08   bf      added line pack support to subract lumas
12/12/07   sm     fix for lint warnings
12/01/07   bf      120: fixed color swapping issue and bug with multiple of 8 
                   issue in ipl_copy_and_paste
...
)
{
  ...
  {
    /* make sure we copy even height area */
    if (hcopy%2) hcopy--;
    if (hpaste%2) hpaste--;
  
  }

  // there are times when input is 420 at say 64 hpaste lines, but customer
  // wants us to go to 63 output lines in 422 space. Since our loop is from 
  // 0 to 63/2, we miss a line. Because of this, the last line has to 
  // be covered with special care
  if (wcopy != wpaste)
  {
  ...
  {
    if (hcopy != hpaste && hcopy != hpaste+1)
    {
      MSG_LOW("ipl_copy_and_paste_ycc420 marker_202\n",0,0,0);
      return IPL_FAILURE;
    }
...
FUNCTION ipl_copy_and_paste_luma

DESCRIPTION
  This function performs copy-and-paste. A region is copy from the input 
  image and pasted onto a region in the output image. 

  Input image must only contain luma and no chroma
...
===========================================================================*/
static ipl_status_type ipl_copy_and_paste_luma
(
  ipl_image_type* in_img_ptr,   /* Points to the input image           */
  ipl_image_type* out_img_ptr,  /* Points to the output image          */
  ipl_rect_type* copy_area,     /* Input region to be copy out         */
...
)
{
  ...
  MSG_LOW("ipl_copy_and_paste_luma marker_0\n",0,0,0);

  if (!in_img_ptr  || !in_img_ptr->imgPtr ||
      !out_img_ptr || !out_img_ptr->imgPtr ||
      !copy_area   || !paste_area)
  {
    MSG_LOW("ipl_copy_and_paste_luma marker_200\n",0,0,0);
    return IPL_FAILURE;
  }
  ...
  if ((in_img_ptr->cFormat != IPL_YCrCb422_LINE_PK) &&
      (in_img_ptr->cFormat != IPL_YCbCr422_LINE_PK))
  {
    MSG_LOW("ipl_copy_and_paste_luma marker_201\n",0,0,0);
    return IPL_FAILURE;
  }
  /* setup our input sizes and such */
  win = in_img_ptr->dx;
  ...
  MSG_LOW("ipl_copy_and_paste_luma marker_1\n",0,0,0);

  if (out_img_ptr->cFormat == IPL_LUMA_ONLY)
  {
    /* go to upper left of where we cut and where we are going to paste */
    inImgPtr += (copy_area->y * win + xscopy);
    outImgPtr += (paste_area->y * wout + xspaste);
  ...
  {
    MSG_LOW("ipl_copy_and_paste_luma marker_203\n",0,0,0);
    return IPL_FAILURE;
  }

  MSG_LOW("ipl_copy_and_paste_luma marker_100\n",0,0,0);
  return IPL_SUCCESS;
} /* End ipl_copy_and_paste_luma */
    ...
    for(pgv = 0; pgv < nmpgv; pgv++)
    {
      ...
      for(pgh = 0; pgh < nmpgh; pgh++)
      {
          ...
          for(Yh = 0; Yh < 2; Yh++)
          {
            ...
            for(j = 0; j < 8; j++)
            {
              for(i = 0; i < 8; i++)
              {
                ...
                inx = num%hin;
                iny = num/hin;
                //printf("should we write luma at inx %d iny %d...", inx, iny);
                if ((inx < cdx) && (iny < cdy))
                {
                  //printf("\n");
                  *outImgPtr++ = *inImgPtr++;
        ...
        for(j = 0; j < 8; j += 2)
        {
          for(i = 0; i < 8; i++)
          {
            num = (pgv*4*hin)+(pgh*16)+j*hin+2*i;
            inx = num%hin;
            iny = num/hin;
            if ((inx < cdx) && (iny < cdy))
            {
            ...
            {
              //printf("skip chroma since ! (%d < %d) && (%d < %d)\n", inx, cdx, iny, cdy);
              outClrPtr += 2;
              inCrPtr++;
              inCbPtr++;
            }
      ...
      for(pgh = 0; pgh < nmpgh; pgh++)
      {
        ...
        for(j = 0; j < 8; j++)
        {
          for(i = 0; i < 8; i+=2)
          {
            ...
            if ((inx < cdx) && (iny < cdy))
            {
              //printf("draw chroma since (%d < %d) && (%d < %d)\n", inx, cdx, iny, cdy);
              // every two horizontal Crs, make this guy's cr
              val = (*inCrPtr + *(inCrPtr+1))/2;
              *outClrPtr++ = (unsigned char) val;
              //*outClrPtr++ = 128;
            ...
            {
              //printf("skip chroma since not (%d < %d) && (%d < %d)\n", inx, cdx, iny, cdy);
              outClrPtr += 2;
              inCbPtr += 2;
              inCrPtr += 2;
            }
...
)
{
  ...
  if (paste_area == NULL)
  {
    ...
    temp2.dx = out_img_ptr->dx;
    temp2.dy = out_img_ptr->dy; 
    paste_area = &temp2;
  }
  ...
  if (in_img_ptr->cFormat == IPL_RGB565) 
  {
    ...
    return(ipl_copy_and_paste_rgb565(in_img_ptr,out_img_ptr,copy_area,
                                   paste_area));
  } 
  else if(in_img_ptr->cFormat == IPL_YCbCr) 
  {
    MSG_LOW("ipl_copy_and_paste marker_101\n",0,0,0);
    return(ipl_copy_and_paste_ycbcr(in_img_ptr,out_img_ptr,copy_area,
                                   paste_area));
  } 
           ...
           (in_img_ptr->cFormat == IPL_YCrCb420_LINE_PK))
  {
    ...
    return(ipl_copy_and_paste_ycc420(in_img_ptr,out_img_ptr,copy_area,
                                   paste_area));
  } 
           ...
           (in_img_ptr->cFormat == IPL_YCrCb422_LINE_PK))
  {
    ...
    return(ipl_copy_and_paste_ycc422(in_img_ptr,out_img_ptr,copy_area,
                                   paste_area));
  } 
  else 
 #endif /*!FEATURE_IPL_NO_CAMERA*/  	
           ...
           (in_img_ptr->cFormat == IPL_MCU_GRAY))
  {
    ...
    return(ipl_copy_and_paste_mcu(in_img_ptr,out_img_ptr,copy_area,
                                   paste_area));
  } 
#ifndef FEATURE_IPL_NO_CAMERA	
  else if(in_img_ptr->cFormat == IPL_RGB888) 
  {
    ...
    return(ipl_copy_and_paste_rgb888(in_img_ptr,out_img_ptr,copy_area,
                                   paste_area));
  }
  else if (in_img_ptr->cFormat == IPL_YCbCr444)
  {
    ...
    memcpy(out_img_ptr->imgPtr, in_img_ptr->imgPtr, 
           ...
           (in_img_ptr->cFormat == IPL_YCrCb444_LINE_PK)) 
  {
    ...
    memcpy(out_img_ptr->clrPtr, in_img_ptr->clrPtr, 
  ...
  else if(in_img_ptr->cFormat == IPL_LUMA_ONLY) 
  {
    ...
    if (copy_area->dx == paste_area->dx && copy_area->dy == paste_area->dy)
    {
      memcpy(out_img_ptr->imgPtr, in_img_ptr->imgPtr, 
    ...
    {
      return(ipl_copy_and_paste_luma(in_img_ptr,out_img_ptr,copy_area, paste_area));
    }
  }
...
)
{
  ...
  unpack_rgb565((uint16)fillerPixel, &r, &g, &b);
...
)
{
  ...
  if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
  {
    MSG_LOW("Max value is %d and pdy is %lu, so skip is %lu\n", 
             max, pdy, yscale>>8);
  ...
  if (i_img_ptr->cFormat == IPL_YCrCb420_LINE_PK)
  {
    ...
    for (chan = 0; chan < maxChan; chan++)
    {
      ...
      for (xpos = 0; xpos < pdx; xpos++)
      {
        ...
        // draw the max
        if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
          MSG_LOW("%d: Going to draw %d tall for hist %lu\n", xpos, max,col);

        lDraw = outPtr + (px + py*odx + 2*(xpos/2));
        cDraw = outCPtr + (px + py*odx/2 + 2*(xpos/2)); 

        if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
          MSG_LOW("%d: Going to draw crycb %d for hist %lu\n",xpos,max,col);
  ...
  else if (i_img_ptr->cFormat == IPL_RGB565)
  {
    ...
    for (xpos = 0; xpos < pdx; xpos++)
    {
      ...
      if (col+(xscale>>8) < hist->size)
      {
        ...
        {
          if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
            MSG_LOW("seeing if %lu is greater than %d\n", hist->v[i], max, 0);
      ...
      // draw the max
      if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
        MSG_LOW("%d: Going to draw line %d for hist %lu\n",xpos,max,col);

      rgbDraw = (uint16*) outPtr + (px + (py-1)*odx + xpos);
      for(row = 0; row < max; row += (yscale>>8))
      {
      	if (rgbDraw >= (uint16*) outPtr && rgbDraw < ((uint16*)outPtr+py*odx))
      	{
          *rgbDraw = gColor;
           rgbDraw -= odx;
        }
  ...
  else if (i_img_ptr->cFormat == IPL_YCbCr)
  {
    ...
    for (xpos = 0; xpos < pdx; xpos++)
    {
      ...
      {
        if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
         MSG_LOW("%d: Going to draw crycb line %d for hist %lu\n",xpos,max,col);
      ...
      {
        if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
         MSG_LOW("%d: Going to draw cbycr line %d for hist %lu\n",xpos,max,col);
...
)
{
  ...
  uint32 xscale = 0;
  uint32 yscale = 0;
  int lastlastMax, lastMax, min, max;
  int compMax[4];
  int32 i;
  int32 cb,cr, luma;
  uint32 odx;
  ...
  for (chan = 0; chan < maxChan; chan++)
  {
    ...
    else
    {
      ...
      {
        if (hist->mv[col][chan] > (uint32)compMax[chan])
  ...
  if (i_img_ptr->cFormat == IPL_YCrCb420_LINE_PK)
  {
    ...
    for (chan = 0; chan < maxChan; chan++)
    {
      ...
      if (rot == IPL_NOROT || rot == IPL_ROT180)
      {
        ...
        if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
        {
          MSG_LOW("Max entry is %3d and pdx is %lu, so skip is %lu\n", 
             hist->size, pdx, xscale>>8);
          MSG_LOW("Max value is %3d and pdy is %lu, so skip is %lu\n", 
             max, pdy, yscale>>8);
          MSG_LOW("Start at %lu, %lu\n", px, py, 0);
        }
      }
      else
      {
        ...
        if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
        {
          MSG_LOW("Max value is %3d and pdx is %lu, so skip is %lu\n", 
             max, pdx, xscale>>8);
      ...
      if (rot == IPL_NOROT || rot == IPL_ROT180)
      {
        ...
        for (xpos = 0; xpos < pdx; xpos++)
        {
          ...
          if (col+(xscale>>8) < hist->size)
          {
            ...
            for (i = col+1; i <= (int)(col+(xscale>>8)); i++)
            {
              if (hist->mv[i][chan] > (uint32)max)
                max = hist->mv[i][chan];
            }
          }
          else
          ...
          if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
            printf("\n%lu: max: %d, lastMax:%d lastlastMax:%d\n", xpos, max, lastMax, lastlastMax);
          ...
          {
            for(row = 0; row < (uint32)max; row += (yscale>>7))
            {
              if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
                printf("drawing (up) from row %lu to max %d\n", row, max);
          ...
          else
          {
            ...
            *(cDraw+1) = (uint8) cb; 
            ...
            min = IPL_MIN(lastMax, lastlastMax);
            for(row = max; row < (uint32)min; row += (yscale>>7))
            {
              if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
      ...
      else if (rot == IPL_ROT90)
      {
        ...
        for (ypos = 0; ypos < pdy; ypos++)
        {
          ...
          if (row+(yscale>>8) < hist->size)
          {
            ...
            for (i = row+1; i <= (int32)(row+(yscale>>8)); i++)
            {
              if (hist->mv[i][chan] > (uint32)max)
                max = hist->mv[i][chan];
            }
          ...
          // draw the max
          if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
            MSG_LOW("%lu: Going to draw %d tall for hist %lu\n", ypos, max,row);
          ...
          if (chan == 0 && lineType == 0 || lineType == 1)
          {
            // shift 8 for Q number, -1 for doing half as many since we do two
            // rows at a time
            for(col = 0; col < (uint32)max; col += (xscale>>7))
            {
              if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
                printf("drawing (up) from col %lu to max %d\n", col, max);
          ...
          else
          {
            ...
            *(cDraw+1) = (uint8) cb; 
            ...
            // value was starting to be drawn
            min = IPL_MIN(lastMax, lastlastMax);
            for(col = max; col < (uint32)min; col += (xscale>>7))
            {
              if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
  ...
  else if (i_img_ptr->cFormat == IPL_RGB565)
  {
    ...
    for (chan = 0; chan < maxChan; chan++)
    {
      ...
      for (xpos = 0; xpos < pdx; xpos++)
      {
        ...
        {
          max = hist->mv[col][chan];
          for (i = col+1; i <= (int32)(col+(xscale>>8)); i++)
          {
            if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
              MSG_LOW("seeing if %lu is greater than %d\n", hist->mv[i][0], max, 0);
        ...
        rgbDraw = (uint16*) outPtr + (px + (py-(uint32)1)*odx + xpos);
...
//          for(row = 0; row < max; row += (yscale>>8))
          for(row = 0; row < (uint32)(max<<8); row += yscale)
          {
	          if (rgbDraw >= (uint16*)outPtr && rgbDraw < ((uint16*)outPtr+py*odx))
	          {
	            *rgbDraw = gColor;
...
//          for(row = max; row < min; row += (yscale>>8))
          for(row = (max<<8); row < (uint32)(min<<8); row += yscale)
          {
            if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
              printf("drawing (down) from row %lu to min %d\n", row, min);
  ...
  else if (i_img_ptr->cFormat == IPL_YCbCr)
  {
    ...
    for (xpos = 0; xpos < pdx; xpos++)
    {
      ...
      if (xpos % 2)
      {
        ...
        for(row = 0; row < (uint32)max; row += (yscale>>8))
        {
          *(lDraw-2) = (uint8) cb;
          *(lDraw-1) = (uint8) luma; // to prevent color bleeding
          *(lDraw)   = (uint8) cr;
...
) 
{
  ...
  {
    if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
 	printf("Could not copy first image to output\n"); 
    return IPL_FAILURE;
  }
  ...
  for (i = 1; i < numFrames; i++)
  {
    ...
    {
        if (IPL_COMPOSE_DEBUG) //lint !e506 !e774
	  printf("Could not copy image %d to final\n", i); 
        return IPL_FAILURE;
    }
  }
