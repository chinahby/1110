...
#ifdef CUST_EDITION
#define OEM_ADDRESS_CATEGORY_COUNT_EX   (OEM_ADDRESS_CATEGORY_COUNT+4)
static const SupportedCat gSupportedCatList[OEM_ADDRESS_CATEGORY_COUNT_EX] =
#else
...
{
#ifdef CUST_EDITION
   { (int)AEE_ADDR_CAT_VIP, "VIP" },
   { (int)AEE_ADDR_CAT_HOME, "Family" },
   { (int)AEE_ADDR_CAT_WORK, "Colleague" },
   { (int)AEE_ADDR_CAT_FRIEND, "Friends" },
#endif
...
static AddrBkCache     *gpCache;           //Cache of vital fields in AddressBook
static boolean          gNoCache;          //Not using cache
static boolean          gbCacheInited;     //Has cache been read in? Don't check cache pointer itself 
...
static uint16 OEMAddr_GetCapacity(AEEAddrCat c);
static int    OEMAddr_GetCacheinfoByNumber(AECHAR *pwstrNum, AEEAddCacheInfo *pCacheInfo, PFN_NUMBERMATCH pfnMactch);
...
static void OEMAddr_RecordWrite_PBM_CB(pbm_writecb_data_s_type *cb_data)
{
...
#ifdef CUST_EDITION
{
  int nErr = InsertCacheEx(wRecID, cat, pItems, nItemCount);
  if(nErr != AEE_SUCCESS)
  {
     if(pErr)
        *pErr = nErr;
...
{
#ifdef CUST_EDITION
  pbm_return_type pbmRet = pbm_clear_phonebook(PBM_DEFAULT);
  ClearAddrBkCache();
  return pbm_error_to_aee(pbmRet);
#else
                           ...
                           int *pErr)
{
  ...
  *pErr = AEE_SUCCESS;
  ...
  do
  {
    ...
    // If no records left 
    if (rec_id == PBM_RECID_NULL) //Gemsea modify from AEE_ADDR_RECID_NULL
    {
      gbEnumInit = FALSE;
      if (pErr)
        *pErr = pbm_error_to_aee(pbmRet);
...
#ifdef CUST_EDITION
  OEMADDR_SET_AND_CHECK_FOR_END(AEE_ADDR_CAT_VIP);
  OEMADDR_SET_AND_CHECK_FOR_END(AEE_ADDR_CAT_HOME);
  OEMADDR_SET_AND_CHECK_FOR_END(AEE_ADDR_CAT_WORK);
  OEMADDR_SET_AND_CHECK_FOR_END(AEE_ADDR_CAT_FRIEND);
#endif
...
static int InitAddrBkCache(void)
{
   ...
   pbmRet = pbm_enum_rec_init(pb_id, (int)pb_cat, OEMAddr_AEEFieldToPBM(AEE_ADDRFIELD_NONE), NULL, 0, pbm_flags);
   ...
   while(TRUE)
   {
      pbmRet = pbm_enum_next_rec_id(&curRec);
...
static void OEMAddr_GetCapacityCB( pbm_return_type pbm_ret,
                                   ...
                                   pbm_extended_fileinfo_s_type *pInfo )
{
   ...
   g_AddrBootCap    = pInfo->number_of_records;
}
/*===========================================================================
...
static uint16   OEMAddr_GetCapacity(AEEAddrCat c)
{
   ...
   pbm_ret = pbm_extended_file_info(pb_id, OEMAddr_GetCapacityCB);
   
   if (PBM_SUCCESS != pbm_ret) {
      return 0;
   }else{
      return g_AddrBootCap;
   }
}// OEMAddr_GetCapacity
