...
Externalized Functions:
 ...
 - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  Copyright (c) 2002-2008 QUALCOMM Incorporated. All Rights Reserved
...
$Header: //source/qcom/qct/modem/mcs/stm/rel/1.0/inc/stm.h#2 $
...
#define STM_NULL_PAYLOAD_CHECK( payload )                                    \
  if ( (payload) == NULL )                                                   \
  {                                                                          \
...
#define STM_PAYLOAD_IS_EMPTY( payload )                                      \
  (((payload) == NULL ) ||                                                   \
   ((stm_msg_hdr *)((uint32) payload - sizeof(stm_msg_hdr)))->size == 0)

/* Helper function for use within a state for empty payload checking. */
#define STM_EMPTY_PAYLOAD_CHECK( payload )                                   \
  if(STM_PAYLOAD_IS_EMPTY( payload ))                                        \
  {                                                                          \
    MSG_ERROR("Null payload not expected in transition function", 0, 0, 0 ); \
  } 

/* Helper function for use within a state for empty payload checking. */
#define STM_EMPTY_PAYLOAD_CHECK_AND_RETURN( payload )                        \
  if(STM_PAYLOAD_IS_EMPTY( payload ))                                        \
  {                                                                          \
    MSG_ERROR("Null payload not expected in transition function", 0, 0, 0 ); \
    return(STM_SAME_STATE);                                                  \
