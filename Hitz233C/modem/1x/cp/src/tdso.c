...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/tdso.c#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $
...
const snm_neg_par_type  ftdso_neg_par = /* Full TDSO negotiation parmeters */
{
  6,                                    /* Minimum PREV allowed = 6        */
...
#else
static byte sch_rx_circ_buffer[1530];
static byte sch_tx_circ_buffer[1530];
#endif /* Rs2 */
#endif /* FEATURE_IS2000_SCH */
...
#else
  byte                sch_tx_circ_buffer[1530];
  byte                sch_rx_circ_buffer[1530];
#endif /* rs2 */
#endif /* FEATURE_IS2000_SCH */
} tdso_pre_sync;
...
/* Watchdog Timer */
LOCAL rex_timer_type tdso_rpt_timer;    /* Watch Dog Timer               */
...
)
{
  ...
  ** Initialize timer
  ** ---------------- */
  rex_def_timer(&tdso_rpt_timer, &tdso_tcb, TDSO_RPT_TIMER_SIG);

  /* -----------------------------------------------------------------
  ** Process task startup procedure from task controller.
  ** ----------------------------------------------------------------- */
  task_start(TDSO_RPT_TIMER_SIG, DOG_TDSO_RPT, &tdso_rpt_timer);
  ...
  ** ------------------------------------------------------------------ */
  ...
  for (;;)   /* Infinite loop */
  {
    sigs = rex_wait(TDSO_RPT_TIMER_SIG    |   /* Watchdog report timer */
                    TDSO_CMD_SIG          |   /* TDSO pre-sync         */
                    TASK_STOP_SIG         |   /* Power down            */
                    TASK_OFFLINE_SIG          /* Offline               */
                   );

    if ((sigs & TDSO_RPT_TIMER_SIG) != 0 )
    {
      /* -----------------------------
    ...
    ** ------------------------------------------------------- */
    ...
    {
      (void) rex_clr_sigs(&tdso_tcb, TASK_STOP_SIG);
      task_stop();
    }
    ...
    ** ------------------------------------------------------- */
    ...
    {
      (void) rex_clr_sigs(&tdso_tcb, TASK_OFFLINE_SIG);
      task_offline();
    }
    ...
    ** ------------------------------------------------------ */
    ...
    {
      (void) rex_clr_sigs(&tdso_tcb, TDSO_CMD_SIG);

      /* ------------------------
      ** TDSO Pre-Synchronization
      ** ------------------------ */
      tdso_pre_sync_proc(tdso_pre_sync.time, tdso_pre_sync.direction);
    }
  } /* end for (;;) */
...
)
{
  ...
  ** ----------------------------------- */
  ...
  MSG_MED( "SNM called TDSO initializer for SO %d ", tdso_so, 0, 0);
  ...
  ** ----------------------------------------------- */
  if (cfg->fch_cc_incl)
  {
    ...
    MSG_MED("TDSO Connect: F_FCH_RC %d, R_FCH_RC %d",
      fch_parm.pn_info[FWD_LINK].rc, fch_parm.pn_info[REV_LINK].rc, 0);
  }
  ...
  ** ------------------------------------------------ */
  if (cfg->dcch_cc_incl)
  {
    ...
    MSG_MED("TDSO Connect: F_DCCH_RC %d, R_DCCH_RC %d",
      dcch_parm.pn_info[FWD_LINK].rc, dcch_parm.pn_info[REV_LINK].rc, 0);
  }
#endif /* FEATURE_IS2000_P2 */
  ...
  ** ----------------------------------- */
  if (cfg->for_sch_cc_incl && (cfg->num_for_sch != 0))
  {
    ...
    MSG_MED("FOR_SCH_RC = %d", sch0_parm.pn_info[FWD_LINK].rc ,0, 0);
  }
  if(cfg->rev_sch_cc_incl && (cfg->num_rev_sch != 0))
  {
    sch0_parm.pn_info[REV_LINK].rc = cfg->rev_sch[0].sch_chn_cfg.sch_rc;
    MSG_MED("REV_SCH_RC = %d", sch0_parm.pn_info[REV_LINK].rc ,0, 0);
  }
#endif
  ...
  ** ------------------------------------------------------------------ */
  ...
  fch_parm.pn_info[FWD_LINK].tx_on_period =
  fch_parm.pn_info[FWD_LINK].tx_on_period_cnt =
  fch_parm.pn_info[FWD_LINK].tx_off_period =
  fch_parm.pn_info[FWD_LINK].tx_off_period_cnt =
  fch_parm.pn_info[REV_LINK].tx_on_period =
  fch_parm.pn_info[REV_LINK].tx_on_period_cnt =
  fch_parm.pn_info[REV_LINK].tx_off_period =
  fch_parm.pn_info[REV_LINK].tx_off_period_cnt = 0;

#ifdef FEATURE_IS2000_P2
  dcch_parm.pn_info[FWD_LINK].tx_on_period =
  dcch_parm.pn_info[FWD_LINK].tx_on_period_cnt =
  dcch_parm.pn_info[FWD_LINK].tx_off_period =
  dcch_parm.pn_info[FWD_LINK].tx_off_period_cnt =
  dcch_parm.pn_info[REV_LINK].tx_on_period =
  dcch_parm.pn_info[REV_LINK].tx_on_period_cnt =
  dcch_parm.pn_info[REV_LINK].tx_off_period =
  dcch_parm.pn_info[REV_LINK].tx_off_period_cnt = 0;
#endif /* FEATURE_IS2000_P2 */

#ifdef FEATURE_IS2000_SCH
  sch0_parm.pn_info[FWD_LINK].tx_on_period =
  sch0_parm.pn_info[FWD_LINK].tx_on_period_cnt =
  sch0_parm.pn_info[FWD_LINK].tx_off_period =
  sch0_parm.pn_info[FWD_LINK].tx_off_period_cnt =
  sch0_parm.pn_info[REV_LINK].tx_on_period =
  sch0_parm.pn_info[REV_LINK].tx_on_period_cnt =
  sch0_parm.pn_info[REV_LINK].tx_off_period =
  ...
  sch0_parm.pn_info[FWD_LINK].pre_sync_circ_buffer =
    tdso_pre_sync.sch_rx_circ_buffer;
  sch0_parm.pn_info[REV_LINK].pre_sync_circ_buffer =
    tdso_pre_sync.sch_tx_circ_buffer;
#endif
...
{
  #if (defined FEATURE_IS2000_SCH && defined FEATURE_IS2000_REL_A_MUXPDU5)
  ...
  #endif /* FEATURE_IS2000_SCH && FEATURE_IS2000_REL_A_MUXPDU5 */
  ...
  ** ----------------------------------------------------------------------*/
  ...
  {
    MSG_MED("Registering SO Initializer %x Failed", CAI_SO_TDSO, 0, 0);
  }

  if (snm_reg_connection_initializer( CAI_SO_FULL_TDSO, tdso_initializer))
  {
    MSG_MED("Registering SO Initializer %x Failed", CAI_SO_FULL_TDSO, 0, 0);
  }
} /* tdso_init_so_cfg */
...
)
{
    ...
    INTLOCK_SAV(i_sav); /* Disable interrupts */
    ...
    ** ------------------------------------------------------------------- */
        ...
        direction))
    {
      ...
      MSG_MED("Pre TDSO sync point, Building Circular Buff", 0, 0, 0);
      tdso_pre_sync.direction = direction;
      tdso_pre_sync.time = frame_blk_ptr->fr_time + PRE_SYNC_COUNT;
      (void) rex_set_sigs(&tdso_tcb, TDSO_CMD_SIG);
    }
    ...
    ** ---------------------------------------------------------------------- */
        ...
        (direction == FWD_LINK))
    {
      ...
      MSG_MED("1sec before SYNC: Defer SOCM processing", 0, 0, 0);
      socm_deferred = TRUE;
    }
    ...
    ** ------------------------------------------------------------------ */
    ...
    {
      MSG_MED( "TDSO sync point, SYNC procedures ",0,0,0);
      ...
      ** -----------------------------------------------------------------------*/
      ...
      {
        MSG_MED("Process deferred SOCM, Effects next SYNC",0,0,0 );
        socm_ignored = FALSE;
    ...
    for (fr_num =0; fr_num < frame_blk_ptr->num_frames; fr_num++)
    {
      ...
      {
          ERR("Phy %d Dir %d fr num %d",tx_frame_ptr->fr_channel,
               direction ,fr_num);
        INTFREE_SAV(i_sav); /* Restore interrupts */
        return TDSO_INVALID_CHAN;
      }
    ...
    ** ---------------------------------------------------------------*/
    ...
    if ((tx_frame_ptr != NULL) && (tx_frame_ptr->fr_channel == TDSO_SCH0))
    {
      chan_pn_info_ptr = &sch0_parm.pn_info[direction];
    ...
    if (log_count == 200) /* Logging the Statistics every 200 data blocks */
    {
      tdso_log_stats();
      log_count = 0;
    }
...
)
{
  ...
  if (frame_blk_ptr->num_frames > 0) /* Loop until all PDUs */
  {
    ...
    {
      ERR("Invalid Service %d", rx_frame_ptr->srvc_type,
          0, 0);
    ...
    {
      ERR("Invalid Channel TDSO %d",
          rx_frame_ptr->fr_channel, 0, 0);
      return TDSO_INVALID_SRVC;
    }
    ...
    if (rx_frame_ptr->srvc_type == TDSO_SECONDARY_SRVC)
    {
      ...
      {
        ERR("Non-Full Frames not supported.", 0, 0, 0);
        return TDSO_INVALID_SRVC;
      }
      else if (frame_blk_ptr->sec_len == 0)
      {
        ERR("Zero length frame received on Secondary!", 0, 0, 0);
        return TDSO_INVALID_SRVC;
      }
      else
      {
    ...
    ** -------------------------------------------------------------------*/
    for (i=0; i<frame_blk_ptr->num_frames; i++)
    {
      ...
      if (rx_frame_ptr->fr_rate >= TDSO_MAX_RATE)
      {
        ...
        ERR_FATAL("Rate out of range %d", rx_frame_ptr->fr_rate, 0, 0);
      }

	  /* check the valid range of forward rate set value */
      if (frame_blk_ptr->fdch_fr_mux >= TDSO_MAX_RATE_SET)
      {
        ERR("forward rate out of range %d", frame_blk_ptr->fdch_fr_mux, 0, 0);
		return TDSO_INVALID_SRVC;
      }
      ...
      {
        ERR("Invalid TDSO rate set= %d", frame_blk_ptr->sup_fr_mux, 0, 0);
		return TDSO_INVALID_SRVC;
      }
      ...
      else
      {
        ...
        if (rx_frame_ptr->mux_pdu_type == TDSO_MUX_PDU_5)
        {
          ...
          */
          ...
          {
            MSG_HIGH("Invalid SCH RATE", 0, 0, 0);
...
uint8 local_fch_data[34];        /* maximum RS1 & RS2 data length in bytes */
  ...
  ** -----------------------------------------------------------------*/
  ...
  {
    ERR( "Invalid FCH MUX_PDU_TYPE type = %d",frame_ptr->mux_pdu_type,0,0);
    return TDSO_INVALID_SRVC;
  }
  ...
  ** ----------------------------------- */
  ...
  {
    ERR("Invalid TDSO rate set= %d", fdch_tdso_rate_set, 0, 0);
    return TDSO_INVALID_SRVC;
  }
  ...
  if((fch_parm.pn_info[FWD_LINK].tdso_state) && (frame_rate == TDSO_RATE_1)) /*lint !e641 */
  {
    ...
    if (tdso_secondary_len) //* if this value is set Service is on Secondary
    {
      /* Copy the generated expected frame into the local buffer */
       memcpy(local_fch_data, tdso_rx_block->frame_type.rev_arr[0].fr_item_ptr,
...
#if (defined FEATURE_IS2000_REL_A && defined FEATURE_IS2000_SCH)
/*===========================================================================
...
)
{
  ...
  {
    ERR( "Invalid SRVC or CHAN TDSO %d", rx_frame_ptr->srvc_type,
          rx_frame_ptr->fr_channel, 0);
    return TDSO_INVALID_SRVC;
  }
  ...
  ** Check if the received frame RATE is valid for SCH.
  ...
  {
    ERR("Invalid SCH TDSO Rate type = %d", rx_frame_ptr->fr_rate, 0, 0);
    return TDSO_INVALID_SRVC;
  }
...
#endif /* FEATURE_IS2000_REL_A && FEATURE_IS2000_SCH */
...
)
{
  ...
  ** ----------------------------------- */
  if (frame_blk_ptr->sup_fr_mux >= TDSO_MAX_RATE_SET)
  {
    MSG_HIGH("Invalid TDSO rate set= %d", frame_blk_ptr->sup_fr_mux, 0, 0);
    return TDSO_INVALID_SRVC;
  }
...
#endif /* FEATURE_IS2000_REL_A */
  {
    ...
    for (i=1; i<frame_blk_ptr->num_frames; i++ )
    {
      rx_frame_ptr = &(frame_blk_ptr->frame_type.fwd_arr[i]);
    ...
    */
        ...
        (rx_frame_ptr->mux_pdu_type == TDSO_MUX_PDU_5))
    {
      ...
      if (frame_blk_ptr->num_frames - 1 < num_pdu)
      {
        ...
        MSG_ERROR("%d PDUs dropped by MUX", num_pdu, 0, 0);
        ...
        */
        rx_frame_ptr->fr_rate = TDSO_BIT_ERROR;
        MSG_ERROR("Frame marked as BIT_ERROR", 0, 0, 0);
      }
    }
#endif /* FEATURE_IS2000_REL_A */
...
)
{
  ...
  switch (chan)
  {
    ...
    default:
      ERR("Invalid Channel!! %d", chan, 0 , 0);
      return;
  }
...
#else
  log_stdso_type *stdso = NULL;

  stdso = (log_stdso_type*) log_alloc(LOG_STDSO_C,
          sizeof(log_stdso_type));
#endif /* FEATURE_IS2000_P2 */
...
#else
  log_ftdso_type *tdso = NULL;

  tdso = (log_ftdso_type*) log_alloc(LOG_FTDSO_C,
         sizeof( log_ftdso_type));
#endif /* FEATURE_IS2000_P2 */
  ...
  ** -------------------------------------------------------- */
      ...
      ((config->num_rev_sch > 0) && (direction == REV_LINK)))
  {
    ...
    *seed_ptr <<= 1;
    ...
    MSG_MED("Pre sync: dir %d, fr %l, seed %l", direction, time_val, *seed_ptr);
  }
#endif /* FEATURE_IS2000_SCH */
...
)
{
  ...
  if (tdso_pre_sync.prev_sync_time != frame_blk_ptr->fr_time)
  {
    /* Get the Current Configuration information from the SNM */
    config = snm_get_current_config();
    ...
    {
      MSG_HIGH( "REV Link Sync: %lx ", frame32, 0, 0);
      seed_ptr = &tdso_tx_seed;  /* Point to TX seed */
      frame32 ^= DEF_TX_SEED;    /* XOR frame with TX SEED */
    }
    else  /* Forward Link Sync frame */
    {
      MSG_HIGH("FOR Link Sync: %lx ", frame32, 0, 0);
      seed_ptr = &tdso_rx_seed; /* Point to RX seed */
      frame32 ^= DEF_RX_SEED;   /* XOR frame with RX SEED */
    }
    ...
    *seed_ptr <<= 1;
    ...
    MSG_HIGH("PN = %lx", (chan_pn_info_ptr->pn)/2 ,0,0);
    ...
    if (!chan_pn_info_ptr->pn_valid)
    {
      ...
      *seed_ptr = mar_ran_next(*seed_ptr);
      ...
      if (config->dcch_cc_incl)
      {
        MSG_MED("DCCH PN = %lx, direction = %d", chan_pn_info_ptr->pn, direction,
        0);
...
#endif /* FEATURE_IS2000_P2 */
      {
        MSG_MED("FCH PN = %lx, direction = %d", chan_pn_info_ptr->pn, direction,
        0);
    ...
    ** ------------------- */
        ...
        ((config->num_rev_sch > 0) && (direction == REV_LINK)))
    {
      ...
      if (!chan_pn_info_ptr->pn_valid)
      {
        ...
        chan_pn_info_ptr->pn = pnval;
        chan_pn_info_ptr->pn_valid = TRUE;
        ERR("SCH0 PN = %lx, direction = %d", chan_pn_info_ptr->pn, direction,
            0);
      ...
      if (sch0_parm.pn_info[direction].tst_par.data_src)
      {
        ...
        memcpy(chan_pn_info_ptr->circ_buffer,
    ...
    tdso_pre_sync.prev_sync_time = frame_blk_ptr->fr_time;
...
)
{
    ...
    ** ----------------------------------------------------------------*/
    if (chan_pn_info_ptr->tdso_state)
    {
      ...
      if (frame_size/8 > byte_count)
      {
        ...
        memcpy(tx_frame_ptr->fr_item_ptr, &chan_pn_info_ptr->circ_buffer
               [chan_pn_info_ptr->data_offset], byte_count);

        memcpy(&tx_frame_ptr->fr_item_ptr[byte_count],
  ...
  else if (tx_frame_ptr->fr_channel == TDSO_SCH0)
  {
    ...
    {
      ERR( "Invalid SCH TDSO Rate type = %d",tx_frame_ptr->fr_rate,0,0);
      return;
    }
    ...
    if (chan_pn_info_ptr->tdso_state)
    {
      ...
      {
        /* Pack the SCH TDSO data block from the Circular buffer */
        memcpy(tx_frame_ptr->fr_item_ptr,
      ...
      ** based on the Current MUX configuration
           ...
           (tx_frame_ptr->fr_rate == TDSO_BLANK)))
      {
        ...
        if ((for_mux >> 8) == 0x8)
        {
          ...
          MSG_MED("SCH Erasure, Single PDU", 0, 0, 0);
          sch_pdu_type = 0x1;
        }
        ...
        {
          ERR("Invalid for MUX", 0, 0, 0);
          return;
        }

        num_erasure_pdu = (word) ((pow((double)2,
          (double) tx_frame_ptr->sch_rate) / sch_pdu_type));
        ERR("Moving Ref By PDU %d, size %d", num_erasure_pdu,
          num_erasure_pdu*tx_frame_ptr->max_frame_size/8,0);
      }
  ...
  {
    MSG_MED( "Unsupported TDSO Physical Channel ",0,0,0);
  }
  ...
  ** --------------------- */
     ...
     )
  {
    ...
    if (chan_pn_info_ptr->tst_par.frame_src == 0x1)
    {
      ...
      {
        ERR("Unsupported # of Circ Buffer frames %d",
            chan_pn_info_ptr->tst_par.num_circ_buf_frames,0,0);
      }
    }
    ...
    if (direction == FWD_LINK)
    {
      ...
      {
        ERR("Unsupported TDSO FWD RC %d", chan_pn_info_ptr->rc, 0, 0);
      }
    }
    else /* Reverse Link */
    {
      ...
      {
        ERR("Unsupported TDSO REV RC %d", chan_pn_info_ptr->rc, 0, 0);
      }
    }
    ...
    MSG_MED("FCH: dir %d circ fr %d NUM_RAND %d", direction, num_circ_buffer,
      chan_pn_info_ptr->num_rand);
  }
#ifdef FEATURE_IS2000_SCH
  /* --------------
  ...
  ** -------------- */
  else if (channel == TDSO_SCH0)
  {
    ...
    switch( chan_pn_info_ptr->rc )
    {
      ...
      default:
        ERR ("Unsupported RC %d for SCH", chan_pn_info_ptr->rc, 0,0);
        break;
    }
    ...
    MSG_MED("SCH: dir %d circ fr %d NUM_RAND %d", direction,
...
void tdso_copy_counter_buffer(tdso_link_dir_type direction)
{
    ...
    if (dcch_parm.pn_info[direction].tst_par.cpy_cnts)
    {
      ...
      ** ----------------------------------------------------------------- */
      if (direction == REV_LINK)
      {
        ...
        memcpy(dcch_tx_rpt, dcch_tx_table, sizeof(dcch_tx_table));
        memcpy(dcch_tx_blank_rpt, dcch_tx_blank_table,
      ...
      ** ------------------------------------------------------------------*/
      {
        memcpy(dcch_rx_rpt, dcch_rx_table, sizeof(dcch_rx_table));
        memcpy(dcch_rx_null_rpt, dcch_rx_null_table, sizeof(dcch_rx_null_table));
      }
    ...
    if (fch_parm.pn_info[direction].tst_par.cpy_cnts)
    {
      ...
      ** ----------------------------------------------------------------- */
      if (direction == REV_LINK)
      {
        ...
        memcpy(fch_tx_rpt, fch_tx_table, sizeof(fch_tx_table));
        memcpy(fch_tx_blank_rpt, fch_tx_blank_table,
      ...
      ** ------------------------------------------------------------------*/
      {
        memcpy(fch_rx_rpt, fch_rx_table, sizeof(fch_rx_table));
        memcpy(fch_rx_null_rpt, fch_rx_null_table, sizeof(fch_rx_null_table));
      }
  ...
  if (sch0_parm.pn_info[direction].tst_par.cpy_cnts)
  {
    ...
    ** ------------------------------------------------------------------*/
    ...
    {
      /* Copy the FCH reverse Link counters */
      memcpy(sch_tx_rpt, sch_tx_table, sizeof(sch_tx_table));
    ...
    ** ------------------------------------------------------------------*/
    else
    {
      ...
      memcpy(sch_rx_pdu_buffer, sch_rx_pdu_table, sizeof(sch_rx_pdu_table));

      /* Copy the SCH receive frame counters from the table */
      memcpy(sch_rx_rpt, sch_rx_table, sizeof(sch_rx_table));
      memcpy(sch_rx_blank_rpt, sch_rx_blank_table, sizeof(sch_rx_blank_table));
    }
...
)
{
  ...
  if (!chan_parm_ptr->pn_info[direction].tst_par.frame_act)
  {
    ...
    MSG_MED("Direction %d, ON/OFF cnts : ON %d, OFF %d ", direction,
      chan_parm_ptr->pn_info[direction].tx_on_period_cnt,
      chan_parm_ptr->pn_info[direction].tx_off_period_cnt );
  }
  else
  {
    ...
    MSG_MED("Act Thresh: ON %x, OFF %x",
            chan_parm_ptr->pn_info[direction].tx_on_thresh,
            chan_parm_ptr->pn_info[direction].tx_off_thresh,0);
  }
...
)
{
  ...
  if (tdso_sch.sch_tx_rate >= 5)
  {
	  MSG_HIGH("Invalid SCH TX RATE", 0, 0, 0);
	  return;
  }
...
)
{
  if (tdso_state) /* TDSO state is ON */
  {
	...
	if  (rx_frame_ptr->fr_rate >= TDSO_MAX_RATE)
	{
	    MSG_HIGH ("FR Rate is out of range",0,0,0);
	    return;
	}
    ...
    if (tdso_so == CAI_SO_FULL_TDSO)
    {
          ...
          (rx_frame_ptr->fr_rate == TDSO_RATE_EIGHTH))
      {
        ...
        MSG_HIGH("TDSO ON: Received DIM & BURST %d", rx_frame_ptr->fr_rate, 0, 0);
      }
      else
      {
...
)
{
  ...
  #ifdef FEATURE_IS2000_REL_A
   if (tdso_sch.sch_rx_rate >= 6)
  #else
   if (tdso_sch.sch_rx_rate >= 5)
  #endif
   {
	  MSG_HIGH("Invalid SCH RX RATE", 0, 0, 0);
	  return;
   }
    ...
    case TDSO_CTL_ALL:
          ...
          (tst_par.ch_dir == FWD_LNK_ONLY))  /*lint !e641 */
      {
        if (socm_deferred)
        {
          ...
          memcpy(&fch_parm.pn_info[FWD_LINK].pending_tst_par, &tst_par,
                 sizeof(tst_par));
#ifdef FEATURE_IS2000_P2
          dcch_parm.pn_info[FWD_LINK].socm_deferred = TRUE;
          memcpy(&dcch_parm.pn_info[FWD_LINK].pending_tst_par, &tst_par,
                 sizeof(tst_par));
#endif /* FEATURE_IS2000_P2 */
#ifdef FEATURE_IS2000_SCH
          sch0_parm.pn_info[FWD_LINK].socm_deferred = TRUE;
          memcpy(&sch0_parm.pn_info[FWD_LINK].pending_tst_par, &tst_par,
                 sizeof(tst_par));
#endif /* FEATURE_IS2000_SCH */
        }
        else
        {
          memcpy(&fch_parm.pn_info[FWD_LINK].tst_par, &tst_par,
                 sizeof(tst_par));
#ifdef FEATURE_IS2000_P2
          memcpy(&dcch_parm.pn_info[FWD_LINK].tst_par, &tst_par,
                 sizeof(tst_par));
#endif /* FEATURE_IS2000_P2 */
#ifdef FEATURE_IS2000_SCH
          memcpy(&sch0_parm.pn_info[FWD_LINK].tst_par, &tst_par,
                 sizeof(tst_par));
#endif /* FEATURE_IS2000_SCH */
        }
      }
          ...
          (tst_par.ch_dir == REV_LNK_ONLY))  /*lint !e641 */
      {
        if (socm_deferred)
        {
          ...
          memcpy(&fch_parm.pn_info[REV_LINK].pending_tst_par, &tst_par,
                 sizeof(tst_par));
#ifdef FEATURE_IS2000_P2
          dcch_parm.pn_info[REV_LINK].socm_deferred = TRUE;
          memcpy(&dcch_parm.pn_info[REV_LINK].pending_tst_par, &tst_par,
                 sizeof(tst_par));
#endif /* FEATURE_IS2000_P2 */
#ifdef FEATURE_IS2000_SCH
          sch0_parm.pn_info[REV_LINK].socm_deferred = TRUE;
          memcpy(&sch0_parm.pn_info[REV_LINK].pending_tst_par, &tst_par,
                 sizeof(tst_par));
#endif /* FEATURE_IS2000_SCH */
        }
        else
        {
          memcpy(&fch_parm.pn_info[REV_LINK].tst_par, &tst_par,
                 sizeof(tst_par));
#ifdef FEATURE_IS2000_P2
          memcpy(&dcch_parm.pn_info[REV_LINK].tst_par, &tst_par,
                 sizeof(tst_par));
#endif /* FEATURE_IS2000_P2 */
#ifdef FEATURE_IS2000_SCH
          memcpy(&sch0_parm.pn_info[REV_LINK].tst_par, &tst_par,
                 sizeof(tst_par));
#endif /* FEATURE_IS2000_SCH */
        }
      }
  ...
  ** --------------------------------------------------- */
  ...
  if (i < tsf_len)
  {
    ...
    ** ----------------------------- */
    ...
    MSG_MED("CH_DIR = %d,CPY_CNT= %d,CLR_CNT = %d",
              tst_par.ch_dir, tst_par.cpy_cnts, tst_par.clr_cnts);
    ...
    ** ----------------------------- */
    ...
    /* Display the control parameters on DM */
    MSG_MED(" RAND_SRC = %d, FRAME_SRC = %d, FRAME_ACT = %d ",
              tst_par.data_src, tst_par.frame_src,
              tst_par.frame_act);
  }
  ...
  ** ---------------------------------------------------------------- */
  if ((tdso_fr_src_type) tst_par.frame_src == N_FULL_RATE_FRAME)
  {
    if (i < (tsf_len - 1))
    {
      ...
      MSG_MED(" NUM CIR BUFF = %d", tst_par.num_circ_buf_frames,0,0);
    }
  }
  else
  {
  ...
  ** ------------------------------------------------------------------ */
  if (tst_par.frame_act)
  {
    if (i < (tsf_len - 2))
    {
      ...
      MSG_MED(" ONtoOFF prob = %d, OFFtoON prob = %d ",
                tst_par.on_to_off_prob, tst_par.off_to_on_prob,0);
    }
  }
  else
  {
    if (i < (tsf_len - 2))
    {
      ...
      MSG_MED(" ON period= %d, OFF period = %d",
                tst_par.tx_on_period, tst_par.tx_off_period,0);
    }
  ...
  ** --------------------------- */
  ...
  if (!tst_par.data_src)
  {
    if (i < (tsf_len - 1))
    {
      ...
      MSG_MED(" Data Pattern = %d", tst_par.data_pattern,0,0);
    }
  }
  ...
  ** --------------------------------------------------------------------- */
  ...
  if (socm_deferred)
  {
    /* Mark that we have deferred the update of TDSO test parameters from
       SOCM */
    socm_ignored = TRUE;
    MSG_MED("Deferred update of TDSO test parameter", 0, 0, 0);
  }
  else
  {
...
)
{
  if (direction == FWD_LINK)
  {
    if (fch_parm.pn_info[FWD_LINK].socm_deferred)
    {
      ...
      memcpy(&fch_parm.pn_info[FWD_LINK].tst_par,
             &fch_parm.pn_info[FWD_LINK].pending_tst_par,
             sizeof(fch_parm.pn_info[FWD_LINK].pending_tst_par));
    }
#ifdef FEATURE_IS2000_P2
    if (dcch_parm.pn_info[FWD_LINK].socm_deferred)
    {
      dcch_parm.pn_info[FWD_LINK].socm_deferred = FALSE;
      memcpy(&dcch_parm.pn_info[FWD_LINK].tst_par,
             &dcch_parm.pn_info[FWD_LINK].pending_tst_par,
             sizeof(dcch_parm.pn_info[FWD_LINK].pending_tst_par));
    }
#endif /* FEATURE_IS2000_P2 */
#ifdef FEATURE_IS2000_SCH
    if (sch0_parm.pn_info[FWD_LINK].socm_deferred)
    {
      ...
      memcpy(&sch0_parm.pn_info[FWD_LINK].tst_par,
             &sch0_parm.pn_info[FWD_LINK].pending_tst_par,
             sizeof(sch0_parm.pn_info[FWD_LINK].pending_tst_par));
    }
#endif /* FEATURE_IS2000_SCH */
  }
  else
  {
    if (fch_parm.pn_info[REV_LINK].socm_deferred)
    {
      ...
      memcpy(&fch_parm.pn_info[REV_LINK].tst_par,
             &fch_parm.pn_info[REV_LINK].pending_tst_par,
             sizeof(fch_parm.pn_info[REV_LINK].pending_tst_par));
    }
#ifdef FEATURE_IS2000_P2
    if (dcch_parm.pn_info[REV_LINK].socm_deferred)
    {
      dcch_parm.pn_info[REV_LINK].socm_deferred = FALSE;
      memcpy(&dcch_parm.pn_info[REV_LINK].tst_par,
             &dcch_parm.pn_info[REV_LINK].pending_tst_par,
             sizeof(dcch_parm.pn_info[REV_LINK].pending_tst_par));
    }
#endif /* FEATURE_IS2000_P2 */
#ifdef FEATURE_IS2000_SCH
    if (sch0_parm.pn_info[REV_LINK].socm_deferred)
    {
      ...
      memcpy(&sch0_parm.pn_info[REV_LINK].tst_par,
             &sch0_parm.pn_info[REV_LINK].pending_tst_par,
             sizeof(sch0_parm.pn_info[REV_LINK].pending_tst_par));
    }
#endif /* FEATURE_IS2000_SCH */
  }
...
void tdso_notify_hho_success( void )
{
  ...
  MSG_HIGH( "TDSO informed about HHO success",0,0,0);
  ...
  ** ---------------------------------------------------------------------- */
  ...
  MSG_HIGH(" Sending SOCM to BS ",0,0,0);
...
void tdso_sch_chng_cback( mccsch_sch_info_type* sch_info)
{
  if ((sch_info->f_sch_0.changed) && (sch_info->f_sch_0.active))
  {
    ...
    MSG_MED("F-SCH config changed: Reset SYNC ", 0, 0, 0);
  }
  ...
  if ((sch_info->r_sch_0.changed) && (sch_info->r_sch_0.active))
  {
    ...
    MSG_MED("R-SCH config changed: Reset SYNC ", 0, 0, 0);
  }
} /* tdso_sch_chng_cback */
/*lint +e818*/
#endif /* FEATURE_IS2000_SCH */
    ...
    switch (vect_counter_id)
    {
      ...
      ** --------------------------- */
      case TDSO_FDCH_FER_CNT:
        ...
        MSG_HIGH("Retrieving FCH FER CNT",0,0,0);
      ...
      ** ------------------------------------- */
      case TDSO_FDCH_RXEX_CNT:
        ...
        MSG_HIGH("Retrieving FCH RXEX CNT",0,0,0);
      ...
      ** ---------------------------- */
      case TDSO_FDCH_TX_CNT:
        ...
        MSG_HIGH("Retrieving FCH TX CNT",0,0,0);
    ...
    switch (vect_counter_id)
    {
      ...
      ** --------------------------- */
      case TDSO_FDCH_FER_CNT:
        ...
        MSG_HIGH("Retrieving DCCH FER CNT",0,0,0);
      ...
      ** ------------------------------------- */
      case TDSO_FDCH_RXEX_CNT:
        ...
        MSG_HIGH("Retrieving DCCH RXEX CNT",0,0,0);
      ...
      ** ---------------------------- */
      case TDSO_FDCH_TX_CNT:
        ...
        MSG_HIGH("Retrieving DCCH TX CNT",0,0,0);
    ...
    switch (vect_counter_id)
    {
      ...
      ** --------------------- */
      case TDSO_SCH_FER_CNT:
        ...
        MSG_HIGH("Retrieving SCH FER CNT",0,0,0);
        ...
        ** ---------------------------------------------------------- */
        ...
        {
          MSG_MED("TDSO SCH Rx FER counters are all zeros", 0, 0, 0);
        ...
        ** ------------------------------------------------------ */
        else
        {
          ...
          MSG_HIGH( "Retrieve F-SCH CNTS: Rate :%d", tdso_sch.sch_rx_rate,0,0);
      ...
      ** --------------------- */
      case TDSO_SCH_PER_CNT:
        ...
        MSG_HIGH("Retrieving SCH PER CNT",0,0,0);
        ...
        ** ---------------------------------------------------------- */
        if ((tdso_sch.sch_rx_rate == TDSO_SCH_RATE_NONE) ||
            (tdso_sch.sch_rx_rate > TDSO_SCH_RATE_16X))
        {
          MSG_MED("TDSO SCH Rx PER counters are all zeros", 0, 0, 0);
      ...
      ** ----------------------------- */
      case TDSO_SCH_TX_CNT:
        ...
        MSG_HIGH("Retrieving SCH TX CNT",0,0,0);
        ...
        ** ---------------------------------------------------------- */
        ...
        {
          MSG_MED("TDSO SCH Tx counters are all zeros", 0, 0, 0);
...
)
{
  ...
  ** ----------------------------------- */
  ...
  {
    ERR("Invalid DCCH MUX_PDU_TYPE type = %d", rx_frame_ptr->mux_pdu_type, 0, 0);
    return TDSO_INVALID_SRVC;
  }
  ...
  ** --------------------------------------------------------------------*/
  ...
  {
    /* Copy the generated expected frame into the local buffer */
    memcpy(expected_data, frame_blk_ptr->frame_type.rev_arr[0].fr_item_ptr,
           tdso_bytes[TDSO_RATE_1][fdch_tdso_rate_set]);
