...
/*=============================================================================
  ...
  $Header: //depot/asic/qsc1100/tools/headergen/reserve_hash_seg.c#2 $
...
int modify_offsets_after_hash_segment(FILE *src_elf,FILE *dest_elf)
{
  ...
  fpos_t phdr_pos;
  int size, ret, i;
  uint32_t file_offset=0, phdr_size,src_offset=0,src_file_offset=0,dest_file_offset=0,section_header_table_size=0;
  uint32_t hash_seg_offset_adjust = 0;
  uint32_t dest_phdr_table_offset_adjust = 0;
  uint32_t hash_seg_offset=0, hash_seg_found=0;
  ...
  src_file.elf_hdr.e_phoff = ELF_HDR_SIZE;
  ...
  ret = write(dest_file.fd, (char*) &src_file.elf_hdr, ELF_HDR_SIZE);
  ...
  DEBUG("Source File offset is %x \n", src_file_offset,0,0);

  dest_file_offset = ELF_HDR_SIZE;

  if (phdr_size > UINT32_MAX)
  {
	  printf("phdr_size reached limit phdr_size(%d) - errno(%d)\n", phdr_size, errno);
	  return errno;
  }
  
  dest_phdr_table_offset_adjust = ((src_file.elf_hdr.e_phnum) * phdr_size);
  ...
  for(i=1; i<=src_file.elf_hdr.e_phnum ; i++)
  {
	...
	else if (hash_seg_found == 1)
    {
     ...
     DEBUG("p_flags  i_hdr->p_offset Before Change [%#x] :\n",i_hdr->p_offset,0,0);
	    src_offset = i_hdr->p_offset;
        i_hdr->p_offset = (i_hdr->p_offset + hash_seg_offset_adjust + dest_phdr_table_offset_adjust);
     DEBUG("p_flags  i_hdr->p_offset After Change [%#x] :\n",i_hdr->p_offset,0,0);
     ...
     if(i_hdr->p_filesz > UINT32_MAX)
     { 
       printf("p_filesz is grater than the max uint 32 value. i_hdr->p_filesz (%d) - errno(%d)\n", i_hdr->p_filesz , errno);
	   return errno;
	 }

	 /* Copy The actual Segment here */
     ret = fcpy(src_elf,  src_offset, dest_elf, i_hdr->p_offset, i_hdr->p_filesz);
     if(ret)
     {
  ...
  /* Copy The Section Headers Here */
  src_file_offset = src_file.elf_hdr.e_shoff;
  dest_file_offset = src_file.elf_hdr.e_shoff;
  section_header_table_size = ((src_file.elf_hdr.e_shentsize) * (src_file.elf_hdr.e_shnum));

  
    if(section_header_table_size > UINT32_MAX)
	{
	 printf(" section_header_table_size [%d] is greater than the max limit of UINT32_MAX - errno(%d)\n",section_header_table_size, errno);
	 return errno;
	}


     ret = fcpy(src_elf,  src_file_offset, dest_elf, dest_file_offset, section_header_table_size);
     if(ret)
     {
	  printf("Copy Segment[%d] failed! ret(%d) - errno(%d)\n",i, ret, errno);
	  return errno;
