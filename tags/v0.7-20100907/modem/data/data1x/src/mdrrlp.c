...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/data/1x/1xrlp/main/lite/src/mdrrlp.c#5 $ $DateTime: 2008/07/30 14:06:44 $ $Author: nsivakum $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/08/08   sn      Fixed klockwork error.
02/07/08   sn      Mainlined DSM_PS_RLP flag code as PS_RLP flag does not 
                   exist anymore.
02/04/08   sn      Merged lint error fix.
09/08/06   az      Fixed a lint error.
...
#include "ds.h"  /* Contains the enum for the UI last call status function */
#include "ps.h"
#include "mdrrlp.h"
...
---------------------------------------------------------------------------*/
LOCAL dword rlp_rx_stat[RLP_BLANK+1][4];
LOCAL dword rlp_tx_stat[RLP_BLANK+1][4];
LOCAL dword rlp_rx_stat_tot[RLP_BLANK+1][4];
LOCAL dword rlp_tx_stat_tot[RLP_BLANK+1][4];

/*---------------------------------------------------------------------------
  This structure holds the error return values from the Validate Generic
  frame function. There are 3 different error values.
---------------------------------------------------------------------------*/
LOCAL dword rlp_gen_rx_errs[RLP_ERASURE][RLP_FRAME_CHK_VLDN_MAX];
...
/*---------------------------------------------------------------------------
  ...
  parameters.
---------------------------------------------------------------------------*/
LOCAL void              (*rlp_post_rx_func_ptr)(void)  = NULL;
LOCAL q_type             *rlp_post_rx_q_ptr            = NULL;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  MSG_MED("Preparing to re-xmit: %u - %u", first_seq_num, last_seq_num, 0);
  /*-------------------------------------------------------------------------
    Return if no data frames are stored.
  -------------------------------------------------------------------------*/
  if( rlp_state_info.saved_frames_cnt == 0)
  {
    ERR("No Saved Frames to Re-xmit", 0, 0, 0);
    return;
  }
  /*-------------------------------------------------------------------------
  ...
  {
    ERR( "Invalid Re-xmit, reset RLP", 0, 0, 0);
  ...
  {
    ERR("Re-xmit requested is too old, re-xmit/min = %u/%u", first_seq_num, min_seq_num, 0);
    rlp_gen_stats.resets++;
    rlp_reset();
    return;
  }
  ...
  {
    ERR( "Re-xmit frame not found: %u", first_seq_num, 0, 0);
...
#error code not present
  ...
  {
    ERR( "Suspicious Re-xmit truncated", 0, 0, 0);
  ...
  {
    ERR("%u re-xmits not queued", frames_to_queue, 0, 0);
    rlp_gen_stats.re_xmits_missed++;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  switch( frame_ptr->gen_data.ctl_len & RLP_GEN_CTL_CHK_MASK)
  {
    ...
    case RLP_REXMIT_UPPR_BITS:
      ...
      *frame_typ = RLP_REXMIT_DATA_FR;
      ...
      {
         ERR("Rexmit bit set for RLP Type 1 frame", 0, 0, 0);
      ...
      *frame_typ = RLP_CONTROL_FR;
      ...
      if( rlp_type == RLP_TYPE_I )
      {
        ...
        else
        {
          ...
          {
            return_val = RLP_CTL_LEN_ERR;
            ERR("RLP Type 1 frame rx'ed with unknown CTL value: %#X",
                 frame_ptr->gen_ctl.ctl_len, 0, 0);
          }
        }
      } /* if RLP Type 1 */
      ...
      else if( rlp_type == RLP_TYPE_II )
      {
        ...
        else if( ctl == RLP_TYPE2_NAK )
        {
          ...
          else
          {
            ...
            ----------------------------------------------------------------*/
            return_val = RLP_CTL_LEN_ERR;
            ERR("RLP Type 2 frame rx'ed with unknown NAK_TYPE: %#X", 
                 nak_type, 0, 0);
          }
        }
        else
        {
          ...
          -----------------------------------------------------------------*/
          return_val = RLP_CTL_LEN_ERR;
          ERR("RLP Type 2 frame rx'ed with unknown CTL value: %#X", 
               ctl, 0, 0);
        }
     ...
     ======================================================================*/
           ...
           RLP_SEGFR_FILL_FR_BITS) 
      {
         ...
         /*-----------------------------------------------------------------
          ...
          -----------------------------------------------------------------*/
           MSG_HIGH("Fill frame received",0,0,0);
       ...
       /* Greater than length check only useful with smaller rate frames*/
        {
          ...
          -----------------------------------------------------------------*/
          ERR("Segment with invalid length received",0,0,0);
          rlp_pri_frate_errs.rx_seg_fr_errs++;
          return_val   = RLP_DATA_LEN_ERR;
        }
        else if (rlp_rx_segfr_active)
        {
          ...
          {
            ERR("Invalid 2nd or Last segmented frame received",0,0,0);
            rlp_pri_frate_errs.rx_seg_fr_errs++;
            return_val   = RLP_CTL_LEN_ERR;
          }
        ...
        {
          ERR("Invalid first segmented frame received",0,0,0);
          return_val   = RLP_CTL_LEN_ERR;
          rlp_pri_frate_errs.rx_seg_fr_errs++;
        }
      }
    ...
    default:
      ...
      *frame_typ   = RLP_CONTROL_FR;
      return_val   = RLP_CTL_LEN_ERR;
      ERR("RLP frame with invalid CTL-LEN field",0,0,0);
  } /* switch */
  ...
  {
   seg_fr_cnt            = 0;
   ERR("Resetting segmented receive",0,0,0);
...
{
  word          isave;                        /* holds program status word */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( (IS_NAK_FR(frame_type)) && (rlp_state_info.state != RLP_SYNC_STATE) )
  {
    ...
    if( rlp_type == RLP_TYPE_I )
    {
      ...
      ---------------------------------------------------------------------*/
      MSG_MED("Rx'ed NAK: First = %u, Last = %u", 
               frame_ptr->gen_ctl.first_seq, frame_ptr->gen_ctl.last_seq, 0);
    ...
    else if( rlp_type == RLP_TYPE_II )
    {
      ...
      if( frame_type == RLP_NAK_TYPE0_FR )
      {
        ...
        MSG_MED("Rx'ed NAK: First = %u, Last = %u", first_seq, last_seq, 0);
      ...
      else if( frame_type == RLP_NAK_TYPE1_FR )
      {
        ...
        -------------------------------------------------------------------*/
        MSG_MED("Rx'ed Bitmap NAK:", 0, 0, 0);
        for(i = 0; i <= nak_map_count; i++)
        {
          ...
          MSG_MED(" 1st seq = %u, Bitmap = 0x%x", first_seq, bitmap, 0);
    ...
    switch( rlp_state_info.state)
    {
      case RLP_SYNC_STATE:
        ...
        if( rlp_state_info.first_sync_rcvd == FALSE)
        {
          ...
          MSG_HIGH("1st SYNC Received", 0, 0, 0);
        }
        break;
      ...
      case RLP_ESTABLISHED_STATE:
        ...
        -------------------------------------------------------------------*/
        ERR( "Invalid SYNC, reset RLP", rlp_state_info.state, 0, 0);
        rlp_gen_stats.resets++;
        rlp_reset();
        return;
    ...
    switch( rlp_state_info.state)
    {
      case RLP_SYNC_STATE:
        ...
        if( rlp_state_info.first_sync_rcvd)
        {
          ...
          MSG_HIGH("Received ACK, state = RT_MEAS", 0, 0, 0);
        }
        break;
      ...
      case RLP_ESTABLISHED_STATE:
        ...
        -------------------------------------------------------------------*/
        ERR( "ACK when ESTABLISHED, reset RLP", 0, 0, 0); 
        rlp_gen_stats.resets++;
        rlp_reset();
        break;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    ERR("RLP Reset: Max V(N/R), %d/%d", rlp_state_info.needed_seq_num, current_seq_num, 0);
    rlp_gen_stats.resets++;
    rlp_reset();
    return(TRUE);
  }
    ...
    if( nak_ptr->tx_done )
    {
      ...
      if( rlp_new_or_idle_rcvd )
      {
        ...
        if( nak_ptr->age == (rlp_state_info.rlp_delay + RLP_RTT_MARGIN) )
        {
          ...
          -----------------------------------------------------------------*/
          MSG_MED("Sending 2nd NAK for seq #%u", nak_ptr->seq, 0, 0);
...
#error code not present
        ...
        else if( nak_ptr->age == 2 * (rlp_state_info.rlp_delay + RLP_RTT_MARGIN) )
        {
          ...
          if( rlp_type == RLP_TYPE_I )
          {
            MSG_MED("Sending 3rd NAK for seq #%u", nak_ptr->seq, 0, 0);
            nak_ptr->repeat_cnt = 3;
            nak_ptr->tx_done = FALSE;
            naks_to_send_cnt++;
          }
...
#error code not present
                 ...
                 (nak_ptr->age ==  RLP_MAX_HOLE_AGE) )
        {
          ...
          {
            ERR("NAK entry too old, seq/age: %u/%u", nak_ptr->seq, nak_ptr->age, 0);
          }
        } /* if age == 3*(RTT + margin) */
    ...
    if( abort )
    {
      if( nak_ptr->seq >= rlp_state_info.needed_seq_num )
      {
        ...
        -------------------------------------------------------------------*/
        ERR("RLP frame aborted: %u, V(N/R): %u/%u", 
             nak_ptr->seq,
      ...
      else  /* seq < V(N) */
      {
        ...
        -------------------------------------------------------------------*/
        MSG_MED("NAK entry for seq #%u died natural death", nak_ptr->seq, 0, 0);
      }
  ...
  for(i = 0; i < rlp_rx_frame_count; i++)
  {
                     ...
                     rlp_state_info.expected_seq_num) )
    {
          ...
          current_seq = (current_seq + 1) % RlpSeqModulus)
      {
        ...
        {
          ERR("No free NAKs V(R)/rx'ed: %u/%u", 
              rlp_state_info.expected_seq_num, rlp_rx_frame_list[i].frame_seq, 0);
        }
        else
        {
          /*-------------------------------------------------------------------
...
#error code not present
          ...
          {
            MSG_HIGH("Hole in RLP seq#, V(R)/rx'ed: %u/%u", 
                      rlp_state_info.expected_seq_num,
                      rlp_rx_frame_list[i].frame_seq, 
                      0 );
            MSG_HIGH("Rx'ed Frame Rate/Type/Ch # = %u/%u/%u", 
                      rlp_rx_frame_list[i].frame_rate,
                      rlp_rx_frame_list[i].frame_type, 
                      rlp_rx_frame_list[i].ch_num);
            MSG_HIGH("Rx'ed Frame Data: 0x%x 0x%x 0x%x", 
      ...
      (rlp_state_info.needed_seq_num == rlp_state_info.expected_seq_num) )
  {
    ...
    -------------------------------------------------------------------------*/
    MSG_HIGH("V(N) = V(R)", rlp_state_info.expected_seq_num, 0, 0);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  while( (rlp_reassembly_buf[needed_index].item_ptr != NULL) )
  {
    ...
    {
      ERR("Wrapped entry for V(N) = %u in reassembly buffer", rlp_state_info.needed_seq_num, 0, 0);
    }
    else
    {
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  else if( GT_SEQ_CHECK( current_seq_num, rlp_state_info.needed_seq_num))
  {
    ...
    if( rlp_reassembly_buf[buf_index].item_ptr != NULL )
    {
      ...
      {
        MSG_MED("Reassembly duplicate %u, V(N/R) = %u/%u", 
                 current_seq_num,
                 rlp_state_info.needed_seq_num,
                 rlp_state_info.expected_seq_num);
      ...
      {
        ERR("%u exceeds Max V(N)<->V(R) delta, V(N/R) = %u/%u",
             current_seq_num,
             rlp_state_info.needed_seq_num,
             rlp_state_info.expected_seq_num);
      }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
       ...
       RLP_SEGFR_LASTSEG_MASK)
  {
        ...
        rlp_rx_segfr_data_len) <= rlp_prifull_table[rlp_rate_set].format_b_payload )
    {
      /*---------------------------------------------------------------------
        ...
        primary Full Rate frame. Set Set destination pointer.
      ---------------------------------------------------------------------*/
    if( ((frame_ptr->gen_data.ctl_len & RLP_GEN_CTL_LEN_MASK) +
         rlp_rx_segfr_data_len) < rlp_prifull_table[rlp_rate_set].format_b_payload )
       {
      ...
      /*---------------------------------------------------------------------
        Now copy the payload data and set return value.
      ---------------------------------------------------------------------*/
      (void)memcpy( dest_ptr + rlp_rx_segfr_data_len,
                    (void *)frame_ptr->gen_data.data,
                    (frame_ptr->gen_data.ctl_len & RLP_GEN_CTL_LEN_MASK));
      (void)memcpy( dest_ptr, rlp_rx_segfr_data, rlp_rx_segfr_data_len);
  ...
  else /* Not last segment */
  {
    ...
    -----------------------------------------------------------------------*/
    (void)memcpy( rlp_rx_segfr_data + rlp_rx_segfr_data_len,
                  (void *)frame_ptr->gen_data.data,
                  (frame_ptr->gen_data.ctl_len & RLP_GEN_CTL_LEN_MASK) );
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*-------------------------------------------------------------------------
    Set the tracer field to indicate where the data item is being stored.
  -------------------------------------------------------------------------*/
  data_frame_ptr->tracer = DSM_RLP_POST_RX_Q;
#endif
  
  q_put( rlp_post_rx_q_ptr, 
         &data_frame_ptr->link);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( rlp_type == RLP_TYPE_I )
  {
    ...
    if( (frame_type == RLP_UNSEG_DATA_FR) && GT_SEQ_CHECK(rlp_state_info.expected_seq_num, frame_seq) )
    {
      ...
      if( nak_ptr == NULL )
      {
        MSG_MED("No NAK list entry for re-tx frame: %u", frame_seq, 0, 0);
      }
      else
      {
#ifdef FEATURE_Q_NO_SELF_QPTR
  ...
  else if( rlp_type == RLP_TYPE_II )
  {
    ...
    else if( frame_type == RLP_REXMIT_DATA_FR )
    {
      ...
      if( nak_ptr == NULL )
      {
        MSG_MED("No NAK list entry for re-tx frame: 0x%x", 
                frame_ptr->seq_num, 0, 0);
      }
      else
      {
       ...
       if( nak_ptr->age < (rlp_state_info.rlp_delay + RLP_RTT_MARGIN) )
       {
        ...
        {
          MSG_MED("Re-tx rx'ed for seq #%u after 1st timeout", nak_ptr->seq, 0, 0);
    ...
    {
      MSG_HIGH("Unknown RLP frame type, can't compute L_SEQ", 0, 0, 0);
    }
  } /* RLP Type 2 */
  else
  {
    ERR("Unknown RLP Type, can't get frame seq num", 0, 0, 0 );
  }
    ...
    case RLP_EIGHTH_RATE:
      ...
      if( rlp_traffic_type == RLP_TRAFFIC_TYPE_PRI )
      {
        if( nr_table[ frame_ptr->pri_idle.seq_num] != frame_ptr->pri_idle.nr_crc )
        {
          ...
          if( rlp_state_info.state == RLP_ESTABLISHED_STATE )
          {
            ERR("RLP Idle CRC error: 0x%x, 0x%x, 0x%x", 
...
#error code not present
                 ...
                 ((frame_ptr->pri_idle.rs2_filler & RLP_RS2_IDLE_FILL_MASK) == RLP_RS2_FILL_LAST_BITS) )
        {
          ...
          -----------------------------------------------------------------*/
          MSG_HIGH("Received an intersegment fill frame",0,0,0);
                 ...
                 ((frame_ptr->pri_idle.rs2_filler & RLP_RS2_IDLE_FILL_MASK) != RLP_RS2_IDLE_LAST_BITS) )
        {
          ...
          if( rlp_state_info.state == RLP_ESTABLISHED_STATE)
          {
            ERR("RLP RS 2 non-zero filler", 0, 0, 0);
          }
          rlp_pri_frate_errs.idle_fr_err++;
          frame_status = RLP_RX_FRAME_ERR;
          break;
                 ...
                 (frame_ptr->pri_idle.seq_num != rlp_saved_eighth.seq_num) )
        {
          /*-----------------------------------------------------------------
            ...
            not incremented.
          -----------------------------------------------------------------*/
          ERR("Idle no match previous, was/is: %u/%u", 
        ...
        else   /* idle frame is good - so far */
        {
          ...
          else if( last2_idles_same )
          {
            ...
            if( GT_SEQ_CHECK(frame_seq, rlp_state_info.expected_seq_num) )
            {
              ...
              -------------------------------------------------------------*/
              ERR("3 Matching 1/8, now: %u, was: %u", 
                   frame_seq, rlp_state_info.expected_seq_num, 0);
              frame_type = RLP_IDLE_FR;
              frame_status = RLP_RX_NO_DATA_AVAILABLE;
            }
            else /* RLP must Re-sync */
            {
              /*-------------------------------------------------------------
                ...
                RLP and return.
              -------------------------------------------------------------*/
              ERR("RLP Idle Seq err, Re-Syncing", 0, 0, 0);
      ...
      else /* Invalid Secondary Frame */
      {
        ...
        -------------------------------------------------------------------*/
        ERR( "Invalid Secondary RLP Frame RX'd", 0, 0, 0);
        rlp_state_info.contig_erase++;
#ifdef FEATURE_DS_PSTATS
#error code not present
#endif /* FEATURE_DS_PSTATS */
    ...
    case RLP_FULL_RATE:
               ...
               rlp_prifull_table[rlp_rate_set].type_mask) != rlp_prifull_table[rlp_rate_set].format_a )
      {
        ...
        -------------------------------------------------------------------*/
        MSG_HIGH("Full rate type error: 0x%x, ch: %d",
                  frame_ptr->pri_fr_data.data[rlp_prifull_table[rlp_rate_set].format_b_payload],
                  ch_num,
                  0);
    ...
    case RLP_QUARTER_RATE:
      ...
      if ( rlp_gendata_maxlen_table[frame_rate][rlp_params_offset] == 0 )
      {
        ...
        {
          MSG_HIGH("Rate 1/4 frame rx'ed for Rate Set 1", 0, 0, 0);
        }
        else
        {
          /*-----------------------------------------------------------------
          ...
          -----------------------------------------------------------------*/
          ERR("Pathological erasure received from Mux Layer", 0, 0, 0);
        }
        rlp_prev_pri_fr_rate = RLP_ERASURE; 
        frame_status = RLP_RX_FRAME_ERR;
        break;
      ...
      if( gen_frame_status != RLP_FRAME_OK )
      {
        ...
        {
          ERR("Control frame with bad CRC rxed", 0, 0, 0);
...
#error code not present
        ...
        {
          ERR("Frame Err: %u, Rate: %u, Chan: %u", gen_frame_status, frame_rate, rlp_rate_set);
        }
    ...
    default:
      ...
      ---------------------------------------------------------------------*/
      ERR("Unknown RLP data frame rate: %u", frame_rate, 0, 0);
    ...
    if( frame_status == RLP_RX_FRAME_ERR )
    {
      ...
      if( rlp_state_info.state == RLP_ESTABLISHED_STATE ) 
      {
        ...
        {
          ERR("RLP Reset, Max Erasures: %d", rlp_state_info.contig_erase, 0, 0);
    ...
    if( rlp_state_info.state == RLP_RT_MEAS_STATE )
    {
      ...
      {
        ERR("Bad 1st Non-ctl frame, Re-sync", 0, 0, 0);
...
#ifndef FEATURE_DATA_MM
      /*-----------------------------------------------------------------------
        Setting state for the UI last call status function. Note, there is a 
        check to see if this is the beginning of a call or a RLP reset in the 
        middle of call. The variable is not set in the latter case.
      -----------------------------------------------------------------------*/
      if ( ds_last_call_var == DS_LAST_RLP_NOT_ESTABLISHED )
      {
        ds_last_call_var = DS_LAST_TCP_NOT_ESTABLISHED;
      }
#endif /* FEATURE_DATA_MM */
      MSG_HIGH("RLP established, RLP_DELAY = %d", rlp_state_info.rlp_delay, 0, 0);
    }
    ...
    {
      MSG_MED("Processing Segmented Data Frame", 0, 0, 0);
    ...
    MSG_LOW("L_V(R), L_V(N), L_SEQ = %d, %d, %d", rlp_state_info.expected_seq_num, rlp_state_info.needed_seq_num, frame_seq);
    ...
    if( frame_seq == 0xFFFF )
    {
      MSG_MED("Error in computing L_SEQ", 0, 0, 0);
      (void)dsm_free_buffer(item_ptr);
      continue;
    }
  ...
  {
    MSG_MED("Dispatch frame from Re-assembly", 0, 0, 0);
    (rlp_post_rx_func_ptr)();
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    ERR("Unknown RLP Type: %d, can't build Type 0 NAK", rlp_type, 0, 0);
  }
} /* build_type0_nak() */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( rlp_type == RLP_TYPE_I )
  {
    ERR("Bitmap NAK is not defined for RLP Type 1", 0, 0, 0);
  }
  ...
  else if( rlp_type == RLP_TYPE_II )
  {
    ...
    {
      MSG_MED(" 1st seq = %u, Bitmap = 0x%x", nak_map[i].first_seq, nak_map[i].bitmap, 0);
      nak_map_seq_fpos = RLP_TYPE2_NAK1_NAK_MAP_FPOS + (RLP_TYPE2_NAK1_NAK_MAP_SEQ_LEN + RLP_TYPE2_NAK1_NAK_MAP_LEN) * i;
      nak_map_fpos = nak_map_seq_fpos + RLP_TYPE2_NAK1_NAK_MAP_SEQ_LEN;     
  ...
  {
    ERR("Unknown RLP Type: %d, can't build Type 1 NAK", 0, 0, 0);
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    ERR("Unknown RLP Type: %d, can't get a NAK frame", 0, 0, 0);
    return;
  }
  ...
  if( do_first_last_nak && found_first )
  {
    ...
    -------------------------------------------------------------------------*/
    MSG_MED("Sending NAK: first = %u, last = %u", first, last, 0);
  ...
  else if( do_bitmap_nak && (nak_map_cnt > 0) )
  {
    ...
    MSG_MED("Sending NAK: #maps = %u", nak_map_cnt, 0, 0);
    build_type1_nak(nak_ptr, current_seq, nak_map_cnt, nak_map);
  }
  else
  {
    ERR("%u NAKs to send, but NAK list is empty!", naks_to_send_cnt, 0, 0);
  }
...
)
{
  ...
  word                  isave;              /* PSW when disabling ints     */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  else if( rlp_state_info.re_xmit_frames_cnt != 0)
  {
    ...
    if( rlp_tx_segfr_active)
    {
      ...
      *frame_ptr                       = (byte *)&tx_frame.frame;
      ...
      (void)memcpy((void *)tx_frame.frame.gen_data.data, rlp_tx_segfr_src_ptr, 
                    actual_fr_payload);
            ...
            rlp_tx_watermark_ptr->current_cnt != 0))
  {
    ...
    if( rlp_tx_data_ptr == NULL)
    {
      ...
      ----------------------------------------------------------------*/
      rlp_tx_data_ptr->tracer = DSM_RLP_TX;
#endif
      meta_info_ptr =
           (ps_meta_info_type *) rlp_tx_data_ptr->app_field;
      ...
      ( cur_packet_length = dsm_length_packet( rlp_tx_data_ptr)))
    {
      ...
      if(( next_item_ptr = dsm_dequeue( rlp_tx_watermark_ptr)) != NULL)
      {
        ...
        /*----------------------------------------------------------------
         Set tracer field to indicate where the data item is being stored
        ----------------------------------------------------------------*/
        next_item_ptr->tracer = DSM_RLP_TX;
#endif
        meta_info_ptr =
             (ps_meta_info_type *) next_item_ptr->app_field;
        if ( meta_info_ptr != NULL )
        {
          PS_META_INFO_FREE ( &meta_info_ptr );
        }
    ...
    else /* Generic frame format */
    {
      ...
      if( data_filler_cnt != 0)
      {
        /* Below if check is just to fix KW error. Revisit for any issues. */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  MSG_MED("RLP initialized with:", 0, 0, 0);
  ...
  if( rlp_traffic_type == RLP_TRAFFIC_TYPE_PRI )
    {
      ...
      rlp_ct_offset = 0;
      MSG_MED("  Traffic Type = Primary", 0, 0, 0);
    }
  else if( rlp_traffic_type == RLP_TRAFFIC_TYPE_SEC )
    {
      /* RLP on secondary traffic */
      rlp_ct_offset = 1;
      MSG_MED("  Traffic Type = Secondary", 0, 0, 0);
    }
  ...
  if( rlp_rate_set == RLP_RATE_SET_1 )
    {
      rlp_rs_offset = 0;
      MSG_MED("  Rate Set = Rate Set 1", 0, 0, 0);
    }
  else if( rlp_rate_set == RLP_RATE_SET_2 )
    {
      rlp_rs_offset = 2;
      MSG_MED("  Rate Set = Rate Set 2", 0, 0, 0);
    }
  ...
  if( rlp_type == RLP_TYPE_I )
  {
    ...
    MSG_MED("  RLP Type = RLP Type 1 (IS-707)", 0, 0, 0);
  }
  else if( rlp_type == RLP_TYPE_II )
  {
    ...
    MSG_MED("  RLP Type = RLP Type 2 (IS-707-A/MDR)", 0, 0, 0);
  }
