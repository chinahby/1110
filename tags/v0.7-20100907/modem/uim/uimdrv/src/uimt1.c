...
$Header: //source/qcom/qct/modem/uim/uimdrv/rel/07H1_2/src/uimt1.c#3 $ $DateTime: 2009/05/04 18:36:44 $ $Author: shanmugh $
...
uim_transport_protocol_type uim_operating_protocol;

/* This holds the current protocol used by the server with the card */
uim_transport_protocol_type uim_current_protocol;

/* This holds the current protocol used by the server with the card */
byte uim_t_1_node_address = 0; /* Set to this value by default */

/* This holds the last S-block code sent by the MSM */
byte uim_t_1_s_block_code;
/* This holds the last S-block data sent by the MSM */
byte uim_t_1_s_block_data;
/* This holds the last S-block data size sent by the MSM */
byte uim_t_1_s_block_data_size;
...
int uim_num_txd_bytes;

/* This holds the prologue field of the transmitted T=1 block */
uim_prologue_field_type uim_tx_prologue_field;

/* This holds the length of the last sent Chained I Block */
word uim_last_tx_i_block_length;

/* This points to the transmitted prologue field */
byte *uim_rx_prologue_field_buffer_ptr = (byte *)&uim_tx_prologue_field;

/* This holds the epilogue field of the T=1 block */
uim_epilogue_field_type uim_tx_epilogue_field;

/* This holds the transmitted prologue field of the T=1 block */
uim_prologue_field_type uim_rx_prologue_field_buffer;

/* This holds the size of the epilogue field */
int uim_epilogue_field_size;

/* This holds the current EDC selection */
uim_t_1_edc_type uim_t_1_edc;

/* This holds the EDC for Rx block */
word uim_error_detection_code;

/* Holds the block waiting time index */
rex_timer_cnt_type uim_t_1_bwi;

/* Holds the character waiting time index */
rex_timer_cnt_type uim_t_1_cwi;

/* This holds the character waiting time */
rex_timer_cnt_type uim_t_1_cwt;

/* This holds the block waiting time */
rex_timer_cnt_type uim_t_1_bwt;

/* This holds the block waiting time */
rex_timer_cnt_type uim_t_1_wtx = 1;

/*===========================================================================
  I N T E R N A L   F U N C T I O N   P R O T O T Y P E S
===========================================================================*/
LOCAL void uim_t_1_resynch_w_card
(
  void
);

LOCAL void uim_t_1_resend_s_block
(
  void
);

LOCAL uim_cmd_type *uim_t_1_process_s_block
(
  uim_rsp_buf_type *rsp_ptr, /* points to the response buffer */
  uim_cmd_type     *cmd,     /* points to current command */
...
LOCAL void uim_t_1_process_bad_block
(
  void
);

LOCAL uim_cmd_type *uim_process_t_1_command_response
(
  uim_rsp_buf_type *rsp_ptr, /* points to the response buffer */
  uim_cmd_type     *cmd,     /* points to the pending command */
  rex_sigs_type    *i_mask   /* Mask of signals interested in */
);

LOCAL boolean is_t_1_block_received_valid
(
  uim_rsp_buf_type const *rsp_ptr,
  uim_cmd_type     const *cmd
);

LOCAL uim_block_params_type uim_get_rx_block_type(void);
...
LOCAL uim_block_params_type uim_get_rx_block_type(void)
{
  ...
  uim_block_params.node_address_byte = uim_rx_prologue_field_buffer.nad;
  /* -------------------------------------------------- */
  /* Determine if the block type is R or S */
  if (uim_rx_prologue_field_buffer.pcb & UIM_T_1_PCB_R_BLOCK)
  {
    /* Determine if the block type is S */
    if ((uim_rx_prologue_field_buffer.pcb & UIM_T_1_PCB_S_BLOCK) ==
         UIM_T_1_PCB_S_BLOCK)
    {
      /* The block type is S */
      ...
      uim_block_params.block_params.s_block =
        (uim_s_block_type)
        (uim_rx_prologue_field_buffer.pcb & ~UIM_T_1_PCB_S_BLOCK);
    }
    else /* The block type is R */
    {
      uim_block_params.block_type = UIM_T_1_BLOCK_TYPE_R;
      uim_block_params.block_params.r_block.error_code =
         uim_rx_prologue_field_buffer.pcb & UIM_T_1_PCB_ERR_CODE_MASK;
      /* Check the send sequence number */
      if (uim_rx_prologue_field_buffer.pcb & UIM_T_1_PCB_NEXT_SEQ_NUM)
      {
  ...
  else /* The block type is I */
  {
    ...
    /* Check the send sequence number */
    if (uim_rx_prologue_field_buffer.pcb & UIM_T_1_PCB_SEND_SEQ_NUM)
    {
    ...
    /* Check the more bit */
    if (uim_rx_prologue_field_buffer.pcb & UIM_T_1_PCB_MORE_DATA_BIT)
    {
...
{
  MSG_LOW( "UIM T=1 sending a Resynch", 0, 0, 0 );

  /* Set the S-block parameters for resynch */
  uim_t_1_s_block_code = UIM_T_1_PCB_RESYNC_REQ;
  uim_t_1_s_block_data_size = 0;
  /* Set the T=1 protocol state for sending an S-block request */
  uim_t_1_protocol_state = UIM_T_1_SENT_SUPERVISORY_BLOCK_ST;

  /* Send the resynch block*/
  uim_t_1_send_s_block (uim_t_1_node_address,
                        uim_t_1_s_block_code,
                        &uim_t_1_s_block_data,
                        uim_t_1_s_block_data_size);
...
{
  MSG_LOW( "UIM T=1 re-sending an S-block", 0, 0, 0 );
  /* Process received block with errors */
  if (++uim_t_1_error_count > UIM_MAX_BAD_BLOCK_RETRIES)
  {
    /* Determine if resynch was last sent */
    if (UIM_T_1_PCB_RESYNC_REQ == uim_t_1_s_block_code ||
        UIM_T_1_PCB_IFS_REQ    == uim_t_1_s_block_code )
    {
...
{
  MSG_LOW( "UIM T=1 processing an S-block", 0, 0, 0 );
  if (rsp_ptr == NULL)
  {
  ...
  if (UIM_T_1_SENT_SUPERVISORY_BLOCK_ST == uim_t_1_protocol_state)
  {
    /* Make sure this S-block is the response to the S-block sent */
    if ((int)uim_block_params.block_params.s_block ==
        (uim_t_1_s_block_code | UIM_T_1_PCB_S_RSP))
    {
      uim_cmd_type* result = NULL;
        ...
        case UIM_S_BLOCK_RESYNCH_RESPONSE:
          {
            ...
            uim_t_1_send_i_block (uim_t_1_node_address);
        ...
        case UIM_S_BLOCK_IFS_RESPONSE:
          {
            ...
            if ((rsp_ptr->rsp.data[uim_apdu_rsp_num_bytes] >= UIM_T_1_MIN_IFS) &&
                (rsp_ptr->rsp.data[uim_apdu_rsp_num_bytes] <= UIM_T_1_MAX_IFS))
            {
              uim_t_1_ifsd = rsp_ptr->rsp.data[uim_apdu_rsp_num_bytes];
            }
            ...
            result = ( uim_process_card_response(cmd, i_mask));
  ...
  else /* not waiting for an S-block response */
  {
    ...
    switch (uim_block_params.block_params.s_block)
    {
      case UIM_S_BLOCK_RESYNCH_REQUEST:
        {
          /* Reset the sequence numbers */
          uim_me_send_sequence_num = 0;
          uim_card_send_sequence_num = 0;

          /* Respond with a resynch response */
          /* Set the S-block parameters for resynch */
          uim_t_1_s_block_code = UIM_T_1_PCB_RESYNC_RSP;
          uim_t_1_s_block_data_size = 0;
          ...
          uim_t_1_protocol_state = UIM_T_1_FAILED_CMD_ST;
      ...
      case UIM_S_BLOCK_IFS_REQUEST:
        {
          ...
          if ((rsp_ptr->rsp.data[uim_apdu_rsp_num_bytes] >= UIM_T_1_MIN_IFS) &&
              (rsp_ptr->rsp.data[uim_apdu_rsp_num_bytes] <= UIM_T_1_MAX_IFS))
          {
            uim_t_1_ifsc = rsp_ptr->rsp.data[uim_apdu_rsp_num_bytes];
          }

          /* Respond with an IFS response block */
          /* Set the S-block parameters for IFS */
          uim_t_1_s_block_code = UIM_T_1_PCB_IFS_RSP;
          uim_t_1_s_block_data = rsp_ptr->rsp.data[uim_apdu_rsp_num_bytes];
          uim_t_1_s_block_data_size = 1;
      ...
      case UIM_S_BLOCK_ABORT_REQUEST:
        {
          ...
          uim_t_1_s_block_code = UIM_T_1_PCB_ABORT_RSP;
          uim_t_1_s_block_data_size = 0;
          ...
          if ( uim_t_1_protocol_state == UIM_T_1_SENT_CHAINED_APDU_ST )
          {
            ...
            uim_me_send_sequence_num ^= uim_t_1_changed_me_send_seq_number;
          ...
          else if ( uim_t_1_receiving_chained_block )
          {
            /* If we are receiving a chain, we need to abort the
             * current reception and start receiving fresh
             * chained block
             */
            uim_t_1_changed_me_send_seq_number = 0;
            
            /* Reset the response buffer to the initial values */
            uim_apdu_rsp_buffer_ptr = back_up_uim_apdu_rsp_buffer_ptr;

            uim_apdu_rsp_num_bytes = 0;
          }
          else
          {
            /* Set the protocol state to reflect a failed command */
      ...
      case UIM_S_BLOCK_WTX_REQUEST:
        {
          ...
          uim_t_1_wtx = rsp_ptr->rsp.data[uim_apdu_rsp_num_bytes];

          /* Respond with an IFS response */
          /* Set the S-block parameters for IFS */
          uim_t_1_s_block_code = UIM_T_1_PCB_WTX_RSP;
          uim_t_1_s_block_data = rsp_ptr->rsp.data[uim_apdu_rsp_num_bytes];
          uim_t_1_s_block_data_size = 1;
...
{
  MSG_LOW( "UIM T=1 processing a bad block", 0, 0, 0 );
  /* Process received block with errors */
  if (++uim_t_1_error_count > UIM_MAX_BAD_BLOCK_RETRIES)
  {
    /* Reset the error count */
  ...
  {
    /* Send an R-block */
    uim_t_1_send_r_block (uim_t_1_node_address,
...
{
  MSG_LOW( "uim_process_t_1_command", 0, 0, 0 );
  if (uim_req == NULL)
  {
  ...
  uim_t_1_apdu_req_buf_ptr = uim_req;
  uim_t_1_apdu_callback_func = uim_req->rpt_function;
  uim_t_1_apdu_resp_buf = uim_req->rsp_ptr;

  /* Initialize the APDU request buffer pointer */
  uim_apdu_req_buffer_ptr = (byte *)&uim_req->apdu_hdr;

  /* Initialize the APDU response buffer pointer and response size */
  uim_apdu_rsp_buffer_ptr = (byte *)&uim_req->rsp_ptr->rsp;
  
  /* back-up the initial response buffer pointer */
  back_up_uim_apdu_rsp_buffer_ptr = uim_apdu_rsp_buffer_ptr;
  ...
  switch (uim_req->instrn_case)
  {
    case UIM_INSTRN_CASE_1:
      {
        ...
        uim_apdu_request_bytes_to_send = NUM_BYTES_APDU_HDR - 1;
    ...
    case UIM_INSTRN_CASE_2:
      {
        /* Send only the APDU */
        uim_apdu_request_bytes_to_send = NUM_BYTES_APDU_HDR;
    ...
    case UIM_INSTRN_CASE_3:
      {
        ...
        uim_apdu_request_bytes_to_send = NUM_BYTES_APDU_HDR;
        uim_apdu_request_bytes_to_send += uim_req->apdu_hdr.p3;
    ...
    case UIM_INSTRN_CASE_4:
      {
        ...
        uim_apdu_request_bytes_to_send++;
    ...
    case UIM_INSTRN_IFS_BLOCK:
      {
        /* Set the S-block parameters for resynch */
        uim_t_1_s_block_code = UIM_T_1_PCB_IFS_REQ;
        uim_t_1_s_block_data_size = 1;
        uim_t_1_s_block_data = uim_req->req.data[0];
    ...
    default:
      {
        ...
        ERR( "Received a bad instrn case from instn %d",
             uim_req->apdu_hdr.instrn,0,0);
        /* Emulate an error for this command */
        uim_t_1_apdu_resp_buf->rsp.data[0] = (int)SW1_PROBLEM;
        uim_t_1_apdu_resp_buf->rsp.data[1] = (int)SW1_PROBLEM;
  ...
  uim_t_1_changed_me_send_seq_number = 1;
...
)
{
  ...
  MSG_LOW( "UIM process t=1 response", 0, 0, 0 );
  ...
  rsp_ptr->cmd_rsp_size = uim_apdu_rsp_num_bytes - 2;
  response_buffer_index = uim_apdu_rsp_num_bytes - 2;
  ...
  switch (rsp_ptr->sw1)
  {
    ...
    case SW1_END_RESP:      /* Normal ending of the command with a rsp */
      {
        ...
        rsp_ptr->cmd_status = UIM_PROBLEM;
        ERR ("Response procedure byte received in T=1 protocol",0,0,0);
    ...
    case SW1_END_RETRY:     /* Command successful with update retries or
                               memory error                       */
      {
        ...
        else /* successful after retries */
        {
          ...
          rsp_ptr->cmd_rsp_size = uim_apdu_rsp_num_bytes;

          /* Indicate APDU result */
          rsp_ptr->cmd_status = UIM_SUCCESS_AFTER_MULTI_TRIES;
    ...
    case SW1_SECURITY:      /* A security problem */
      {
        ...
        switch (rsp_ptr->sw2)
        {
...
#if defined(FEATURE_UIM_USIM) || defined(FEATURE_UIM_UICC)
          case SW2_APPLN_SPECIFIC_AUTH_ERR:  /* Applcn specific auth error */
            ...
            break;
#endif /* FEATURE_UIM_USIM || FEATURE_UIM_UICC */
    ...
    default:
      {
        /* Log the error */
        ERR ("The status word is not expected - %x",rsp_ptr->sw1,0,0);
  ...
  /* Call the response callback function */
  return( uim_process_card_response(cmd, i_mask));
...
)
{
  ...
  uim_block_params = uim_get_rx_block_type();
  
  /* check if we have the right NAD with which we have transmitted */
  if ( uim_block_params.node_address_byte != 
       uim_tx_prologue_field.nad )
  {
    /* The received block should be of the same NAD */
    ERR("Block received with node address %x (should be %x)",
             uim_block_params.node_address_byte,uim_tx_prologue_field.nad,
             0);
    return FALSE;
  }
  ...
  if (uim_block_params.block_type == UIM_T_1_BLOCK_TYPE_S)
  {
    ...
    if (uim_rx_prologue_field_buffer.pcb & UIM_T_1_PCB_S_RSP  && 
        uim_t_1_protocol_state != UIM_T_1_SENT_SUPERVISORY_BLOCK_ST )
    {
      /* The s block should not be a response */
      ERR("S block received with response bit set (should be a request)",
               0,0,0);
      return FALSE;
    }
       ...
       )
    {
      if ( rsp_ptr->cmd_rsp_size != 1 )
      {
        ...
        ERR("S block IFS OR WTX received with length %x (should be 1)",
            rsp_ptr->cmd_rsp_size,0,0);
        return FALSE;
      }
    }
    else if ( rsp_ptr->cmd_rsp_size != 0 )
    {
      ...
      ERR("S block RESYNC OR ERROR or ABORT received with length %x (should be 0)",
          rsp_ptr->cmd_rsp_size,0,0);
      return FALSE;
    }

    if ( ( uim_rx_prologue_field_buffer.pcb & UIM_T_1_SBLK_B1_B5_MASK ) >
         UIM_T_1_SBLK_B1_B5_MAX_VAL )
    {
      /* Value encoded is RFU */
      ERR("S block received with RFU values encoded",
          (uim_rx_prologue_field_buffer.pcb & UIM_T_1_SBLK_B1_B5_MASK),0,0);
      return FALSE;
    }
  ...
  else if (uim_block_params.block_type == UIM_T_1_BLOCK_TYPE_R)
  {
    if ( rsp_ptr->cmd_rsp_size != 0 )
    {
      ...
      ERR("R block received with length %x (should be 0)",
          rsp_ptr->cmd_rsp_size,0,0);
      return FALSE;
    }
    else if ( ( uim_rx_prologue_field_buffer.pcb & UIM_T_1_RBLK_B1_B6_MASK ) >
                UIM_T_1_RBLK_B1_B6_MAX_VAL )
    {
      /* RFU bits populated in the PCB */
      ERR("R block received with RFU values encoded",
          (uim_rx_prologue_field_buffer.pcb & UIM_T_1_RBLK_B1_B6_MASK),0,0);
      return FALSE;
    }
  }

  
  MSG_LOW("Validated the received t_1 block returning TRUE",0,0,0);
  return TRUE;
} /*lint !e715 */
...
===========================================================================*/
uim_cmd_type *uim_process_t_1_block
(
...
)
{
  /* Holds the parsed parameters from a received T=1 block */
  uim_block_params_type uim_block_params;
  /* Holds the return cmd pointer value */
  uim_cmd_type *return_cmd_ptr = cmd;
  /* Holds if the block received is valid or not */
  boolean received_block_is_valid;

  /*-----------------------------------------------------------------*/

  MSG_LOW( "uim_process_t_1_block", 0, 0, 0 );
  if (rsp_ptr == NULL)
  {
    ...
    case UIM_T_1_FAILED_CMD_ST:
      {
        ...
        else
        {
          ...
          {
            /* Process the S-block */
            (void)uim_t_1_process_s_block (rsp_ptr,
                                     cmd,
                                     i_mask,
                                     uim_block_params);
          }
    ...
    case UIM_T_1_CACHED_SEL_RSP_ST: /* The SELECT response was already cached */
      {
        ...
        return_cmd_ptr = uim_process_t_1_command_response(rsp_ptr, cmd, i_mask);
      }/* end case - UIM_T_1_CACHED_SEL_RSP_ST */
      break;

    case UIM_T_1_SENT_APDU_ST:  /* An APDU response is expected */
      {
        received_block_is_valid = is_t_1_block_received_valid(rsp_ptr, cmd);
            ...
            case UIM_T_1_BLOCK_TYPE_I:
              {
                    ...
                    uim_card_send_sequence_num)
                {
                  ...
                  /* Change the send sequence number for the next I-block from
                     the card */
                  uim_card_send_sequence_num ^= UIM_T_1_CHANGE_SEQ_NUM_MASK;
  
                  /* Adjust the response buffer pointer for the next block */
                  uim_apdu_rsp_buffer_ptr += rsp_ptr->cmd_rsp_size;
  
                  /* Keep a total of the total number of bytes in the
                     response */
                  uim_apdu_rsp_num_bytes += rsp_ptr->cmd_rsp_size;
                  ...
                  if (uim_block_params.block_params.i_block.more_bit)
                  {
                    ...
                    uim_t_1_receiving_chained_block = TRUE;
                  ...
                  else /* This is the entire response */
                  {
                    ...
                    uim_t_1_receiving_chained_block = FALSE;
                    ...
                    /* Process the received response for the upper layers */
                    return_cmd_ptr =
                      uim_process_t_1_command_response(rsp_ptr, cmd, i_mask);
            ...
            case UIM_T_1_BLOCK_TYPE_R:
              {
                ...
                if ((uim_block_params.block_params.r_block.send_seq_num ==
                    uim_me_send_sequence_num) && uim_t_1_error_count < 2 )
                {
                  /* Send out the I-block again */
                  uim_t_1_send_i_block (uim_t_1_node_address);
                }
            ...
            case UIM_T_1_BLOCK_TYPE_R:
              {
                ...
                else /* I-block is acknowledged, send next I-block */
                {
                  ...
                  /* Change the send sequence number for the next I-block */
                  uim_me_send_sequence_num ^= UIM_T_1_CHANGE_SEQ_NUM_MASK;

                  /* Update the pointer and size of the request buffer */
                  uim_apdu_req_buffer_ptr += uim_last_tx_i_block_length;
                  uim_apdu_request_bytes_to_send -= uim_last_tx_i_block_length;
    ...
    default:
      {
        ...
        ERR ("UIM T=1 protocol state is out of range",0,0,0);
      ...
      break;
  } /* end switch - uim_t_1_protocol_state */

  /* Return the command pointer */
  return( return_cmd_ptr );
...
)
{
  ...
  MSG_LOW( "uim_process_t_1_timeout", 0, 0, 0 );
