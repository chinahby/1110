...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_public.c#23 $ $DateTime: 2009/07/06 18:43:01 $ $Author: ebuller $
...
#if defined (FS_STANDALONE) || defined (FEATURE_EFS_BOOT_ENV)
  int efs_errno = 0;
#endif
...
/***********************************************************************
FUNCTION      efs_boot_initialize

DESCRIPTION   This function should only be called to initialize efs
              from boot code. Upon return EFS and HFAT will be initialized
              and any inserted sd/mmc devices will be mounted and
              available to access.
...
efs_boot_initialize ()
{
  ...
  /* Create all the basic inodes to be available to EFS while booting from
   * ROMFS. We can later mount an actual device over any of these and start
   * accessing it. We need an entry here for each block device otherwise we
   * err_fatal when trying to create them in hotplug_init.
   */
  static struct efs_romfs_node fs_boot_nodes[] =
  {
    {"/", NULL, 0},
    {"mmc1", NULL, 0},
    {"mmc2", NULL, 0},
    ...
    /* If feature is not available add the rest of the devices so we
     * can stil compile for off-target testing */
#ifndef FEATURE_EFS_BOOT_ENV
    {"mmc1_p", NULL, 0},
    {"ftl1", NULL, 0},
    {"ftl2", NULL, 0},
  ...
  /* If coldplug is enabled this will force a poll which should
   * mount any inserted hotplug devices. */
...
#elif defined(FEATURE_EFS_BOOT_ENV)
  (void) dev_attr;
  result = -1;                   /* Operation invalid on ROMFS */
#else
...
efs_image_prepare (void)
{
#if defined(FEATURE_EFS_QEFS2)
  {
    ...
    fs_global_lock ();
    rv = fs_efs2_image_prepare ();
    fs_global_unlock ();
    return rv;
  }
#elif defined(FEATURE_EFS_FDI) || defined (FEATURE_EFS_BOOT_ENV)
  return 0;
#else
