...
$Header: //source/qcom/qct/multimedia/qtv/legacymedia/filemedia/mp4parser/main/latest/src/tsmlparse.cpp#9 $
...
void TsmlHandle :: ResetSubStringAttributes(void)
{
  ...
  {
    QTV_Free(CurLinkInfo.linkValue);
    CurLinkInfo.linkValue = NULL;
  }
}
...
void TsmlHandle :: CopyLinkInfo ()
{
  if (S){
    ...
    S->linkValue = (char*)QTV_Malloc((CurLinkInfo.linkSize) + 1);
    (void)std_strlcpy(S->linkValue, CurLinkInfo.linkValue, (CurLinkInfo.linkSize) + 1);
    S->oLinking = 1;
  }
}
...
void TsmlHandle :: LinkInfo ()
{
  ...
  if ( (CurLinkInfo.linkValue = (char*)QTV_Malloc((sizeOfWord-8) + 1)) == NULL )
  {
...
{
  S = (SubStrings*)QTV_Malloc(sizeof(SubStrings));
  ...
  if ( S )
  {
    ...
    stringFontColor = stringFontColor;
...
void TsmlHandle :: SetStringPara ()
{
  if ( stringFlag && S)
  {
    ...
    if ( (S->textSubString = (char*)QTV_Malloc(S->sizeofTextSampleInBytes + 1)) == NULL )
    {
      errorFlag = ERROR_IN_ALLOCATING_MEMORY_FOR_STRING_MEMBER;
    }
    else
    {
     (void)std_strlcpy(S->textSubString, stringPointer,(S->sizeofTextSampleInBytes + 1));
