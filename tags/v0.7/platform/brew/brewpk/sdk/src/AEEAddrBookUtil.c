...
Copyright © 2000-2005,2007-2008 QUALCOMM Incorporated.
All Rights Reserved.
...
#if !defined (offset)
#define offset(x,s) (((char*)(&((s*)1)->x))-((char*)1))
#endif
...
uint32 AEEAddrFieldComplex_GetSize( const AEEAddrFieldComplex *pfc )
{
   ...
   pSubfield = (AEEAddrSubfield *)pfc->buf;
   ...
   while( nSubfields-- )
   {
      ...
      pSubfield = (AEEAddrSubfield *)((byte *)pSubfield + nBytesThisSubfield);
...
int AEEAddrFieldComplex_AddSubField( AEEAddrFieldComplex **pfc, const AEEAddrField *paf )
{
   ...
   if( pNew )
   {
      *pfc = pNew;
      ...
      pSubfield = (AEEAddrSubfield *)((byte *)pNew + nBytesOrig);
      pSubfield->fID = paf->fID;
      pSubfield->wDataLen = paf->wDataLen;
      pSubfield->fType = paf->fType;
      MEMCPY( pSubfield->buf + nPadBytes, paf->pBuffer, paf->wDataLen );
...
static AEEAddrSubfield * GetSubField( const AEEAddrFieldComplex *pfc, int nFieldIdx)
{
   ...
   if( nFieldIdx >= 0 && nFieldIdx < pfc->wNumSubfields )
   {
      ...
      {
         nPadBytes = CALC_PAD_BYTES(pSubfield->buf);
         pSubfield = (AEEAddrSubfield *)((byte *)pSubfield + pSubfield->wDataLen + offset(buf, AEEAddrSubfield) + nPadBytes);
      }
...
int AEEAddrFieldComplex_GetSubField( const AEEAddrFieldComplex *pfc, int nFieldIdx, AEEAddrField *paf )
{
   ...
   if( pSubfield )
   {
      ...
      paf->pBuffer = pSubfield->buf + nPadBytes;
