...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_namei.c#1 $ $DateTime: 2008/05/29 19:19:11 $ $Author: richardp $
...
fs_namei (struct nameidata *ndp, const char *path, int flag)
{
  ...
  if (path_len > FS_PATH_MAX)
    return -ENAMETOOLONG;
  fs_strlcpy (ndp->namebuf, path, sizeof (ndp->namebuf));
   ...
   * Make sure we only change path_last if we are altering string.  */
  if (path_len > 0)
  {
    while ((*(path_last - 1) == '/') && ((path_last - 1) != ndp->path))
    {
      path_last--;
      *path_last = 0;
    }
