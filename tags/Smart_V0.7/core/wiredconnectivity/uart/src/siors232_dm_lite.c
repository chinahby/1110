...
/*===========================================================================
  ...
  $Header: //depot/asic/msmshared/drivers/uart/02.00.XX/siors232_dm_lite.c#15 $ $DateTime: 2009/06/16 14:32:17 $ $Author: cmihalik $
...
#if(( defined( FEATURE_FIRST_UART  ) && ( SIORS_FIRST_UART_TYPE  == HW_UARTDM_LT ))  ||\
    ( defined( FEATURE_SECOND_UART ) && ( SIORS_SECOND_UART_TYPE == HW_UARTDM_LT ))  ||\
    ( defined( FEATURE_THIRD_UART  ) && ( SIORS_THIRD_UART_TYPE  == HW_UARTDM_LT ))) //{ 
...
static void siors_dml_dp_rx_isr1(void)
{
...
static void siors_dml_dp_rx_isr2(void)
{
...
===========================================================================*/
static void siors_dml_set_uart_clock_freq
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  MSMUDW_OUT( hwreg->msmu_ipr, rs_port->gv.sio_ipr_shadow);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*-----------------------------------------------------------------------
   ...
   register is written.
  -----------------------------------------------------------------------*/
  MSMUDW_OUT( rs_port->hw.msmu_ipr, rs_port->gv.sio_ipr_shadow);
...
#ifdef FEATURE_FIRST_UART
static void siors_dml_isr1(void)
{
...
#ifdef FEATURE_SECOND_UART
static void siors_dml_isr2(void)
{
...
static void siors_dml_uart1dm_init( siors_port_type *rs_port )
{
  ...
  /* This is to Deassert the carrier detect pin (DCD) */
    BIO_OUT(BIO_DCD_REG, BIO_DCD_M, BIO_DCD_OFF_V);
    BIO_TRISTATE( BIO_DCD_REG, BIO_DCD_M, BIO_OUTPUT_ENA_V);

  /*-------------------------------------------------------------------------
   DTR is an input.
  -------------------------------------------------------------------------*/
    BIO_TRISTATE( BIO_DTR_REG, BIO_DTR_M, BIO_OUTPUT_DIS_V);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  *byte_count = 0;
  ...
  if(((uint32)current_rx_ptr & 0x0003) != 0) 
  {
     ...
     *byte_count = current_rx_ptr - rx_ptr;
  }
  else
  {
     ...
     *byte_count = ((byte*) dword_rx_ptr) - rx_ptr;
  }

  //  The byte_count is bytes_to_rx rounded up to a 4 byte boundary 
  //  (since only dwords are transferred).  Set *byte_count to its correct 
  //  value.  (Think of the case where bytes_to_rx is 1.) 

  if( *byte_count > bytes_to_rx ) 
    *byte_count = bytes_to_rx;

  rs_port->gv.last_snap_value = snap_value - *byte_count;
