...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/data/1x/mip/main/lite/src/dsmip_solsm.c#1 $ $DateTime: 2007/11/06 01:07:58 $ $Author: nsivakum $
...
---------------------------------------------------------------------------*/
char *ssmi_state_names[SSMI_MAX_STATE] =
{
  "Null",
  "Waiting to Solicit",
  "Send Solicitation"
...
char *ssmi_event_names[SSM_MAX_EV] =
{
...
)
{
  ...
  {
    ASSERT(0);
    MSG_ERROR("Failed to allocate PS timer for SSM!", 0, 0, 0);
    return -1;
  }
...
)
{
...
#ifndef T_ARM
  MSG_MED("'%s' event posted to Sol SM in state '%s'.",
          ssmi_event_names[event],
          ssmi_state_names[ssmi_info.state], 0);
#else
  MSG_MED("Event %d posted to Sol SM. in state %d",
          event,
          ssmi_info.state, 0);
#endif
  ...
  case SSM_SOL_TRIGGER_EV:
    ...
    case SSMI_WAIT_TO_SOL_STATE:
      MSG_MED("Ignoring %d in state %d", event, ssmi_info.state,0);
      /* ignore */
      break;
  ...
  case SSM_EXIT_MIP_EV:
    ...
    {
    case SSMI_NULL_STATE:
      MSG_MED("Got event %d in NULL state!", event, 0, 0);
      /* ignore */
      break;
  ...
  case SSM_IMMED_RETRY_EV:
    ...
    case SSMI_NULL_STATE:
    case SSMI_WAIT_TO_SOL_STATE:
      MSG_MED("Ignoring %d event in %d state.\n",
              event,
              ssmi_info.state,
              0);
      /* ignore */
...
)
{
...
#ifndef T_ARM
  MSG_MED("Sol SM from '%s' to '%s' state",
          ssmi_state_names[ssmi_info.state],
          ssmi_state_names[new_state],
          0);
#else
  MSG_MED("Sol SM from %d to %d state", ssmi_info.state, new_state, 0);
#endif
  ...
  switch(new_state)
  {
    /*-----------------------------------------------------------------------
      ...
      Cancel the timer, and enable inbound flow.
    -----------------------------------------------------------------------*/
  case SSMI_NULL_STATE:
    timer_result = ps_timer_cancel(ssmi_info.timer);
    ASSERT(timer_result != PS_TIMER_FAILURE);
    mip_meta_sm_set_in_flow(DS_FLOW_MIP_SOL_MASK, TRUE);
    break;
    ...
    /*-----------------------------------------------------------------------
      ...
      Reset retry counter, disable inbound flow and start timer.
    -----------------------------------------------------------------------*/
  case SSMI_WAIT_TO_SOL_STATE:
    mip_meta_sm_set_in_flow(DS_FLOW_MIP_SOL_MASK, FALSE);
    ssmi_info.retry_cnt = 0;
    timeout = SSMI_SOL_DELAY_TIME + mip_sol_sm_get_random_byte(); 
    timer_result = ps_timer_start(ssmi_info.timer, timeout);
    MSG_MED("Set sol delay time to %d.", timeout, 0, 0);
    ASSERT(timer_result != PS_TIMER_FAILURE); 
    break;
  ...
  case SSMI_SEND_SOL_STATE:
    if(ssmi_info.retry_cnt <= ssmi_info.max_retries)
    {
      ...
      timer_result= ps_timer_start(ssmi_info.timer, timeout);
      MSG_MED("Set retry time to %d.", timeout, 0, 0);
      ASSERT(timer_result != PS_TIMER_FAILURE);
      ssmi_info.retry_cnt += 1;
    }
    else
    {
      MSG_HIGH("Agent solicitation failed",0,0,0);
