...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/data/1x/1xrlp/main/lite/src/dsrlpi.c#5 $ $DateTime: 2008/06/24 01:30:38 $ $Author: nsivakum $

when       who        what, where, why
--------   ---        -------------------------------------------------------
06/18/08   sn         Fixed compiler warnings.
04/08/08   sn         Fixed klocwork error.
02/04/08   sn         Fixed lint error.
10/18/07   sn         Removed variables 'rlp_payload', 'header_bytes' and 
                      'payload_length' that are not used.
...
---------------------------------------------------------------------------*/
dsrlpi_sess_ctl_blk_type  dsrlpi_rscb_array[ DSRLPI_MAX_RSCB_ELEMENTS ];
...
{                                  
  ...
  /* MuxPDU type 3 */
  0x03, NULL, 0x02, 0x03, 0x00,  /* primary                                */
  0x03, NULL, 0x02, 0x03, 0x00,  /* secondary                              */ 
  0x03, NULL, 0x02, 0x03, 0x00,  /* Generic Service                        */ 
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  if (dsrlprtx_get_rexmit_first_seq(rscb_ptr, &nak_first_seq) == FALSE)
  {
    ERR(" No frames to rexmit, but in rexmit func", 0, 0, 0);
    return FALSE;
  }

  if (dsrlprtx_get_rexmit_last_seq(rscb_ptr, &nak_last_seq) == FALSE)
  {
    ERR(" bad last seq info in rtx queue",0,0,0);
    dsrlprtx_reset(rscb_ptr);
    return FALSE;
  }
  ...
  do
  {
    ...
    data_payload_len  = dsrlptxq_access_frame(rscb_ptr,
                                              fr_info,
                                              &next_seq_num);
    ...
    else if (next_seq_num == DSRLPSEQ_NO_SEQ)
    {
      ...
      ---------------------------------------------------------------------*/
      ERR("Need to Rxmit, but txq is empty",0,0,0);
      dsrlprtx_reset(rscb_ptr);
      return FALSE;
    }
    else
    {
      MSG_MED("Did not find first_seq 0x%x in txq",nak_first_seq,0,0);
      /*---------------------------------------------------------------------
      ...
      else
      {
        ...
        if (dsrlprtx_get_rexmit_first_seq(rscb_ptr,&nak_first_seq) == FALSE)
        {
          MSG_HIGH(" No more rxmit info", 0, 0, 0);
          return FALSE;
        }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  ASSERT(fr_len != NULL);
  ASSERT(( rev_fr_ptr->mux_pdu_type == DSRLP_MUX_PDU_1) || 
  ...
  {
    if (dsrlp_len_table[DSRLP_RATE_1]
  ...
  if( (rev_fr_ptr->fr_rate_mask & DSRLP_RATE_1_16_MASK) &&
      (dsrlp_len_table[ DSRLP_RATE_1_16]
                       [rev_fr_ptr->srvc_type]
                       [rev_fr_ptr->mux_pdu_type] >= fr_len))
  {
  ...
  if( (rev_fr_ptr->fr_rate_mask & DSRLP_RATE_1_8_MASK) &&
      (dsrlp_len_table[ DSRLP_RATE_1_8]
                       [rev_fr_ptr->srvc_type]
                       [rev_fr_ptr->mux_pdu_type] >= fr_len))
  {
  ...
  if( (rev_fr_ptr->fr_rate_mask & DSRLP_RATE_3_16_MASK) &&
      (dsrlp_len_table[ DSRLP_RATE_3_16]
                       [rev_fr_ptr->srvc_type]
                       [rev_fr_ptr->mux_pdu_type] >= fr_len))
  {
  ...
  if( (rev_fr_ptr->fr_rate_mask & DSRLP_RATE_1_4_MASK) &&
      (dsrlp_len_table[ DSRLP_RATE_1_4]
                       [rev_fr_ptr->srvc_type]
                       [rev_fr_ptr->mux_pdu_type] >= fr_len))
  {
  ...
  if( (rev_fr_ptr->fr_rate_mask & DSRLP_RATE_3_8_MASK) &&
      (dsrlp_len_table[ DSRLP_RATE_3_8]
                       [rev_fr_ptr->srvc_type]
                       [rev_fr_ptr->mux_pdu_type] >= fr_len))
  {
  ...
  if( (rev_fr_ptr->fr_rate_mask & DSRLP_RATE_7_16_MASK) &&
      (dsrlp_len_table[ DSRLP_RATE_7_16]
                       [rev_fr_ptr->srvc_type]
                       [rev_fr_ptr->mux_pdu_type] >= fr_len))
  {
  ...
  if( (rev_fr_ptr->fr_rate_mask & DSRLP_RATE_1_2_MASK) &&
      (dsrlp_len_table[ DSRLP_RATE_1_2]
                       [rev_fr_ptr->srvc_type]
                       [rev_fr_ptr->mux_pdu_type] >= fr_len))
  {
  ...
  if( (rev_fr_ptr->fr_rate_mask & DSRLP_RATE_3_4_MASK) &&
      (dsrlp_len_table[ DSRLP_RATE_3_4]
                       [rev_fr_ptr->srvc_type]
                       [rev_fr_ptr->mux_pdu_type] >= fr_len))
  {
  ...
  if( (rev_fr_ptr->fr_rate_mask & DSRLP_RATE_7_8_MASK) &&
      (dsrlp_len_table[ DSRLP_RATE_7_8]
                       [rev_fr_ptr->srvc_type]
                       [rev_fr_ptr->mux_pdu_type] >= fr_len))
  {
...
#ifdef FEATURE_DS_RLP3
     case DSRLP_REV_3:
       rscb_ptr->ver_ptr = &dsrlp3_constants;
       break;
#endif 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  ...
  rscb_ptr->state.reset_active = TRUE;
  ...
  -------------------------------------------------------------------------*/
  rscb_ptr->stats.resets++;                          
#if defined (FEATURE_HDR)
#error code not present
#endif
  /*-------------------------------------------------------------------------
    Reset the state info...this is kept only per RLP session. Also, free the 
    tx_item_ptr, which holds current item to be transmitted.
  -------------------------------------------------------------------------*/
  {
    rscb_ptr->state.state             = DSRLPI_SYNC_STATE;
  }

  rscb_ptr->state.ack_to_send    = FALSE;
  rscb_ptr->state.expected_seq_num  = 0;
  rscb_ptr->state.needed_seq_num    = 0;
  rscb_ptr->state.peer_vn           = 0;
  rscb_ptr->state.contig_erase      = 0;
  rscb_ptr->state.rlp_delay         = 0;
  rscb_ptr->state.hole_active       = FALSE;
  rscb_ptr->state.saved_frames_index = 0;
  rscb_ptr->state.saved_frames_cnt   = 0;
  rscb_ptr->state.next_seq_num       = 0;
  rscb_ptr->state.naks_to_send       = 0;

  rscb_ptr->state.last_fill_nak_cnt  = 0;

  rscb_ptr->state.mux_erasure_cnt    = 0;
  
  dsm_free_packet( &((rscb_ptr->state).tx_item_ptr));
  rscb_ptr->state.tx_item_ptr        = NULL;

  rscb_ptr->state.idle_timer_active  = FALSE;
  rscb_ptr->state.idle_timer         = 0;
  rscb_ptr->state.idle_fr_xmit_cntr  = 0;

  rscb_ptr->state.rnd_trip_fr_cntr   = 0;

  rscb_ptr->state.old_frames         = 0;
  rscb_ptr->state.big_nak_gap        = FALSE;
  rscb_ptr->state.reset_rlp          = FALSE;
  /*------------------------------------------------------------------------
    Clean up the nak list for this RLP instance.
  -------------------------------------------------------------------------*/
  dsrlpnak_reset(rscb_ptr);

  /*------------------------------------------------------------------------
    Clean up the saved frame array (the tx queue).
  -------------------------------------------------------------------------*/
  dsrlptxq_reset(rscb_ptr);

  /*------------------------------------------------------------------------
    Clean up the retransmit queue.
  -------------------------------------------------------------------------*/
  dsrlprtx_reset(rscb_ptr);
  ...
  -------------------------------------------------------------------------*/
  dsrlprsq_reset(rscb_ptr);

  /*------------------------------------------------------------------------
    Clean up the delayed frame arrays
  -------------------------------------------------------------------------*/
#ifdef FEATURE_DS_RLP3
  dsrlpdel_reset(rscb_ptr->state.del_fr_id);
#endif /* FEATURE_DS_RLP3 */

  rscb_ptr->state.reset_active = FALSE;
} /* dsrlpi_reset() */

#ifdef FEATURE_DS_RLP3
/*===========================================================================
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
                             ...
                             == CURR_FRAME_SYNC_MASK)
  {
    switch(state_info_ptr->state)
    {
      ...
      case DSRLPI_ESTABLISHED_STATE:
        ...
        ERR( "Invalid SYNC, reset RLP, state=%d", state_info_ptr->state, 
             0, 0);
        break;
            ...
            == (CURR_FRAME_SYNC_MASK | CURR_FRAME_ACK_MASK))
  {
    switch(state_info_ptr->state)
    {
      ...
      case DSRLPI_RT_MEAS_STATE:
      case DSRLPI_ESTABLISHED_STATE:
        ERR( "Invalid SYNC/ACK, drop frame, state=%d", state_info_ptr->state, 
             0, 0);
        break;
                                 ...
                                 ==  CURR_FRAME_ACK_MASK)
  {
    switch(state_info_ptr->state)
    {
      ...
      case DSRLPI_ESTABLISHED_STATE:
        ERR( "Invalid ACK, drop frame, state=%d", state_info_ptr->state, 0, 0);
        break;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  ASSERT(curr_info_ptr != NULL);

  if (*fr_item_pkt_ptr == NULL)
  {
    ERR("null dsm item ptr",0,0,0);
    return (DSRLPI_DONE);
  }

  exp_seq  = rscb_ptr->state.expected_seq_num;
  need_seq = rscb_ptr->state.needed_seq_num;
  ...
  {
    MSG_HIGH("New seq 0x%x < Vr 0x%x Vn 0x%x",
             rxed_seq,
             exp_seq, 
             need_seq
            );
    dsm_free_packet(fr_item_pkt_ptr);
    rscb_ptr->state.old_frames++;
    rscb_ptr->state.big_nak_gap = FALSE;
    return (DSRLPI_DONE);
  }
  
  else if ((rscb_ptr->state.nak_rounds_rev == 0) 
           || ((rxed_seq == exp_seq) && (exp_seq == need_seq)) )
  {
    ...
    if( (rscb_ptr->state.nak_rounds_rev == 0) && 
        !(curr_info_ptr->fr_cat & CURR_FRAME_IN_DELQ_MASK))
    {
       /*-------------------------------------------------------------------
        ...
        dsrlpdel_add( rscb_ptr, *fr_item_pkt_ptr, curr_info_ptr);
        return( DSRLPI_DONE);
      }
    ...
    temp_item_ptr->tracer = DSM_RXC_RLP;
#endif //FEATURE_DSM_MEM_CHK
        q_put(rscb_ptr->cfg.rx_q_ptr, &temp_item_ptr->link);
      }while (*fr_item_pkt_ptr != NULL);
      rscb_ptr->cfg.rx_func_ptr();
      ...
      ---------------------------------------------------------------------*/
      rscb_ptr->stats.rx_total_bytes  += data_len;
    }
    else
    {
      dsrlprsq_insert(rscb_ptr,curr_info_ptr,*fr_item_pkt_ptr);
    }
    ...
    /*-----------------------------------------------------------------------
      ...
      the frame.
    -----------------------------------------------------------------------*/
    rscb_ptr->state.expected_seq_num = new_seq_num;
    rscb_ptr->state.needed_seq_num   = new_seq_num;
  }
  else
  {
    rscb_ptr->state.old_frames = 0;
          ...
          == TRUE)
    {
      /*-------------------------------------------------------------------
        ...
        holes.
      -------------------------------------------------------------------*/
      MSG_HIGH("Too few small items, dump new frame",0,0,0);
      dsm_free_packet(fr_item_pkt_ptr);
      return (DSRLPI_DONE);
    }
    ...
    if (rxed_seq != exp_seq)
    {
      ...
      else
      {
        if (DSRLPSEQ_GT_SEQ_CHECK(rxed_seq,exp_seq,modulus))
        {
          ...
          -----------------------------------------------------------------*/
          nak_gap_seq = DSRLPSEQ_ADD_SEQ(rscb_ptr->state.expected_seq_num,
                                         DSRLPI_BIG_NAK_GAP,
                                         modulus);
          ...
          -----------------------------------------------------------------*/
          vr_seq_hi_plus = DSRLPSEQ_ADD_SEQ(rscb_ptr->state.expected_seq_num, 
                                            0xFF, 
                                            modulus);
             ...
             )
          {
            ...
            {   /* changed MSG_HIGH to MSG_ERROR to help testers. */
                MSG_ERROR("BURST GAP seq %x Vr %x",
                          curr_info_ptr->seq_num, 
            ...
            else
            {
              /*-------------------------------------------------------------
                ...
                regardless.
              -------------------------------------------------------------*/
              rscb_ptr->state.big_nak_gap = TRUE;
                /* changed MSG_HIGH to MSG_ERROR to help testers. */
              MSG_ERROR("FCH RDA gap->seq %x Vr %x",
                        curr_info_ptr->seq_num,
                        rscb_ptr->state.expected_seq_num, 
                        0
                      );
        ...
        -------------------------------------------------------------------*/
        curr_info_ptr->fr_cat &= ~CURR_FRAME_IN_DELQ_MASK;
        MSG_MED("hole-Rx=0x%x, V(R)=0x%x, V(N)=0x%x",
    ...
    if (DSRLPSEQ_GT_SEQ_CHECK(rxed_seq,exp_seq,modulus))
    {
      if (dsrlpnak_insert(rscb_ptr,exp_seq,rxed_seq) == FALSE)
      {
        ERR("Could not insert into NAK list",0,0,0);
        rscb_ptr->state.reset_rlp = TRUE;
      }
    }
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    ERR("Null ptr to rexmit processing",0,0,0);
    return (DSRLPI_DONE);
  }
  ...
  -----------------------------------------------------------------------*/
  fr_len = dsm_length_packet(fr_ptr);
  ...
  -----------------------------------------------------------------------*/
  if( dsrlpnak_get_seq_hi( rscb_ptr, fr_info_ptr, fr_len, &nak_id)
      == TRUE)
  {
    if (DSM_LESS_THAN_LINK_LAYER_FEW_FREE_ITEMS(DSM_DS_SMALL_ITEM_POOL) 
          == TRUE)
    {
      ...
      {
        MSG_HIGH(" Few small items, rexmit not first hole",0,0,0);
    ...
    -------------------------------------------------------------------*/
    dsrlprsq_insert(rscb_ptr, fr_info_ptr, fr_ptr);
    if (dsrlpnak_remove(rscb_ptr, fr_info_ptr, fr_len, nak_id) == FALSE)
    {
      ERR("nak remove failed, so reset",0,0,0);
      rscb_ptr->state.reset_rlp = TRUE;
    }
  }
  else
  {
    MSG_MED(" Got dup rxit Rx=%x, V(R)=0x%x, V(N)=%x", 
              fr_info_ptr->seq_num,
              rscb_ptr->state.expected_seq_num,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  tx_wm_ptr           = rscb_ptr->cfg.tx_wm_ptr;
  ...
  peer_num_shift = DSRLPSEQ_GET_12BIT_FRAME_SEQ(rscb_ptr->state.peer_vn);
  ...
  -------------------------------------------------------------------------*/
  if ( rscb_ptr->cfg.octet_sequenced == TRUE)
  {
    tx_s_seq  = TRUE;
    tx_seq_hi = TRUE;
  }
...
#endif /*FEATURE_IS2000_REL_A_MUXPDU5*/
        (rscb_ptr->cfg.octet_sequenced == TRUE)        ||
        (tx_s_seq == TRUE)                             ||
        (tx_seq_hi == TRUE)
       )
      )                                                               ||
  ...
  {
    MSG_HIGH("Can't make data frame size=%d", 
              rev_fr_ptr->max_frame_size, 0, 0);
    return FALSE;
  }
  ...
  if ( state_ptr->tx_item_ptr == NULL)
  {
    ...
    tx_item_len = dsm_length_packet( state_ptr->tx_item_ptr);
    rscb_ptr->stats.tx_total_bytes +=  tx_item_len;
    meta_info_ptr = (ps_meta_info_type *) state_ptr->tx_item_ptr->app_field;
  ...
  {
    MSG_HIGH("Memory allocation failed", 0, 0, 0);
    return FALSE;
  }
...
/*-------------------------------------------------------------------------
  ...
  {
    new_fr_item_ptr->data_ptr[ new_fr_item_ptr->used++]= rscb_ptr->sr_id << 5;    
    txq_fr_param.footer ++;
  }
  ...
  else if( rev_fr_ptr->mux_pdu_type == DSRLP_MUX_PDU_5 ) 
  {
    ...
    {
       MSG_FATAL( "payload_in_frame : %ld:Feature unsupported", 
                  payload_in_frame, 0, 0 );
       return FALSE;
    }
    ...
    /*-----------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------*/
    /*-----------------------------------------------------------------------
        Calculate RLP Header based on Seq,PDU5_FmtD_Ctl_Len, Seq_hi, S_Seq,Padding.
     ------------------------------------------------------------------------*/
    ...
    if(mux_len_bits != 0)
    {
      b_packw(  (payload_in_frame +rlp_hdr_bytes), 
       ...
       (tx_seq_hi==TRUE))
   {
        ...
        ( rev_fr_ptr->mux_pdu_type == DSRLP_MUX_PDU_2))
    {
      ...
      if ( (tx_seq_hi==TRUE) || (tx_s_seq==TRUE))
      {
        ...
        b_packw( payload_in_frame, ctl_field_ptr, 7, 5);
      }
      ...
      else
      {
        ...
        b_packw( payload_in_frame, ctl_field_ptr, 2, 6);
      }
    }
  ...
  -------------------------------------------------------------------------*/
  txq_fr_param.header  = new_fr_item_ptr->used;
  txq_fr_param.footer -= ( payload_in_frame + txq_fr_param.header);
  ...
  while( payload_in_frame >0)
  {
    ...
    if ( state_ptr->tx_item_ptr == NULL)
    {
      ...
      else
      {
        ...
        rscb_ptr->stats.tx_total_bytes += tx_item_len;
        meta_info_ptr = 
             (ps_meta_info_type *) state_ptr->tx_item_ptr->app_field;
        if ( meta_info_ptr != NULL )
        {
          PS_META_INFO_FREE ( &meta_info_ptr );
        }
    ...
    else /* tx_item_ptr != NULL */
    {
      ...
      if( tx_item_len == 0)
      {
        ERR("Zero len item curr=%d dont_ex=%d",
  ...
  {
    ERR("%d bytes missing from watermark on making RLP frame", 
        payload_in_frame, 0, 0);
    dsm_free_packet( &head_item_ptr);
    return( FALSE);
  }
  ...
  if ( rev_fr_ptr->fr_rate_mask == DSRLP_RATE_1_MASK)
  {
        ...
        (rev_fr_ptr->mux_pdu_type == DSRLP_MUX_PDU_2))
    {
      ...
      ---------------------------------------------------------------------*/
      frame_len  = dsrlp_len_table[ rev_fr_ptr->fr_rate]
                                  [ rev_fr_ptr->srvc_type]
  ...
  {
    frame_len     = dsm_length_packet( head_item_ptr);
    rev_fr_ptr->fr_rate = dsrlpi_get_best_fr_rate( rev_fr_ptr, frame_len); 

    if ( rev_fr_ptr->fr_rate == DSRLP_RATE_ERASURE)
    {
      ERR(" fr_rate mask=%x len=%d Donot match", 
          rev_fr_ptr->fr_rate_mask, frame_len, 0);
      dsm_free_packet( &head_item_ptr);
      return( FALSE);
    }
    frame_len  = dsrlp_len_table[ rev_fr_ptr->fr_rate]
...
#ifdef FEATURE_DSM_MEM_CHK
  txq_fr_param.fr_ptr->tracer = DSM_RLP_TX;
#endif //FEATURE_DSM_MEM_CHK
  dsrlptxq_add_frame ( rscb_ptr, &txq_fr_param);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( (rev_fr_ptr->srvc_type >= DSRLP_GENERIC_SRVC) ||     /* Klocwork Fix */
      (rev_fr_ptr->mux_pdu_type >= DSRLP_MUX_PDU_3) )
  {
    ERR("Invalid Service type index %d or MUX PDU type index %d", 
        rev_fr_ptr->srvc_type, rev_fr_ptr->mux_pdu_type, 0);
    return FALSE;
  }
  ...
  ------------------------------------------------------------------------*/
  *fr_len = dsrlp_len_table[rev_fr_ptr->fr_rate]
                           [rev_fr_ptr->srvc_type]
                           [rev_fr_ptr->mux_pdu_type];
...
)
{
  ...
  byte                    length_bits = 0;   /* To save the number of bits
                                                used for length in PDU5    */
  uint16                  rexmit_frame_length  = 0;
  byte                    max_mux_header_bytes = 0;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  state_ptr   = &rscb_ptr->state;
  modulus     = rscb_ptr->ver_ptr->seq_modulus;
  octet_seq   = rscb_ptr->cfg.octet_sequenced;
  seg_allowed = (rscb_ptr->ver_ptr)->seg_allowed;
  ...
  if (rlpi_get_rexmit_data(rscb_ptr,&fr_info, &rexmit_info) == FALSE)
  {
    rscb_ptr->stats.re_xmits_not_found++;
    return FALSE;
  }
  ...
  if (octet_seq == TRUE)
  {
    ...
    {
      copy_len = DSRLPSEQ_DIFF_SEQ(fr_last_seq, nak_first_seq, modulus);
    }
    else
    {
      copy_len = DSRLPSEQ_DIFF_SEQ(nak_last_seq, nak_first_seq, modulus);
    }
  ...
  else
  {
        ...
        (DSRLPSEQ_HAS_S_SEQ(nak_last_seq)))
    {
      /*---------------------------------------------------------------------
        ...
        again.
      ---------------------------------------------------------------------*/
      MSG_HIGH("Strange NAK info from peer",0,0,0);
      dsrlprtx_remove_first_element(rscb_ptr);
      return FALSE;
    }
  ...
  /*-------------------------------------------------------------------------
    ...
    packet.
  -------------------------------------------------------------------------*/
  dsm_offset = DSRLPSEQ_DIFF_SEQ(nak_first_seq, fr_first_seq, modulus);
      ...
      (mux_pdu == DSRLP_MUX_PDU_2))
  {
             ...
             (octet_seq == FALSE))
    {
      /*---------------------------------------------------------------------
        ...
        subtracted.
      ---------------------------------------------------------------------*/
      table_len = dsrlp_len_table[DSRLP_RATE_1]
                                 [srvc]
                                 [mux_pdu];
      ...
      {
        MSG_MED("tbl_len:%d;max_size:%d;payld:%d; Format B frame, but not correct length",
            table_len,
  ...
  if(( snd_fr_type == RLPI_SND_FR_FMT_D)  && ( mux_pdu == DSRLP_MUX_PDU_5)) {
    ...
    rexmit_frame_length = copy_len + max_mux_header_bytes + max_rlp_header_bytes;
    
    fr_item_ptr = dsm_new_buffer( DSM_DS_SMALL_ITEM_POOL );  //Saritha: Think if we should use large item.
    /*fr_item_ptr = dsm_new_buffer(DSM_DS_POOL_SIZE( (word) 
                    (rexmit_frame_length >> DSRLPI_ITEM_SIZ_DIVISOR)));*/
  }
  else 
#endif /*FEATURE_IS2000_REL_A_MUXPDU5*/
  {
      ...
      (snd_fr_type == RLPI_SND_FR_FMT_C))
  {
    ...
    if (snd_fr_type == RLPI_SND_FR_FMT_B)
    {
      typebit_pos = dsrlp_len_table[DSRLP_RATE_1][srvc][mux_pdu];
      ...
      ASSERT(curr_item_ptr->used == (typebit_pos + 1));
    ...
    else
    {
      /*-------------------------------------------------------------------
        ...
        type bits.
      -------------------------------------------------------------------*/
      fr_item_ptr->data_ptr[0]  = rscb_ptr->sr_id << 5;;
  ...
  else if ((seg_allowed == FALSE) || (seg_frame_fits == TRUE))
  {
    ...
    ASSERT((mux_pdu == DSRLP_MUX_PDU_1) || (mux_pdu == DSRLP_MUX_PDU_2));

    ASSERT((snd_fr_type == RLPI_SND_FR_FMT_A)     || 
           (snd_fr_type == RLPI_SND_FR_FMT_NONE));

    ASSERT(data_payload_len == copy_len);
    ASSERT(nak_first_seq == fr_first_seq);
    ...
    if ((copy_len + RLP3_DATA_UNSEG_HDR_SIZE) > rev_fr_ptr->max_frame_size)
    {
      MSG_HIGH("Frame not large enough for rexmit",0,0,0);
      dsm_free_packet(&fr_item_ptr);
      return FALSE;
    }
    ...
    ASSERT(copy_len <= RLP3_DATA_UNSEG_DATA_MAX_LEN);
    ...
    *byte_ptr |= dsrlpi_type_mask_table[mux_pdu][srvc].format_a;
    ...
    fr_item_ptr->used = dsrlp_used_len_table[rev_fr_ptr->fr_rate]
                                            [srvc]
                                            [mux_pdu];
           ...
           (mux_pdu == DSRLP_MUX_PDU_2))
  {
    ...
    *byte_ptr |= dsrlpi_type_mask_table[mux_pdu][srvc].format_a;
    ...
    if (rev_fr_ptr->fr_rate == DSRLP_RATE_ERASURE)
    {
      ERR("seg frame not big enough",0,0,0);
      dsm_free_buffer(fr_item_ptr);
      return FALSE;
    }
  ...
  else   /* MUXPDU3or5 Format D  */
  {
    ...
    -----------------------------------------------------------------------*/
    fr_item_ptr->data_ptr[fr_item_ptr->used]= rscb_ptr->sr_id << 5;    
  ...
  MSG_MED("Sending Rexmit, with SEQ=%x", nak_first_seq,0,0);
  ...
  if (octet_seq == TRUE)
  {
    ...
    else
    {
      nak_first_seq = DSRLPSEQ_INCREMENT_S_SEQ(nak_first_seq,
                                        rscb_ptr->ver_ptr->seq_modulus);
      dsrlprtx_update_first_element(rscb_ptr,
                                    nak_first_seq,
                                    nak_last_seq);
    }
...
SIDE EFFECTS   None
===========================================================================*/
byte dsrlpi_get_rscb_index_for_sr_id(byte sr_id)
{
   /* If sr_id is valid, return the corresponding index */
   if (sr_id < DSRLPI_MAX_SP_SR_ID_VALUE)
   {
      return dsrlpi_sr_id_rscb_map[ sr_id ];
   }
   else
   {
      return DSRLPI_INVALID_RSCB_INDEX;
   }
}
...
SIDE EFFECTS   Invalidates the old mapping, if any between an SR_ID and the
               RLP instance.
===========================================================================*/
void dsrlpi_set_rscb_index_for_sr_id(byte sr_id, byte rscb_index)
{
   byte old_sr_id;

   /* Sanity checks */
   if ((sr_id >= DSRLPI_MAX_SP_SR_ID_VALUE) || (rscb_index >= DSRLPI_MAX_RSCB_ELEMENTS))
   {
      return; /* Invalid SR_ID, do not proceed */
   }

   /* Clear the old mapping, if any */
   old_sr_id = dsrlpi_rscb_array[ rscb_index ].sr_id;
   if( old_sr_id < DSRLPI_MAX_SP_SR_ID_VALUE )
   {
      dsrlpi_sr_id_rscb_map[ old_sr_id ] = DSRLPI_INVALID_RSCB_INDEX;
   }

   /* Setup the new mapping */
   dsrlpi_sr_id_rscb_map[sr_id] = rscb_index;
...
SIDE EFFECTS   None
===========================================================================*/
void  dsrlpi_sridmap_init(void)
{
    int i;
    for(i=0;i<DSRLPI_MAX_SP_SR_ID_VALUE;i++)
    {
        dsrlpi_sr_id_rscb_map[ i ] = DSRLPI_INVALID_RSCB_INDEX;
    }
}
