...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/mmode/cm/rel/08H1/src/cmxsms.c#1 $
...
)
{
  ...
  CM_MSG_HIGH( "Send MO DBM(SMS) to MC", 0, 0, 0 );
  ...
  */
  ...
  if (uptime > sms_ptr->max_wait_for_status_time)
  {
    ...
    */
    ...
    CM_MSG_HIGH("MO SMS-WaitForStatus timeout",0,0,0);
    return;
  }
...

{
  ...
  switch( sms_cmd_ptr->cmd )
  {
    ...
    default:
      ...
      CM_SWITCH_ERR( "SMS cmd %d",sms_cmd_ptr->cmd,0,0);
...
void cmxsms_mo_msg_retry( void)
{
  ...
  */
  ...
  {
    CM_MSG_HIGH( "DCM: SMS(DBM) is waiting for HDR session close!",0,0,0 );
    return;
  }
  #endif
  ...
  */
  ...
  if( sms_ptr->is_ok_to_send )
  {
    ...
    sms_ptr->max_wait_for_status_time =
        clk_uptime() + MAX_SMS_WAIT_FOR_STATUS_TIME;
    return;
  }
...
)
{
  ...
  if( ss_ptr->is_hdr_session_open )
  {
    CM_MSG_HIGH("DCM: Closing HDR session for SMS(DBM)",0,0,0);
  ...
  overall_call_state = cmcall_overall_call_state();
...
)
{
  ...
  CM_FUNC_START( "cmsms_mc_rpt_proc(), cmd=%d",
    ...
    */
    ...
    case CM_MO_DBM_STATUS_F:
      if(cm_mc_rpt_ptr->mo_dbm_status.msg_type == CAI_SHORT_MSG_SERVICES)
      {
        ...
        */
        CM_ELSE_ERR( "CM_MO_DBM_STATUS_F" );
      }
    ...
    case CM_N_VOICE_F:
      memcpy(sms_ptr->data, cm_mc_rpt_ptr->n_voice.data, 14);
...
)
{
  ...
  CM_FUNC_START( "cmsms_sd_rpt_proc(), cmd=%d", sd_rpt_ptr->hdr.cmd, 0, 0);
...
)
{
    ...
    mt_sms.sms.data_len = MIN( len, CM_SMS_MAX_LEN );

    memcpy( mt_sms.sms.data, data, mt_sms.sms.data_len );
