 ...
 * Copyright (C) 2004-2009, QUALCOMM, Inc.
 *
...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_factory.c#5 $ $DateTime: 2009/05/20 16:03:23 $ $Author: richardp $
...
static FS_ALIGN byte fs_factory_page_buffer[EFS_PAGE_SIZE];
...
#else
      pm->parent.ops.page_read (&pm->parent, cluster, fs_factory_page_buffer);
      memcpy (page_data, fs_factory_page_buffer, EFS_PAGE_SIZE);
#endif
      info->cluster_data_seqno++;
      return 0;
    }
...
void
spm_init (fs_super_t super, efs_device_t dev,
    struct fsb_nand_data *buf,
    int start_buffer)
{
  int i, j;
...
factory_scan (efs_device_t dev, uint8 *buffer)
{
  ...
  while (1) {
    ...
    {
      MSG_HIGH ("Fact img dev layout differs PGSz: %d, BlkSz: %d, BlkCnt: %d",
                head->page_size, head->block_size, head->block_count);
      MSG_HIGH ("Expected : PGSz: %d, BlkSz: %d, BlkCnt: %d",
                  fs_device_page_size (dev), fs_device_block_size (dev),
                  fs_device_block_count (dev));
      return FS_INVALID_PAGE_ID;
    }
