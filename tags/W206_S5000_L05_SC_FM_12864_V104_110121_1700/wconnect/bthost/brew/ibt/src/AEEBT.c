...
$Header: //source/qcom/qct/wconnect/bthost/brew/ibt/rel/00.00.26/src/AEEBT.c#7 $ 
$DateTime: 2009/09/16 16:07:32 $
$Author: phuongn $

when        who  what, where, why
----------  ---  ------------------------------------------------------------
2009-09-16  pn   Fixed more compile errors when FEATURE_BMP is not defined.
2009-09-03  gb   Fixing compile errors when FEATURE_BMP is not defined
2009-09-02  pn   Added IBTAGEx entry to mod table.
2009-07-31  gb   Adding FTP client entry to mod table 
...
#define AEEBT_CMD_CANCEL_BOND 		0x0108f2e7
...
extern int AEEBTAGEx_New (IShell* pIShell, AEECLSID cls, void** ppObj);
...
#ifdef FEATURE_BMP
   {AEECLSID_BTFTPClient,   ASCF_UPGRADE,           0,  NULL, AEEBTFTPClient_New},
#endif
   {AEECLSID_IBTAGEX,       ASCF_UPGRADE,           0,  NULL, AEEBTAGEx_New},
                          ...
                          int                 outBufSize)
{
   ...
   case AEEBT_CMD_GET_BONDABLE:
      ...
      break;
   case AEEBT_CMD_CANCEL_BOND:
      if ( pInBuffer == NULL )
      {
         nErr = EBADPARM;
      }
      else if ( inBufSize != sizeof( BDAddress ) )
      {
         nErr = EVERSIONNOTSUPPORT;
      }
      else
      {
	     nErr = OEMBT_CancelBond( ((BDAddress*) pInBuffer) );
      }
      break;
   default:
      break;
