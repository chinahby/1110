...
$Header: //source/qcom/qct/wconnect/bthost/brew/ibt/rel/00.00.26/src/OEMBTAG.c#7 $ 
...
void OEMBTAG_ApplySpkrGain (OEMAG* pMe, uint8 gain)
{
   if ((NULL != pMe) && (pgTheOEMAG == pMe))
   {
     snd_set_volume (
       SND_DEVICE_BT_HEADSET, SND_METHOD_VOICE, gain, NULL, NULL);
     snd_set_volume (
       SND_DEVICE_BT_HEADSET, SND_METHOD_KEY_BEEP, gain, NULL, NULL);
     snd_set_volume (
       SND_DEVICE_BT_HEADSET, SND_METHOD_MESSAGE, gain, NULL, NULL);
     snd_set_volume (
       SND_DEVICE_BT_HEADSET, SND_METHOD_RING, gain, NULL, NULL);
#if defined(FEATURE_AUDIO_FORMAT) || defined(FEATURE_MIDI_OUT)
     snd_set_volume (
       SND_DEVICE_BT_HEADSET, SND_METHOD_MIDI, gain, NULL, NULL);
#endif
   }
}

int OEMBTAG_SetMicGain (OEMAG* pMe, uint8 gain)
{
   int result = AEE_EFAILED;
   if ((NULL != pMe) && (pgTheOEMAG == pMe))
   {
      bt_cmd_status_type stat = bt_cmd_ag_set_ad_mic_gain (pMe->appId, gain);
      result = OEMBT_CmdStatus2AEEResult (stat);
   }
   return result;
}
...
int OEMBTAG_SendResponse (OEMAG* pMe, const char* pszRsp)
{
   int result = AEE_EFAILED;
#ifndef BT_SIMULATION
