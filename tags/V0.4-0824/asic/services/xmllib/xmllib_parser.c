...
/*===========================================================================
  ...
  $Header: //depot/asic/msmshared/services/xmllib/xmllib/xmllib_parser.c#13 $
...
)
{
  ...
  -------------------------------------------------------------------------*/
  pnode = metainfo->memalloc( sizeof(xmllib_parsetree_node_s_type) );
...
)
{
  ...
  {
    strptr->len    = numbytes;
    strptr->string = metainfo->memalloc( numbytes );
...
)
{
  ...
  ret = xmllib_tok_tokenize( encoding,
    ...
    switch( token )
    {
      ...
      ---------------------------------------------------------------------*/
      default:
        MSG_HIGH( "Invalid occurance of token %d", token, 0, 0 );
        ret         = XMLLIB_ERROR;
        *error_code = XMLLIB_ERROR_INVALID_TEXT;
        break;
    } /* switch( token ) */
...
)
{
  ...
  while( NULL != pnode )
  {
    switch( pnode->nodetype )
    {
      ...
      default:
        /* confused node */
        ERR_FATAL( "Unexpected node-type: %d in node: 0x%p",
                   pnode->nodetype,
                   pnode,
                   0 );
        break;
