...
$Header: //source/qcom/qct/multimedia/audio/codec/sbc/main/latest/src/audiosbc.c#5 $ $DateTime: 2009/10/14 07:34:24 $ $Author: c_ypaida $
...
)
{
  ...
  context->alloc = (uint8) audiosbc_alloc_mode_conv[audiosbc_alloc].oi_value;
  context->mode  = (uint8)audiosbc_mode_conv[audiosbc_channel_mode].oi_value;
  ...
  {
    context->bitrate = (uint32)AUDIOSBC_BITRATE_HIGH * 2;
  }
  else
  {
    context->bitrate    = (uint32)AUDIOSBC_BITRATE_HIGH;
  }
  context->num_blocks   =(uint8)
                               audiosbc_block_conv[audiosbc_block_len].oi_value;
  context->num_subbands =(uint8) 
                          audiosbc_subband_conv[audiosbc_num_subbands].oi_value;

  context->sample_rate  = 
             (uint8)audiosbc_sample_rate_conv[audiosbc_sample_rate].oi_value;
...
uint16 audiosbc_encode(
  ...
  int16 (*pcm_data)[2]  = NULL;/* points pcm data for SBC frame will be taken */
...
#error code not present
      ...
      ((uint32)dsp_bytes < audiosbc_context.num_pcm_sampleb_in_frame))
  {
    ...
    if (need_bytes <= dsp_bytes)
    {
      ...
      pcm_data = (int16 (*)[2])audiosbc_context.hold_buffer;
...
void audiosbc_audio_flush(boolean send_held_frame)
{
      ...
      (audiosbc_context.hold_buffer_valid) && send_held_frame)
  {
    ...
    memset ((void*) ((uint8*)(audiosbc_context.hold_buffer)+
                              audiosbc_context.hold_buf_len),0, 
                              num_sampleb_left);
    pcmData = (int16 (*)[2])audiosbc_context.hold_buffer;
