...
// Format variables, strings, characters and patterns for Slovak (Slovakia)
...
static const AECHAR Currency_NegativePattern []           = { '-','#',' ','S','k',0 }; 
static const AECHAR Currency_PositivePattern[]            = { '#',' ','S','k',0 }; 
static const AECHAR Currency_CurrencySymbol[]             = { 'S','k',0 }; 
...
/* -----------------------------------------------------------------------
    LocaleSK_Ctor 
   --- */
static void LocaleSK_Ctor(LocaleSK *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleSK_New  
   --- */
int LocaleSK_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleSK *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleSK, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleSK_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule);
...
int LocaleSK_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALESK) {
      return LocaleSK_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
