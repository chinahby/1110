...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_mnand_super.c#1 $ $DateTime: 2008/12/12 11:11:10 $ $Author: richardp $
                     ...
                     int *fresh_start)
{
  ...
  if (super->data->page_size != EFS_PAGE_SIZE)
    FS_ERR_FATAL ("EFS_PAGE_SIZE mismatch...",0,0,0);
  if (super->data->sect_per_clust != FS_MNAND_SECTORS_PER_CLUSTER)
    FS_ERR_FATAL ("FS_MNAND_SECTORS_PER_CLUSTER mismatch...",0,0,0);
  if (super->data->block_size != FS_MNAND_CLUSTERS_PER_BLOCK)
    FS_ERR_FATAL ("FS_MNAND_CLUSTERS_PER_BLOCK mismatch...",0,0,0);
