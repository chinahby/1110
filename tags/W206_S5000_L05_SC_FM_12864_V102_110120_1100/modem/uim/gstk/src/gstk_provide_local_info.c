...
$Header: //source/qcom/qct/modem/uim/su/baselines/qsc1110/rel/3.3.71/uim/gstk/src/gstk_provide_local_info.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/09/10   dd      Provide local time and time zone in PLI
02/09/10   bd      Fixed fall through in swich-case and corrected log statements
08/31/09   xz      Fix issue of PLI date/time on SC1X
08/13/09   dd      Fixed compile error
...
#error code not present
...
#if defined(FEATURE_CDMA_800) || defined(FEATURE_CDMA_1900)
      if(gstk_next_curr_rat == GSTK_ACCESS_TECH_CDMA)
        {
        db_get(DB_LTM_OFF, &db);
        tz_seconds = (int32)(db.ltm_off & 0x3F);
        if (tz_seconds >= 32)
        {
          tz_seconds -= 64;
        }
        tz_seconds = (int32)(tz_seconds*30*60);
      
        MSG_HIGH("Time zone from CDMA in seconds is : 0x%x ", tz_seconds, 0, 0);
      }	
#endif /* FEATURE_CDMA_800 || FEATURE_CDMA_1900 */
#ifdef FEATURE_SEC_TIME
      ...
      secs = time_jul_to_secs(&rtc_julian);
      secs = secs + tz_seconds;
...
#error code not present
      ...
      if(gstk_next_curr_rat == GSTK_ACCESS_TECH_CDMA)
      {
        /* Time zone from CP is 6 bit 2's compliment number in 30 minute increments */
        db_get(DB_LTM_OFF, &db);
        local_info.info_data.time.zone = (uint8)(db.ltm_off & 0x3F);
        MSG_HIGH("Setting CDMA time zone to: 0x%x ",local_info.info_data.time.zone, 0, 0);
      }
#endif /* FEATURE_CDMA_800 || FEATURE_CDMA_1900 */
      break;
