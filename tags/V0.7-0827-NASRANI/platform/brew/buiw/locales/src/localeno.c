...
// Format variables, strings, characters and patterns for Norwegian (Norway)
...
/* -----------------------------------------------------------------------
    LocaleNO_Ctor 
   --- */
static void LocaleNO_Ctor(LocaleNO *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleNO_New  
   --- */
int LocaleNO_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleNO *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleNO, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleNO_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule);
...
int LocaleNO_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALENO) {
      return LocaleNO_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
