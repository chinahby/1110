 ...
 * @file  
 *
 * This file contains common definitions from version 1.1 of the Bluetooth specification.
 *
 */
 ...
 * the most significant 32 bits are stored with the same endian-ness as is native on the target machine.
 * The remainder of the 128-bit UUID is stored as bytes in big-endian order.
 */
typedef struct {
    OI_UINT32 ms32bits;                                    /**< most significant 32 bits of 128-bit UUID */
    OI_UINT8 base[OI_BT_UUID128_SIZE - sizeof(OI_UINT32)];    /**< remainder of 128-bit UUID, stored as an array of 8-bit values */
} OI_UUID128;
 ...
 */
typedef enum  {
    OI_LINK_KEY_TYPE_COMBO,          /**< combination key */
    OI_LINK_KEY_TYPE_LOCAL_UNIT,     /**< local unit key */
    OI_LINK_KEY_TYPE_REMOTE_UNIT,    /**< remote unit key */
    OI_LINK_KEY_TYPE_RESERVED        /**< reserved key type */
} OI_BT_LINK_KEY_TYPE ;
...
/** maximum number of SCO connections per device, which is 3 for version 1.1 of the Bluetooth specification */
#define OI_BT_MAX_SCO_CONNECTIONS  3

/** data type for clock offset */
typedef OI_UINT16   OI_BT_CLOCK_OFFSET ;

/** data type for baseband connection handle */
typedef OI_UINT16   OI_HCI_CONNECTION_HANDLE ;
...
/** Bit 13 in device class indicates limited discoverability mode (GAP V1.1, section 4.1.2.2) */
...
#define OI_BT_TICKS_TO_SECONDS(ticks)(((ticks)*5)/8000)
