#******************************************************************************
# FILE:     mediaplayer.smk
#
# PURPOSE:
#    Makefile script for the hs sub-build 
#
# DESCRIPTION:
...
#
#         make -f mediaplayer.smk [make-target] 
#
...
SOURCES := $(filter-out $(FILTERED_SOURCES), $(wildcard *.c) $(wildcard *.s))

# Log level for this subsystem
SMK_CDEFINES := $(OEM_MSG_LVL)
...
SMK_CINCPATH := $(MEDIAPLAYER_CINCPATH)

# List of assembler include header directory search paths
SMK_AINCPATH := $(MEDIAPLAYER_AINCPATH)

# Additional compile flags
STD_CFLAGS += -DBREW_STATIC_APP
