...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/hfat/main/latest/src/hfat_chkfat.c#10 $ $DateTime: 2009/04/06 18:51:13 $ $Author: richardp $
...
#define BOUNDS_CHECK(cluster)  do { \
        ...
        ERR_FATAL ("Accessing cluster out of bounds\n:0x%08X",cluster>>3,0,0);\
        } while(0) 
...
#else
  #define LOG(fmt,x,y) MSG_HIGH(fmt, x, y, 0)
#endif
...
/* Concatenate an wchar string to a wchar string */
static wchar *
wstrlcat (wchar *dst, const wchar *src, unsigned int len)
{
  unsigned int c = 0;
  /* Go to the end of the wchar dst */
  while (*dst && c < len)
  {
    dst++;
    c++;
...
/* Get number of characters in wide string */
static size_t
chkfat_wstrlen (const wchar *src)
{
  unsigned long len = 0;
  while (*src++ != 0) len++;
  return len;
...
chkfat_getlostname (F_MULTI * fm, wchar *path, char *lost_name)
{
  ...
  {
    (void) fs_strlcpy (lost_name, "LOST0000", LOST_LEN);
    do
    {
    ...
    if (chkfat_checklostname (fullname) == FC_NO_ERROR)
    {
      ...
      if (i < 8)
      {
        ...
        if (start_cluster == dst_pos->cluster)
        {
          ...
          (void) fs_strlcpy (tmpname, fullname, sizeof (tmpname));
                    ...
                    int lost, int depth)
{
  ...
  if (lost && cluster == 0)
  {
    (void) fs_strlcpy (fstr, "LOST*.*", sizeof (fstr));
    ++cluster;
  }
  else
  {
    if (!cluster)
      return 0;
    (void) fs_strlcpy (fstr, "*.*", sizeof (fstr));
  }
  ...
  if (fn_wfindfirst (fm, towchar (fstr), find) == 0)
  {
    do
    {
      if ((find->attr) & F_ATTR_DIR)
      {
        ...
        if (find->filename[0] != '.')
        {
          len = chkfat_wstrlen (full_path);
          if (len + chkfat_wstrlen (find->filename) + 2 < F_MAXPATH - 1)
          {
            if (full_path[1])
