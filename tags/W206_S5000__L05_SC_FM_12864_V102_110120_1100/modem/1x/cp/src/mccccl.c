...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/mccccl.c#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $
...
static boolean sup_pilots_are_valid
(
  byte num_sup_pilots,              // Number of valid pilots
  caii_scam_var_type pilot_info[]   // Array containing the pilots
);
static void update_sups
(
   byte num_sup_pilots, byte num_sup,
   caii_scam_var_type pilot_info[]
);
#endif

#if defined(FEATURE_IS95B_MDR)
static byte generate_aset_sups
(
  srch_tc_sup_type* sups,
...
// Function to read number of supplementals that are active currently
static byte get_number_of_supplementals( void );
...
#if defined(FEATURE_IS2000_REL_C_DV) || defined(FEATURE_IS2000_DV_SIG_TEST)
#error code not present
#endif /* defined(FEATURE_IS2000_REL_C_DV) || defined(FEATURE_IS2000_DV_SIG_TEST) */
...
#if defined(FEATURE_IS95B_MDR)
  ,sup_pilots_are_valid,
  update_sups
#endif

#if defined(FEATURE_IS95B_MDR)
  ,generate_aset_sups,
  get_number_of_supplementals
#endif
  ,save,
  restore
  ,update_ccl_with_uhdm
...
{
  qword  timestamp;          // Current timestamp
  qword  timestamp_diff;     // Difference between current timestamp 
  ...
  ts_get_20ms_frame_time( timestamp );
...
#else // ifdef FEATURE_IS2000_RELEASE_A_LOGGING
/*===========================================================================
...
void mccccl_log_aset_info (void)
{
  ...
  if (log_status(LOG_ASET_INFO_C))
  {
    ...
    ts_get((unsigned long *) log.hdr.ts);
...
#error code not present
  ...
  // N6m active pilots.
  ASSERT( num_pilots > 0 && num_pilots <= CAI_N6M );
...
#error code not present
  ...
  {
    MSG_ERROR("First pilot should have PWR_COMB_IND=0", 0,0,0);
  }
...
static void update_sups
(
  byte num_sup_pilots,              // Total number of pilots carrying supplementals
  byte num_sup,                     // Total number of supplementals per pilot
  caii_scam_var_type pilot_info[]   // Pilot PNs, fundamental and supplemental code channels
)
{
  byte cclp, supp, bn, bm;  // Some loop variables, ccl pilot, sup pilot, etc
  // Used to mark supplementals that are processed
  boolean processed[CAI_MAX_SUP_PILOTS];

  ASSERT( num_sup_pilots <= CAI_MAX_SUP_PILOTS && num_sup <= CAI_MAX_NUM_SUP );

  // Just in case ASSERT didn't catch all problem during development, fix
  // num_sup_pilots and num_sup here.
  if ( num_sup_pilots > CAI_MAX_SUP_PILOTS )
  {
    num_sup_pilots = CAI_MAX_SUP_PILOTS;
  }
  ...
  for ( bn = 0; bn < CAI_MAX_SUP_PILOTS; ++bn )
  {
    processed[bn] = FALSE;
  }

  // This function is only used to configure SCCH
  ccl.sup_state = MCCCCL_SCCH;

  // Step 1:
  // Look at each pilot in the code channel list.  If the pilot does not match
  // anyone specified in pilot_info, then we delete all supplementals from that
  // pilot.  If it does match one in pilot_info, then we replace the supplemental
  // code channels for that pilot with the ones supplied.  This is done for pilots
  // labelled as MCCCCL_SUP_ONLY or ACITVE.
  for ( cclp = 0; cclp < CCL_SIZE; ++cclp )
  {
    // If pilot entry is invalid, continue with next one.
    if ( ccl.pilots[cclp].pilot_state == MCCCCL_INVALID )
    {
      continue;
    }

    // Check if pilot is specified in pilot_info
    for ( supp = 0; supp < num_sup_pilots; ++supp )
    {
      // A pilot matches one in the CCL, then...
      if ( ccl.pilots[cclp].pilot_pn == pilot_info[supp].plt_pn )
      {
        // Mark as processed
        processed[supp] = TRUE;

        // ... update number of supplementals and the code channels
        ccl.pilots[cclp].sup_info.scch.num_sup_chan = num_sup;
        for ( bn = 0; bn < num_sup; ++bn )
        {
          // If explicit code channel specified...
          if ( pilot_info[supp].expl_code_chan == TRUE )
          {
            ccl.pilots[cclp].sup_info.scch.sup_code_chan[bn] =
              pilot_info[supp].chan_lst.sup_code_chan[bn];
          }
          else
          {
            // ...otherwise, use base code channel + n
            ccl.pilots[cclp].sup_info.scch.sup_code_chan[bn] = 
              pilot_info[supp].chan_lst.base_code_chan + bn;
          }
        }  // of for ( bn = 0...
        // Done processing this pilot.  Continue with next pilot in CCL.
        break;
      }
    }  // of for ( supp = 0...
    if ( supp == num_sup_pilots ) // This means pilot not in sup. pilots
    {
      // If the pilot state is MCCCCL_ACTIVE, we need to clear out the supplementals.
      // If it is MCCCCL_SUP_ONLY, we can just remove it.
      if ( ccl.pilots[cclp].pilot_state == MCCCCL_ACTIVE )
      {
        // Clear out its supplementals
        ccl.pilots[cclp].sup_info.scch.num_sup_chan = 0;
      }
      else  // Must be MCCCCL_SUP_ONLY
      {
        mark_pilot_as_invalid( cclp );
      }
    }
  ...
  // Step 2:
  // For each pilot in pilot_info that's not in CCL, we need to add the entry
  // in CCL and mark as MCCCCL_SUP_ONLY.
  for ( cclp = supp = 0; supp < num_sup_pilots; ++supp )
  {
    // Skip ones that have been processed above
    if ( processed[supp] )
    {
      continue;
    }

    // Find a slot in CCL for this new sup. pilot
    for ( bm = cclp; bm < CCL_SIZE; ++bm )
    {
      if ( ccl.pilots[bm].pilot_state == MCCCCL_INVALID )
      {
        // Found an empty entry, populate it...
        ccl.pilots[bm].pilot_state = MCCCCL_SUP_ONLY;
        // ...with pilot PN, and...
        ccl.pilots[bm].pilot_pn = pilot_info[supp].plt_pn;
        #ifdef FEATURE_IS2000_REL_A
        /* Pilot rec types coming in SCAM can only be comon pilots */
        ccl.pilots[bm].pilot_rec_type = CAI_PILOT_1X_COMMON;
        #endif
        // ...number of supplementals and the code channels
        ccl.pilots[bm].sup_info.scch.num_sup_chan = num_sup;
        for ( bn = 0; bn < num_sup; ++bn )
        {
          // If explicit code channel specified...
          if ( pilot_info[supp].expl_code_chan == TRUE )
          {
            // ...then use the explicitly specified code channel number
            ccl.pilots[bm].sup_info.scch.sup_code_chan[bn] = pilot_info[supp].chan_lst.sup_code_chan[bn];
          }
          else
          {
            // ...otherwise, use base code channel + n
            ccl.pilots[bm].sup_info.scch.sup_code_chan[bn] = pilot_info[supp].chan_lst.base_code_chan + bn;
          }
        }  // of for ( bn = 0...
    ...
    // If an empty entry is not found (shouldn't happen), just stop right here
    // and return.
    if ( bm == CCL_SIZE )
    {
      ERR( "CCL out of entries: Pilot %d", pilot_info[supp].plt_pn, 0, 0 );
      return;
    }
  }
  /* Log the active set */
...
#error code not present
  ...
  for ( uhdmp = 0; uhdmp < uhdm->num_pilots; ++uhdmp )
  {
    ...
    if ( cclp == CCL_SIZE )     // New pilot not in CCL
    {
      ...
      {
        ERR( "CCL out of entries: Pilot %d", uhdm->pilot[uhdmp].pilot_pn, 0, 0 );
        return;
      }
      else
      {
...
)
{
  ...
  for ( bn = newp = 0; newp < num_pilots; ++newp )
  {
    ...
    //lint +e771
    {
      ...
      if ( cclp == CCL_SIZE )
      {
        ERR( "CCL out of entries: Pilot %d", pilot_array[newp].pilot_pn, 0, 0 );
        return;
      }
    }
  }
...
)
{
  ...
  else
  {
    ...
    // Some error checking
    ASSERT ( num_sup <= CAI_MAX_NUM_SUP );
    ...
    for ( bn = 0; bn < num_pilots; ++bn )
    {
      ...
      ccl.pilots[bn].srch_offset_nghbr = 0;
#if defined(FEATURE_IS95B_MDR)
      // If supplemental channels are include, do the update.
      if ( pilot_info[bn].for_sup_inc )
      {
        // ... update number of supplementals and the code channels
        ccl.pilots[bn].sup_info.scch.num_sup_chan = num_sup;
        for ( sup = 0; sup < num_sup; ++sup )
        {
          // If explicit code channel specified...
          if ( pilot_info[bn].expl_code_chan == TRUE )
          {
            // ...then use the explicitly specified code channel number
            ccl.pilots[bn].sup_info.scch.sup_code_chan[sup] = 
              pilot_info[bn].chan_rec.sup_code_chan[sup];
          }
          else
          {
            ccl.pilots[bn].sup_info.scch.sup_code_chan[sup] = 
              pilot_info[bn].chan_rec.base_code_chan + sup;
          }
        }  // of for ( sup = 0...
      }  // of if ( pilot_info[bn].for_sup_inc )
      else
#endif /* FEATURE_IS95B */
      {
...
#error code not present
...
#if defined(FEATURE_IS95B_MDR)
  // Rememeber the active status of supplementals
  ccl.sups_active = use_sups;
#endif
  ...
  for ( count1 = 0; count1 < num_pilots; count1++ )
  {
...
#endif
    {
      ...
      if( !use_sups )
      {
        /* Sups are not requested. So for each active pilot,
        Zero out the number of supplementals */
        aset[count1].scch_info.num = 0;
      }
      else
      {
        byte count2;               /* loop counter                        */

        /* Sups are requested. So for each active pilot,
        copy all of the supplementals info */
        aset[count1].scch_info.num = ccl.pilots[this_entry].sup_info.scch.num_sup_chan;
                                               /* Number of supplementals*/
        /* Copy all supplemental walsh codes for this pilot */
        for( count2 = 0; count2 < ccl.pilots[this_entry].sup_info.scch.num_sup_chan; count2++ )
        {
          aset[count1].scch_info.walsh_chn[count2] =
             ccl.pilots[this_entry].sup_info.scch.sup_code_chan[count2];
        }/* end for */
      }/* end if use_sups ... */
#endif // defined(FEATURE_IS95B_MDR)
    }
    ...
    else // This must be SCH
    {
      ...
      // SCCH is not active, zero it out
      aset[count1].scch_info.num = 0;
      #endif /* FEATURE_IS95B_MDR */
      for ( sch = 0; sch < CAI_MAX_NUM_FOR_SCH; ++sch )
      {
...
DESCRIPTION
  ...
  made. This function is used for SCCH only.

DEPENDENCIES

  FEATURE_IS95B_MDR must be turned on. An MDR call must be up and the service
...
{
  byte num_pilots=0; /* To store the number of pilots       */
  byte count1;       /* loop counter                        */
  byte count2;       /* loop counter                        */
  ...
  /* Find out the active pilots from the entire ccl list */
  /* Note: we are looping through 12 ccl members, but at
  any given time only 6 will contain active pilots       */
  for( count1 = 0; count1 < CCL_SIZE; count1++ )
  {
    /* Check which pilots are currently active. Only their
    supplemental information will be copied */
    if( ccl.pilots[count1].pilot_state == MCCCCL_ACTIVE )
    {
      /* Now copy this pilot's information to the given pointer */
      /* Typecast to a signed 16 bit value, but it's always positive */
      sups[num_pilots].pilot = (int2)ccl.pilots[count1].pilot_pn; /* Pilot's PN*/

      /* Check if supplemental channel information is requested
      by the calling function. Note that we don't give
      the fundamental walsh codes info in this function */
      if( !use_sups )
      {
        /* Sups are not requested. So for each active pilot,
        Zero out the number of supplementals */
        sups[num_pilots].num_sup_channels = 0;
      }
      else
      {
        /* Sups are requested. So for each active pilot,
        copy all of supplementals info */
        sups[num_pilots].num_sup_channels = ccl.pilots[count1].sup_info.scch.num_sup_chan;
                                               /* Number of supplementals*/
        /* Copy all supplemental walsh codes for this pilot */
        for( count2 = 0; count2 < ccl.pilots[count1].sup_info.scch.num_sup_chan; count2++ )
        {
          sups[num_pilots].sup_walsh_codes[count2] =
             ccl.pilots[count1].sup_info.scch.sup_code_chan[count2];
        }/* end for */
      }/* end if sup_usage ... */

      num_pilots++; /* Now increment the active pilot count */
      /* This should not happen, but let's check if it's above CAI_N6M */
      if( num_pilots > CAI_N6M )
      {
        ERR(" More than CAI_N6M Active Pilots in ccl! %d", num_pilots, 0, 0);
        return( CAI_N6M ); /* Calling function can only store CAI_N6M members */
      }/* end if (num_pilots > CAI_N6M) */
    }/* end if (pilot is active) */
  }/* end for (pilot count) */
...
)
{
  ...
  for ( sch = 0; sch < mccap_num_f_sch_supported(); ++sch )
  {
    for ( ccl_plt = aset_plt = 0; ccl_plt < CCL_SIZE; ++ccl_plt )
    {
...
)
{
  ...
  // Do it for all SCH's (MSM5000 supports only 1 - SCH 0)
  for ( sch = 0; sch < mccap_num_f_sch_supported() && !pilot_found; ++sch )
  {
...
(
  byte num_sup_pilots,              // Total number of pilots carrying supplementals
  caii_scam_var_type pilot_info[]   // Array of pilots carrying supplementals
)
{
  int scamp, cclp;

  if ( num_sup_pilots > CAI_MAX_SUP_PILOTS || num_sup_pilots == 0 )
  {
    ERR( "Invalid number of pilots", num_sup_pilots, 0, 0 );
    return FALSE;
  }

  // Now look at each pilot and see if it's in the active set
  for ( scamp = 0; scamp < num_sup_pilots; ++scamp )
  {
    ...
    for ( cclp = 0; cclp < CCL_SIZE; ++cclp )
    {
      ...
      if ( ccl.pilots[cclp].pilot_pn == pilot_info[scamp].plt_pn )
      {
    ...
    // Now if the pilot in scam is not found in CCL (indicated by cclp ==
    // CCL_SIZE), then we can return FALSE.
    if ( cclp == CCL_SIZE )
    {
      return FALSE;
    }
  ...
  // If we can ever get here, that means all the pilots are in the active set,
...
DESCRIPTION
  This function returns the number of supplemental code channels (SCCH) that
  are currently active.  If the supplemental channels exist in the CCL but
  are not active, 0 is returned.  This function has nothing to do with SCH.

DEPENDENCIES
  NONE

RETURN VALUE
  The number of supplemental code channels (SCCH) that are active.

SIDE EFFECTS
  None.

===========================================================================*/
static byte get_number_of_supplementals
(
  void
)
{
  // Initialize the default return of 0
  byte  num_sup = 0;
  byte  bn; // Index variable

  // Only need to do something if the supplementals are active; otherwise
  // we can return 0
  if ( ccl.sups_active )
  {
    // We have supplementals, we need to know how many by looking for the first
    // acitve pilot carrying supplementals.  The number of supplementals that it
    // carries will be the number of active supplementals (since pilot cannot
    // have different number of supplementals.  Note that if for some reason such
    // a pilot is not found, 0 is returned.
    for ( bn = 0; bn < CCL_SIZE; ++bn )
    {
      if ( ccl.pilots[bn].pilot_state == MCCCCL_ACTIVE &&
           ccl.pilots[bn].sup_info.scch.num_sup_chan != 0 )
      {
        num_sup = ccl.pilots[bn].sup_info.scch.num_sup_chan;
        break;
      }
    }
  }
...
)
{
  ...
  if (sch_id >= CAI_MAX_NUM_FOR_SCH)
  {
    ERR_FATAL("sch_in out of range", 0,0,0);
    /* return; */
  }
  ...
  /* Log the active set */
  mccccl_log_aset_info();
#endif // ifdef FEATURE_IS2000_RELEASE_A_LOGGING
} // activate_f_sch_assign
...
)
{
  ...
  if (sch_id >= CAI_MAX_NUM_FOR_SCH)
  {
    ERR_FATAL("sch_id is out of range", 0,0,0);
    /* return; */
  }
...
)
{
  ...
  if (sch_id >= CAI_MAX_NUM_FOR_SCH)
  {
    ERR_FATAL("sch_id out of range", 0,0,0);
    /* return; */
  }

  //make sure that SCH Rate is valid
  if (sch_rate >= CAI_MAX_NUM_PER_SCH_CFG)
  {
	  ERR_FATAL("sch_rate out of range", 0,0,0);
	  /* return; */
  }
...
)
{
  ...
  ASSERT( sch_id < CAI_MAX_NUM_FOR_SCH && index < CAI_MAX_NUM_PER_SCH_CFG );
...
)
{
  ...
  if((sch_id >= CAI_MAX_NUM_REV_SCH) || (rate >= CAI_MAX_NUM_PER_SCH_CFG))
  {
    ERR_FATAL("sch_id or rate OuOfRange %d %d", sch_id, rate, 0);
  }

  return ccl.rev_sch_cfg[sch_id].walsh_id[rate]; /*lint !e662 */
} // get_r_sch_walsh
...
{
  MSG_HIGH("OTA: chind: 0x%x  extchind: 0x%x", ota_chind, ota_extchind, 0);
  MSG_HIGH("MASK: fwd: 0x%x  rev: 0x%x ", ccl.fwd_chan_mask, ccl.rev_chan_mask, 0);
...
#error code not present
#else
    MSG_ERROR("No DV support, EXT_CH_IND should not be in use", 0,0,0);
#endif /* FEATURE_IS2000_REL_C_DV */
  }
  return fwd_mask;
}
...
)
{
  if (!in_control_hold())
  {
    MSG_ERROR("Exit Control Hold?, When MS is Active", 0, 0, 0);
  }
