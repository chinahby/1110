...
$Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/snm.c#1 $ $DateTime: 2009/05/29 03:54:56 $ $Author: prabhuk $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/20/09   pk      Added the check to see if an SO is enabled in case of 
                   evaluating a SO.
02/10/09   jj      Modified the check for fields of Service Connect Message to 
...
/* The maximum size that the buffer in LOG_SRV_CONFIG_VER_3_C or 
  ...
  #else /* FEATURE_IS2000_REL_C_FCS || FEATURE_IS2000_REL_D */
    #define LOG_SRV_CONFIG_BUFFER_REQ_FOR_REL_C 1
  #endif /* FEATURE_IS2000_REL_C_FCS || FEATURE_IS2000_REL_D */
...
#ifdef FEATURE_IS2000_REL_C_DV
#error code not present
#else /* FEATURE_IS2000_REL_C_DV */
  #define LOG_SRV_CONFIG_BUFFER_REQ_FOR_REL_C_DV 0
#endif /* FEATURE_IS2000_REL_C_DV */

#ifdef FEATURE_IS2000_REL_D_DV
#error code not present
#else /* FEATURE_IS2000_REL_D_DV */
...
)
{
  ...
  {
    ERR("Num RCs is 0 for SO %d",so_number, 0, 0);
    return SNM_FAILURE;
  }
  ...
  if (snp.total_so < MAX_SUPPORTED_SO)
  {
    ...
    for (n = 0; n < snp.total_so; n++)
    {
      ...
      {
        ERR("SO added twice: %d", so_number, 0, 0);
        break;
      }
    }
  ...
  {
    ERR("Too many SOs. MAX: %d, SO: %d", MAX_SUPPORTED_SO, so_number, 0);
  }
...
)
{
  ...
  if (snp.total_so > MAX_SUPPORTED_SO)
  {
    ERR("SNM not initialized properly: %d", so, 0, 0);
    return SNM_FAILURE;
  }
...
)
{
  ...
  if (index >= snp.total_so)
  {
    MSG_ERROR("SO index does not exist: %d", index, 0, 0);
    return CAI_SO_REJ;
  }
...
#error code not present
  ...
  MSG_MED("Eval, return: %d, %d", paged_so, so_to_return, 0);
...
)
{
  ...
  for (i = 0, so_list = snp.so[so_index].negotiation_pars.alternate_so;
       i < MAX_ALT_SO && so_list[i] != CAI_SO_NULL; ++i)
  {
...
)
{
  ...
  if (max_alt_so > MAX_ALT_SO)
  {
    max_alt_so = MAX_ALT_SO;
  }

  if (snm_find_so(so_num, &so_index) == SNM_SUCCESS) /* if SO is in SNP */
  {
    /* go through the alt so list */
    for (index = 0; (index < MAX_ALT_SO) && (num_alt_so < max_alt_so); index++)
    {
  ...
  *so_bitmap_ind = SO_BITMAP_SIZE_0;
  ...
  if (snm_find_so(so_num, &so_index) == SNM_SUCCESS)
  {
    *so_group_num = snp.so[so_index].negotiation_pars.so_group;
    ...
    for (index = 0 ; index < *num_alt_so ; index++ )
    {
...
#error code not present
  ...
  if (snm_find_so(so, &so_index) == SNM_FAILURE)
  {
    ...
    MSG_MED("Service Option %d not found in SNM", so, 0, 0);
...
)
{
  ...
  MSG_MED("Searching neg info based on sr_id =%d",use_sr_id,0,0);
  ...
  {
    MSG_MED("Did not find a matching sr_id.. starting a fresh neg on new sr_id",
             0,0,0);
...
)
{
  ...
  if (sch_mux == 0xf20)
  {
    MSG_ERROR("Wrong function used for rate calculation for 0xf20 mux", 0, 0, 0);
    return 0;
  }
...
)
{
     ...
     )
  {
    ...
    found_mux = FALSE;
    for (i=index+1; i<so_info->negotiation_pars.fwd_sch_mo_list.num_mos; i++)
    {
    ...
    if (!found_mux)
    {
      /* Okay we couldn't find RCs and mux consistent with each other, do
         not propose SCH */
      cfg_ptr->for_sch_cc_incl = FALSE;
      MSG_ERROR("F-SCH not proposed because of RC/MUX inconsistency",0,0,0);
      return;
    }
  }
...
)
{
          ...
          (MUX_TO_RATE_SET(cfg_ptr->rev_sch[0].rev_sch_mux) & 1)))
  {
    /* The SCH RC we proposed is not matching the SCH
     ...
     found_mux = FALSE;
     for (i=index+1; i<so_info->negotiation_pars.rev_sch_mo_list.num_mos; i++)
     {
     ...
     if (!found_mux)
     {
       /* Okay we couldn't find RCs and mux consistent with each other, do
          not propose SCH */
       cfg_ptr->rev_sch_cc_incl = FALSE;
       MSG_ERROR("R-SCH not proposed because of RC/MUX inconsistency",0,0,0);
       return;
     }
  }
...
)
{
  ...
  {
    MSG_ERROR("Illegal index in to negotiation_info.con_list[] %d", con_index,
      0, 0 );
    return SNM_FAILURE;
  }
  ...
  if (snm_find_so(so, &so_index) == SNM_FAILURE)
  {
    MSG_ERROR("SO %d not found", so, 0, 0);
    return SNM_FAILURE;
  }
  ...
  ** ************************************************ */
  if ((fwd_scr_chan_mask & SNM_DCCH) || (rev_scr_chan_mask & SNM_DCCH))
  {
    ...
    if (!P_REV_IS_6_OR_GREATER || !mccap_item_is_supported(MCCAP_DF_DCCH))
    {
      if (!suggest_alt)
      {
        MSG_MED("MS does not support DCCH, reject SCR", 0, 0, 0);
  ...
  ** ************************************************** */
  if (fwd_scr_chan_mask & SNM_PDCH)
  {
    if (!P_REV_IS_9_OR_GREATER || !mccap_is_for_pdch_supported())
    {
      if (!suggest_alt)
      {
        MSG_MED("MS does not support F-PDCH, reject SCR", 0, 0, 0);
        return SNM_REJECT_CFG;
      }
      else
      {
...
#error code not present
  ...
  ** ************************************************* */
      ...
      (!P_REV_IS_6_OR_GREATER || !mccap_item_is_supported(MCCAP_DF_FOR_SCH)))
  {
    if (!suggest_alt)
    {
      MSG_MED("MS does not support fwd SCH, reject SCR", 0, 0, 0);
      return SNM_REJECT_CFG;
    }
    else
    {
  ...
  ** ************************************************* */
      ...
      (!P_REV_IS_6_OR_GREATER || !mccap_item_is_supported(MCCAP_DF_REV_SCH) ))
  {
    if (!suggest_alt)
    {
      MSG_MED("MS does not support rev SCH, reject SCR", 0, 0, 0);
      return SNM_REJECT_CFG;
    }
    else
    {
  ...
  ** ********************* */
  if (frame_size_chan_mask == SNM_DCCH)
  {
    if (cfg_ptr->dcch_frame_size != CAI_DCCH_20MS_FRAME)  /*lint !e641 */
    {
      if (!suggest_alt)
      {
        MSG_MED("BS proposed 5ms DCCH frame size, reject SCR", 0, 0, 0);
        return SNM_REJECT_CFG;
      }
      else
      {
        MSG_MED("BS proposed 5ms DCCH frame size, propose SCR", 0, 0, 0);
        cfg_ptr->dcch_cc_incl = TRUE;
        cfg_ptr->dcch_frame_size = CAI_DCCH_20MS_FRAME; /*lint !e641 */
        status = SNM_ALT_CFG;
      }
  ...
  if (frame_size_chan_mask == SNM_FCH)
  {
    ...
    ** ******************** */
    if (cfg_ptr->fch_frame_size)
    {
      if (!suggest_alt)
      {
        MSG_MED("BS proposed 5ms FCH frame size, reject SCR", 0, 0, 0);
        return SNM_REJECT_CFG;
      }
      else
      {
        MSG_MED("BS proposed 5ms FCH frame size, propose SCR", 0, 0, 0);
        cfg_ptr->fch_cc_incl = TRUE;
  ...
  if (P_REV_IS_8_OR_GREATER)
  {
        ...
        ((fwd_scr_chan_mask | rev_scr_chan_mask) & SNM_DCCH))
    {
          ...
          (cfg_ptr->fch_dcch_mux_option_ind == CAI_MUX_OPTION_ONLY_FOR_DCCH))  /*lint !e641 */
      {
        ...
        if (!suggest_alt)
        {
          MSG_MED("Bad Mux Opt Ind %d, reject SCR",
            cfg_ptr->fch_dcch_mux_option_ind, 0, 0);
          return SNM_REJECT_CFG;
        }
        else
        {
          MSG_MED("Bad Mux Opt Ind %d, propose SCR",
            cfg_ptr->fch_dcch_mux_option_ind, 0, 0);
  ...
  ** ********** */
  ...
  if (mux_chan_mask & SNM_DCCH)
  {
          ...
          fwd_rc, rev_rc, *fwd_mux_ptr, *rev_mux_ptr))
    {
      ...
      if (!suggest_alt)
      {
        MSG_MED("Bad DCCH Mux: fwd %d rev %d, reject SCR", *fwd_mux_ptr, 
          *rev_mux_ptr, 0);
        return SNM_REJECT_CFG;
      }
      else
      {
        MSG_MED("Bad DCCH Mux: fwd %d rev %d, propose SCR", *fwd_mux_ptr, 
          *rev_mux_ptr, 0);
        cfg_ptr->dcch_cc_incl = TRUE;
  ...
  if (mux_chan_mask & SNM_FCH)
  {
          ...
          fwd_rc,rev_rc, *fwd_mux_ptr, *rev_mux_ptr))
    {
      ...
      if (!suggest_alt)
      {
        MSG_MED("Bad FCH Mux: fwd %d rev %d, reject SCR", *fwd_mux_ptr, 
          *rev_mux_ptr, 0);
        return SNM_REJECT_CFG;
      }
      else
      {
        MSG_MED("Bad FCH Mux: fwd %d rev %d, propose SCR", *fwd_mux_ptr, 
          *rev_mux_ptr, 0);
        cfg_ptr->fch_cc_incl = TRUE;
...
)
{
  ...
  if (so_info != NULL)
  {
    ...
    if ((fwd_scr_chan_mask & SNM_FCH) && (cfg_ptr->fwd_rates != so_info->default_config.forward.tx_rates))
    {
      if (!suggest_alt)
      {
        MSG_MED("Fwd rate %d is not good, reject SCR", cfg_ptr->fwd_rates, 0, 0);
        return SNM_REJECT_CFG;
      }      
      else
      {
        MSG_MED("Fwd rate %d is not good, propose SCR", cfg_ptr->fwd_rates, 0, 0);
    ...
    if ((rev_scr_chan_mask & SNM_FCH) && (cfg_ptr->rev_rates != so_info->default_config.reverse.tx_rates))
    {
      if (!suggest_alt)
      {
        MSG_MED("Rev rate %d is not good, reject SCR", cfg_ptr->rev_rates, 0, 0);
        return SNM_REJECT_CFG;
      }
      else
      {
        MSG_MED("Rev rate %d is not good, propose SCR", cfg_ptr->rev_rates, 0, 0);
...
)
{
  ...
  {
    MSG_MED("Signalling SO %d don't care fundicated config", so_info->service_option_num, 0, 0);
    return status;
  }
  ...
  ** ************ */
  ...
  { 
    ...
    if (!snm_is_fundicated_rc_match(fwd_scr_chan_mask, rev_scr_chan_mask, cfg_ptr)) 
    {
      if (!suggest_alt)
      {
        MSG_MED("Mismatch RC class or rate set, reject SCR", 0, 0, 0);
        return SNM_REJECT_CFG;
      }
      else
      { 
        ...
        if (compatible_chan_mask & SNM_DCCH)
        {
          ...
          {
            MSG_MED("Mismatch RC class or rate set, propose SCR", 0, 0, 0);
          ...
          else
          {
            ...
            {
              MSG_MED("SO %d: None of FCH RC can be proposed, reject SCR", 
                so_info->service_option_num, 0, 0);
              return SNM_REJECT_CFG;
            }
          }
        ...
        else 
        {
          ...
          else
          {
            ...
            if (!snm_gen_fundicated_rc(so_info, snm_con_index, 
                ...
                &cfg_ptr->rev_dcch_rc)) 
            {
              MSG_MED("SO %d: None of DCCH RC can be proposed, reject SCR", 
                so_info->service_option_num, 0, 0);
              return SNM_REJECT_CFG;
            }
          }
      ...
      !(compatible_chan_mask & so_preferred_fundicated_chan_mask))
  {
    if (!suggest_alt)
    {
      ...
      if ((so_preferred_fundicated_chan_mask & SNM_DCCH) &&
          !(compatible_chan_mask & SNM_DCCH))
      {
        MSG_MED("Bad DCCH RCs: fwd %d, rev %d Alt: %d, reject SCR", cfg_ptr->for_dcch_rc, 
          cfg_ptr->rev_dcch_rc, suggest_alt);
      }
#endif /* FEATURE_IS2000_P2 */
      ...
      if ((so_preferred_fundicated_chan_mask & SNM_FCH) && 
          !(compatible_chan_mask & SNM_FCH))
      { 
        MSG_MED("Bad FCH RCs: fwd %d, rev %d Alt: %d, reject SCR", cfg_ptr->for_fch_rc, 
          cfg_ptr->rev_fch_rc, suggest_alt);
      }
    ...
    else
    {
      ...
      {
        MSG_MED("Bad DCCH RCs: fwd %d, rev %d, propose SCR", 
          cfg_ptr->for_dcch_rc, cfg_ptr->rev_dcch_rc, 0);
      ...
      {
        MSG_MED("Bad FCH RCs: fwd %d, rev %d, propose SCR", 
          cfg_ptr->for_fch_rc, cfg_ptr->rev_fch_rc, 0);
...
) 
{
  ...
  if (bad_cfg)
  {
    if (!suggest_alt)
    {
      MSG_MED("Bad traffic type Fwd:%d Rev:%d, reject SCR", 
        cfg_ptr->con[con_index].fwd_traf, cfg_ptr->con[con_index].rev_traf, 0);
      return SNM_REJECT_CFG;
    }
    else
    {
      MSG_MED("Bad traffic type Fwd:%d Rev:%d, propose SCR", 
        cfg_ptr->con[con_index].fwd_traf, cfg_ptr->con[con_index].rev_traf, 0);
...
) 
{
      ...
      /* we have function to evaluate it */
  {
    ...
    {
      MSG_MED("QoS blob not acceptable, reject SCR", 0, 0, 0);
      return SNM_REJECT_CFG;
    }
    else
    {
      ...
      if (qos_eval_status == SNM_ALT_CFG)
      {
        ...
        MSG_MED("QoS blob not acceptable, propose SCR", 0, 0, 0);
...
) 
{
      ...
      (cfg_ptr->con[con_index].sr_id > MAX_SR_ID_VALUE))
  {
    ...
    {
      MSG_MED("Invalid SR ID %d, reject SCR", cfg_ptr->con[con_index].sr_id, 0, 0);
      return SNM_REJECT_CFG;
    }
    else
    {
      MSG_MED("Invalid SR ID %d, propose SCR", cfg_ptr->con[con_index].sr_id, 0, 0);
...
) 
{
  ...
  {
    /* Shall never happen */
    MSG_ERROR("Invalid channel link specified %d", link_mask, 0, 0);
    return SNM_REJECT_CFG;
  }
  ...
  {
	  MSG_HIGH("Invalid SCH Index value %d", sch_index,0,0);
	  return SNM_REJECT_CFG;
  }
  ...
  if (link_mask == SNM_FORWARD)  
  {
    if ((cfg_ptr->for_sch[sch_index].sch_chn_cfg.sch_rc <= CAI_RC_5) &&  /*lint !e641 */
    ...
    if (bad_cfg)
    {
      if (!suggest_alt)
      {
        MSG_MED("Bad F-SCH RC %d, reject SCR",  
          cfg_ptr->for_sch[sch_index].sch_chn_cfg.sch_rc, 0, 0);
        return SNM_REJECT_CFG;
      }
      else
      {
        MSG_MED("Bad F-SCH RC %d, propose SCR",  
          cfg_ptr->for_sch[sch_index].sch_chn_cfg.sch_rc, 0, 0);
  ...
  {
    if ((cfg_ptr->rev_sch[sch_index].sch_chn_cfg.sch_rc <= CAI_RC_4) && /*lint !e641 */
    ...
    if (bad_cfg)
    {
      if (!suggest_alt)
      {
        MSG_MED("Bad R-SCH RC %d, reject SCR",  
          cfg_ptr->rev_sch[sch_index].sch_chn_cfg.sch_rc, 0, 0);
        return SNM_REJECT_CFG;
      }
      else
      {
        MSG_MED("Bad R-SCH RC %d, propose SCR",  
          cfg_ptr->rev_sch[sch_index].sch_chn_cfg.sch_rc, 0, 0);
...
) 
{
  ...
  if (link_mask == SNM_FORWARD)
  {
    ...
    if (bad_cfg)
    {
      if (!suggest_alt)
      {
        MSG_MED("Bad F-SCH mux 0x%x, reject SCR", 
          cfg_ptr->for_sch[sch_index].for_sch_mux, 0, 0);
        return SNM_REJECT_CFG;
      }
      ...
      else
      {
        ...
        MSG_MED("Bad F-SCH mux 0x%x, propose SCR", 
          cfg_ptr->for_sch[sch_index].for_sch_mux, 0, 0);
  ...
  else 
  {
    ...
    if (bad_cfg)
    {
      if (!suggest_alt)
      {
        MSG_MED("Bad R-SCH mux 0x%x, reject SCR", 
          cfg_ptr->rev_sch[sch_index].rev_sch_mux, 0, 0);
        return SNM_REJECT_CFG;
      }
      /* Propose alt MUX agsinst supported SO */
      else
      {
        ...
        MSG_MED("Bad R-SCH mux 0x%x, propose SCR", 
          cfg_ptr->rev_sch[sch_index].rev_sch_mux, 0, 0);
...
) 
{
             ...
             just make sure the value given in the max rate is consistent */
  {
    if (suggest_alt)
    {
      MSG_MED("Bad %s max_rate %d, propose SCR", (link_mask == SNM_FORWARD) ? "F--SCH" : "R-SCH", 
        *sch_max_rate_ptr, 0);
      status = SNM_ALT_CFG;
    ...
    {
      MSG_MED("Bad %s max_rate %d, reject SCR", (link_mask == SNM_FORWARD) ? "F--SCH" : "R-SCH", 
        *sch_max_rate_ptr, 0);
      return SNM_REJECT_CFG;
    }
  }
...
) 
{
  ...
  if (*frame_40_used_ptr && !frame_40_supported)
  {
    if (suggest_alt)
    {
      MSG_MED("%s Frame 40 support mismatch, propose SCR", 
        ((link_mask == SNM_FORWARD) ? "F-SCH" : "R-SCH"), 0, 0);
      status = SNM_ALT_CFG;
      *frame_40_used_ptr = FALSE;
    }
    else
    {
      MSG_MED("%s Frame 40 support mismatch, reject SCR", 
        ((link_mask == SNM_FORWARD) ? "F-SCH" : "R-SCH"), 0, 0);
      return SNM_REJECT_CFG;
    }
  } 
  ...
  if (*frame_80_used_ptr && !frame_80_supported)
  {
    if (suggest_alt)
    {
      MSG_MED("%s Frame 80 support mismatch, propose SCR", 
        ((link_mask == SNM_FORWARD) ? "F-SCH" : "R-SCH"), 0, 0);
      status = SNM_ALT_CFG;
    ...
    {
      MSG_MED("%s Frame 80 support mismatch, reject SCR", 
        ((link_mask == SNM_FORWARD) ? "F-SCH" : "R-SCH"), 0, 0);
      return SNM_REJECT_CFG;
    }
  }
...
) 
{
  ...
  if (cdma.p_rev_in_use >= P_REV_IS2000_REL_A)
  {
    ...
    if (bad_cfg)
    {
      ...
      if (suggest_alt)
      {
        MSG_MED("Bad %s SCH coding %d, propose SCR", 
          (link_mask == SNM_FORWARD) ? "F-SCH" : "R-SCH", *sch_coding_ptr, 0);
        *sch_coding_ptr = coding_type; 
        status = SNM_ALT_CFG;
      }
      else
      {
        MSG_MED("Bad %s SCH coding %d, reject SCR", 
          (link_mask == SNM_FORWARD) ? "F-SCH" : "R-SCH", *sch_coding_ptr, 0);
        return SNM_REJECT_CFG;
      }
    }
     ...
     )
  {
    ...
    ** ******************************************************** */
    if (fwd_scr_chan_mask & SNM_PDCH)
    {
      ...
      {     
        ...
        {
          MSG_MED("MS does not support simultaneously F-PDCH and SCH, reject SCR", 0, 0, 0);
          return SNM_REJECT_CFG;
        }
      }
    }
    ...
    ** ********************************************************* */
...

    {
      ...
      {
        MSG_MED("Cannot support SCH on FCH RC class 1, reject SCR", 0, 0, 0);
        return SNM_REJECT_CFG;
      }
    }
    ...
    ** *************************************************** */
        ...
        (so_info->negotiation_pars.fwd_sch_mo_list.num_mos > 0)) 
    {
      ...
      if (cfg_ptr->num_for_sch > max_sch)
      {
        if (!suggest_alt)
        {
          MSG_MED("%d F-SCHs not supported, reject SCR", cfg_ptr->num_for_sch, 0, 0);
          return SNM_REJECT_CFG;
        }
        else
        {
          MSG_MED("%d F-SCHs not supported, propose SCR", cfg_ptr->num_for_sch, 0, 0);
        }
      }
      ...
      for (i = 0; (i < cfg_ptr->num_for_sch && (i < CAI_MAX_NUM_FOR_SCH)); i++)
      {
        ...
        ** ************ */
        ...
        { 
          if (!suggest_alt)
          {
            MSG_MED("F-SCH %d not allowed, reject SCR", 
              cfg_ptr->for_sch[i].for_sch_id, 0, 0);
            return SNM_REJECT_CFG;
          }

          MSG_MED("F-SCH %d not allowed, propose SCR", 
            cfg_ptr->for_sch[i].for_sch_id, 0, 0);
          continue;
        }
        ...
        if (P_REV_IS_7_OR_GREATER)
        {
          ...
          ** ************** */
              ...
              (cfg_ptr->for_sch[sch_index].sch_chn_cfg.sch_rc == CAI_RC_3))  /*lint !e641 */
          {
            if (suggest_alt)
            {
              MSG_MED("Error 32X with RC 3 !!, propose SCR", 0, 0, 0);
              /* Set max rate to be what we computed above */
            ...
            {
              MSG_MED("Error 32X with RC 3 !!, reject SCR", 0, 0, 0);
              return SNM_REJECT_CFG;
            }
          }
    ...
    ** *************************************************** */
        ...
        /* SO supports SCH */
    {
      ...
      if (cfg_ptr->num_rev_sch > max_sch)
      {
        if (!suggest_alt)
        {
          MSG_MED("%d R-SCHs not supported, reject SCR", cfg_ptr->num_rev_sch, 0, 0);
          return SNM_REJECT_CFG;
        }
        else
        {
          MSG_MED("%d R-SCHs not supported, propose SCR", cfg_ptr->num_rev_sch, 0, 0);
        }
      }
        ...
        ** **************** */
        ...
        if (P_REV_IS_7_OR_GREATER)
        {
          ...
          ** ************** */
          ...
          {
            MSG_MED("Error 18X with RC 4 !!, reject SCR", 0, 0, 0);
            return SNM_REJECT_CFG;
          }
          /* ****************
          ** Check frame size
  ...
  ** ********************************** */
...
#endif /* FEATURE_IS2000_REL_A_SVD */
  {
    if (con_index > 0) //lint !e539
    {
      if (!suggest_alt)
      {
        MSG_MED("con_index can't be non-zero %d", con_index,0,0);
        return SNM_REJECT_CFG;
      }
      else
      {
    ...
    ** *************************************** */
       ...
       )  
    {
      ...
      /* FCH assignment is asymmetric to DCCH assignment, simply set RCs of DCCH same as FCH */
               ...
               (rev_has_fch == rev_has_dcch))
      {
        MSG_MED("Bad DCCH RCs: fwd %d, rev %d, propose SCR", cfg_ptr->for_dcch_rc, 
          cfg_ptr->rev_dcch_rc, 0);
        cfg_ptr->dcch_cc_incl = TRUE;
        cfg_ptr->for_dcch_rc = cfg_ptr->for_fch_rc;
        cfg_ptr->rev_dcch_rc = cfg_ptr->rev_fch_rc;
        status = SNM_ALT_CFG;
    ...
    ** ******************************************************** */
    ...
    { 
      ...
      if (!snm_is_fundicated_rc_match(fwd_scr_chan_mask, rev_scr_chan_mask, cfg_ptr)) 
      {
        if (!suggest_alt)
        {
          MSG_MED("FCH and DCCH RC rateset or class not match, reject SCR", 0, 0, 0);
          status = SNM_REJECT_CFG;
        }
        else
        {
          ...
          if ((fwd_has_fch == fwd_has_dcch) &&
              (rev_has_fch == rev_has_dcch))
          {
            MSG_MED("FCH and DCCH RC rateset or class not match, propose SCR", 0, 0, 0);
            cfg_ptr->fch_cc_incl = TRUE;
            cfg_ptr->for_fch_rc = cfg_ptr->for_dcch_rc;
            cfg_ptr->rev_fch_rc = cfg_ptr->rev_dcch_rc;
            status = SNM_ALT_CFG;
    ...
    ** ************************************** */
       ...
       )  
    {
      ...
      /* DCCH assignment is symmetric to FCH assignment, simply set RCs of FCH same as DCCH */
               ...
               (rev_has_fch == rev_has_dcch))
      {
        MSG_MED("Bad FCH RCs: fwd %d, rev %d, propose SCR", cfg_ptr->for_fch_rc, 
          cfg_ptr->rev_fch_rc, 0);
        cfg_ptr->fch_cc_incl = TRUE;
        cfg_ptr->for_fch_rc = cfg_ptr->for_dcch_rc;
        cfg_ptr->rev_fch_rc = cfg_ptr->rev_dcch_rc;
        status = SNM_ALT_CFG;
...
)
{
  ...
  ** ********************************************************************** */
      ...
      !negotiation_info.fwd_sch_cc_verified)  
  {
    ...
    (void) snm_find_so(CAI_SO_PPP_PKT_DATA_3G, &so_index);
    ...
    ** *************************** */
        ...
        !negotiation_info.fwd_sch_cc_verified)  
    {
      ...
      if (cfg_ptr->num_for_sch > max_sch)
      {
        MSG_MED("%d F-SCHs not supported", cfg_ptr->num_for_sch, 0, 0);
        return SNM_REJECT_CFG;
      }
      ...
      for (i = 0; ((i < cfg_ptr->num_for_sch) && (i < CAI_MAX_NUM_FOR_SCH)); i++)
      {
        ...
        ** ************ */
        if (cfg_ptr->for_sch[i].for_sch_id >= max_sch)
        { 
          MSG_MED("F-SCH ID %d not supported", cfg_ptr->for_sch[i].for_sch_id, 0, 0);
          return SNM_REJECT_CFG;
        }
        ...
        ** **************** */
        ...
        if (P_REV_IS_7_OR_GREATER)
        {
          ...
          ** ************** */
          ...
          {
            MSG_MED("Error : 32X with RC3 !!", 0, 0, 0);
            return SNM_REJECT_CFG;
          }
    ...
    ** *************************** */
        ...
        !negotiation_info.rev_sch_cc_verified)  
    {
      ...
      if (cfg_ptr->num_rev_sch > max_sch)
      {
        MSG_MED("%d R-SCHs not supported", cfg_ptr->num_rev_sch, 0, 0);
        return SNM_REJECT_CFG;
      }
      ...
      for (i = 0; ((i < cfg_ptr->num_rev_sch) && (i < CAI_MAX_NUM_REV_SCH)); i++)
      {
        if (cfg_ptr->rev_sch[i].rev_sch_id >= max_sch)
        {
          MSG_MED("R-SCH ID %d not supported", cfg_ptr->for_sch[i].for_sch_id, 0, 0);
          return SNM_REJECT_CFG;
        }
        ...
        ** **************** */
        ...
        if (P_REV_IS_7_OR_GREATER)
        {
          ...
          ** ************** */
          ...
          {
            MSG_MED("Error : 18X with RC4 !!", 0, 0, 0);
            return SNM_REJECT_CFG;
          }
          /* ****************
          ** Check frame size
...
)
{
  ...
  MSG_MED("Eval cfg:Num Connections %d, fwd_mux %d, rev_mux %d", cfg_ptr->num_con,
            cfg_ptr->fwd_mux, cfg_ptr->rev_mux);

  MSG_MED("SO of first connection record- as rxed in OTA %d", 
          cfg_ptr->con[0].so, 0, 0);
...
#error code not present
  ...
  {
    MSG_MED("SVD supported only in PREV >= 7", 0,0,0);
    return SNM_REJECT_CFG;
  }
#endif /* FEATURE_IS2000_REL_A_SVD */
  ...
  if ((cfg_ptr->num_con > MAX_CON) && !suggest_alt)
  {
    ...
    MSG_MED("Too many connections", 0,0,0);
    return SNM_REJECT_CFG;
  }
    ...
    for (so_con = 0, iterations_so_far = 0; so_con < cfg_ptr->num_con;)
    {
          ...
          &con_index))
      {
        ...
        if (!snm_get_free_neg_index(&con_index))
        {
          ...
          MSG_MED("No room for evaluating new connection", 0,0,0);
          return SNM_REJECT_CFG;
        }
      ...
      MSG_MED("con_index calculated = %d",con_index,0,0);
      if (negotiation_info.con_list[con_index].beginning_so == CAI_SO_REJ)
      {
        ...
        if (snm_find_so(cfg_ptr->con[so_con].so, &so_index) != SNM_SUCCESS )
        {
          ...
          MSG_MED("SO not found %d", cfg_ptr->con[so_con].so, 0, 0);
          goto LOOP_CONTROL;  /*lint !e801 */
        }
        MSG_MED("initializing neg_info with con_index=%d with the rxed SO",
                con_index,0,0);
      ...
      MSG_MED("SO of beginning record in neg_info %d", 
              negotiation_info.con_list[con_index].beginning_so,0,0);
          ...
          negotiation_info.con_list[con_index].beginning_so)
      {
        ...
        MSG_MED("beginning and rxed SO are the same .. SO status = %d",
                snp.so[so_index].status,0,0);
        if ((snp.so[so_index].status == SNM_ENABLED) &&
              snm_p_rev_verified(snp.so[so_index].negotiation_pars.min_p_rev))
        {
      ...
      for (wn = 0, wlist = negotiation_info.con_list[con_index].alt_so_list;
           wn < MAX_ALT_SO && wlist[wn] != CAI_SO_NULL; wn++)
      {
        if (wlist[wn] == cfg_ptr->con[so_con].so)
        {
          ...
          {
            /* Should never happen. */
            ERR("Alt.SO not in SNM.  Seed: %d, alt.SO: %d",
              negotiation_info.con_list[con_index].neg_seed, wlist[wn], 0);
            break;
          }
          MSG_MED("SO status = %d",snp.so[so_index].status,0,0);
          if ((snp.so[so_index].status == SNM_ENABLED) &&
              snm_p_rev_verified(snp.so[so_index].negotiation_pars.min_p_rev))
          {
            ...
            {
              MSG_MED("status_list[so_con] =%d , rejecting..",status_list[so_con],0,0);
              return status_list[so_con];
            }
            MSG_MED("status_list[so_con] =%d",status_list[so_con],0,0);
      ...
      if (P_REV_IS_7_OR_GREATER && 
          ...
          ((wn == MAX_ALT_SO) || (wlist[wn] == CAI_SO_NULL)) &&
          ...
          negotiation_info.con_list[con_index].so_can_be_changed) 
      {
        ...
        MSG_MED("Radical change of SO, old seed %d, old beginning so %d, new so %d", 
          negotiation_info.con_list[con_index].neg_seed,
          negotiation_info.con_list[con_index].beginning_so,
          cfg_ptr->con[so_con].so);
...
#error code not present
      ...
      else
      {
        /* Too many iterations of finding alternative for this SVD config, 
           break out. */
        MSG_MED("SNM exceeded limit of iterations, rej cfg", 0, 0, 0);
        return SNM_REJECT_CFG;
      }
    } /* end for so_con... */
  ...
  for (index=0; index < cfg_ptr->num_con && index < MAX_CON; index++)
  {
    ...
    {                                        
      ...
      MSG_MED("SO not found or not in alternate list", 0, 0, 0);
      break;
    }
  }
      ...
      (negotiation_info.current_config->neg_scr.num_con > 1)) 
  {
    ...
    *alt_cfg_ptr = negotiation_info.current_config->neg_scr;
    MSG_MED("Proposing the current configuration", 0, 0, 0);
    return SNM_ALT_CFG;
  }
#endif /* FEATURE_IS2000_REL_A_SVD */
  ...
  if (!snm_get_neg_index_for_sr_id(FALSE, 0, &con_index))
  {
    ...
    MSG_MED("Failed to find entry in negotiation database",0,0,0);
    return SNM_REJECT_CFG;
  }
      ...
      (negotiation_info.con_list[con_index].beginning_so != CAI_SO_REJ))
  {
         ...
         wn < MAX_ALT_SO && wlist[wn] != CAI_SO_NULL ; wn++)
    {
      ...
      if (snm_find_so(wlist[wn], &so_index) != SNM_SUCCESS)
      {
        /* Should never happen. */
        ERR( "Alt.SO not in SNM.  Seed: %d, alt.SO: %d", 
          negotiation_info.con_list[0].neg_seed, wlist[wn], 0 );
  ...
  if (status == SNM_ALT_CFG) /* We found an SO_INDEX in above loop */
  {
        ...
        SNM_MAXIMUM_MUX, con_index, fwd_chan_mask, rev_chan_mask) == SNM_FAILURE)
    {
      ...
      MSG_MED("Failed to generate cfg with alt SO %d", 
        snp.so[so_index].service_option_num, 0, 0);
      status = SNM_REJECT_CFG;
    }
    else
    {
      ...
      MSG_MED("Alt. cfg:SO %d, fwd_mux %d, rev_mux %d", alt_cfg_ptr->con[0].so,
         alt_cfg_ptr->fwd_mux, alt_cfg_ptr->rev_mux);
...
#error code not present
  ...
  ** ********************************************************************** */
  ...
  if (nnscr_for_lpm_eval->lpm_ind > CAI_LPM_LAST)  /*lint !e641 */
  {
    ...
    MSG_MED("Invalid lpm_ind %d",nnscr_for_lpm_eval->lpm_ind, 0, 0);
    return SNM_FAILURE;
  }
  ...
  for (i = 0; i < nnscr_for_lpm_eval->num_lpm_entries; i++)
  {
    ...
    {
      MSG_MED("Invalid sr_id or logical/physical resource", 0, 0, 0);
      return SNM_FAILURE;
    }
    ...
    {
      MSG_MED("DCCH not assigned or not supported, but included in LPM", 0, 0, 0);
      continue;
    }
    ...
    {
      MSG_MED("FCH not assigned, but included in LPM", 0, 0, 0);
      continue;
    }
#endif /* FEATURE_IS2000_P2 */
...
#error code not present
    ...
    {
      MSG_MED("PDCH not assigned or supported, but included in LPM", 0, 0, 0);
      continue;
    }
#endif /* FEATURE_IS2000_REL_C */
      ...
      switch (nnscr_for_lpm_eval->lpm_entries[i].physical_resource)
      {
        case CAI_LPM_FCH:
          if (so_chan_mask & SNM_FCH)
          {
            ...
            {
              MSG_MED("LPM: SRID %d SO %d not compatible with FCH RC, reject NNSCR", 
                sr_id, so, 0); 
              return SNM_FAILURE;
            }
          }
        ...
        case CAI_LPM_DCCH:
          if (so_chan_mask & SNM_DCCH)
          {
            ...
            {
              MSG_MED("LPM: SRID %d SO %d not compatible with DCCH RC, reject NNSCR", 
                sr_id, so, 0); 
              return SNM_FAILURE;
            }
          }
...
#error code not present
  ...
  {
    /* reject */
    MSG_MED("No legal reverse signaling channel provided in LPM, reject NNSCR", 0, 0, 0);
    return SNM_FAILURE;
  }
  ...
  {
    /* reject */
    MSG_MED("No legal forward signaling channel provided in LPM, reject NNSCR", 0, 0, 0);
    return SNM_FAILURE;
  }
  ...
  for (i=0; i<neg_cfg_ptr->num_con; i++)
  {
    ...
    {
      MSG_MED("LPM did not specify fwd/rev entry for sr_id %d, reject NNSCR", 
        neg_cfg_ptr->con[i].sr_id, 0, 0);
      return SNM_FAILURE;
    }
  } /* end for */
  ...
  if (nn_cfg_ptr->fpc_incl)
  {
    ...
    if (nn_cfg_ptr->fpc_pri_chan == CAI_FPC_PRI_CHAN_CPCCH) /*lint !e641 */
    {
      ...
      {
        MSG_MED("Unacceptable FPC channel %d, reject NNSCR",
          nn_cfg_ptr->fpc_pri_chan, 0, 0);
        return SNM_FAILURE;
      }
    }
    else
  ...
  if (P_REV_IS_7_OR_GREATER)
  {
    ...
    {
      MSG_ERROR("No flex rates support, reject NNSCR", 0, 0, 0);
      return SNM_FAILURE;
    }
    if (nn_cfg_ptr->use_var_rate && !mccap_is_sch_var_rates_supported())
    {
      MSG_ERROR("No var rates support, reject NNSCR", 0, 0, 0);
      return SNM_FAILURE;
    }
  }
#endif /* FEATURE_IS2000_REL_A */
...
#error code not present
  ...
  if (neg_scr_ptr->num_con == 0)
  {
    ...
    MSG_MED("Received SCR with zero con rec", 0, 0, 0);
...
)
{
  ...
  switch (new_lpm_nnscr->lpm_ind)
  {
    case CAI_LPM_DEFAULT:
      ...
      if (P_REV_IS_7_OR_GREATER) //lint !e506 !e774
      {
        for (index=0; index < new_scr->num_con; index++)
        {
...
)
{
  ...
  if (P_REV_IS_7_OR_GREATER)
  {
    ...
    {
        MSG_MED("numrec %d exceeds the maximum possible values %d", 
         negotiation_info.current_config->non_neg_scr.numrec, CAI_NNSCR_MAX_NUM_SR_REC, 0);
    }

    if (negotiation_info.current_config->neg_scr.num_con > CAII_SRV_CON_MAX)
    {
        MSG_MED("num_con %d exceeds the maximum possible value %d", 
         negotiation_info.current_config->neg_scr.num_con, CAII_SRV_CON_MAX, 0);
    }
...
static snm_status_type snm_initialize_connections()
{
...
*/
  ...
  if (!scr_ptr)
  {
     MSG_MED( "Unable to allocate log buffer", 0, 0, 0 );
     return;
  }
...
/* FC: Tools not ready
#ifdef FEATURE_IS2000_REL_C_DV
  provisional_size = sizeof(LOG_SRV_CONFIG_VER_4_C_type) - 1; 
#elsif FEATURE_IS2000_REL_B
*/
#ifdef FEATURE_IS2000_REL_B
...
//  }
  ...
  scr_ptr->reference_buf[offset] = 0; 
  offset += 1;
#endif /* FEATURE_IS2000_REL_C_FCS || FEATURE_IS2000_REL_D */
...
)
{
...
#ifndef FEATURE_IS2000_RELEASE_A_LOGGING
  log_svc_config_type *log_ptr = NULL;
  uint16 log_len;
#endif /* !FEATURE_IS2000_RELEASE_A_LOGGING */
  ...
  log_len = FPOS(log_svc_config_type, sch_info) +
              FSIZ(log_svc_config_type, sch_info[0]) *
              (negotiation_info.current_config->neg_scr.num_for_sch +
               negotiation_info.current_config->neg_scr.num_rev_sch); //lint !e734

  log_ptr = (log_svc_config_type *) log_alloc(LOG_SVC_CONFIG_C, log_len); //lint !e713
...
#else /* !FEATURE_IS2000_RELEASE_A_LOGGING */
  /*
...
)
{
  ...
  if (!snm_get_free_neg_index(&con_index))
  {
    MSG_MED("Failed to get free entry in SNM negotiation_info database",0,0,0);
    return FALSE;
  }
...
)
{
  ...
  {
    MSG_MED("SO %d not enabled", so_to_compare, 0, 0);
    return FALSE;
  }
...
#endif /* !FEATURE_IS2000_REL_B */
  {
    (void) snm_find_so(so_to_compare, &so_index_to_compare); /* can't fail as we've verified it above */
    ...
    if (!found_fwd_rc_match || !found_rev_rc_match)
    {
      ...
      MSG_MED("SO %d not rate set-compatible with cur cfg", so_to_compare, 0, 0);
      return FALSE;
    }
  }
...
)
{
  ...
  if (!snm_so_is_enabled(so_to_compare))
  {
    MSG_MED("SO not enabled %d", so_to_compare, 0, 0);
    return FALSE;
  }
  ...
  for (i = 0; i < cfg_ptr->num_con; i++)
  {
    if (snm_find_so(cfg_ptr->con[i].so, &cfg_so_index) != SNM_SUCCESS)
    {
      ...
      MSG_MED("SO of cur cfg not found! %d", cfg_ptr->con[i].so, 0,0);
      return FALSE;
    }
    ...
    {                                                                        // support both types
      ...
      MSG_MED("SO %d traffic type not compatible with cur cfg SO %d",
        snp.so[so_index_to_compare].service_option_num, cfg_ptr->con[i].so, 0);
      return FALSE;
    }
  ...
  if (cfg_ptr->num_con == 0)
  {
    ...
    {
      MSG_MED("SO of cdma.so_req not found! %d", cdma.so_req, 0,0);
      return TRUE;
    }
    ...
    {   
      /* This means we can't propose the SO. */
      MSG_MED("SO %d traffic type not compatible with cur cfg SO %d",
        snp.so[so_index_to_compare].service_option_num, cdma.so_req, 0);
      return FALSE;
    }
  ...
  if (traffic_type_present == CAI_PRI_OR_SEC_TRAF)
  {
    /* Okay Both pri and sec traffic types have been taken by
       ...
       so return FALSE */
    MSG_MED("Have Pri & Sec, can't originate SO %d", so_to_compare, 0, 0);
    return FALSE;
  }
...
)
{
  ...
  if (!snm_so_is_enabled(*so))
  {
    MSG_MED("EOM SO not enabled %d", *so, 0, 0);
    return FALSE;
  }
  ...
  (void) snm_find_so(*so, &so_index); /* can't fail as we've verified it above */
  MSG_MED("Trying alternates for SO %d", *so, 0, 0);

  for (i = 0;
       (i < MAX_ALT_SO) && (snp.so[so_index].negotiation_pars.alternate_so[i] != CAI_SO_NULL);
       i++)
  {
            ...
            snm_is_so_traffic_type_compatible_with_cfg(alt_so, cur_cfg)))
    {
      ...
      *so = alt_so;
      MSG_MED("Found alternate SO %d for Origination", *so, 0, 0);
      return TRUE;
    }
  } /* end for */

  MSG_MED("Failed to get alternate for SO %d", *so, 0, 0);
...
)
{
  ...
  if (!snm_get_neg_index_for_sr_id(use_sr_id, deleted_sr_id, &con_index))
  {
    ...
    MSG_MED("Entry for deleted connection not found in snm", 0, 0, 0);
    if (use_sr_id)
    {
      MSG_MED("sr_id for missing entry is %d", deleted_sr_id, 0, 0);
    }
...
)
{
  ...
  {
    MSG_ERROR("Invalid gating rate %d", gating_rate,0 ,0);
  }
...
)
{
  ...
  {
    MSG_ERROR("num_for_sch %d is incorrect ", num_for_sch, 0, 0);
    return SNM_FAILURE;
  }
  ...
  for (i=0; i < num_for_sch ; i++)
  {
    ...
    {
      MSG_ERROR("%d for SCH Id out of range", for_sch_cfg[i].for_sch_id, 0, 0);
    }
    ...
    if (bad_cfg)
    {
      MSG_ERROR("Bad RC %d",for_sch_cfg[i].sch_chn_cfg.sch_rc, 0, 0); 
      return SNM_FAILURE;
    }
    ...
    if (bad_cfg)
    {
      MSG_ERROR("Bad mux 0x%x",for_sch_cfg[i].for_sch_mux, 0, 0); 
      return SNM_FAILURE;
    }
    ...
    if (for_sch_cfg[i].for_sch_mux == 0xf20)
    {
      ...
      {
        MSG_ERROR("Bad max rate %d",for_sch_cfg[i].sch_chn_cfg.max_rate, 0, 0); 
        return SNM_FAILURE;
      }
    }
    else
    ...
    {
      MSG_ERROR("Bad max rate RC combo",0,0,0); 
      return SNM_FAILURE;
    }
    ...
    {
      MSG_ERROR("For SCH frame 40 mismatch",0,0,0); 
      return SNM_FAILURE;
    }
    ...
    {
      MSG_ERROR("For SCH frame 80 mismatch",0,0,0); 
      return SNM_FAILURE;
    }
      ...
      case CAI_CONVOLUTIONAL_CODING:
            ...
            == 0)
        {
              ...
              snm_conv_turbo_rate_table[for_sch_fields.for_sch_recs[0].for_max_conv_block_size])
          {
            ...
            MSG_ERROR("Conv is too small",0,0,0);
            bad_cfg = TRUE;
          }
        }
        else
      ...
      case CAI_TURBO_CODING:
        if (RATE_SET_OF_FWD_RC(for_sch_cfg[i].sch_chn_cfg.sch_rc) == 0)
        {
              ...
              snm_conv_turbo_rate_table[for_sch_fields.for_sch_recs[0].for_max_turbo_block_size])
          {
            /* Cannot do this rate with conv then it must be possible with turbo
               since we already verified max rate */
            MSG_ERROR("Turbo is too small",0,0,0);
            bad_cfg = TRUE;
          }
        }
        else
    ...
    if (bad_cfg)
    {
      MSG_ERROR("Bad Coding type %d",for_sch_cfg[i].sch_chn_cfg.coding,0,0); 
      return SNM_FAILURE;
    }
...
)
{
  ...
  {
    MSG_ERROR("num_rev_sch %d is incorrect ", num_rev_sch, 0, 0);
    return SNM_FAILURE;
  }
  ...
  for (i=0; i < num_rev_sch ; i++)
  {
    ...
    {
      MSG_ERROR("%d rev SCH Id out of range", rev_sch_cfg[i].rev_sch_id,0,0);
    }
    ...
    if (bad_cfg)
    {
      MSG_ERROR("Bad RC %d",rev_sch_cfg[i].sch_chn_cfg.sch_rc, 0, 0); 
      return SNM_FAILURE;
    }
    ...
    if (bad_cfg)
    {
      MSG_ERROR("Bad rev SCH mux 0x%x",rev_sch_cfg[i].rev_sch_mux, 0, 0); 
      return SNM_FAILURE;
    }
    ...
    if (rev_sch_cfg[i].rev_sch_mux == 0xf20)
    {
      ...
      {
        MSG_ERROR("Bad max rate %d",rev_sch_cfg[i].sch_chn_cfg.max_rate, 0, 0); 
        return SNM_FAILURE;
      }
    }
    else
    ...
    if (bad_cfg)
    {
      MSG_ERROR("Bad Coding type %d",rev_sch_cfg[i].sch_chn_cfg.coding, 0, 0); 
      return SNM_FAILURE;
    }
  }
