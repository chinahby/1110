...
// Format variables, strings, characters and patterns for Chinese (simplified)
...
/* -----------------------------------------------------------------------
    LocaleZHCN_Ctor 
   --- */
static void LocaleZHCN_Ctor(LocaleZHCN *pMe, AEEVTBL(ILocale) *pvt, IShell *piShell, IModule *piModule)
{
...
/* -----------------------------------------------------------------------
    LocaleZHCN_New  
   --- */
int LocaleZHCN_New(ILocale **ppo, IShell *piShell, IModule *piModule)
{
   LocaleZHCN *pMe;
   ...
   pMe = MALLOCREC_VTBL(LocaleZHCN, ILocale);
   if (!pMe) {
      return ENOMEMORY;
   }
   LocaleZHCN_Ctor(pMe,GETVTBL(pMe,ILocale),piShell,piModule); 
...
int LocaleZHCN_CreateInstance(IModule *module, IShell *shell, AEECLSID idClass, void **pp)
{
   if (idClass == AEECLSID_LOCALEZHCN) {
      return LocaleZHCN_New((ILocale **)pp, shell, module);
   }
   *pp = 0;
