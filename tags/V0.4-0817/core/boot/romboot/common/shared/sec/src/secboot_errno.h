...
$Header: //depot/asic/msmshared/secboot2/shared/1_0/sec/secboot_errno.h#2 $
$DateTime: 2008/02/20 11:56:15 $
$Author: wctan $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/10/08   MJS     Updated comment block.
...
typedef enum
{
  SBL_SEC_NO_ERR= 0,                    /* 0x000                       */
  SBL_SEC_CERT_CHAIN_VERIFY_ERR,        /* 0x1                         */
  SBL_SEC_SIGNATURE_VERIFY_ERR,         /* 0x2                         */
  SBL_SEC_MISC_OR_EXCEPTION_ABORT_ERR,  /* 0x3                         */
  SBL_SEC_MAX_ERR_CODE = 0x7FFFFFFF     /* To ensure it's 32 bits wide */
}sbl_sec_err_code_enum_type;

extern uint32 secboot_err_code;

/* Defintion of logging error code */
#ifdef USE_ARM
  #define  SECBOOT_RET   SBL_RET
  #define  SBL_RET( x )  {secboot_err_code = (uint32)x;  return; }
  #define  SECBOOT_CHECK SBL_CHECK
  #define  SBL_CHECK( y, z )   {secboot_err_code = (uint32)z;  return y; }
  //#define  SBL_CHECK( x , y )  { if(y) QCSBL_CHECK(x); }
#endif
