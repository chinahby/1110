...
/*=============================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_ftl_cache.c#5 $
...
ftl_cache_open (int16 driveno)
{
  ...
  {
    ERR_FATAL("Misconfigured constant for this flash device", 0, 0, 0);
  }
                ...
                uint16 count)
{
  ...
  while (count > 0)
  {
    ...
    {
      memcpy (buffer, temp_cached_page->buffer, DEFAULT_MS_SECTOR_SIZE);
    }
                 ...
                 uint16 count)
{
  ...
  while (count > 0)
  {
    ...
    memcpy (temp_cached_page->buffer, buffer, DEFAULT_MS_SECTOR_SIZE);
...
int ftl_cache_force_sync_all (void)
{
  ...
  for (i = 0; i < MAX_FTL_PARTITIONS; i++) {
    if (cpartitions[i].ftl_raw_device != FTL_NO_DEVICE) {
      ...
      fs_os_enter_crit_sect (&cpartitions[i].fs_ftl_cache_crit_sec);
      while (cache_sync_oldest_block (pt))
        ;
      fs_os_leave_crit_sect (&cpartitions[i].fs_ftl_cache_crit_sec);
      pt->sync_timer_active = 0;
