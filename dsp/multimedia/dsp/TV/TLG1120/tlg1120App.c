...
const tlg_reg_pair tlg_init_reg_1_1 []=
{
...
const tlg_reg_pair tlg_init_reg_1_2_common []=
{
...
const tlg_reg_pair tlg_init_reg_2_1_common []=
{
...
const tlg_reg_pair *getTlgInitReg()
{
    uint32 version;
...
int TLG_Ini_WritePair(uint32 base_addr, int start, const tlg_reg_pair *p_tbl)
{
...
TLGDLL_API void TLGAPP_Ini_Reg(int start)
{
    ...
    const tlg_reg_pair *tlg_init_reg ;
	...
	{
	//Vincent modify for search channel
       // TLG_WriteReg(0x2c, 0x52,0);
	}
...
TLGDLL_API int TLGAPP_SetFreq(int freq)
{
    if (!tlg_init_done) 
    {
        TLGAPP_Init(tlg_i2c_addr);
        //TLGAPP_ChannelSetup(TLG_MODE_FM_RADIO, TLG_VSTD_NTSC_M, TLG_BW_6);
        //TLGAPP_ChannelSetup(tlg_cur_mode, p_tlg_cur_map[i] .chstd, p_tlg_cur_map[i] .bw);
    }    

    /* change LNA if required */
    TLGAPP_SetLnaGpio(freq); 
    TLGAPP_ChannelSetup(TLG_MODE_FM_RADIO, TLG_VSTD_NTSC_M, TLG_BW_6);
    TLG_Set_FM_StepSize(tlg_i2c_addr, 100000);


	
    if (tlg_cur_mode == TLG_MODE_FM_RADIO)
        return TLG_ScanFMChn(tlg_i2c_addr, freq, tlg_slope, tlg_offset);
    else
        return TLG_ScanTVChn(tlg_i2c_addr, freq);
}
...
extern void dbg_printf(int num, char * format, ...);
...
static int TLGAPP_FMScanFrequency(int32 center_freq_hz)
{
    int32   freqOffset;
    int     result = 0;
    int32   searchOffset =  50000; 
    int32   halfMargin   = 250000; 
    ...
    result = TLG_FMScanCheckNotchPower(tlg_i2c_addr, freqOffset,  searchOffset);
