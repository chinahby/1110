...
#else	//D-TV
//RDA5888
#define TLG_PREVIEW_VH_POLARITY  (A8_SENSOR_HOR_POS|A8_SENSOR_VER_POS) //(A8_SENSOR_HOR_POS|A8_SENSOR_VER_POS)
#endif

#define TLG_FULL_VH_POLARITY     (A8_SENSOR_HOR_POS|A8_SENSOR_VER_POS)
...
#define TLG_PREVIEW_YUV_FORMAT   (A8_SENSOR_CRYCBY) //(A8_SENSOR_YCRYCB)//A8_SENSOR_YCBYCR	//A8_SENSOR_CRYCBY
#define TLG_FULL_YUV_FORMAT      (A8_SENSOR_CBYCRY)
#define TLG_LATCH_EDGE        	 (A8_SENSOR_NEG_EDGE)//(A8_SENSOR_NEG_EDGE)	//Wind@20091224 (A8_SENSOR_POS_EDGE)

#define	TLG_I2C_ID				(0x2c<<1)

#define	TLG_PREVIEW_WIDTH  	   320	//(320)//360)
#define	TLG_PREVIEW_HEIGHT	   240	//(240)//(288)

#define	TLG_FULL_WIDTH			(1280)
#define	TLG_FULL_HEIGHT  	    		(960)
...
extern	u_char	g_ATV_Flag;
	...
	{	
		...
		if (g_ATV_Flag == 0)
		{
			...
			//ATV off
			analogtv_module_power_on(1);//PowerOffFM();
...
//	extern char TLG1120_TestPatternOn;
	...
	if(vhpolarity==4)	
		...
		preview_mode.latch_edge = !preview_mode.latch_edge ;
...
//#define CHECK_YUV
		...
		preview_mode.yuv_format  = yuvformat;	
		yuvformat++;
		if(yuvformat==4)	
			yuvformat = 0;
#endif

	AIT_Message_P1("vhpolarity:%d",preview_mode.hv_polarity);
	AIT_Message_P1("yuv_format:%d",preview_mode.yuv_format);
	AIT_Message_P1("latch edge:%d",preview_mode.latch_edge );// 1
		...
		TLGAPP_TurnOnTestChannel(); // Must open! 2010/02/02
		//TLG1120_TestPatternOn = 1;

		sys_IF_ait_delay1ms(1000); 
	...
	AIT_Message_P2("DSC TEST %s, %d\r\n",__FILE__,__LINE__);

	AIT_Message_P1("[!!]Pixel delay=%x\n\r",GetA8RegB(VIF_PXL_CLK_DELAY));

        return  fRet;
}

#if 0
uint8 nAITChangeColorCount = 0;
void AITChangeColorOrder(void)
{
	uint8 nWriteValue = 0x00;

    AIT_Message_P0("[!!]AITChangeColorOrder\n");
    AIT_Message_P1("[!!]nAITChangeColorCount=(%d)\n", nAITChangeColorCount);

	switch(nAITChangeColorCount)
	{
		default:
		case 0:
			nWriteValue = 0x03; //0x00;
			break;
		case 1:
			nWriteValue = 0x02; //0x01;
			break;
		case 2:
			nWriteValue = 0x02;
			break;
	...
	SetA8RegB(VIF_SEN_YUV_CTL, SEN_YUV_EN|(nWriteValue <<4));

    AIT_Message_P1("[!!]Write(0x%x)\n", GetA8RegB(VIF_SEN_YUV_CTL) );

	nAITChangeColorCount = (nAITChangeColorCount+1)%2;
}

				
uint8 nAITChangePixelDelay = 0;
void AITChangePixelDelay(void)
{
	uint8 nWriteValue = 0x00;

    AIT_Message_P0("[!!]AITChangePixelDelay\n");
    AIT_Message_P1("[!!]nAITChangePixelDelay=(%d)\n", nAITChangePixelDelay);

	switch(nAITChangePixelDelay)
	{
		default:
		case 0:
			nWriteValue = 0x00;
	...
	SetA8RegB(VIF_PXL_CLK_DELAY, nWriteValue);

    AIT_Message_P1("[!!]Write(0x%x)\n", GetA8RegB(VIF_PXL_CLK_DELAY) );

	nAITChangePixelDelay = (nAITChangePixelDelay+1)%4;
}
#endif

t_sensor_manager tv_rda5888 = 
{
	SENSOR_ID,
	TLG_I2C_ID,
	TLG_I2C_TYPE,
