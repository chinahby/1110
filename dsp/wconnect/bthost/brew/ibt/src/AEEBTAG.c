...
$Header: //source/qcom/qct/wconnect/bthost/brew/ibt/rel/00.00.26/src/AEEBTAG.c#10 $ 
$DateTime: 2009/09/03 16:49:16 $
$Author: phuongn $

when        who  what, where, why
----------  ---  ------------------------------------------------------------
2009-09-03  pn   Remote speaker gain is applied when SCO is up.
2009-09-02  pn   Added support for private interface IBTAGEx.
2009-08-27  co   Fixed bugs in Private Mode with inband ringer in use.
...
struct IBTAGEx
{
   const IBTAGExVtbl* pvt;
   uint32               uRefs;
   IBTAudioGateway*     pParent;
   ISignal*             piATCmdSig;
   char*                pszATCmds;
};
...
struct IBTAudioGateway
{
   ...
   int                  nConnAttempts;
   int                  nConnIdx;
   BTAGConnInfo         sConnections[OEMBTAG_MAX_CONNECTIONS];
   BTAGConnInfo*        pOpConn; // connection associated with current op
   int                  nNumConnections;
...
// supplemental config bits
#define AEEBTAG_CFG_MOS_DISABLED_B  0x0008
...
#define CONN_STATE(pc) (pc)->publicInfo.dConnState
#define AUDIO_STATE(pc) (pc)->publicInfo.dAudioConnState
#ifndef RELEASEIF
...
static BTAGConnInfo* AEEBTAG_FindActiveConn (IBTAudioGateway* pMe)
{
   BTAGConnInfo* pConn = NULL;
   // the first connection is always the active one
   if (AEEBT_CONN_STATE_CONNECTED == pMe->sConnections[0].publicInfo.dConnState)
   {
...
static int AEEBTAG_StopSCO (IBTAudioGateway* pMe)
{
   ...
   else if (AEEBT_CONN_STATE_CONNECTED == AUDIO_STATE(pConn))
   {
      pMe->uFlags |= AEEBTAG_FLAG_OP_BY_ME;
      result = AEEBTAG_DisconnectAudio (pMe, &pConn->publicInfo.remoteAddr);
      if (AEE_SUCCESS != result)
      {
         pMe->uFlags &= ~AEEBTAG_FLAG_OP_BY_ME;
      }
...
static int AEEBTAG_StartSCO (IBTAudioGateway* pMe)
{
   ...
   DBGPRINTF ("Starting SCO - state=%d", pMe->dState);
   ...
   else
   {
      pMe->uFlags |= AEEBTAG_FLAG_OP_BY_ME;
      pMe->uFlags |= AEEBTAG_FLAG_BRINGING_UP_AUDIO;
      result = AEEBTAG_ConnectAudio (pMe, &pConn->publicInfo.remoteAddr);
      if (AEE_SUCCESS != result)
      {
         pMe->uFlags &= ~AEEBTAG_FLAG_OP_BY_ME;
      }
...
static void AEEBTAG_SendErrResponse (IBTAudioGateway* pMe, uint32 uErrCode)
{
   ...
   (void) OEMBTAG_SendResponse (pMe->pOEMAG, szErrMsg);
}
...
static boolean AEEBTAG_ATCmdProcSetCOPS (IBTAudioGateway* pMe, char* args[])
{
   ...
   if ((*args[0] == '3') && (*args[1] == '0'))
   {
      ...
      (void) OEMBTAG_SendResponse (pMe->pOEMAG, "OK");
   }
   else
   {
   ...
   if (pMe->ATCmdHndlr.uFlags & AEEBTAG_FLAG_ATCMD_COPS_SET_RCVD)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            (void) OEMBTAG_SendResponse (pMe->pOEMAG, pszMsg);
            (void) OEMBTAG_SendResponse (pMe->pOEMAG, "OK");
            FREE (pszMsg);
         }
      }
...
static boolean AEEBTAG_ATCmdProcCMEE (IBTAudioGateway* pMe, char* args[])
{
   ...
   if ((NULL == pMe->piBTAGEx) || (NULL == pMe->piBTAGEx->piATCmdSig))
   {
      (void) OEMBTAG_SendResponse (pMe->pOEMAG, "OK");
      bProcessed = TRUE;
   }
   /* else, let app response */
   return bProcessed;
}
      ...
      switch (snd)
      {
         case OEMBT_SND_TYPE_NONE:
            if ((AEET_CALL_STATE_NONE == callState) &&
                ((pMe->CallMgr.uFlags & AEEBTAG_FLAG_CALLMGR_DEV_DIALED) == 0))
            {
            ...
            /* else, SCO is manipulated upon call events */
            break;
         case OEMBT_SND_TYPE_BT_OTHER:
            (void) AEEBTAG_StopSCO (pMe);
            break;
         ...
         case OEMBT_SND_TYPE_BT_ALL:
            if (((pMe->uFlags & AEEBTAG_FLAG_A2DP_CONNECTED) == 0) &&
                ((pMe->dCfgFlags & AEEBTAG_CFG_MOS_DISABLED_B) == 0))
            {
               (void) AEEBTAG_StartSCO (pMe);
            }
            break;
...
static BTAGConnInfo* AEEBTAG_FindConnectionEntry (
   IBTAudioGateway* pMe, 
...
)
{
   ...
   for (i = 0; i < ARR_SIZE(pMe->sConnections); i++)
   {
      ...
      if ((AEEBT_BD_ADDRS_EQUAL (&pMe->sConnections[i].publicInfo.remoteAddr, 
                                 pAddr) != FALSE) &&
          (AEEBT_CONN_STATE_DISCONNECTED != 
           pMe->sConnections[i].publicInfo.dConnState))
      {
   ...
   if ((NULL == pConn) && (FALSE == bMustExist))
   {
      ...
      for (i = 0; i < ARR_SIZE(pMe->sConnections); i++)
      {
         if (AEEBT_CONN_STATE_DISCONNECTED == 
             pMe->sConnections[i].publicInfo.dConnState)
         {
            pConn = &pMe->sConnections[i];
            pConn->publicInfo.remoteAddr = *pAddr;
            break;
         }
      }
   }
...
{
   if (AEEBT_CONN_STATE_DISCONNECTED == CONN_STATE(pConn))
   {
      int i;
...
{
   BTAGConnInfo* pConn = AEEBTAG_FindConnectionEntry (pMe, &pEv->bdAddr, TRUE);
   if (pConn == NULL)
   {
      DBGPRINTF_ERROR ("HndlSpkrGain - conn entry not found");
   }
   else if (pConn->uRemoteSpkrGain != pEv->u8Buf[0])
   {
      DBGPRINTF ("HndlSpkrGain - got %d", pEv->u8Buf[0]);
      pConn->uRemoteSpkrGain = pEv->u8Buf[0];
      if (AEEBT_CONN_STATE_CONNECTED == AUDIO_STATE(pConn))
      {
         OEMBTAG_ApplySpkrGain (pMe->pOEMAG, pConn->uRemoteSpkrGain);
      }
      /* else, will save speaker gain in AVS table when SCO comes up */
   }
}
...
static void AEEBTAG_HandleMicGain (IBTAudioGateway* pMe, OEMAGEvent* pEv)
{
   ...
   if (pConn == NULL)
   {
      DBGPRINTF_ERROR ("HndlMicGainEv - conn entry not found");
   }
   else if (pConn->uRemoteMicGain != pEv->u8Buf[0])
   {
      DBGPRINTF ("HndlMicGain - got %d", pEv->u8Buf[0]);
      pConn->uRemoteMicGain = pEv->u8Buf[0];
   }
}
...
static void AEEBTAG_HandleConnFailedEv (IBTAudioGateway* pMe, OEMAGEvent* pEv)
{
   ...
   if ((pConn == NULL) || (AEEBT_CONN_STATE_CONNECTING != CONN_STATE(pConn)))
   {
   ...
   else
   {
      ...
      if ((AEEBTAG_OP_CONNECT == pMe->dPendingOperation) &&
          (AEEBT_CONN_STATE_CONNECTING == CONN_STATE(pConn)))
      {
      ...
      {
        CONN_STATE(pConn) = AEEBT_CONN_STATE_DISCONNECTED;
      }
   }
}
...
{
   BTAGConnInfo* pConn = AEEBTAG_FindConnectionEntry (pMe, &pEv->bdAddr, FALSE);
   ...
   else
   {
      ...
      CONN_STATE(pConn) = AEEBT_CONN_STATE_CONNECTED;
      pMe->nNumConnections++;
      pMe->nConnIdx = 0;
      pMe->nConnAttempts = 0;
...
static void AEEBTAG_HandleDisconnEv (IBTAudioGateway* pMe, OEMAGEvent* pEv)
{
   ...
   if ((pConn == NULL) || (AEEBT_CONN_STATE_DISCONNECTED == CONN_STATE(pConn)))
   {
   ...
   else
   {
      if ((AEEBTAG_OP_DISCONNECT == pMe->dPendingOperation) &&
          (AEEBT_CONN_STATE_DISCONNECTING == CONN_STATE(pConn)))
      {
         AEEBTAG_OpCmpltCleanup (pMe);
      }
      ...
      AUDIO_STATE(pConn) = AEEBT_CONN_STATE_DISCONNECTED;
      CONN_STATE(pConn)  = AEEBT_CONN_STATE_DISCONNECTED;
      pMe->nNumConnections--;
      pMe->nConnIdx = 0;
      pMe->nConnAttempts = 0;
...
)
{
   ...
   if ((AEEBTAG_OP_CONNECT_AUDIO == pMe->dPendingOperation) &&
       (AEEBT_CONN_STATE_CONNECTING == AUDIO_STATE(pConn)))
   {
      AEEBTAG_OpCmpltCleanup (pMe);
      AUDIO_STATE(pConn) = AEEBT_CONN_STATE_DISCONNECTED;
   }
...
static void AEEBTAG_HandleAudioDisconnEv (IBTAudioGateway* pMe, OEMAGEvent* pEv)
{
   ...
   if ((pConn == NULL) || (AEEBT_CONN_STATE_DISCONNECTED == AUDIO_STATE(pConn)))
   {
   ...
   else
   {
      if ((AEEBTAG_OP_DISCONNECT_AUDIO == pMe->dPendingOperation) &&
          (AEEBT_CONN_STATE_DISCONNECTING == AUDIO_STATE(pConn)))
      {
         AEEBTAG_OpCmpltCleanup (pMe);
      }
      else
...
static void AEEBTAG_HandleAudioConnEv (IBTAudioGateway* pMe, OEMAGEvent* pEv)
{
   ...
   if ((pConn == NULL) || (AEEBT_CONN_STATE_CONNECTED == AUDIO_STATE(pConn)))
   {
   ...
   else
   {
      ...
      AUDIO_STATE(pConn) = AEEBT_CONN_STATE_CONNECTED;
      AEEBTAG_ResolveActiveConn (pMe, pConn);
...
static void AEEBTAG_ParseATCmd (IBTAudioGateway* pMe)
{
   ...
   while (pszATCmd < (pMe->ATCmdHndlr.pszATCmd + pMe->ATCmdHndlr.uNumChars))
   {
      ...
      if (FALSE == bCmdValid)
      {
         if ((NULL != pMe->piBTAGEx) && (NULL != pMe->piBTAGEx->piATCmdSig))
         {
            std_strlcat(pMe->piBTAGEx->pszATCmds, pszATCmd, OEMBTAG_AT_CMD_MAX_LEN);
            if (NULL != pszEnd)
            {
               std_strlcat(pMe->piBTAGEx->pszATCmds, "\r", 
                           OEMBTAG_AT_CMD_MAX_LEN);
            }
            ISignal_Set(pMe->piBTAGEx->piATCmdSig);
         }
         else
         {
...
/************************************************************************ 
                             IBTAGEx FUNCTIONS
************************************************************************/
static void AEEBTAGEx_Cleanup(IBTAGEx* pMe)
{
   DBGPRINTF_LOW ("AGEx_Cleanup");
   RELEASEIF(pMe->piATCmdSig);
   FREEIF(pMe->pszATCmds);
   FREEIF(pMe);
}

static uint32 AEEBTAGEx_AddRef(IBTAGEx* pMe)
{
   pMe->uRefs++;
   return pMe->uRefs;
}

static uint32 AEEBTAGEx_Release(IBTAGEx* pMe)
{
   uint32 uRefs = pMe->uRefs;
   if (0 < uRefs)
   {
      uRefs = --pMe->uRefs;
      ...
      if (0 == uRefs)
      {
         AEEBTAGEx_Cleanup(pMe);
      }
   }
   return uRefs;
}

static int AEEBTAGEx_QueryInterface(IBTAGEx* pMe, AEEIID iid, void** ppo)
{
   int result = AEE_ECLASSNOTSUPPORT;
  
   if ((AEEIID_IQI == iid) || (AEEIID_IBTAGEx == iid))
   {
      if (NULL != ppo)
      {
         *ppo = pMe;
         AEEBTAGEx_AddRef (pMe);
         result = AEE_SUCCESS;
      }
      else
      {
...
static AEEResult AEEBTAGEx_OnATCommand(IBTAGEx* pMe, /*in*/ ISignal* piSig)
{
   AEEResult result = AEE_ENOMEMORY;
   RELEASEIF(pMe->piATCmdSig);
   if (NULL != piSig)
   {
      result = ERR_MALLOC(OEMBTAG_AT_CMD_MAX_LEN + 1, &pMe->pszATCmds);
      if (AEE_SUCCESS == result)
      {
         pMe->piATCmdSig = piSig;
         ISignal_AddRef(pMe->piATCmdSig);
      }
   }
   else
   {
...
static AEEResult AEEBTAGEx_GetATCommand(
  IBTAGEx* pMe, 
  /*rout*/ char* pszATCmd, 
  /*in*/ int atCmdLen, 
  /*rout*/ int* atCmdLenReq
)
{
   AEEResult result = AEE_EBADPARM;
   if (NULL != atCmdLenReq)
   {
      *atCmdLenReq = std_strlen(pMe->pszATCmds);
      if ((*atCmdLenReq <= atCmdLen) && (NULL != pszATCmd))
      {
         std_strlcpy(pszATCmd, pMe->pszATCmds, atCmdLen);
         *pMe->pszATCmds = '\0';
         result = AEE_SUCCESS;
      }
      else if (*atCmdLenReq > atCmdLen)
      {
...
static AEEResult AEEBTAGEx_SendResponse(
  IBTAGEx* pMe, 
  /*in*/ const char* pszResponse
)
{
   AEEResult result = AEE_EBADSTATE;
   if (AEEBTAG_STATE_CONNECTED == pMe->pParent->dState)
   {
      result = OEMBTAG_SendResponse(pMe->pParent->pOEMAG, pszResponse);
   }
   return result;
}

static AEEResult AEEBTAGEx_GetMicGain(
  IBTAGEx* pMe, 
  /*in*/ const BDAddress* pBDAddr, 
  /*rout*/ uint8* pLevel
)
{
   AEEResult result = AEE_EBADPARM;
   if ((NULL != pBDAddr) && (NULL != pLevel))
   {
      BTAGConnInfo* pConn = 
         AEEBTAG_FindConnectionEntry (pMe->pParent, pBDAddr, TRUE);
      if (pConn == NULL)
      {
         DBGPRINTF_ERROR ("AGEx_GetMicLevel - conn entry not found");
         result = AEE_EBADSTATE;
      }
      else
      {
         DBGPRINTF_LOW ("AGEx_GetMicLevel - %d", pConn->uRemoteMicGain, 0, 0);
         *pLevel = pConn->uRemoteMicGain;
         result = AEE_SUCCESS;
      }
   }
   return result;
...
static AEEResult AEEBTAGEx_SetMicGain(
  IBTAGEx* pMe, 
  /*in*/ const BDAddress* pBDAddr, 
  /*in*/ uint8 level
)
{
   ...
   if (NULL != pBDAddr)
   {
      ...
      if (pConn == NULL)
      {
         DBGPRINTF_ERROR ("AGEx_SetMicLevel - conn entry not found");
         result = AEE_EBADSTATE;
      }
      else
      {
         DBGPRINTF_LOW ("AGEx_SetMicLevel - %d", level, 0, 0);
         pConn->uRemoteMicGain = level;
         result = OEMBTAG_SetMicGain(pMe->pParent->pOEMAG, 
                                     pConn->uRemoteMicGain);
      }
   }
   return result;
}

static AEEResult AEEBTAGEx_SetMOSUsage(IBTAGEx* pMe, boolean bUse)
{
   DBGPRINTF_LOW ("AGEx_SetMOSUsage - %d", bUse, 0, 0);
   if (FALSE == bUse)
   {
      pMe->pParent->dCfgFlags |= AEEBTAG_CFG_MOS_DISABLED_B;
   }
   else
   {
      pMe->pParent->dCfgFlags &= ~AEEBTAG_CFG_MOS_DISABLED_B;
...
int AEEBTAGEx_New (IShell* pIShell, AEECLSID cls, void** ppObj)
{
   int result = ENOTALLOWED;
   ...
   {
      IBTAudioGateway* pMe = pgTheOnlyAG;
      uint8* pMem = NULL;
      IBTAGExVtbl* pvt;

      if (NULL == pMe->piBTAGEx)
      {
         if (SUCCESS == 
             ERR_MALLOC (sizeof(IBTAGEx) + sizeof(AEEVTBL(IBTAGEx)), &pMem))
         {
            pMe->piBTAGEx = (IBTAGEx*)pMem;
            pvt = (IBTAGExVtbl*)(pMem + sizeof(IBTAGEx));
            pvt->AddRef                   = AEEBTAGEx_AddRef;
            pvt->Release                  = AEEBTAGEx_Release;
            pvt->QueryInterface           = AEEBTAGEx_QueryInterface;
            pvt->OnATCommand              = AEEBTAGEx_OnATCommand;
            pvt->GetATCommand             = AEEBTAGEx_GetATCommand;
            pvt->SendResponse             = AEEBTAGEx_SendResponse;
            pvt->GetRemoteMicrophoneLevel = AEEBTAGEx_GetMicGain;
            pvt->SetRemoteMicrophoneLevel = AEEBTAGEx_SetMicGain;
            pvt->SetMOSUsage              = AEEBTAGEx_SetMOSUsage;
            AEEINITVTBL(pMe->piBTAGEx, IBTAGEx, *pvt);

            pMe->piBTAGEx->pParent = pMe;
         }
      }
      if (NULL != pMe->piBTAGEx)
      {
      ...
      *pdOp = pMe->dLastCmpltOperation;
      ...
      switch (*pdOp)
      {
         ...
         case AEEBTAG_OP_CONNECT:
            *pdResult = 
               (AEEBT_CONN_STATE_CONNECTED == CONN_STATE(pMe->pOpConn)) ?
               AEE_SUCCESS : AEE_EFAILED;
            break;
         case AEEBTAG_OP_DISCONNECT:
            *pdResult = 
               (AEEBT_CONN_STATE_DISCONNECTED == CONN_STATE(pMe->pOpConn)) ?
               AEE_SUCCESS : AEE_EFAILED;
            break;
         case AEEBTAG_OP_CONNECT_AUDIO:
            *pdResult = 
               (AEEBT_CONN_STATE_CONNECTED == AUDIO_STATE(pMe->pOpConn)) ?
               AEE_SUCCESS : AEE_EFAILED;
            break;
         case AEEBTAG_OP_DISCONNECT_AUDIO:
            *pdResult = 
               (AEEBT_CONN_STATE_DISCONNECTED == AUDIO_STATE(pMe->pOpConn)) ?
               AEE_SUCCESS : AEE_EFAILED;
            break;
      }
      result = AEE_SUCCESS;
...
)
{
   ...
   else
   {
      int i;
      *pnNumConn = pMe->nNumConnections;
      for (i=0; i<pMe->nNumConnections; i++)
      {
         std_memmove(psConnList, &pMe->sConnections[i].publicInfo,
...
static int AEEBTAG_Connect (IBTAudioGateway* pMe, const BDAddress* pAddr)
{
   ...
   else
   {
      ...
      else if (AEEBT_CONN_STATE_CONNECTED == CONN_STATE(pMe->pOpConn))
      {
      ...
      else
      {
         ...
         if ((result = OEMBTAG_Connect (
                         pMe->pOEMAG, 
                         &pMe->pOpConn->publicInfo.remoteAddr)) != AEE_SUCCESS)
         {
            pMe->dPendingOperation = AEEBTAG_OP_NONE;
         }
         else
         {
            CONN_STATE(pMe->pOpConn) = AEEBT_CONN_STATE_CONNECTING;
         }
...
static int AEEBTAG_Disconnect (IBTAudioGateway* pMe, const BDAddress* pAddr)
{
   ...
   else
   {
      ...
      else
      {
         ...
         {
            CONN_STATE(pMe->pOpConn) = AEEBT_CONN_STATE_DISCONNECTING;
         }
      }
   }
   return result;
...
static int AEEBTAG_ConnectAudio (IBTAudioGateway* pMe, const BDAddress* pAddr)
{
   ...
   else
   {
      ...
      else
      {
         ...
         {
            AUDIO_STATE(pMe->pOpConn) = AEEBT_CONN_STATE_CONNECTING;
         }
      }
   }
   return result;
...
static int AEEBTAG_DisconnectAudio (IBTAudioGateway* pMe, const BDAddress* pAddr)
{
   ...
   else
   {
      ...
      else if (AEEBT_CONN_STATE_CONNECTED != AUDIO_STATE(pMe->pOpConn))
      {
      ...
      else
      {
         ...
         {
            AUDIO_STATE(pMe->pOpConn) = AEEBT_CONN_STATE_DISCONNECTING;
         }
      }
   }
   return result;
