...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/1x/cp/rel/1h08/src/mc.c#5 $ $DateTime: 2009/11/26 01:47:01 $ $Author: ssaranu $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/26/09   ss      update the last entry reason for OFFLINE_CDMA operating mode.
11/24/09   jj      Fixed the issue of phone becoming offline upon removing 
                   the card.
08/14/09   jj      Added support for CSIM feature.
07/08/09   jj      Featurized writing of NV_SLOT_CYCLE_INDEX_I only when the phone is 
                   operating in NV mode or RUIM mode with RTRE being NV or when 
                   FEATURE_UIM_RUIM_SUPPORT_SCI is inactive.
...
)
{
  ...
  ** type.
  */
  MSG_MED("Rxed MC_CDMA_PROT_PH_STAT_CHGD_CMD_F with chg_type =%d",
           ph_stat_ptr->chg_type,0,0);
  switch ( ph_stat_ptr->chg_type )
  {
    ...
    case PROT_PH_STAT_CHG_SUBSC:
      ...
      */
      MSG_MED("Rxed CHG_NAM/CHG_SUBSC subsc_avail_status =%d",
              ph_stat_ptr->prot_subsc_chg,0,0);
      if (CHECK_PROTO(ph_stat_ptr->prot_subsc_chg, PROT_SUBSC_CHG_CDMA))
      {
        cdma.is_cdma_subs_avail = ph_stat_ptr->is_cdma_subs_avail;
        MSG_MED(" CDMA subsc available = %d",
                 ph_stat_ptr->is_cdma_subs_avail,0,0);
        #if ( defined(FEATURE_MMGSDI) ) && ( defined(FEATURE_CSIM) )
#error code not present
        #endif /*( defined(FEATURE_MMGSDI) ) && ( defined(FEATURE_CSIM) )*/
...
#error code not present
    ...
    {
      MSG_MED("Rxed PROT_PH_STAT_CHG_OPRT_MODE with optr_mode =%d",
              ph_stat_ptr->oprt_mode,0,0);
      ...
      )
  {
    ...
    switch ( cmd_ptr->hdr.cmd )
    {
      case MC_CDMA_PROT_ACT_CMD_F:
      {
        ...
        MSG_MED("Rxed MC_CDMA_PROT_ACT_CMD_F ",0,0,0);
...
#error code not present
      ...
      case MC_AMPS_PROT_DEACT_CMD_F:
        ...
        */
        MSG_MED("Rxed MC_CDMA_PROT_DEACT_CMD_F ",0,0,0);
      ...
      case MC_AMPS_PROT_GEN_CMD_F:
        ...
        ** set to TRUE.
        */
        MSG_MED("Rxed MC_CDMA_PROT_GEN_CMD_F",0,0,0);
      ...
      case MC_CDMA_PROT_PH_STAT_CHGD_CMD_F:
        ...
        MSG_MED("Rxed MC_CDMA_PROT_PH_STAT_CHGD_CMD_F ",0,0,0);
