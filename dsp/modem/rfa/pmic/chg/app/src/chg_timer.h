...
/* =======================================================================
  ...
  $Header: //source/qcom/qct/modem/rfa/pmic/chg_task/main/latest/app/src/chg_timer.h#11 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/07/09   hs      Removed unnecessary header file inclusion.
08/05/09   hs      Removed supprot for the end-of-fast-charging detection.
                   Added support for maximum-charging-timeout. 
...
public:
    ...
    /*! \brief This member function provides an interface for adding the listener for
     *  maximum-charging-time-detection timer in the autonomous charging 
     *  (::AUTO_CHARGING_MAX_CHARGE_T)
     *  \param listener A listener
     *  \return void
     *  \note The maximum-charging-time-detection timer is a one-shot timer. This function should be 
     *  called only when a ChargingEventListener class wants to get informed when the 
     *  maximum-charging-time-detection timer expires.
     */ 
    virtual void AddListenerForMaxChargeTimeDetection(ChargingEventListener *listener);
    /*! \brief This member function provides an interface for removing the listener from
     *  listening for the maximum-charging-time-detection timer (::AUTO_CHARGING_MAX_CHARGE_T)
     *  \param listener A listener
     *  \return void
     *  \note This function should be called only when a ChargingEventListener class no
     *  longer wants to get informed when the maximum-charging-time-detection timer expires.
     */ 
    virtual void RemoveListenerForMaxChargeTimeDetection(ChargingEventListener *listener);
