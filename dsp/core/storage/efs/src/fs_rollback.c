 ...
 *
 * Copyright (C) 2006-2007, 2009 Qualcomm, Inc.
 *
 ***********************************************************************/
...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/core/storage/efs/main/latest/src/fs_rollback.c#4 $ $DateTime: 2009/05/20 16:03:23 $ $Author: richardp $
  ...
  const struct fs_stat *fstat_ptr)
{
  ...
  memcpy (g_fs_rbk_file.hdr.magic_str, fs_rollback_file_magic_string,
          sizeof (g_fs_rbk_file.hdr.magic_str));
  ...
  /* make sure our _read and _write should always succeed. */
  ASSERT ( sizeof (g_fs_rbk_file.hdr) <= FS_MAX_TRANSACTION );
  ...
  fs_off_t src_offset, fs_off_t dest_offset, fs_size_t write_size)
{
  ...
  /* make sure our _read and _write should always succeed. */
  ASSERT ( sizeof (fs_temp_buff[0]) <= FS_MAX_TRANSACTION );
  ...
  while (write_size > 0) {
    ...
    if (efs_read (src_fd, src_data_ptr, curr_write_size) !=
        curr_write_size)
    {
      ROLLBACK_ASSERT (FALSE);
      goto Error;
    ...
    if (write_result != (fs_ssize_t)curr_write_size) {
      ROLLBACK_ASSERT (FALSE);
      goto Error;
    }
  ...
  long orig_file_offset, fs_size_t rec_size, int orig_fd)
{
  ...
  /* make sure our _read and _write should always succeed. */
  ASSERT ( sizeof (g_fs_rbk_file.write_rec_hdr) <= FS_MAX_TRANSACTION );
